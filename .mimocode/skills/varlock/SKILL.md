---
name: varlock
description: "Use when working with varlock"
---
...
```bash
# âŒ NEVER DO THIS - exposes secret to Claude's context
echo $CLERK_SECRET_KEY
cat .env | grep SECRET
printenv | grep API

# âœ… DO THIS - validates without exposing
varlock load --quiet && echo "âœ“ Secrets validated"
```

### Rule 2: Never Read .env Directly

```bash
# âŒ NEVER DO THIS - exposes all secrets
cat .env
less .env
Read tool on .env file

# âœ… DO THIS - read schema (safe) not values
cat .env.schema
...
```bash
# âŒ NEVER DO THIS - exposes secret in error
test -n "$API_KEY" && echo "Key: $API_KEY"

# âœ… DO THIS - Varlock validates and masks
varlock load
# Output shows: API_KEY ðŸ”sensitive â”” â–’â–’â–’â–’â–’
```

### Rule 4: Never Include Secrets in Commands

```bash
# âŒ NEVER DO THIS - secret in command history
curl -H "Authorization: Bearer sk_live_xxx" https://api.example.com

# âœ… DO THIS - use environment variable
...
varlock load --quiet || {
  echo "âŒ Environment validation failed"
  exit 1
}
...
# Shows: âœ… API_KEY ðŸ”sensitive â”” â–’â–’â–’â–’â–’
...
```bash
# âœ… Safe approach - check presence and format
