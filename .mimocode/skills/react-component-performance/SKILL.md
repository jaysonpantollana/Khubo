...
description: "Use when working with react-component-performance"
---
...
6. **Validate**: open React DevTools Profiler â†’ record the interaction â†’ inspect the Flamegraph for components rendering longer than ~16 ms â†’ compare against a pre-optimization baseline recording.
...
```tsx
// âŒ Before â€“ entire parent (and list) re-renders every second
...
// âœ… After â€“ only <Clock> re-renders; list is untouched
function Clock() {
  const [tick, setTick] = useState(0);
  useEffect(() => {
    const id = setInterval(() => setTick(t => t + 1), 1000);
...
```tsx
// âŒ Before â€“ new handler reference on every render busts Row memo
...
// âœ… After â€“ stable handler; Row only re-renders when its own item changes
