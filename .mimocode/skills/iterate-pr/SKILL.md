---
name: iterate-pr
description: "Use when working with iterate-pr"
---
...
Review bot feedback (from Sentry, Warden, Cursor, Bugbot, CodeQL, etc.) appears in `high`/`medium`/`low` with `review_bot: true` â€” it is NOT placed in the `bot` bucket.
...
- Real issue found â†’ fix it
- False positive â†’ skip, but explain why in a brief comment
- Never silently ignore review bot feedback â€” always verify the finding
...
**Skip silently:**
- `resolved` threads
- `bot` comments (informational only â€” Codecov, Dependabot, etc.)
...
**When to reply:**
- `high` and `medium` items â€” whether fixed or determined to be false positives
- `low` items â€” whether fixed or declined by the user
...
- Before replying, check if the thread already has a reply ending with `*- Claude Code*` or `*â€” Claude Code*` to avoid duplicates on re-loops
- If the `gh api` call fails, log and continue â€” do not block the workflow

### 4. Check CI Status

Run `${CLAUDE_SKILL_ROOT}/scripts/fetch_pr_checks.py` to get structured failure data.

**Wait if pending:** If review bot checks (sentry, warden, cursor, bugbot, seer, codeql) are still running, wait before proceedingâ€”they post actionable feedback that must be evaluated. Informational bots (codecov) are not worth waiting for.

### 5. Fix CI Failures

For each failure in the script output:
1. Read the `log_snippet` and trace backwards from the error to understand WHY it failed â€” not just what failed
...
Do NOT assume what failed based on check name aloneâ€”always read the logs. Do NOT "quick fix and hope" â€” understand the failure thoroughly before changing code.
...
If local verification fails, fix before proceeding â€” do not push known-broken code.
...
2. If all checks passed â†’ proceed to exit conditions
3. If any checks failed (none pending) â†’ return to step 5
...
5. After all checks pass, do a final feedback check: `sleep 10`, then run `uv run ${CLAUDE_SKILL_ROOT}/scripts/fetch_pr_feedback.py`. Address any new high/medium feedback â€” if changes are needed, return to step 6.
