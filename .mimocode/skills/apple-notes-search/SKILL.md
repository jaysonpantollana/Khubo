...
description: "Use when working with apple-notes-search"
---
...
user's own Apple Notes â€” hybrid search, Swanson-ABC bridges, entity threads, and cited
...
If `apple-notes` tools are not available, the server isn't registered yet â€” do the
...
## Setup (walk the user through this â€” it's the skill's real value)
...
2. **Clone + install deps:**
   ...
   bun install
   ```
3. **Grant Full Disk Access to bun.** Run `which bun`, then open System Settings â†’
   Privacy & Security â†’ Full Disk Access, click `+`, and add that exact `bun` binary
...
## Tool map â€” which tool for which job
...
| `bridge-notes` | **Swanson-ABC bridges** â€” non-obvious connections: pairs (A, C) not directly similar but both strongly tied to a shared intermediary B. "Find non-obvious connections across my notes." Optional `folder`, `limit`. No LLM. |
| `feed` | Ranked evidence-first connection stream (bridges + abstraction pairs + entity threads) as JSON. Optional `limit`. |
| `entity-notes` / `list-entities` | "Where else do I talk about Mercedes?" Entity chips â†’ notes by mention weight. **Needs the optional entity graph db** (`~/.mcp-apple-notes/layered_graph.db`); if absent these report how to generate it. |
...
- **Index before the first search.** No index â†’ empty/garbage results; run `index-notes`.
- **Auto re-index:** each search does ~1ms change detection and kicks ONE background
  incremental index if notes changed â€” search returns immediately from the current index
  and catches up when the job lands. If a just-edited note is missing, it's the catch-up
  lag; re-run the search.
- **Score:** `score = RRF(vector, BM25) Ã— title_boost Ã— recency_factor`.
...
- **Synthesis is the only cloud-capable part.** It needs an LLM: local via LM Studio /
  ...
  Everything else â€” embeddings, search, BM25, clustering, bridges, entities â€” is on-device.
