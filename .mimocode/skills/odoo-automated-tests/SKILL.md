...
description: "Use when working with odoo-automated-tests"
---
...
- âœ… **Do:** Use `setUpClass()` with `cls.env` instead of `setUp()` â€” it is dramatically faster for large test suites.
- âœ… **Do:** Use `@tagged('post_install', '-at_install')` to run tests after all modules are installed.
- âœ… **Do:** Test both the happy path and error conditions (`ValidationError`, `AccessError`, `UserError`).
- âœ… **Do:** Use `self.with_user(user)` to test access control without calling `sudo()`.
- âŒ **Don't:** Use a production database for tests â€” always use a dedicated test database.
- âŒ **Don't:** Rely on test execution order â€” each `TransactionCase` test is rolled back in isolation.
- âŒ **Don't:** Hardcode passwords in `HttpCase.authenticate()` â€” use `self.env.user.login` or a fixture user.
