...
description: "Use when working with tao-setup-nvidia-gpu-host"
---
...
- Docker engine â€” only installed for `docker` / `local-docker` backends and
...
The check is safe and read-only by default â€” it works on any Linux
...
| other (Arch, Alpine, Gentoo, NixOS, FreeBSD, â€¦) | n/a | n/a | `--install` exits with a clear error listing the version targets and the NVIDIA install-guide URLs. Install manually, then rerun `--check-only`. |
...
> âš ï¸ **Note â€” running non-interactively (agent / skill runs):** a skill run has
...
> NVIDIA Container Toolkit, and â€” for Docker backends â€” Docker) and modifies the
...
6. Adds the invoking user (`$SUDO_USER` if available, else `$USER`) to the
   `docker` group so subsequent shells can run `docker` without `sudo` â€”
...
portable â€” it only queries the binaries / package databases â€” so once the
...
`VERSION_CODENAME` (`focal`/`jammy`/`noble` â†’ Ubuntu 20.04/22.04/24.04;
`bullseye`/`bookworm`/`trixie` â†’ Debian 11/12/12). If the host's codename
...
the detected distro, which is always â‰¥ 580. If your host needs a stricter
