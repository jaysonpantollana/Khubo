---
name: fp-option-ref
description: "Use when working with fp-option-ref"
---
...
O.some(5)              // Some(5)
O.none                 // None
O.fromNullable(x)      // null/undefined â†’ None, else Some(x)
O.fromPredicate(x > 0)(x) // false â†’ None, true â†’ Some(x)
```
...
```typescript
// âŒ Nullable - easy to forget checks
const name = user?.profile?.name ?? 'Guest'

// âœ… Option - explicit, composable
pipe(
  O.fromNullable(user),
