...
description: "Use when working with latex-paper-conversion"
---
...
- âœ… Always write a Python extraction script; DO NOT manually copy-paste thousands of lines of LaTeX.
- âœ… Always run `pdflatex` and verify the `.log` to ensure the final output compiles.
- âœ… Explicitly ask the user for the structural mapping if the source and target differ drastically (e.g., merging abstract and keywords).
- âŒ Don't assume all math packages automatically exist in the new template (e.g., add `\usepackage{amsmath}` if missing).
