...
description: "Use when working with odoo-edi-connector"
---
...
Electronic Data Interchange (EDI) is the standard for automated B2B document exchange â€” purchase orders, invoices, ASNs (Advance Shipping Notices). This skill guides you through mapping EDI transactions (ANSI X12 or EDIFACT) to Odoo business objects, setting up trading partner configurations, and automating inbound/outbound document flows.
...
def process_850(edi_file_path):
    ...
    with x12file.X12File(edi_file_path) as f:
        for transaction in f.get_transaction_sets():
            ...
            # Extract partner (N1 segment â€” Buyer)
...
- âœ… **Do:** Store every raw EDI transaction in an audit log table before processing.
- âœ… **Do:** Always send a **997 Functional Acknowledgment** within 24 hours of receiving a transaction.
- âœ… **Do:** Negotiate a test cycle with trading partners before going live â€” use test ISA qualifier `T`.
- âŒ **Don't:** Process EDI files synchronously in web requests â€” queue them for async processing.
- âŒ **Don't:** Hardcode trading partner qualifiers â€” store them in a configuration table per partner.
