---
name: skill-scanner
description: "Use when working with skill-scanner"
---
...
Returns JSON with findings, URLs, structure info, and severity counts. The script catches patterns mechanically â€” your job is to evaluate intent and filter false positives.
...
- **Tool assessment**: Review `allowed-tools` â€” is Bash justified? Are tools unrestricted (`*`)?
...
3. Skills about security, testing, or education commonly reference injection patterns â€” this is expected
...
This phase is agent-only â€” no pattern matching. Read the full SKILL.md instructions and evaluate:
...
5. Check PEP 723 `dependencies` â€” are they legitimate, well-known packages?
...
- **Trusted domains**: GitHub, PyPI, official docs â€” normal
- **Untrusted domains**: Unknown domains, personal sites, URL shorteners â€” flag for review
...
Example assessments:
- `Read Grep Glob` â€” Low risk, read-only analysis skill
- `Read Grep Glob Bash` â€” Medium risk, needs Bash justification (e.g., running bundled scripts)
- `Read Grep Glob Bash Write Edit WebFetch Task` â€” High risk, near-full access
