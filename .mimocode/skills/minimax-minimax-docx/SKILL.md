...
description: "Use when working with minimax-minimax-docx"
---
...
description: >
  ...
  MUST use this skill whenever the user wants to produce, modify, or format a Word document â€”
...
**First operation in session:** `scripts/env_check.sh` â€” do not proceed if `NOT READY`. (Skip on subsequent operations within the same session.)
...
**Before writing any C#, read the relevant `Samples/*.cs` file** â€” they contain compilable, SDK-version-verified patterns. The Samples table in the References section below maps topics to files.
...
```
User task
â”œâ”€ No input file â†’ Pipeline A: CREATE
â”‚   signals: "write", "create", "draft", "generate", "new", "make a report/proposal/memo"
â”‚   â†’ Read references/scenario_a_create.md
â”‚
â””â”€ Has input .docx
    â”œâ”€ Replace/fill/modify content â†’ Pipeline B: FILL-EDIT
    â”‚   signals: "fill in", "replace", "update", "change text", "add section", "edit"
    â”‚   â†’ Read references/scenario_b_edit_content.md
    â”‚
    â””â”€ Reformat/apply style/template â†’ Pipeline C: FORMAT-APPLY
        signals: "reformat", "apply template", "restyle", "match this format", "å¥—æ¨¡æ¿", "æŽ’ç‰ˆ"
        â”œâ”€ Template is pure style (no content) â†’ C-1: OVERLAY (apply styles to source)
        â””â”€ Template has structure (cover/TOC/example sections) â†’ C-2: BASE-REPLACE
            (use template as base, replace example content with user content)
        â†’ Read references/scenario_c_apply_template.md
```
...
Convert `.doc` â†’ `.docx` if needed: `scripts/doc_to_docx.sh input.doc output_dir/`
...
Read `references/scenario_a_create.md`, `references/typography_guide.md`, and `references/design_principles.md` first. Pick an aesthetic recipe from `Samples/AestheticRecipeSamples.cs` that matches the document type â€” do not invent formatting values. For CJK, also read `references/cjk_typography.md`.

**Choose your path:**
- **Simple** (plain text, minimal formatting): use CLI â€” `$CLI create --type report --output out.docx --config content.json`
...
Read `references/scenario_b_edit_content.md` first. Preview â†’ analyze â†’ edit â†’ validate.
...
For complex template operations (multi-template merge, per-section headers/footers, style merging), write C# directly â€” see Critical Rules below for required patterns.
...
These prevent file corruption â€” OpenXML is strict about element ordering.
...
| `w:p`  | `pPr` â†’ runs |
| `w:r`  | `rPr` â†’ `t`/`br`/`tab` |
| `w:tbl`| `tblPr` â†’ `tblGrid` â†’ `tr` |
| `w:tr` | `trPr` â†’ `tc` |
| `w:tc` | `tcPr` â†’ `p` (min 1 `<w:p/>`) |
| `w:body` | block content â†’ `sectPr` (LAST child) |

**Direct format contamination:** When copying content from a source document, inline `rPr` (fonts, color) and `pPr` (borders, shading, spacing) override template styles. Always strip direct formatting â€” keep only `pStyle` reference and `t` text. Clean tables too (including `pPr/rPr` inside cells).

**Track changes:** `<w:del>` uses `<w:delText>`, never `<w:t>`. `<w:ins>` uses `<w:t>`, never `<w:delText>`.

**Font size:** `w:sz` = points Ã— 2 (12pt â†’ `sz="24"`). Margins/spacing in DXA (1 inch = 1440, 1cm â‰ˆ 567).

**Heading styles MUST have OutlineLevel:** When defining heading styles (Heading1, ThesisH1, etc.), always include `new OutlineLevel { Val = N }` in `StyleParagraphProperties` (H1â†’0, H2â†’1, H3â†’2). Without this, Word sees them as plain styled text â€” TOC and navigation pane won't work.
...
- Each content paragraph must appear exactly ONCE â€” never duplicate when inserting section breaks.
...
- NEVER recreate headers/footers from scratch â€” copy template header/footer XML byte-for-byte.
...
Load as needed â€” don't load all at once. Pick the most relevant files for the task.

**The C# samples and design references below are the project's knowledge base ("encyclopedia").** When writing OpenXML code, ALWAYS read the relevant sample file first â€” it contains compilable, SDK-version-verified patterns that prevent common errors. When making aesthetic decisions, read the design principles and recipe files â€” they encode tested, harmonious parameter sets from authoritative sources (IEEE, ACM, APA, Nature, etc.), not guesses.
...
### C# code samples (compilable, heavily commented â€” read when writing code)
...
| `Samples/StyleSystemSamples.cs` | Styles: Normal/Heading chain, character/table/list styles, DocDefaults, latentStyles, CJK å…¬æ–‡, APA 7th, import, resolve inheritance |
...
| `Samples/TableSamples.cs` | Tables: borders, grid, cell props, margins, row height, header repeat, merge (H+V), nested, floating, three-line ä¸‰çº¿è¡¨, zebra striping |
| `Samples/HeaderFooterSamples.cs` | Headers/footers: page numbers, "Page X of Y", first/even/odd, logo image, table layout, å…¬æ–‡ "-X-", per-section |
...
| `Samples/ListAndNumberingSamples.cs` | Numbering: bullets, multi-level decimal, custom symbols, outlineâ†’headings, legal, Chinese ä¸€/ï¼ˆä¸€ï¼‰/1./(1), restart/continue |
...
| `references/cjk_typography.md` | CJK fonts, å­—å· sizes, RunFonts mapping, GB/T 9704 å…¬æ–‡ standard |
| `references/cjk_university_template_guide.md` | Chinese university thesis templates: numeric styleIds (1/2/3 vs Heading1), document zone structure (coverâ†’abstractâ†’TOCâ†’bodyâ†’references), font expectations, common mistakes |
| `references/design_principles.md` | **Aesthetic foundations**: 6 design principles (white space, contrast/scale, proximity, alignment, repetition, hierarchy) â€” teaches WHY, not just WHAT |
...
| `references/troubleshooting.md` | **Symptom-driven fixes**: 13 common problems indexed by what you SEE (headings wrong, images missing, TOC broken, etc.) â€” search by symptom, find the fix |
