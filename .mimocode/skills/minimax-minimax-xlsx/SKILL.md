...
description: "Use when working with minimax-minimax-xlsx"
---
...
| Task | Method | Guide |
|------|--------|-------|
| **READ** â€” analyze existing data | `xlsx_reader.py` + pandas | `references/read-analyze.md` |
| **CREATE** â€” new xlsx from scratch | XML template | `references/create.md` + `references/format.md` |
| **EDIT** â€” modify existing xlsx | XML unpackâ†’editâ†’pack | `references/edit.md` (+ `format.md` if styling needed) |
| **FIX** â€” repair broken formulas in existing xlsx | XML unpackâ†’fix `<f>` nodesâ†’pack | `references/fix.md` |
| **VALIDATE** â€” check formulas | `formula_check.py` | `references/validate.md` |

## READ â€” Analyze data (read `references/read-analyze.md` first)

Start with `xlsx_reader.py` for structure discovery, then pandas for custom analysis. Never modify the source file.

**Formatting rule**: When the user specifies decimal places (e.g. "2 decimal places"), apply that format to ALL numeric values â€” use `f'{v:.2f}'` on every number. Never output `12875` when `12875.00` is required.

**Aggregation rule**: Always compute sums/means/counts directly from the DataFrame column â€” e.g. `df['Revenue'].sum()`. Never re-derive column values before aggregation.

## CREATE â€” XML template (read `references/create.md` + `references/format.md`)

Copy `templates/minimal_xlsx/` â†’ edit XML directly â†’ pack with `xlsx_pack.py`. Every derived value MUST be an Excel formula (`<f>SUM(B2:B9)</f>`), never a hardcoded number. Apply font colors per `format.md`.

## EDIT â€” XML direct-edit (read `references/edit.md` first)

**CRITICAL â€” EDIT INTEGRITY RULES:**
...
3. Only modify the specific cells the task asks for â€” everything else must be untouched.
4. **After saving output.xlsx, verify it**: open with `xlsx_reader.py` or `pandas` and confirm the original sheet names and a sample of original data are present. If verification fails, you wrote the wrong file â€” fix it before delivering.

Never use openpyxl round-trip on existing files (corrupts VBA, pivots, sparklines). Instead: unpack â†’ use helper scripts â†’ repack.

**"Fill cells" / "Add formulas to existing cells" = EDIT task.** If the input file already exists and you are told to fill, update, or add formulas to specific cells, you MUST use the XML edit path. Never create a new `Workbook()`. Example â€” fill B3 with a cross-sheet SUM formula:
```bash
python3 SKILL_DIR/scripts/xlsx_unpack.py input.xlsx /tmp/xlsx_work/
# Find the target sheet's XML via xl/workbook.xml â†’ xl/_rels/workbook.xml.rels
...
## FIX â€” Repair broken formulas (read `references/fix.md` first)

This is an EDIT task. Unpack â†’ fix broken `<f>` nodes â†’ pack. Preserve all original sheets and data.

## VALIDATE â€” Check formulas (read `references/validate.md` first)
...
2. **CREATE â†’ XML template**: Copy minimal template, edit XML directly, pack with `xlsx_pack.py`
3. **EDIT â†’ XML**: Never openpyxl round-trip. Use unpack/edit/pack scripts
4. **Always produce the output file** â€” this is the #1 priority
