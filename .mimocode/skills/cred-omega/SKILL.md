---
name: cred-omega
description: "Use when working with cred-omega"
---
...
## Fase 1 â€” Descoberta (Varredura Profunda)

#### 1A. Varredura de Codigo (padroes de alta precisao)

```bash

## Scanner Principal â€” Padroes Regex De Alta Cobertura
...
## Github Actions â€” Checar Se Secrets Estao Sendo Logados
...
## Fase 2 â€” Classificacao De Risco

Para cada achado, classificar usando esta matriz:

| Nivel | Criterio | Acao | SLA |
|-------|----------|------|-----|
| **P0 â€” CRITICO** | Segredo confirmado exposto em repo publico ou produÃ§Ã£o | Revogar AGORA, rotacionar, notificar | < 1 hora |
| **P1 â€” ALTO** | Segredo em repo privado, historico git, ou CI logs | Revogar, rotacionar, limpar historico | < 24 horas |
| **P2 â€” MEDIO** | Permissoes excessivas, chave sem restricao, sem rotacao | Restringir, adicionar restricoes, agendar rotacao | < 1 semana |
| **P3 â€” BAIXO** | Chave dormante, sem dono identificado, best practice faltando | Documentar, atribuir dono, planejar melhoria | < 1 mes |
...
## Fase 3 â€” Contencao (Acao Imediata)

Para P0 e P1, executar imediatamente:

1. **Revogar** â€” invalidar a chave/token no painel do provedor
2. **Rotacionar** â€” gerar nova credencial com escopo minimo
3. **Substituir** â€” atualizar em todos os locais que usam a credencial antiga
4. **Verificar** â€” confirmar que servicos voltaram a funcionar com nova credencial
5. **Limpar** â€” remover do historico git se necessario:
   ```bash
...
## Fase 4 â€” Hardening (Protecao Profunda)
...
- O front chama SEU endpoint â†’ sua VPS chama o provedor com segredo em Secret Store
...
- Chaves sem dono e sem data = lixo perigoso â†’ revogar
...
[Cliente/Browser]
       |
       v
[Seu Proxy (VPS)] â† autenticacao do usuario (JWT/session)
       |             rate limiting por usuario/rota
       |             logging (sem segredos)
       |             quota por ambiente
       |             kill switch
       v
[API do Provedor] â† chave injetada do Secret Store
```
...
- Chaves sem uso 30 dias â†’ investigar â†’ revogar se inativas
- Permissoes excedentes â†’ reduzir
...
**Risco tipico:** Chave vazada â†’ consumo/custo descontrolado â†’ milhares de dolares em horas.

**Hardening:**
- Chave SO no servidor (VPS) â€” nunca no front
...
- EVITAR service account keys long-lived â€” preferir Workload Identity Federation
- Aplicar least privilege (IAM minimo â€” usar IAM Recommender)
...
- Webhooks com validacao de assinatura (HMAC-SHA256) â€” OBRIGATORIO
- Revisar permissoes/roles no Business Manager â€” principio do menor privilegio
...
- Modo teste (sk_test_) para dev â€” NUNCA sk_live_ em dev
...
1. **CONTER** â€” Revogar chave/token, desativar webhooks, travar proxy (kill switch)
2. **ERRADICAR** â€” Remover do codigo, reescrever historico git, scan amplo
3. **RECUPERAR** â€” Gerar novas credenciais com escopo minimo, reimplantar
4. **APRENDER** â€” Adicionar regra anti-regressao, post-mortem, atualizar playbook
...
## Verificar Custos/CobranÃ§as Anomalos Nos Provedores
...
Esta skill opera de forma TRANSVERSAL â€” mesmo quando outras skills estao ativas:

- Se durante QUALQUER tarefa detectar uma chave exposta em codigo â†’ alertar imediatamente
- Se um usuario pedir para "colocar a chave no config.js" â†’ explicar o risco e oferecer alternativa segura
- Se detectar .env sendo commitado â†’ bloquear e orientar .gitignore
- Se ver hardcoded credentials â†’ sugerir refatoracao para env vars
...
| Skill | Integracao |
|-------|-----------|
| **007** | Threat modeling + Red Team â€” cred-omega cuida de segredos, 007 de arquitetura |
