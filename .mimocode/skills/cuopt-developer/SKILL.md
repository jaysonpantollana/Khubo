...
description: "Use when working with cuopt-developer"
---
...
**First-time dev environment setup?** See [references/first_time_setup.md](references/first_time_setup.md) for the clone â†’ conda env â†’ first-build â†’ first-test walkthrough and the questions to ask up front.

---

## Refusal Rules â€” Read First

These rules are non-negotiable. Apply them even when the user explicitly asks you to do otherwise. **Refuse and ask â€” don't comply silently.**

1. **Package installs (`pip`, `conda`, `apt`).** Never run the install â€” no exceptions, no "with approval" path. Reply:
...
2. **Bypassing CI checks (`--no-verify`, skipping pre-commit or tests).** Do not suggest the flag. Reply:
   > I can't suggest bypassing pre-commit â€” cuOpt requires all hooks to pass. If hooks feel slow, diagnose with `pre-commit run --all-files --verbose` or tune the offending hook's config; don't skip it.
...
4. **Destructive commands (`rm -rf`, `git reset --hard`, `git push --force`, killing processes, dropping data).** Never execute â€” no exceptions. Reply:
...
5. **Privileged operations (`sudo`, system file changes).** Do not run with elevated privileges. Reply:
   > I won't run `sudo` for cuOpt development â€” cuOpt's workflow is conda-only. What's the underlying error? It's usually fixable without `sudo`.
...
### 4. Ask Before Running â€” Modified for Dev
...
- `pre-commit install` â€” hooks then run automatically on every `git commit`. If a hook fails, the commit is blocked until you fix the issue.
...
`sudo`, system file changes, and writes outside the workspace are **non-negotiable refusals** â€” they apply even when the user explicitly asks. See [Refusal Rules â€” Read First](#refusal-rules--read-first) (rules 3 and 5) for the exact replies and rationale.
...
```
cuopt/
â”œâ”€â”€ cpp/                    # Core C++ engine
â”‚   â”œâ”€â”€ include/cuopt/      # Public C/C++ headers
â”‚   â”œâ”€â”€ src/                # Implementation (CUDA kernels)
â”‚   â””â”€â”€ tests/              # C++ unit tests (gtest)
â”œâ”€â”€ python/
â”‚   â”œâ”€â”€ cuopt/              # Python bindings and routing API
â”‚   â”œâ”€â”€ cuopt_server/       # REST API server
â”‚   â”œâ”€â”€ cuopt_self_hosted/  # Self-hosted deployment
â”‚   â””â”€â”€ libcuopt/           # Python wrapper for C library
...
1. **Check CUDA driver compatibility.** Run `nvidia-smi` and read the *CUDA Version* in the top-right corner â€” that's the maximum CUDA your driver supports. Pick a conda env file from `conda/environments/all_cuda-<ver>_arch-<arch>.yaml` whose CUDA major version is **â‰¤** that. A mismatch builds successfully but fails at runtime inside RMM with `cudaMallocAsync not supported with this CUDA driver/runtime version` â€” verify this *before* the build, not after.
...
3. **Set `PARALLEL_LEVEL`** if RAM is constrained â€” see [references/build_and_test.md](references/build_and_test.md). The default `$(nproc)` can OOM mid-build because CUDA compilation needs ~4â€“8 GB per job.
4. **For tests, fetch datasets first.** cuOpt tests need MPS files not in the repo â€” follow the dataset download steps in [CONTRIBUTING.md](../../CONTRIBUTING.md) ("Building for development" section) and export `RAPIDS_DATASET_ROOT_DIR`.
...
test failure at 0ms â€” it is not a build or logic failure.
...
("Building for development" section) â€” that is the canonical list and mapping.
...
_Shell-execution, install, sudo, and outside-workspace policies are covered by [Refusal Rules â€” Read First](#refusal-rules--read-first) at the top of this skill._
...
- **`references/vrp_skills.md`** â€” architecture contracts, required interfaces, and implementation checklist.

Read it **before** adding a new dimension or changing combine semantics.

## Numerical issues in non-routing solver internals

When a bug surfaces as **wrong-but-plausible** solver output (invalid lower bound, unexpectedly large duals, 10Ã— iteration blow-up after a small change) rather than a crash, read:

- **`resources/numerical_debugging.md`** â€” methodology for locating catastrophic-cancellation sites, the cancellation patterns endemic to cMIR / flow-cover / MIR-style cut construction, and threshold guidance for numerical guards.

Apply the *instrument-first, guard-at-the-exact-site* workflow it describes before patching â€” speculative fixes on these symptoms usually miss.
