...
description: "Use when working with cupynumeric-parallel-data-load"
---
...
| Sharded multi-file (any format), Parquet/Arrow, raw binary, custom layouts | **No built-in loader â€” this skill.** |
...
range/slice reads (raw binary, HDF5, Parquet/Arrow â€” see "Other
...
This skill is purely about **loading** â€” it assumes the data is already
...
- All shards share the same `dtype` and the same trailing axes
  (`shape[1:]`); **axis 0 (rows per shard) may differ across files** â€”
...
If your data lives in a different layout â€” fixed-stride raw binary, an
HDF5 file with one dataset per shard, a directory tree, ... â€” only the
...
No layout flags â€” the read driver walks every `.npy` header to recover
...
worked example's defaults â€” total rows split across 4 GPUs at
`~1M` per tile â€” fall below the threshold and would otherwise be
...
fence) are reused unchanged across formats â€” see "Other formats" below
for the swap-points.
...
`shape[1:]`) across files. **Per-shard row counts may differ** â€” the
...
from `trailing_shape` unchanged. Use `cn.empty` â€” the task overwrites
...
short â€” that's exactly what `partition_by_tiling` supports â€” so the
...
â€” `PhysicalStore` itself has no `.shape` attribute, so going through
...
(`__dlpack__` for cupy, `__array_interface__` for `np.asarray`) â€”
zero-copy views of the local tile. Bisect cost is `O(log num_shards)`
and the inner loop typically iterates 1â€“2 times (tiles overlap at
...
1. **All shards must share `dtype` and trailing axes (`shape[1:]`).**
   ...
   freely differ â€” the cumulative-offset table handles them. The
...
2. **Pick the consumer that matches the variant.** `cp.from_dlpack`
   ...
   its own consumer â€” see step 4.

3. **mmap views aren't always C-contiguous** â€” wrap each per-file
...
The same `load_tile` task body still works in either mode â€” the inner
...
`K * num_processors`. The same task body still works â€” bisect just lands
...
Cheap range/slice reads are required â€” formats that only support
...
Raw binary doesn't â€” sidecar or derive from file size.
...
runtime â€” `cn.asarray(store)`, slice assignment `cn_dst[s] = host_np` â€”
...
are independent â€” the runtime doesn't catch a mismatch. Larger launch
domain â†’ out-of-range tiles; smaller â†’ unwritten tiles. Always derive
