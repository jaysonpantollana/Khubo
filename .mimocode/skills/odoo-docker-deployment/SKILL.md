...
description: "Use when working with odoo-docker-deployment"
---
...
workers = 9                ; (CPU cores Ã— 2) + 1
max_cron_threads = 2
limit_memory_soft = 1610612736   ; 1.5 GB â€” soft kill threshold
limit_memory_hard = 2147483648   ; 2.0 GB â€” hard kill threshold
limit_time_cpu = 600
...
# Restart Odoo only (not DB â€” avoids data risk)
...
- âœ… **Do:** Store all secrets in a `.env` file and reference them with `${VAR}` â€” never hardcode passwords in `docker-compose.yml`.
- âœ… **Do:** Use `depends_on: condition: service_healthy` with a PostgreSQL healthcheck to prevent Odoo starting before the DB is ready.
- âœ… **Do:** Put Nginx in front of Odoo for SSL termination (Let's Encrypt / Certbot) â€” never expose Odoo directly on port 80/443.
- âœ… **Do:** Set `workers = (CPU cores Ã— 2) + 1` in `odoo.conf` â€” `workers = 0` uses single-threaded mode and blocks all users.
- âŒ **Don't:** Expose port 5432 (PostgreSQL) to the public internet â€” keep it on the internal Docker network only.
- âŒ **Don't:** Use the `latest` or `17` Docker image tags in production â€” always pin to a specific patch-level tag (e.g., `odoo:17.0`).
- âŒ **Don't:** Mount `odoo.conf` and rely on it for secrets in CI/CD â€” use Docker secrets or environment variables instead.

## Limitations

- This skill covers **self-hosted Docker deployments** â€” Odoo.sh (cloud-managed hosting) has a completely different deployment model.
...
- Does not include an Nginx configuration template â€” consult the [official Odoo Nginx docs](https://www.odoo.com/documentation/17.0/administration/install/deploy.html) for the full reverse proxy config.
- The `addons_path` inside the Docker image may change with new base image versions â€” always verify after upgrading the Odoo image.
