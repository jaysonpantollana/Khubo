...
description: "Use when working with gh-review-requests"
---
...
Accept either a team slug (`streaming-platform`) or a display name ("Streaming Platform") â€” convert to lowercase-hyphenated slug before passing to the script.
...
- `"review requested from: <Team Name>"` â€” the team is a requested reviewer
- `"opened by: <login>"` â€” the PR author is a team member
...
- `gh api repos/{repo}/pulls/{number}` â€” skip if `state == "closed"` or `merged_at` is set
- `gh api repos/{repo}/pulls/{number}/requested_reviewers` â€” check `teams[].name`
- `gh api orgs/{org}/teams/{slug}/members` â€” check if author is a member
