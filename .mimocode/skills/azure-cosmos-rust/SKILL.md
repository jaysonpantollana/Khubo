...
description: "Use when working with azure-cosmos-rust"
---
...
1. **Always specify partition key** â€” required for point reads and writes
2. **Use `into_model()?`** â€” to deserialize responses into your types
3. **Derive `Serialize` and `Deserialize`** â€” for all document types
4. **Use Entra ID auth** â€” prefer `DeveloperToolsCredential` over key auth
5. **Reuse client instances** â€” clients are thread-safe and reusable
