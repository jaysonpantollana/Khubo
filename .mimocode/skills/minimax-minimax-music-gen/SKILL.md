...
description: "Use when working with minimax-minimax-music-gen"
---
...
description: >
  ...
  a mood/scene and wants background music. Supports multilingual triggers â€” match equivalent
...
- **Music Generation**: `mmx music generate` â€” model: `music-2.6-free`
...
- **Cover**: `mmx music cover` â€” model: `music-cover-free`
...
- ALL text shown to the user â€” including preview labels, field names, confirmations, status
  messages, playback info, feedback prompts, **and the prompt/description preview** â€” MUST
...
Q2: Creation mode?
  - Basic â€” one-line description, auto-generate
  - Advanced â€” edit lyrics, refine prompt, plan
```

If the user gives a clear one-liner like "make me a sad piano piece", skip the questions â€”
...
2. **Show the user a preview** before generating. Translate all labels AND the prompt
   ...
   calling the API â€” the user should never see it. Example template (English reference â€”
...
**Reference audio requirements**: mp3, wav, flac â€” duration 6s to 6min, max 50MB.
If no lyrics are provided, the original lyrics are extracted via ASR automatically.

### Workflow

When the user selects Cover mode:
1. Ask for the source audio â€” a local file path or URL
