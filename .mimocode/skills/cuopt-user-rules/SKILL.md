...
description: "Use when working with cuopt-user-rules"
---

---
name: cuopt-user-rules
version: "26.08.00"
description: Base rules for end users calling NVIDIA cuOpt (routing/LP/MILP/QP/install/server). Not for cuOpt internals â€” use cuopt-developer for those.
license: Apache-2.0
metadata:
...
**Don't guess â€” ask.** A brief clarifying question saves time vs. solving the wrong problem.
...
4. **State assumptions explicitly:**
   - "I'm assuming [X] â€” let me know if this differs from your scenario"
...
- If user provides partial code, extend itâ€”don't rewrite from scratch
...
- **Objective value with highlight** â€” easy to spot (bold or code block). Example: **Objective value (min total cost):** &lt;value&gt; or `Objective value: <value>`.
...
**Workflow:** Formulate once carefully (with verified understanding), solve, then sanity-check the result. If something is wrong, fix it with a targeted changeâ€”avoid spinning through many model variants. Decide, implement, verify, then move on.
...
2. **Different packages by language/interface:**
   ...
   |----------------------|---------|-------|
   | **Python** | `cuopt` (pip/conda) â€” also pulls in `libcuopt` | `import cuopt` |
   | **C** | `libcuopt` (pip/conda) â€” already present if `cuopt` is installed | `find libcuopt.so` or header check |
...
4. **Never assume installation is needed** â€” the user may:
...
| Package installs | **Never** run installs yourself â€” give the exact command, user runs it (see below). |
...
> **ðŸ”’ MANDATORY â€” You MUST NOT install, upgrade, or modify packages.** Provide the exact command; the user runs it. No exceptions.
