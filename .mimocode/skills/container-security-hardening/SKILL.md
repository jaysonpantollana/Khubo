...
description: "Use when working with container-security-hardening"
---
...
A production-focused guide for building, scanning, and running containers securely â€” from Dockerfile authoring through runtime enforcement and supply chain integrity.
...
- The user is primarily asking about GitHub Actions CI/CD â†’ recommend `github-actions-advanced`
- The user needs general Docker usage help (not security) â†’ recommend `docker-expert`
- The user is working with Kubernetes orchestration beyond security â†’ recommend `kubernetes-architect`
- The user needs application-level security (SQL injection, XSS) â†’ recommend `api-security-best-practices`
...
- The tech stack (Node, Python, Go, Java â€” affects base image choice)
...
```
1. Image Build        â†’ Minimal base, no secrets, non-root, read-only FS
2. Image Scanning     â†’ CVE scanning, SBOM, secret detection, Dockerfile lint
3. Runtime Security   â†’ Capabilities, seccomp, AppArmor, resource limits
4. Supply Chain       â†’ Signed images, pinned digests, trusted registries
5. Kubernetes Layer   â†’ Pod Security Admission, NetworkPolicy, RBAC, Kyverno
```

> Work through layers in order â€” hardening the image first gives the most leverage.
...
```dockerfile
# âŒ AVOID â€” massive attack surface (~100â€“200 CVEs typical)
FROM ubuntu:latest
FROM node:20

# âœ… BETTER â€” slim variants (glibc, smaller apt footprint)
FROM node:20-slim
FROM python:3.12-slim

# âœ… BEST â€” distroless (no shell, no package manager, built-in nonroot user)
FROM gcr.io/distroless/nodejs20-debian12
FROM gcr.io/distroless/python3-debian12
FROM gcr.io/distroless/static-debian12   # Go/Rust fully-static binaries

# âœ… ALSO GREAT â€” Alpine (musl libc; verify app compatibility first)
FROM alpine:3.20

# âœ… ZERO ATTACK SURFACE â€” for fully static binaries only
FROM scratch
```

See `references/base-image-comparison.md` for the full trade-off matrix.

### 1.2 Multi-Stage Build â€” Separate Build from Runtime
...
# â”€â”€ Stage 1: Install & Build â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Stage 2: Runtime â€” minimal, no build tools â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
USER nonroot:nonroot                # UID 65532 â€” built into distroless
EXPOSE 3000
CMD ["dist/server.js"]
```
...
```dockerfile
# For debian/ubuntu-based images â€” create dedicated user
RUN groupadd -r appgroup --gid 10001 && \
    useradd -r -g appgroup --uid 10001 --no-log-init appuser

COPY --chown=appuser:appgroup . /app

USER appuser    # Switch before CMD/ENTRYPOINT â€” never run as root
...
# For distroless â€” nonroot (UID 65532) is already built in
...
```dockerfile
# âŒ UNSAFE â€” tags are mutable; image can be silently overwritten (supply chain attack)
FROM node:20-slim

# âœ… SAFE â€” SHA256 digest is cryptographically immutable
...
```dockerfile
# âŒ NEVER â€” secret in ENV or RUN; visible in `docker history` and layer cache
ENV AWS_SECRET_ACCESS_KEY=supersecret
RUN curl -H "Authorization: Bearer $TOKEN" https://api.example.com > config.json
ARG API_KEY                         # Also unsafe â€” visible in build args history

# âœ… CORRECT â€” BuildKit secret mount (never persisted in any layer)
# syntax=docker/dockerfile:1
...
```dockerfile
# In the Dockerfile â€” use exec form (no shell interpretation)
ENTRYPOINT ["node", "server.js"]    # âœ… exec form
# ENTRYPOINT /bin/sh -c "node..."  # âŒ shell form â€” spawns extra process
...
# â”€â”€ Build stage â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
FROM node:20-slim AS builder
WORKDIR /build
COPY package*.json ./
...
# â”€â”€ Runtime stage â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
### 2.1 Trivy (Recommended â€” Fast, Comprehensive)
...
**.trivyignore example:**
```
# CVE-2023-1234 â€” only exploitable via X feature, not used in this app
CVE-2023-1234

# CVE-2023-5678 â€” fix not yet available; tracked in issue #42
CVE-2023-5678
```
...
### 2.3 Hadolint â€” Dockerfile Linting
...
### 2.5 CI Integration (GitHub Actions â€” SHA-Pinned)
...
### 3.2 Linux Capabilities â€” What to Drop and Keep
...
| `CHOWN` | Change file ownership | No â€” set ownership at build time |
| `SETUID` / `SETGID` | Switch user identity | No â€” drop always |
| `SYS_ADMIN` | Broad privileged operations | No â€” most dangerous capability |
| `NET_ADMIN` | Configure network interfaces | No (only network tools) |
| `SYS_PTRACE` | Debug/trace processes | No (only debugger containers) |
| `DAC_OVERRIDE` | Override file permissions | No â€” runs as correct user |
...
aa-genprof myapp   # Interactive â€” run app under aa-complain mode first
```
...
### 4.1 Sign Images with Cosign (Sigstore â€” Keyless)
...
# Sign after push â€” keyless via OIDC (no long-lived keys)
...
**GitHub Actions â€” Sign & Verify Pipeline:**
```yaml
permissions:
...
### 4.4 Admission Control â€” Block Unsigned/Unscanned Images

```yaml
# Kyverno policy â€” require signed images before admission
...
spec:
  ...
  template:
    spec:
      # â”€â”€ Pod-level security context â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      securityContext:
        runAsNonRoot: true
        runAsUser: 10001
        runAsGroup: 10001
      ...
      # â”€â”€ Container-level security context â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      containers:
        - name: app
          ...
          # â”€â”€ Resource limits (required for restricted PSA) â”€â”€
          resources:
            requests:
              memory: "128Mi"
              cpu: "100m"
          ...
          # â”€â”€ Writable tmpfs mounts â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
          volumeMounts:
            - name: tmp
              mountPath: /tmp
            - name: varrun
...
### 5.3 NetworkPolicy â€” Zero-Trust Networking
...
### 5.4 RBAC â€” Least Privilege

```yaml
# Create minimal role â€” never use wildcards
...
| Container needs `SYS_ADMIN` | Missing `--cap-drop` context | Investigate why â€” almost always avoidable |
...
### Dockerfile
- [ ] Minimal base image (distroless, slim, or alpine â€” not full debian/ubuntu)
- [ ] Multi-stage build â€” no build tools, devDependencies, or compilers in runtime image
...
- `references/base-image-comparison.md` â€” Size, CVE count, shell/pkg-manager trade-offs: distroless vs alpine vs slim vs scratch
- `references/seccomp-profile-template.json` â€” Minimal syscall allowlist for typical web servers; start here and extend
- `references/kubernetes-pod-security.md` â€” NetworkPolicy, RBAC, OPA/Kyverno policies, service account hardening, PSA

## Related Skills

- `docker-expert` â€” General Docker usage, Compose orchestration, image optimization
- `gha-security-review` â€” Security audit of GitHub Actions workflows
- `github-actions-advanced` â€” CI pipeline patterns including scanner integration
- `kubernetes-architect` â€” Full Kubernetes architecture, not just security
- `api-security-best-practices` â€” Application-level security (injection, auth, OWASP)
- `k8s-security-policies` â€” Extended Kubernetes security policies
