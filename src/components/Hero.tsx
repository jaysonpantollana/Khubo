...
// @behavior: External click closes dropdowns; announcements overlay trigger; scales to fit smaller viewports
...
// ponytail: reference width for the desktop design — scale is clamped to 1 so desktop never zooms in
...
export default function Hero({
  ...
  // Scale factor — renders desktop layout at natural size when wide enough, scales down on smaller screens
  const [scale, setScale] = useState(1);
  useEffect(() => {
    function updateScale() {
      setScale(Math.min(1, window.innerWidth / REFERENCE_WIDTH));
    }
    updateScale();
  ...
  // Desktop reference height for the hero
  const DESKTOP_HEIGHT = 500;

  return (
    <div
      className="relative w-full z-50 overflow-hidden bg-cover bg-center"
      style={{
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 max-w-[2520px] mx-auto h-full px-12">
          {/* Top bar with Search */}
          <div className="absolute top-0 left-0 right-0 flex items-center justify-between py-6 px-12 gap-3 z-20">
            <button
              aria-label="Home"
              className="flex items-center justify-center overflow-hidden w-16 h-16 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-black/40 rounded-xl"
            >
              <img
                src="/khubo Logo.png"
                alt="Khubo Logo"
            ...
            <button
              ...
              className="flex items-center justify-center w-16 h-16 bg-transparent text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white rounded-full"
            >
          ...
          <div className="flex flex-col items-center justify-center text-center h-full">
            <h1 className="flex flex-row items-center justify-center gap-x-6 text-white px-4 w-full">
              <span className="font-noto-serif italic tracking-[0.3em] opacity-80" style={{ fontSize: 'clamp(1.1rem, 4.5vw, 2.2rem)' }}>
                WELCOME TO
              </span>
              <span className="font-roboto font-bold tracking-[0.08em]" style={{ fontSize: 'clamp(1.25rem, 5vw, 2.2rem)' }}>
                KHUBO
              </span>
            </h1>

            <div
              className="relative mt-12 w-full max-w-none flex justify-center"
              ref={dropdownRef}
            >
              ...
              {/* Pill search bar */}
              <div
                className="bg-white/10 backdrop-blur-md border border-white/20 p-2 rounded-full flex items-center text-white shadow-2xl w-[min(70vw,800px)] relative z-[95] transition-all duration-300 pointer-events-auto cursor-default"
              >
                {isSearchActive ? (
                  <>
                  ...
                  <>
                    ...
                    <div className="flex-1 min-w-0">
                      <div
                        ...
                        className={`w-full flex items-center justify-between px-1 md:pl-6 md:pr-4 py-3.5 transition-all cursor-pointer group select-none focus-visible:outline-none ${
