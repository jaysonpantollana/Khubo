---
name: copilot-sdk
description: "Use when working with copilot-sdk"
---
...
- **GitHub Copilot subscription** (Individual, Business, or Enterprise) â€” not required for BYOK
...
## Core Pattern: Client â†’ Session â†’ Message
...
const getWeather = defineTool("get_weather", {
    ...
    handler: async ({ city }) => ({ city, temperature: "72Â°F", condition: "sunny" }),
});
...
async def get_weather(params: GetWeatherParams) -> dict:
    return {"city": params.city, "temperature": "72Â°F", "condition": "sunny"}
...
getWeather := copilot.DefineTool("get_weather", "Get weather for a city",
    func(params WeatherParams, inv copilot.ToolInvocation) (WeatherResult, error) {
        return WeatherResult{City: params.City, Temperature: "72Â°F"}, nil
    },
)
...
var getWeather = AIFunctionFactory.Create(
    ([Description("The city name")] string city) => new { city, temperature = "72Â°F" },
...
1. **Explicit token** â€” `githubToken` in constructor
2. **Environment variables** â€” `COPILOT_GITHUB_TOKEN` â†’ `GH_TOKEN` â†’ `GITHUB_TOKEN`
3. **Stored OAuth** â€” From `copilot auth login`
4. **GitHub CLI** â€” `gh auth` credentials
...
Use your own API keys â€” no Copilot subscription required.
...
**Common issues:**
- `CLI not found` â†’ Install CLI or set `cliPath`
- `Not authenticated` â†’ Run `copilot auth login` or provide `githubToken`
- `Session not found` â†’ Don't use session after `destroy()`
- `Connection refused` â†’ Check CLI process, enable `autoRestart`
