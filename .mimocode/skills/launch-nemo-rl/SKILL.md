---
name: launch-nemo-rl
description: "Use when working with launch-nemo-rl"
---
...
# launch-nemo-rl â€” running NeMo-RL recipes on Kubernetes via nrl-k8s

This is the playbook for the `nrl-k8s` CLI at `infra/nrl_k8s/`. Follow it when the user asks to launch / iterate / debug a NeMo-RL recipe on a Kubernetes cluster. Verify current state (`kubectl`, `git log`, the recipe + infra files) before acting â€” the cluster is shared and the cost of a wrong action is high.
...
- **Recipe** (e.g. `qwen3_30b_math_8n_4gpu.yaml`) â€” NeMo-RL config: model, GRPO/SFT knobs, `cluster.{gpus_per_node,num_nodes}`. Uses `defaults:` to inherit from `examples/configs/recipes/llm/...`.
- **Infra** (e.g. `*.<profile>.infra.yaml`) â€” K8s/Ray shape: namespace, image, service account, RayCluster spec under `kuberay:`, optional Deployments under `deployments:`, `submit.submitter`, `launch.{mode,codeSource,codePath,entrypoint}`. Pair names follow `<recipe>.<profile>[.prod].infra.yaml` where `<profile>` names the hardware target (e.g. `gb300`).

Example pairs in `infra/nrl_k8s/examples/` â€” read the neighbouring files to see the current conventions for the target profile.
...
```
--mode interactive   â†’ --submitter portForward  --code-source upload  (tails logs)
--mode batch         â†’ --submitter exec         --code-source image   (returns after nohup)
```
...
- **Code source**: `upload` stages a working_dir from the laptop (Ray 100 MiB cap). `image` / `lustre` expect code on the pod's filesystem â€” paired with `--code-path` (typically `/opt/nemo-rl`), which is a subPath of the shared-filesystem PVC mount in the standard infra examples.
- **Wait**: `--wait` tails logs until terminal; `--no-wait` returns as soon as the driver is running.

Other long-lived-only flags:

- `--replace` â€” stop any running training / daemon job before submitting new ones (suffixes daemon submissionIds with a timestamp so Ray accepts the resubmit).
- `--recreate` â€” delete + re-apply a RayCluster whose live spec has drifted from the rendered manifest (default is warn + reuse).
- `--skip-daemons` â€” bring up all declared clusters but only submit training. Use on disagg recipes where gym/generation are already healthy.

Gotcha: on infra where the entrypoint does `cd /opt/nemo-rl` (or another in-image / Lustre path) and loads the recipe from there, **`--code-source upload` does NOT override the recipe on the pod** â€” the uploaded working_dir sits in `/tmp/ray/...` but the entrypoint `cd`s away from it. To actually test a local recipe change, either sync your edits to the shared filesystem mounted into the pods or flip the Hydra overrides in the entrypoint.

## 4. Ephemeral mode flags (`--rayjob`)

When `--rayjob` is set, `run` branches into the RayJob code path. Relevant flags:

- `--rayjob-name NAME` â€” RayJob metadata name (defaults to the training cluster name).
- `--shutdown / --no-shutdown` â€” default `true`: KubeRay deletes the RayCluster once the Ray Job reaches a terminal state.
- `--ttl SECONDS` â€” default 3600s: keep the RayJob object around after the run finishes for post-mortem log access.
- `--wait / --no-wait` â€” default `wait`: poll `jobDeploymentStatus` until Complete/Failed. `--no-wait` returns as soon as the RayJob is applied.
- `--timeout SECONDS` â€” default 86400s (24h): bound the `--wait` poll.
- `--dry-run` â€” render the RayJob manifest and print it; do not apply.
...
**Escape `${â€¦}`** with a backslash. OmegaConf otherwise interprets it as interpolation and errors on shell-style `${VAR:-default}`. `RUN_ID` resolves to `RAY_JOB_SUBMISSION_ID` (injected by KubeRay in rayjob mode) â†’ `NRL_K8S_RUN_ID` (injected by the CLI in long-lived mode) â†’ local timestamp â€” so the name is unique across either path.

## 6. Per-profile concerns (hardware + scheduler + DRA)

Every infra YAML encodes a hardware/scheduler profile. The concrete examples in `infra/nrl_k8s/examples/` are authoritative for the profiles they target â€” read the neighbouring infra file before writing a new one. Things that commonly vary:

- **Per-node GPUs** (e.g. 4 vs 8) â€” must match `cluster.gpus_per_node` in the recipe, otherwise workers stay `Pending`.
- **Node selectors** â€” head pods usually land on a CPU-only node pool; GPU workers match on `nvidia.com/gpu.product` or a node-group label.
- **Scheduler** â€” KAI (`schedulerName: kai-scheduler` + `kai.scheduler/queue` label) with topology annotations (`kai.scheduler/topology`, `kai.scheduler/topology-required-placement`) gang-schedules workers into one clique. Without it, pods may land on different racks and NVLink/RoCE won't span them.
- **DRA claims** â€” ComputeDomain + RoCE are attached via `resourceClaims` referencing `ResourceClaimTemplate`s. The CLI auto-creates/deletes these when the worker pod spec contains DRA claim references â€” no manual setup needed.
- **Secrets** â€” always via `secretKeyRef` (`wandb-api-key`, image pull secret). Never embed.
- **Shared filesystem mounts** â€” typically a Lustre PVC mounted twice: once at the code path (e.g. `/opt/nemo-rl` with a user-scoped `subPath`) and once at a workspace root (e.g. `/mnt/rl-workspace`) for datasets, HF cache, and checkpoints.
...
# Edits in the recipe? Just re-run â€” reuses the live cluster.
...
The `deployments:` section in infra YAML declares Kubernetes Deployments managed alongside RayClusters. The CLI patches image, imagePullSecrets, and serviceAccountName from the top-level infra keys (same as RayClusters). Deployments start in parallel with cluster bring-up â€” no ordering dependency.
...
`type=DRIVER` with `submission_id=null` means an exec-submitter run (no dashboard log endpoint â€” use `nrl-k8s job logs` instead). `type=SUBMISSION` has `submission_id` set and `/api/jobs/<id>/logs` works.
...
| A RayJob (ephemeral)             | `kubectl delete rayjob <name> -n default` â€” only if `shutdownAfterJobFinishes` didn't fire |
...
- **DTensor vs Megatron** â€” MoE recipes typically use `megatron_cfg.enabled=true`; ensure `dtensor_cfg.enabled=false` in inherited defaults.
- **Shared filesystem vs git divergence** â€” `codeSource: image|lustre` reads from the pod filesystem. If your local edits aren't on the shared filesystem the pods mount, the run is testing the on-disk version, not yours. Either sync via a helper pod (head pod exec is often blocked) or override via Hydra flags.
...
- **Dashboard blank page** â€” Ray 2.52 installs dashboard assets as symlinks by default; `nrl-k8s cluster dashboard <name>` auto-reinstalls `ray[default] --link-mode=copy` to fix it. Bake `ENV UV_LINK_MODE=copy` in the image to avoid this entirely.
- **`kubectl exec` is usually blocked** in automation â€” route around with `kubectl get ... -o yaml`, `kubectl logs`, and `kubectl port-forward` + Ray dashboard APIs.
...
3. Driver log contains `wandb.ai/<project>/runs/<id>` (if wandb is enabled) â€” share the URL with the user.
...
# Create pod and exec in (idempotent â€” reuses existing pod)
nrl-k8s dev connect

# Switch image (must stop first â€” image change is warned but not auto-applied)
nrl-k8s dev stop
...
- Tests: `infra/nrl_k8s/tests/unit/` â€” run with `uv run --extra test pytest -x -q` from `infra/nrl_k8s/`.
- Recipe + infra examples: `infra/nrl_k8s/examples/`.
- Base recipes this tool wraps: `examples/configs/recipes/llm/â€¦` and `examples/nemo_gym/â€¦`.
