---
name: nosql-expert
description: "Use when working with nosql-expert"
---
...
âŒ **Scatter-Gather:** Querying *all* partitions to find one item (Scan).
âŒ **Hot Keys:** Putting all "Monday" data into one partition.
âŒ **Relational Modeling:** Creating `Author` and `Book` tables and trying to join them in code. (Instead, embed Book summaries in Author, or duplicate Author info in Books).
