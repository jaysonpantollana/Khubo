...
description: "Use when working with odoo-l10n-compliance"
---
...
|---|---|---|
| ðŸ‡ºðŸ‡¸ USA | `l10n_us` | GAAP CoA, Payroll (ADP bridge), 1099 reporting |
| ðŸ‡²ðŸ‡½ Mexico | `l10n_mx_edi` | CFDI 4.0 e-invoicing, SAT integration, IEPS tax |
| ðŸ‡ªðŸ‡¸ Spain | `l10n_es` | SII real-time VAT, Modelo 303/390, AEAT |
| ðŸ‡®ðŸ‡¹ Italy | `l10n_it_edi` | FatturaPA XML, SDI submission, reverse charge |
| ðŸ‡µðŸ‡± Poland | `l10n_pl` | SAF-T JPK_FA, VAT-7 return |
| ðŸ‡§ðŸ‡· Brazil | `l10n_br` | NF-e, NFS-e, SPED, ICMS/PIS/COFINS |
| ðŸ‡©ðŸ‡ª Germany | `l10n_de` | SKR03/SKR04 CoA, DATEV export, UStVA |
| ðŸ‡¨ðŸ‡´ Colombia | `l10n_co_edi` | DIAN e-invoicing, UBL 2.1 |
...
```
Menu: Accounting â†’ Configuration â†’ Taxes â†’ New
...
Fiscal Position (created separately):
  ...
  Tax Mapping: Standard VAT Rate â†’ 0% Intra-Community
```
...
# Verify in Odoo:
# Apps â†’ Installed â†’ Search "l10n_mx" â†’ Should show as Installed
```

## Best Practices

- âœ… **Do:** Install the localization module **before** creating any accounting entries â€” it sets up the correct accounts.
- âœ… **Do:** Use **Fiscal Positions** to automate tax switching for international customers (B2B vs B2C, domestic vs export).
- âœ… **Do:** Test e-invoicing in the **SAT/tax authority test environment** before going live.
- âŒ **Don't:** Manually create a chart of accounts if a localization module exists for your country.
- âŒ **Don't:** Mix localization tax accounts with custom accounts â€” it breaks fiscal reports.
