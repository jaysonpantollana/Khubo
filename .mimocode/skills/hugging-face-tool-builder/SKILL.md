...
description: "Use when working with hugging-face-tool-builder"
---
...
Reference examples:
- `references/hf_model_papers_auth.sh` â€” uses `HF_TOKEN` automatically and chains trending â†’ model metadata â†’ model card parsing with fallbacks; it demonstrates multi-step API usage plus auth hygiene for gated/private content.
- `references/find_models_by_paper.sh` â€” optional `HF_TOKEN` usage via `--token`, consistent authenticated search, and a retry path when arXiv-prefixed searches are too narrow; it shows resilient query strategy and clear user-facing help.
- `references/hf_model_card_frontmatter.sh` â€” uses the `hf` CLI to download model cards, extracts YAML frontmatter, and emits NDJSON summaries (license, pipeline tag, tags, gated prompt flag) for easy filtering.

Baseline examples (ultra-simple, minimal logic, raw JSON output with `HF_TOKEN` header):
- `references/baseline_hf_api.sh` â€” bash
- `references/baseline_hf_api.py` â€” python
- `references/baseline_hf_api.tsx` â€” typescript executable

Composable utility (stdin â†’ NDJSON):
- `references/hf_enrich_models.sh` â€” reads model IDs from stdin, fetches metadata per ID, emits one JSON object per line for streaming pipelines.
