...
description: "Use when working with odoo-backup-strategy"
---
...
echo "âœ… Backup complete: db_$DATE.dump + filestore_$DATE.tar.gz"
```
...
# Step 5: Verify â€” open Odoo in the browser and check:
#   - Can you log in?
...
- âœ… **Do:** Test restores monthly in a staging environment â€” a backup you've never restored is not a backup.
- âœ… **Do:** Follow the **3-2-1 rule**: 3 copies, 2 different media types, 1 offsite copy (e.g., S3 or a remote server).
- âœ… **Do:** Back up **immediately before every Odoo upgrade** â€” this is your rollback point.
- âœ… **Do:** Verify backup integrity: `pg_restore --list backup.dump` should complete without errors.
- âŒ **Don't:** Back up only the database without the filestore â€” all attachments and images will be missing after a restore.
- âŒ **Don't:** Store backups on the same disk or same server as Odoo â€” a disk or server failure destroys both.
- âŒ **Don't:** Run `pg_restore --clean` against a non-existent database â€” always create the database first.

## Limitations

- Does not cover **Odoo.sh built-in backups** â€” Odoo.sh has its own backup system accessible from the dashboard.
