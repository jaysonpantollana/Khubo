...
description: "Use when working with tao-convert-dataset-format"
---
...
**not** trigger for non-DAFT â†’ DAFT conversion (COCO, YOLO, Data
Factory JSONL) â€” redirect to the upstream `nvidia-tao-daft` repo's
converter skills.
...
2. **Path and output are flags** â€” `--path PATH` (source),
   `--output OUTPUT` (destination). Both required at the leaf;
   passing positionally fails.
3. **`--path` accepts both granularities** â€” a single scene/dataset
   or a parent directory; the converter walks the tree.
4. **Per-pair flags live at the leaf** â€” flag sets differ between
   targets (e.g. media-handling). Always check the leaf `--help`.

**Operating procedure:**

1. `tao-daft --version` â€” confirm install, pin version in any report.
2. `tao-daft convert --help` â€” list supported source formats.
3. `tao-daft convert <source> --help` â€” list valid targets for that
   source.
...
5. `tao-daft convert <source> <target> --help` â€” pick flags for the
...
converted dataset is written under `--output` â€” spot-check it with
...
- Supported pairs are whatever `--help` reports for the installed
  version â€” don't pass an unconfirmed pair.
- Source and target are positional; `--path` / `--output` are flags.
- `convert` only â€” `validate` and `info` have their own skills.
- Do not reimplement conversion in Python; the CLI is the spec.

## Troubleshooting

- **`tao-daft: command not found`** â€” wheel not installed; `pip
  install nvidia-tao-daft`, verify with `tao-daft --version`.
- **`error: argument --path/--output is required`** â€” passed
  positionally; move behind the flag.
- **`invalid choice: '<format>'`** â€” slug not wired up in this
  version. Re-run the relevant `--help`.
- **Output rejected by `tao-daft validate`** â€” re-check per-pair
