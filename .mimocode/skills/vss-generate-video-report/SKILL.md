...
description: "Use when working with vss-generate-video-report"
---

---
name: vss-generate-video-report
description: Use this skill when producing a VSS analysis report â€” Mode A per-clip VLM, Mode B incident-range via video-analytics. Not for standalone video summarization, real-time alerts or ad-hoc Q&A.
license: Apache-2.0
metadata:
  version: "3.2.0"
...
Generate a video analysis report by routing to one of two backends â€” **never via** `POST /generate` on the VSS agent.

| Mode | Backend |
|---|---|
| **A. Video clip** | `/vss-manage-video-io-storage` â†’ clip URL â†’ **VLM chat/completions** |
| **B. Incident range** | `/vss-query-analytics` â†’ incident list â†’ narrative report |
...
1. **Pick the mode** â€” Mode A for a single recorded clip/sensor video, Mode B when the request names a time range or incidents/alerts (match against *Examples*).
...
3. **Run that mode's numbered steps** â€” *Mode A* or *Mode B* below.
...
- "Generate a report for this video" / "report on `<sensor-id>`" â†’ **Mode A**
- "Analyze warehouse_01.mp4" / "create an analysis report on the uploaded video" â†’ **Mode A**
- "Report on incidents from 12:31Z to 12:32Z" â†’ **Mode B**
- "Report on alerts today" / "what incidents happened on `<sensor>` last hour" â†’ **Mode B**
- "Summarize alerts on `<sensor>` between `<t1>` and `<t2>`" â†’ **Mode B**
...
- Ad-hoc visual Q&A on a clip that do not ask explicitly for a report ("what color is the truck?", "what happens at 00:12?") â†’ use `/vss-ask-video`.
- Archive/semantic similarity retrieval ("find forklifts", "search all videos for tailgating") â†’ use `/vss-search-archive`.
- Read-only incident/metrics lookup without report rendering needs â†’ use `/vss-query-analytics`.
- Deploy/teardown/profile changes ("deploy alerts", "switch profile", "bring up base") â†’ use `/vss-deploy-profile`.
- Real-time alert/rule management requests â†’ use `/vss-manage-alerts`.
...
profile `.env` â€” Brev or bare-metal â€” so the report-link rewrite is:
...
## Mode A â€” Report on a recorded video clip

**If the VSS `lvs` profile is deployed** â€” `curl -sf --max-time 5 "http://${HOST_IP}:38111/v1/ready"` returns HTTP 200 â€” run `/vss-summarize-video` to produce the summary, then paste its output into the report template in Step 4 and skip Steps 1â€“3 (the VLM-direct path). Run Steps 1â€“3 only when `/v1/ready` is non-200.

### Step 1 â€” Resolve the clip URL
...
3. Request a clip URL:
   ...
   That gives a direct `mp4` URL that the local / in-cluster VLM can pull frames from. Bind it to `VIDEO_URL` (used by the VLM in Step 3) and set `RAW_URL="$VIDEO_URL"` before applying the report-link rewrite to produce `BROWSER_CLIP_URL` for Step 4 â€” the user's browser cannot reach `$VIDEO_URL` directly.
...
### Step 2 â€” Resolve VLM endpoint and model

The deploy may serve the VLM through either of two stacks. Both expose an OpenAI-compatible `chat/completions` API â€” pick whichever is live:

| Backend | Env vars | Typical host endpoint | Picked when |
|---|---|---|---|
| **NIM Cosmos** | `VLM_BASE_URL`, `VLM_NAME`, `VLM_MODE`, `VLM_MODEL_TYPE` | `${VLM_BASE_URL}/v1` (no trailing `/v1` on the env var; the agent appends it) | `VLM_MODEL_TYPE != rtvi` **and** `VLM_MODE` âˆˆ {`local`, `local_shared`, `remote`} **and** `VLM_BASE_URL` is non-empty |
| **RT-VLM Cosmos** | `RTVI_VLM_BASE_URL`, `RTVI_VLM_MODEL_TO_USE`, `VLM_MODEL_TYPE` | `${RTVI_VLM_BASE_URL}/v1` â€” if unset, derive from `${HOST_IP}` (`http://${HOST_IP}:8018/v1` for alerts, `http://${HOST_IP}:30082/v1` for base) | `VLM_MODEL_TYPE = rtvi`, or `VLM_MODE=none`, or `VLM_BASE_URL` empty; also the only path for `warehouse` |

Read the live values off the running agent container â€” do not guess:
...
If the probe fails or the listed ids don't include `${VLM_MODEL}`, fall back to the other backend (or surface the error â€” never silently pick a model that isn't on the server).

### Step 3 â€” Call the VLM directly

Use the OpenAI-compatible `chat/completions` endpoint with a `video_url` content block â€” the same payload shape **and multimodal settings** `video_understanding` builds in `src/vss_agents/tools/video_understanding.py` (`_build_vlm_messages` + the Cosmos `base_vlm.bind(...)` call).

The frame sampling and visual-token (pixel) budget must mirror the **live** `video_understanding` settings for the active profile. **Send `mm_processor_kwargs` and `media_io_kwargs`** so the direct call uses the same frame sampling and pixel budget as the in-agent `video_understanding` tool â€” omitting them lets the VLM apply its own defaults, so the output diverges from the agent path.

```bash
PROMPT='Describe in detail what happens in the video, with timestamps (startâ€“end in seconds from clip start) for each segment or event. Cover scenes, objects, people, vehicles, and notable actions.'

# Reasoning is OFF by default â€” matches the base-profile video_understanding config (`reasoning: false`).
...
# Multimodal settings â€” resolve from the live agent config file path, not hardcoded candidates.
CFG_JSON=$(
...
# num_frames = min(int(clip_seconds) * max_fps, max_frames), min 1 â€” matches video_understanding.py.
# clip_seconds (Step 1 endTime-startTime) may be fractional; truncate to integer seconds â€” bash $((...))
...
If the VLM returns a `<think>â€¦</think>` block (Cosmos Reason reasoning mode), keep only the text after `</think>` as the report body.

### Step 4 â€” Fill the Video Analysis Report template
...
## Mode B â€” Report on incidents in a time range

### Step 1 â€” Resolve the time range and (optionally) sensor
...
### Step 2 â€” Fetch incidents via `/vss-query-analytics`

Hand off to `/vss-query-analytics` (initialize â†’ `tools/call`) with:
...
For each incident keep: `id`, `sensorId`, `timestamp`, `end`, `category`, `place.name`, `info.verdict`, `info.reasoning`, `objectIds`, and the clip URL (commonly `info.clip_url`, `clip_url`, or whichever clip-pointer field the response carries). **Apply the `$VSS_PUBLIC_HOST:$VSS_PUBLIC_PORT` rewrite (see *Browser-playable clip URL* above) to every clip URL before pasting it into the report** â€” the raw value is a `HOST_IP:30888` URL the user's browser cannot reach.

### Step 3 â€” Fill the Incident Range Report template
...
- **`/vss-manage-video-io-storage`** â€” sensor list, timelines, and clip URL for Mode A Step 1.
- **`/vss-query-analytics`** â€” incident retrieval (and verdict / reasoning enrichment) for Mode B Step 2.
- **`/vss-ask-video`** â€” ad-hoc VLM Q&A on a single clip (not a structured report).
- **`/vss-summarize-video`** â€” used by Mode A to produce the summary body when the `lvs` profile is deployed; the report template (Step 4) is still filled here.
