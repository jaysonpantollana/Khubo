---
name: schema-markup
description: "Use when working with schema-markup"
---
...
- **0â€“15% of band:** Schema describes none of the visible content (e.g. you would mark `description` for a `Recipe` page that has no recipe markup yet).
- **16â€“40% of band:** Partial alignment â€” the schema describes some but not all of the visible content, OR maps to a less-common schema.org type.
- **41â€“80% of band:** Strong alignment â€” the schema describes the bulk of the visible content with a common schema.org type.
- **81â€“100% of band:** Exemplary â€” the schema covers all visible content, uses a Google-supported rich-result type, and includes all required properties.

Sum the per-category scores to compute the Eligibility Index used in Â§"Eligibility Bands" below.
