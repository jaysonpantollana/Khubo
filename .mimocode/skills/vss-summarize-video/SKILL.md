...
description: "Use when working with vss-summarize-video"
---
...
- Live RTSP captioning â€” use `vss-deploy-dense-captioning`.
- Report generation, including incident or alert-window reports â€” use `vss-generate-video-report` Mode B.
- Semantic search across the archive â€” use `vss-search-archive`.
...
- Direct VLM fallback uses a single fixed prompt and cannot target
  scenario/events â€” output quality is lower than the LVS path.
...
*Setup â†’ Availability checks* below). **Duration does not drive routing.**
...
Fallback message when the LVS service is unreachable â€” copy verbatim above the summary:

> âš  **Note:** Input video `<name>` is `<N>`s long.
...
`/v1/ready` probe (see *Setup â†’ Availability checks*) returns anything
...
- **Yes** â†’ hand off to `/vss-deploy-profile`, then re-probe and continue with Step 2 (LVS + HITL).
- **No** â†’ go straight to **Step 2 fallback (VLM with default prompt)** and prepend the Routing fallback note. Do not ask again, and do not run scenario/events HITL.
- **Pre-authorized to deploy autonomously** (caller said so explicitly) â†’ skip the confirmation and invoke `/vss-deploy-profile` directly.
- **Pre-authorized to use VLM fallback** ("skip lvs, just use the VLM") â†’ go straight to Step 2 fallback without prompting.
...
- VLM / RT-VLM: `${VLM_BASE_URL}` â€” default `${RTVI_VLM_BASE_URL:-http://${HOST_IP:-localhost}:8018}`
- LVS service: `${LVS_BACKEND_URL}` â€” default `http://${HOST_IP:-localhost}:38111`
- VIOS: owned by `vss-manage-video-io-storage`; refer there.

Use env vars when set (strip trailing `/v1` from the VLM base â€” the skill appends it). Otherwise use the defaults. If neither works, ask the user â€” do not scan ports or read config files to guess.
...
**Readiness is determined by the HTTP status code only** â€” the LVS
...
- `video_sum_code = 200` â†’ **Step 2 (LVS + HITL)** for every video.
- `video_sum_code != 200`, `vlm_code = 200` â†’ **Step 2 fallback (VLM)**; prepend the Routing fallback note.
- `vlm_code != 200` â†’ fail; at least one backend must be reachable.
...
**Use the `vss-manage-video-io-storage` skill for all VIOS interactions** â€” it
...
This step is a sub-task â€” do NOT end your turn here; do NOT return the clip
URL as the final answer. From VIOS collect three values:

1. **`streamId`** (via `sensor/list` â†’ `sensor/<id>/streams`, or directly from an upload response).
...
3. **Temporary MP4 clip URL** â€” the `/storage/file/<streamId>/url` variant with `container=mp4`. Response field: `.videoUrl`. Both backends need an HTTP(S) URL they can `GET`.

Everything else (auth, upload, `disableAudio`, expiry, etc.) lives in the
`vss-manage-video-io-storage` skill â€” refer users there if VIOS fails.

---

## Step 2 â€” Primary: video summarization microservice with HITL
...
### HITL: collect scenario and events first (REQUIRED â€” do not skip)
...
**verbatim** â€” do not infer from filename or sensor name. Note the
...
If both `video_summary` and `events` are empty, the clip probably doesn't contain the requested events â€” re-run with broader `scenario`/`events`, don't report "no content".
...
## Step 2 fallback â€” VLM direct with default prompt

Use this path **only** when `/v1/ready` did not return 200 after warmup. Do NOT run HITL â€” the user did not opt in; you fell back because the service was missing. Prepend the Routing fallback note to the response.
...
- **VLM** returns an OpenAI chat-completion envelope; summary is
  `choices[0].message.content`.
- **LVS service** returns the same envelope but `content` is a JSON string â€”
...
- **Errors** surface as HTTP non-2xx plus JSON `{error: ...}`. LVS `503` usually
  means warmup â€” retry `/v1/ready`.

### Presenting the output to the user

Surface backend output with **minimal transformation** â€” do not paraphrase,
re-voice, add emojis, or reformat. **One backend call â†’ one rendering**: no
...
report â€” rewriting loses fidelity). Render each `events` entry with its
...
produced `<think>â€¦</think><answer>â€¦</answer>` blocks, drop the `<think>`
...
- **Route by service availability, not by duration.** Probe `/v1/ready` once
  in Setup; HTTP 200 â†’ LVS+HITL for every clip; anything else â†’ VLM fallback.
...
- **Readiness = HTTP 200 on `/v1/ready`. Nothing else.** Body may be empty.
  Always use `curl -s -o /dev/null -w '%{http_code}'` â€” never pipe through
  `jq`/`grep`/`head`.
- **Delegate VIOS to `vss-manage-video-io-storage`** â€” it is a sub-task; the
...
- **Render output verbatim** â€” no paraphrasing, no reformatting, no rewriting
  the `video_summary` or `choices[0].message.content`.
- **One call, one render.** No parallel hedging, no double renderings.

## Cross-reference

- **vss-deploy-profile** â€” bring up the `base` (VLM only) or `lvs` (VLM + video summarization service) profile
- **vss-manage-video-io-storage** (VIOS API) â€” upload videos, list streams, get clip URLs
- **vss-search-archive** â€” semantic search across the archive (different profile)
- **vss-query-analytics** â€” query incidents/events from Elasticsearch
- **video summarization API reference** â€” [`references/video-summarization-api.md`](references/video-summarization-api.md)
- **video summarization service ops reference** â€” [`references/video-summarization-deployment.md`](references/video-summarization-deployment.md)
