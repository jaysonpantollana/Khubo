---
name: nv-reason-cxr
description: "Use when working with nv-reason-cxr"
---
...
- `--backend local|hf-space-api` â€” inference backend, default `local`.
- `--model-id` â€” Hugging Face model id, default `nvidia/NV-Reason-CXR-3B`.
- `--device auto|cuda|cpu` â€” default `auto`, using CUDA when available.
- `--allow-cpu` â€” required for live CPU inference; CPU runs can be very slow.
- `--torch-dtype auto|float16|bfloat16|float32` â€” default `auto`, using
  bfloat16 on CUDA and float32 on CPU, matching the published BF16 model.
- `--max-new-tokens` â€” generation cap, default 2048.
- `--local-files-only` â€” use only locally cached Hugging Face assets.
- `--mock` â€” deterministic dry-run response for CI and wiring checks.
- `--prompt-preset findings|comprehensive|educational|structured` â€” optional
  known-good prompt presets from the model card/demo behavior.
- `--out-dir` â€” optional artifact directory. Required for generated JSON
