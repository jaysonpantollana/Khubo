...
description: "Use when working with github-actions-advanced"
---
...
- The user is working with GitLab CI/CD â†’ recommend `gitlab-ci-patterns`
- The user is working with CircleCI, Jenkins, or other CI platforms
- The task is purely about Docker image building without GitHub context â†’ recommend `docker-expert`
- The task is about Kubernetes deployment configuration â†’ recommend `kubernetes-architect`
...
permissions:                 # Always declare â€” principle of least privilege
  contents: read
...
jobs:
  job-id:
    ...
    runs-on: ubuntu-24.04    # Pin OS version â€” never use -latest in prod
    timeout-minutes: 15      # Always set â€” prevents runaway jobs
...
on:
  ...
  pull_request_target:                   # Runs with repo secrets â€” use with care!
...
### Language Setup Actions (Preferred â€” No Extra Step Needed)
...
- **Required reviewers** â€” manual approval gate before run
- **Wait timer** â€” delay after approval (e.g., 10-minute buffer)
- **Branch/tag restrictions** â€” only `main` or `v*` tags can deploy to prod
- **Environment-specific secrets** â€” override repo-level secrets per environment
- **Deployment branches** â€” whitelist which branches can target this environment
...
# Auto-provided token â€” no setup needed
...
```bash
# Actions Runner Controller (Kubernetes) â€” recommended for ephemeral runners
helm install arc \
...
```yaml
# Workflow-level default â€” restrict everything
permissions:
  contents: read

jobs:
  publish:
    # Job-level override â€” only expand what's needed
    permissions:
...
```yaml
# âŒ UNSAFE â€” tag can be mutated or hijacked
- uses: actions/checkout@v4

# âœ… SAFE â€” commit SHA is immutable
- uses: actions/checkout@11bd71901bbe5b1630ceea73d27597364c9af683  # v4.2.2
...
```yaml
# âŒ UNSAFE â€” attacker controls PR title, which gets expanded in shell
- run: echo "${{ github.event.pull_request.title }}"

# âœ… SAFE â€” pass through environment variable (shell doesn't evaluate it)
- env:
    PR_TITLE: ${{ github.event.pull_request.title }}
  run: echo "$PR_TITLE"

# âœ… SAFE â€” expressions in if: conditions are evaluated by Actions, not shell
...
```yaml
# Only run when a maintainer adds a specific label â€” prevents untrusted execution
on:
  pull_request_target:
    types: [labeled]
...
```yaml
# Add to every workflow â€” hardens runner, monitors outbound traffic
...
- name: Debug â€” dump github context
  if: runner.debug == '1'
  env:
...
- name: Debug â€” dump all contexts
  if: runner.debug == '1'
  run: |
...
### Pattern 1: Build â†’ Test â†’ Push â†’ Deploy
...
jobs:
  # â”€â”€ Build & Test â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  build-test:
    runs-on: ubuntu-24.04
    timeout-minutes: 20
    permissions:
  ...
  # â”€â”€ Push Image (main branch only) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  push-image:
    needs: build-test
  ...
  # â”€â”€ Deploy Staging â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  deploy-staging:
    needs: push-image
    runs-on: ubuntu-24.04
    timeout-minutes: 30
  ...
  # â”€â”€ Deploy Production (manual approval required) â”€â”€â”€â”€â”€â”€
  deploy-production:
    needs: deploy-staging
    runs-on: ubuntu-24.04
    timeout-minutes: 30
...
- `gha-security-review` â€” Deep security audit of existing workflow files
- `github-actions-templates` â€” Copy-paste ready workflow templates
- `docker-expert` â€” Container build optimization and Dockerfile best practices
- `kubernetes-architect` â€” Deploying to Kubernetes from GitHub Actions
- `gitlab-ci-patterns` â€” GitLab CI/CD equivalent patterns
