...
description: "Use when working with azure-resource-manager-redis-dotnet"
---
...
â””â”€â”€ SubscriptionResource
    â””â”€â”€ ResourceGroupResource
        â””â”€â”€ RedisResource
            â”œâ”€â”€ RedisFirewallRuleResource
            â”œâ”€â”€ RedisPatchScheduleResource
            â”œâ”€â”€ RedisLinkedServerWithPropertyResource
            â”œâ”€â”€ RedisPrivateEndpointConnectionResource
            â””â”€â”€ RedisCacheAccessPolicyResource
```
...
8. **Enable TLS 1.2 minimum** â€” set `MinimumTlsVersion = RedisTlsVersion.Tls1_2`
9. **Disable non-SSL port** â€” set `EnableNonSslPort = false` for security
10. **Rotate keys regularly** â€” use `RegenerateKeyAsync` and update connection strings
...
1. **SKU downgrades not allowed** â€” You cannot downgrade from Premium to Standard/Basic
2. **Clustering requires Premium** â€” Shard configuration only available on Premium SKU
3. **Geo-replication requires Premium** â€” Linked servers only work with Premium caches
4. **VNet injection requires Premium** â€” Virtual network support is Premium-only
5. **Patch schedules require Premium** â€” Maintenance windows only configurable on Premium
6. **Cache name globally unique** â€” Redis cache names must be unique across all Azure subscriptions
7. **Long provisioning times** â€” Cache creation can take 15-20 minutes; use `WaitUntil.Started` for async patterns
