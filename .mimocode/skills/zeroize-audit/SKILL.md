---
name: zeroize-audit
description: "Use when working with zeroize-audit"
---
...
| `config` | no | â€” | YAML defining heuristics and approved wipes |
...
| `max_tus` | no | â€” | Limit on translation units processed from compile DB |
| `mcp_mode` | no | `prefer` | `off`, `prefer`, or `require` â€” controls Serena MCP usage |
...
| `mcp_timeout_ms` | no | â€” | Timeout budget for MCP semantic queries |
...
|---|---|
| `compile_commands.json` at `compile_db` path | Fail fast â€” do not proceed |
| `clang` on PATH | Fail fast â€” IR/ASM analysis impossible |
...
| `{baseDir}/tools/extract_compile_flags.py` | Fail fast â€” cannot extract per-TU flags |
| `{baseDir}/tools/emit_ir.sh` | Fail fast â€” IR analysis impossible |
...
| Prerequisite | Failure mode if missing |
|---|---|
| `Cargo.toml` at `cargo_manifest` path | Fail fast â€” do not proceed |
| `cargo check` passes | Fail fast â€” crate must be buildable |
| `cargo +nightly` on PATH | Fail fast â€” nightly required for MIR and LLVM IR emission |
| `uv` on PATH | Fail fast â€” required to run Python analysis scripts |
| `{baseDir}/tools/validate_rust_toolchain.sh` | Warn â€” run preflight manually. Checks all tools, scripts, nightly, and optionally `cargo check`. Use `--json` for machine-readable output, `--manifest` to also validate the crate builds. |
| `{baseDir}/tools/emit_rust_mir.sh` | Fail fast â€” MIR analysis impossible (`--opt`, `--crate`, `--bin/--lib` supported; `--out` can be file or directory) |
| `{baseDir}/tools/emit_rust_ir.sh` | Fail fast â€” LLVM IR analysis impossible (`--opt` required; `--crate`, `--bin/--lib` supported; `--out` must be `.ll`) |
...
| `{baseDir}/tools/scripts/check_rust_asm.py` | Warn and skip Rust assembly analysis (`STACK_RETENTION`, `REGISTER_SPILL`, drop-glue checks). Dispatches to `check_rust_asm_x86.py` (production) or `check_rust_asm_aarch64.py` (**EXPERIMENTAL** â€” AArch64 findings require manual verification). |
...
| Prerequisite | Failure mode if missing |
|---|---|
| `{baseDir}/tools/generate_poc.py` | Fail fast â€” PoC generation is mandatory |
...
```
Phase 0: 0-preflight agent â€” Preflight + config + create workdir + enumerate TUs
           â†’ writes orchestrator-state.json, merged-config.yaml, preflight.json
Phase 1: Wave 1:  1-mcp-resolver              (skip if mcp_mode=off OR language_mode=rust)
         Wave 2a: 2-source-analyzer           (C/C++ only; skip if no compile_db)  â”€â” parallel
         Wave 2b: 2b-rust-source-analyzer     (Rust only; skip if no cargo_manifest) â”€â”˜
...
Phase 3: Wave 4:  4-report-assembler          (mode=interim â†’ findings.json; reads all agent outputs)
...
Phase 6: Wave 6: 4-report-assembler           (mode=final â†’ merge PoC results, final-report.md)
Phase 7: Wave 7: 6-test-generator             (optional)
Phase 8: Orchestrator â€” Return final-report.md
```
...
|---|---|---|
| Sensitive object (C/C++) | `SO-0001`â€“`SO-4999` | `2-source-analyzer` |
| Sensitive object (Rust) | `SO-5000`â€“`SO-9999` (Rust namespace) | `2b-rust-source-analyzer` |
...
|---|---|---|---|
| Phase 1 (Source) | 1â€“6 | `MISSING_SOURCE_ZEROIZE`, `PARTIAL_WIPE`, `NOT_ON_ALL_PATHS`, `SECRET_COPY`, `INSECURE_HEAP_ALLOC` | Source + compile DB |
| Phase 2 (Compiler) | 7â€“12 | `OPTIMIZED_AWAY_ZEROIZE`, `STACK_RETENTION`*, `REGISTER_SPILL`*, `LOOP_UNROLLED_INCOMPLETE`â€ , `MISSING_ON_ERROR_PATH`â€¡, `NOT_DOMINATING_EXITS`â€¡ | `clang`, IR/ASM tools |

\* requires `enable_asm=true` (default)
â€  requires `enable_semantic_ir=true`
â€¡ requires `enable_cfg=true`
...
1. **`final-report.md`** â€” Comprehensive markdown report (primary human-readable output)
2. **`findings.json`** â€” Structured JSON matching `{baseDir}/schemas/output.json` (for machine consumption and downstream tools)
...
```json
{
  ...
  "line": 42,
  "symbol": "key_buf",
  "evidence": "store volatile i8 0 count: O0=32, O2=0 â€” wipe eliminated by DSE",
  "compiler_evidence": {
    "opt_levels": ["O0", "O2"],
    "o0": "32 volatile stores targeting key_buf",
    "o2": "0 volatile stores (all eliminated)",
    "diff_summary": "All volatile wipe stores removed at O2 â€” classic DSE pattern"
  },
...
|---|---|---|
| Exit 0 (exploitable) | Yes | Strong signal â€” can upgrade `likely` to `confirmed` |
| Exit 1 (not exploitable) | Yes | Downgrade severity to `low` (informational); retain in report |
| Exit 0 or 1 | No (user accepted) | Weaker signal â€” note verification failure in evidence |
| Exit 0 or 1 | No (user rejected) | No confidence change; annotate as `rejected` |
| Compile failure / no PoC | â€” | No confidence change; annotate in evidence |
...
- *"The compiler won't optimize this away"* â€” Always verify with IR/ASM evidence. Never suppress `OPTIMIZED_AWAY_ZEROIZE` without it.
- *"This is in a hot path"* â€” Benchmark first; do not preemptively trade security for performance.
- *"Stack-allocated secrets are automatically cleaned"* â€” Stack frames may persist; STACK_RETENTION requires assembly proof, not assumption.
- *"memset is sufficient"* â€” Standard `memset` can be optimized away; escalate to an approved wipe API.
- *"We only handle this data briefly"* â€” Duration is irrelevant; zeroize before scope ends.
- *"This isn't a real secret"* â€” If it matches detection heuristics, audit it. Treat as sensitive until explicitly excluded via config.
- *"We'll fix it later"* â€” Emit the finding; do not defer or suppress.
