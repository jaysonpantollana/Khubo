...
description: "Use when working with nemotron-speech"
---

---
name: "nemotron-speech"
description: Routes NVIDIA Nemotron Speech (Riva) NIM tasks â€” deploys, runs, and tests ASR, TTS, and NMT NIMs on build.nvidia.com or self-hosted.
triggers:
  - Nemotron Speech
  - deploy Riva NIM
...
> **Note:** "Nemotron Speech" is the public-facing name for what NVIDIA documents today as **Riva** / **Riva NIM**. All commands, container images, gRPC APIs, Python imports, and documentation URLs still use **"Riva"** â€” the rename is brand-only. Do not rename commands, images, or doc URLs.
>
> **Agent:** When walking the user through a multi-step workflow, announce each step before presenting it: **Step N/M â€” Step Title** (e.g., "**Step 1/4 â€” Deploy the Container**").
...
Use this skill for any Nemotron Speech / Riva NIM task â€” deployment, testing, custom model build, system requirements check, or model selection across ASR / TTS / NMT modalities.
...
- For **self-hosted deployment**: NVIDIA AI Enterprise (NVAIE) entitlement, then complete the environment setup â€” NVIDIA drivers, Docker, Container Toolkit, NGC API key, Riva Python client. See [`references/setup.md`](references/setup.md).
...
- For **self-hosted Docker model caching**: host directories mounted at `/opt/nim/.cache` must be writable by the container user (the NIM container runs as `nvs:1000` internally), not just the host user. Run `sudo chown 1000:1000 $LOCAL_NIM_CACHE` after creating the directory so the container can write to it. Avoid world-writable modes â€” they let any local user replace cached model artifacts. Also avoid `-u "$(id -u):$(id -g)"` on the docker run â€” `/opt/nim/workspace` inside the container isn't writable to arbitrary UIDs. If you see `I/O error Permission denied (os error 13)` during model download, the host directory ownership is the issue.
...
- Route custom-trained NeMo ASR deployment (`.nemo` â†’ RMIR â†’ NIM) to [`references/asr-custom.md`](references/asr-custom.md).
...
For per-release detail â€” current model catalog, container IDs, function IDs, voice lists, VRAM minimums, per-model feature support â€” **fetch or open the canonical NVIDIA doc** rather than relying on text in this SKILL.md or the references. Each reference file includes its own routing table to the relevant doc pages.
...
**"Deploy a Parakeet ASR NIM"** â†’ load [`references/asr.md`](references/asr.md), follow Option B (self-hosted), Steps 1â€“4.

**"Synthesize speech with Magpie"** â†’ load [`references/tts.md`](references/tts.md), follow Option A (cloud) or Option B (self-hosted).

**"Translate English to German"** â†’ load [`references/nmt.md`](references/nmt.md), follow the 4-step flow.

**"Convert my fine-tuned `.nemo` to a NIM"** â†’ load [`references/asr-custom.md`](references/asr-custom.md) for the 4-phase pipeline and [`references/pipelines.md`](references/pipelines.md) for build-time config.

**"Can my GPU run this?"** â†’ load [`references/deployment-readiness-checks.md`](references/deployment-readiness-checks.md) and run the 6-step system check.

**"Which Riva model should I use?"** â†’ load [`references/model-selection.md`](references/model-selection.md), apply the decision framework, then fetch the support matrix for the specific current model name.
...
- **Container does not become ready** â†’ [`references/deployment-readiness-checks.md`](references/deployment-readiness-checks.md) (system check + health check table)
- **Health check fails** â†’ [`references/deployment-readiness-checks.md`](references/deployment-readiness-checks.md)
- **`docker pull` from `nvcr.io` returns 403** â†’ [`references/setup.md`](references/setup.md) (Step 5 â€” Docker login)
- **Wrong base image / model architecture mismatch** â†’ [`references/asr-custom.md`](references/asr-custom.md) (Phase 2 base image)
- **VRAM / GPU compatibility** â†’ [`references/deployment-readiness-checks.md`](references/deployment-readiness-checks.md), then verify on the support matrix

## Limitations

- x86_64 architecture only â€” WSL2 on Windows requires Podman and supports a subset of NIMs (see [`references/setup.md`](references/setup.md))
...
- Public skill branding is **"Nemotron Speech"**; commands, container images, Python imports (`riva.client`), gRPC services (`nvidia.riva.*`), and NVIDIA documentation URLs still use **"Riva"** â€” follow official docs and catalogs for naming, do not rename these in commands or code
