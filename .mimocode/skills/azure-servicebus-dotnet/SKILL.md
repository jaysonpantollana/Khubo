...
description: "Use when working with azure-servicebus-dotnet"
---
...
```
ServiceBusClient
â”œâ”€â”€ CreateSender(queueOrTopicName)      â†’ ServiceBusSender
â”œâ”€â”€ CreateReceiver(queueName)           â†’ ServiceBusReceiver
â”œâ”€â”€ CreateReceiver(topicName, subName)  â†’ ServiceBusReceiver
â”œâ”€â”€ AcceptNextSessionAsync(queueName)   â†’ ServiceBusSessionReceiver
â”œâ”€â”€ CreateProcessor(queueName)          â†’ ServiceBusProcessor
â””â”€â”€ CreateSessionProcessor(queueName)   â†’ ServiceBusSessionProcessor
...
1. **Use singletons** â€” Clients, senders, receivers, and processors are thread-safe
2. **Always dispose** â€” Use `await using` or call `DisposeAsync()`
3. **Dispose order** â€” Close senders/receivers/processors first, then client
4. **Use DefaultAzureCredential** â€” Prefer over connection strings for production
5. **Use processors for background work** â€” Handles lock renewal automatically
6. **Use safe batching** â€” `CreateMessageBatchAsync()` and `TryAddMessage()`
7. **Handle transient errors** â€” Use `ServiceBusException.Reason`
8. **Configure transport** â€” Use `AmqpWebSockets` if ports 5671/5672 are blocked
9. **Set appropriate lock duration** â€” Default is 30 seconds
10. **Use sessions for ordering** â€” FIFO within a session
