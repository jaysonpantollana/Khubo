...
description: "Use when working with youtube-summarizer"
---
...
â•‘     ðŸ“¹  YOUTUBE SUMMARIZER - Processing Video                â•‘
â• â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•£
...
# Pattern 2: youtu.be/VIDEO_ID  
    ...
    echo "âŒ Invalid YouTube URL format"
    exit 1
fi

echo "ðŸ“¹ Video ID extracted: $VIDEO_ID"
```

**If URL is invalid:**

```
âŒ Invalid YouTube URL
...
try:
    ...
    print("ðŸ“ Available transcripts:")
    ...
    print(f"âŒ Transcripts are disabled for video {video_id}")
    sys.exit(1)
    
except NoTranscriptFound:
    print(f"âŒ No transcript found for video {video_id}")
    sys.exit(1)
    
except Exception as e:
    print(f"âŒ Error accessing video: {e}")
    sys.exit(1)
```
...
|-------|---------|--------|
| Video not found | "âŒ Video does not exist or is private" | Ask user to verify URL |
| Transcripts disabled | "âŒ Transcripts are disabled for this video" | Cannot proceed |
| No transcript available | "âŒ No transcript found (not auto-generated or manually added)" | Cannot proceed |
| Private/restricted video | "âŒ Video is private or restricted" | Ask for public video |

### Step 3: Extract Transcript

**Progress:**
```bash
echo "[â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘] 60% - Step 3/5: Extracting Transcript"
```
...
try:
    ...
    print(f"âŒ Error extracting transcript: {e}")
    exit(1)
```
...
**Progress:**
```bash
echo "[â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–‘â–‘â–‘â–‘] 80% - Step 4/5: Generating Summary"
```
...
**Progress:**
```bash
echo "[â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆ] 100% - Step 5/5: Formatting Output"
```
...
**Data de PublicaÃ§Ã£o:** [Date if available]
...
```
âš ï¸  youtube-transcript-api not installed
...
âœ… Installation complete! Proceeding with video summary...
```
...
```
âŒ Invalid YouTube URL format
...
â†’ Summary + raw transcript

âœ… File saved: resumo-exemplo123-2026-02-01.md (includes raw transcript)
[â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆ] 100% - âœ“ Processing complete!
```
