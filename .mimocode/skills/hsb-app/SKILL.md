---
name: hsb-app
description: "Use when working with hsb-app"
---
...
**Gate 2 â€” Present the phase plan and get confirmation.** Before taking any action:
...
- You will NOT add `--headless` automatically â€” only if the user explicitly requests it
...
Then ask explicitly: `Shall I proceed with Phase 0? [Y/n]` â€” do not start Phase 0 until the user confirms.

**Gate 3 â€” Fast path check.** After the user confirms in Gate 2, run this check before executing any Phase 0 commands:
...
If the output contains `_SESSION_VERIFIED=true`, skip Phase 0 and Phase 1 setup discovery â€” go directly to app selection and inform the user.
...
2. Interact with the user to understand their specific setup â€” repo location on the devkit, HSB software version, board type (Lattice, etc.), and connected sensors (e.g., dual IMX274, VB1940). Then scan the repository's user guide and `examples/` directory to build a list of applications compatible with the user's setup. Present the list and let the user choose an app to run.
3. Run the selected application inside the demo container, monitor output, and if the app fails, analyze the log output and guide the user through debugging â€” including suggesting code or environment edits and re-running the app.
4. Produce a summary report of the session â€” issues encountered, fixes applied, and outcome. Offer to save the report to a file.
...
```
Session already verified â€” skipping connectivity checks.
...
Then execute:
- Phase 1 Steps 2â€“3 only (scan examples, present app list, user selects app)
...
## Phase gate â€” user confirmation between phases

After completing each phase (Phases 0â€“2), **always prompt the user for confirmation** before starting the next phase.
...
All prompts in this skill require explicit typed responses. Never treat a blank or Enter-only input as a selection â€” re-prompt the user instead.

- **"y"**, **"yes"**, **"Y"**, **"ok"**, **"go"**, **"continue"**, **"next"** â†’ proceed to the next phase.
- **"n"**, **"no"**, **"stop"**, **"abort"** â†’ stop execution. Print:
  ```
  App workflow paused after Phase N.
  You can resume by re-invoking the skill.
  ```
  Then run session teardown.
- **Any other text** â†’ treat as a question or instruction about the current phase. Answer it, then re-prompt.
- **"retry"** â†’ re-execute the current phase, show summary again, then re-prompt.

### Exceptions

- **Phase 3** (session report) is the final phase â€” do not prompt after it unless the user wants to run another app. Show the report and offer to save.
...
OPTIONS
  ...
  --y               Auto-approve all phase gates (skip user confirmation
                    between phases). Not recommended â€” a confirmation
...
ENVIRONMENT VARIABLES (set before invoking the skill)
  ...
  HSB_REPO_DIR      Repo directory name under REMOTE_ROOT (default: holoscan-sensor-bridge)
                    Example: HSB_REPO_DIR=hololink â†’ repo at $REMOTE_ROOT/hololink
...
```
âš   WARNING: Auto-approve mode (--y) is enabled.
...
will be skipped â€” the app runs once and the result is reported.

Type 'yes' to confirm auto-approve mode, or anything else to cancel:
```

- If the user responds with **"yes"** (exact match, case-insensitive) â†’ enable auto-approve mode.
- Any other response â†’ cancel auto-approve mode and run interactively.

### Behavior when `--y` is active

1. **Phase gates are skipped** between phases.
2. **App selection still requires user input** â€” the user must choose which app to run.
3. **Default app settings are used automatically** â€” the "defaults vs. customize" prompt is skipped and the app runs with its default options.
4. **Timeout defaults to 30 seconds** if no `--timeout` was specified on the command line (to avoid indefinite hangs).
5. **Debug iterations are skipped** â€” if the app fails in Phase 2, the failure is logged but no interactive debugging is performed. The workflow proceeds directly to the report.
