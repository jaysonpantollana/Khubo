...
description: "Use when working with tao-analyze-gaps-vlm-bcq"
---
...
- **predictions_json**: Path to predictions JSON file. Must be a JSON array where each item has `video_id`, `response`, and `gt` fields. `response` and `gt` are parsed with word-boundary matching â€” `'yes'` or `'no'` anywhere in the string is recognized. Samples where both or neither are present are skipped with a warning.
