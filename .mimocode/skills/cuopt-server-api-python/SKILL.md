...
description: "Use when working with cuopt-server-api-python"
---

---
name: cuopt-server-api-python
version: "26.08.00"
description: cuOpt REST server â€” start server, endpoints, Python/curl client examples. Use when the user is deploying or calling the REST API.
license: Apache-2.0
metadata:
...
# cuOpt Server â€” Deploy and client (Python/curl)
...
1. POST to `/cuopt/request` â†’ get `reqId`
...
**Validation errors:** Check field names against OpenAPI (`/cuopt.yaml`). Common mistakes: `transit_time_matrix_data` â†’ `travel_time_matrix_data`; capacities per dimension `[[50, 50]]` not per vehicle `[[50], [50]]`. Capture `reqId` and response body for failed requests.

## Runnable assets

Run from each asset directory (server must be running; scripts exit 0 if server unreachable). All use Python `requests`:

- [assets/vrp_simple/](assets/vrp_simple/) â€” Basic VRP (no time windows)
- [assets/vrp_basic/](assets/vrp_basic/) â€” VRP with time windows
- [assets/pdp_basic/](assets/pdp_basic/) â€” Pickup and delivery
- [assets/lp_basic/](assets/lp_basic/) â€” LP via REST (CSR format)
- [assets/milp_basic/](assets/milp_basic/) â€” MILP via REST
