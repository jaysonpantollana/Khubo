---
name: skill-improver
description: "Use when working with skill-improver"
---
...
Verify it's enabled: run `/plugins` â€” `plugin-dev` should appear in the list. If missing, install from the Trail of Bits plugin repository.
...
- Missing required frontmatter fields (name, description) â€” Claude cannot index or trigger the skill
- Invalid YAML frontmatter syntax â€” Parsing fails, skill won't load
- Referenced files that don't exist â€” Runtime errors when Claude follows links
- Broken file paths â€” Same as above, leads to tool failures

### Major Issues (MUST fix)

These significantly degrade skill effectiveness:

- Weak or vague trigger descriptions â€” Claude may not recognize when to use the skill
- Wrong writing voice (second person "you" instead of imperative) â€” Inconsistent with Claude's execution model
- SKILL.md exceeds 500 lines without using references/ â€” Overloads context, reduces comprehension
- Missing "When to Use" or "When NOT to Use" sections â€” Required by project quality standards
- Description doesn't specify when to trigger â€” Skill may never be selected

### Minor Issues (Evaluate before fixing)

These are polish items that may or may not improve the skill:

- Subjective style preferences â€” Reviewer may have different taste than author
- Optional enhancements â€” May add complexity without proportional value
- "Nice to have" improvements â€” Consider cost-benefit before implementing
- Formatting suggestions â€” Often valid but low impact
...
**Iteration 1 â€” skill-reviewer output:**
```text
...
**Iteration 2 â€” run skill-reviewer again to verify fixes:**
```text
Minor: Line 45 is verbose
```
...
1. **skill-reviewer reports "Pass"** or **no issues found** â†’ output marker immediately
2. **All critical and major issues are fixed** AND you've verified the fixes â†’ output marker
3. **Remaining issues are only minor** AND you've evaluated them as false positives or not worth fixing â†’ output marker
