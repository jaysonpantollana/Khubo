...
description: "Use when working with holoscan-install-conda"
---
...
- **CUDA 13 only** (since v4.3.0 â€” earlier releases were CUDA 12). If the user has a CUDA 12 driver, redirect to `/holoscan-install-container` or `/holoscan-install-wheel` instead.
- Linux x86_64 only â€” no aarch64/iGPU support on conda-forge.
- `ulimit -s 32768` is recommended in every shell that runs Holoscan â€” without it, some apps **may** segfault.

## Step 0: Consult the Official Install Instructions

Always fetch the current Conda section of `https://docs.nvidia.com/holoscan/sdk-user-guide/sdk_installation.html` before installing â€” package names, channel selection, and the runtime/dev split can change between releases. Specifically extract:

- The exact runtime package name (e.g. `holoscan` for Python bindings).
- The C++ dev package name and whether the user needs it. As of v4.1.0, `libholoscan-dev` is a separate package containing headers and CMake config â€” install it whenever the user wants to develop C++ apps. Without it, `find_package(holoscan)` fails and there are no headers to `#include`.
- Supported Python versions for the current release (3.10â€“3.13 for v4.3).
- The current `cuda-version` pin (v4.3 â†’ `13`).

`rmm` and `ucxx` are distributed via the `rapidsai` channel; `holoscan`, `libholoscan`, and `libholoscan-dev` come from `conda-forge`.

If the doc disagrees with anything below, the doc wins â€” update the install commands accordingly and tell the user.
...
- `libholoscan` â€” C++ runtime symbols (`libholoscan_core.so`). Auto-pulled as a dependency.
- `holoscan` â€” Python bindings.
- `libholoscan-dev` â€” C++ headers, `libholoscan_core.so` symlink, and `holoscan-config.cmake` for `find_package(holoscan)`.
- `rmm` â€” RAPIDS Memory Manager (rapidsai channel). Undeclared runtime dep of `holoscan`; `import holoscan` fails without it.
- `ucxx` â€” UCX Python bindings (rapidsai channel), needed for distributed/multi-process apps.
- `cuda-version=13` â€” pins the CUDA 13 metapackage so the solver picks compatible CUDA runtime libs.
...
Pick the packages for the user's goal â€” Python-only needs `holoscan`, C++ dev needs `libholoscan-dev`, both works for combined use:
...
`ulimit -s 32768` is recommended â€” without it, some Holoscan apps may segfault on startup.
...
(`headless: true`) â€” headless works with or without a display attached and avoids GUI
...
# Patch video_replayer.yaml â€” 10 frames, headless.
python3 -c "
...
# hello_world â€” no display, no data needed; expected: "Hello World!"
python3 /tmp/hs_hello_world.py

# video_replayer â€” needs racerx data; expected: frames rendered, "Graph execution finished."
...
conda activate holoscan
ulimit -s 32768   # recommended â€” prevents segfaults in some apps
```
...
- **`ImportError: librmm.so: cannot open shared object file`.** `rmm` was not installed. Re-run the Step 2 `conda install` line â€” `rmm` is an undeclared runtime dependency of `holoscan`.
- **Solver picks an older `holoscan` build than expected.** Channel order may be wrong. Use `-c rapidsai -c conda-forge` (rapidsai first) â€” that's the order in the official install command, and under strict channel priority a conda-forge-first ordering can lock the solver to an older `holoscan` build.
