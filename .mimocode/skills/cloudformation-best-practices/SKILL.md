...
description: "Use when working with cloudformation-best-practices"
---
...
- âœ… **Do:** Use `Outputs` with `Export` for cross-stack references
- âœ… **Do:** Add `DeletionPolicy` and `UpdateReplacePolicy` on stateful resources
- âœ… **Do:** Use `cfn-lint` and `cfn-nag` in CI pipelines
- âŒ **Don't:** Hardcode ARNs or account IDs â€” use `!Sub` with pseudo parameters
- âŒ **Don't:** Put all resources in a single monolithic template
