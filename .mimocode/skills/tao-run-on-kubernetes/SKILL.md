...
description: "Use when working with tao-run-on-kubernetes"
---

---
name: tao-run-on-kubernetes
description: Kubernetes execution platform â€” submits TAO container jobs as single-pod k8s Jobs with NVIDIA GPU scheduling.
...
# 3. NVIDIA GPU Operator present (soft check â€” warn if kubectl available, don't fail)
...
a warning rather than a hard fail â€” `kubectl` isn't always installed. The SDK
...
- **Kubeconfig** (one of):
  - `~/.kube/config` â€” default discovery path
  - `$KUBECONFIG` â€” alternate path
  - In-cluster service account â€” used when running inside a pod (no kubeconfig needed)
...
K8s is SDK-only â€” there is no `kubectl`-only launch path. Read
...
- `resources.limits["nvidia.com/gpu"]: <gpu_count>` â€” schedules onto GPU nodes via the NVIDIA Device Plugin / GPU Operator.
- `env_vars` flowed through, plus auto-injected S3/NGC/HF credentials for `script_runner`.
- `restart_policy=Never` and `backoff_limit=0` â€” failures surface to the user instead of silently retrying.
- `ttl_seconds_after_finished=3600` â€” Job auto-cleans 1 hour after terminal state.

## Status & monitoring

```python
status = sdk.get_job_status(job.id)
# status.status âˆˆ {"Pending", "Running", "Complete", "Error", "Canceled", "Unknown"}

logs = sdk.get_job_logs(job.id, tail=200)  # concatenates logs from all pods of the Job

# For stuck-Pending jobs â€” replica diagnostics:
...
job = sdk.create_job(
    ...
    num_nodes=4,           # 4 Ã— 8 = 32 GPUs total
    inputs={'/data/train.json': 's3://bucket/coco/train.json'},
    outputs=['/results/'],
)
```
...
The capacity check sums across nodes: `gpu_count Ã— num_nodes` â‰¤ cluster's allocatable `nvidia.com/gpu`.
...
- **MPI Operator** â€” <https://github.com/kubeflow/mpi-operator> â€” for MPI / Horovod workloads.
- **Kubeflow Training Operator** (`PyTorchJob`, `TFJob`) â€” <https://www.kubeflow.org/docs/components/training/> â€” for elastic PyTorch training with built-in restart logic.
- **Volcano** â€” <https://volcano.sh/> â€” gang scheduling, queues, fair-share. Useful in shared multi-tenant clusters.
- **Kueue** â€” <https://kueue.sigs.k8s.io/> â€” quota / queue layer on top of any of the above.
...
**`No nvidia.com/gpu resources allocatable on the cluster`** â€” the GPU Operator (or NVIDIA Device Plugin) isn't installed. Install per the link above; verify with `kubectl get nodes -o jsonpath='{.items[*].status.allocatable}'`.

**`ImagePullBackOff` / `ErrImagePull`** â€” the cluster can't pull the image. For nvcr.io: pre-create an image-pull secret in the namespace and pass its name via the `image_pull_secret` argument:
```bash
...
**Pod stays `Pending` forever** â€” `get_job_replicas(job_id)` will show the readiness_issue. Common causes: insufficient GPU capacity (`Insufficient nvidia.com/gpu`), no node matches `node_selector`, missing image-pull secret, or PVC mount failure.

**`OOMKilled` (exit 137)** â€” container exceeded memory. Reduce batch size, lower max_length, or add a memory request/limit and target a larger node.

**`CredentialError: Could not authenticate to a Kubernetes cluster`** â€” neither kubeconfig nor in-cluster auth worked. Run `kubectl get nodes` to verify your config, or set `$KUBECONFIG` to the right path.

## What this skill does NOT support (yet)

- **Elastic / fault-tolerant training.** Indexed Job has `backoff_limit=0` â€” failures fail the whole training run. For elastic restart (e.g., resume from checkpoint after a node death), use Kubeflow's `PyTorchJob` operator instead.
- **Gang scheduling.** Indexed Job pods are scheduled independently â€” no all-or-nothing. Multi-node training will *partially* start if only some pods can be scheduled (rank-0 will hang waiting for peers). For all-or-nothing scheduling on shared clusters, use Volcano or Kueue.
