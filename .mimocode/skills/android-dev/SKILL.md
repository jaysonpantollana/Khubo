---
name: android-dev
description: "Use when working with android-dev"
---
...
This skill guides production-grade Android and cross-platform (non-iOS) app development following practices used at big tech companies. It covers the entire development lifecycle â€” architecture, UI, code quality, testing, error handling, release, and maintenance.

## When to Use This Skill

- Use when deciding on a tech stack (see Â§1 Stack Selection)
- Use when setting up project architecture (see Â§2 Architecture)
- Use when designing UI, screens, or a design system (see Â§3 UI & Design)
- Use when ensuring code quality, patterns, or APIs (see Best Practices)
- Use when implementing error handling or debugging crashes (see Â§5 Error Handling)
- Use when planning testing strategy (see Â§6 Testing)
- Use when configuring build, CI/CD, or release pipelines (see Â§7 Build & Release)
- Use when optimizing performance or memory (see Â§8 Performance)
- Use when debugging or fixing bugs (see Â§9 Debugging)
- Use when following the full development roadmap (see Â§10 Development Roadmap)
...
### Native Android â€” Kotlin + Jetpack Compose
...
### Native Android â€” Java + XML Views
...
- Java and Kotlin **coexist seamlessly** in the same project â€” migrate incrementally
...
|---|---|---|---|---|---|---|
| Android-only (new) | âœ… Best | âœ… | âœ… | âœ… | âœ… | âœ… |
| Android-only (existing Java) | âš ï¸ migrate | âœ… Best | âŒ | âŒ | âš ï¸ | âŒ |
| Android + Web | âŒ | âŒ | âœ… | âœ… | âœ… | âœ… Best |
| Android + Desktop | âŒ | âŒ | âœ… | âš ï¸ | âœ… | âš ï¸ |
| Shared business logic only | N/A | N/A | N/A | N/A | âœ… Best | N/A |
| Native performance | âœ… | âœ… | âœ… | âš ï¸ | âœ… | âŒ |
| JS/TS team | âŒ | âŒ | âŒ | âœ… Best | âŒ | âœ… |
| Custom pixel-perfect UI | âœ… | âš ï¸ | âœ… Best | âš ï¸ | âœ… | âŒ |
...
```
app/
â”œâ”€â”€ ui/              # Composables / Activities / Fragments / Screen states
â”œâ”€â”€ presentation/    # ViewModels, UI State, UI Events
â”œâ”€â”€ domain/          # Use cases, domain models, repository interfaces
â”œâ”€â”€ data/            # Repository impl, remote (API), local (DB), mappers
â””â”€â”€ di/              # Dependency injection modules
```

**Data flow (unidirectional):**
```
User Action â†’ ViewModel/Store â†’ Use Case â†’ Repository â†’ Data Source
                    â†“
...
1. **Color tokens** â€” Primary, secondary, surface, on-surface, error; light + dark variants
2. **Typography scale** â€” Display, headline, title, body, label (Material 3 type system)
3. **Spacing scale** â€” 4dp grid system (4, 8, 12, 16, 24, 32, 48dp)
4. **Shape tokens** â€” Corner radii per component family
5. **Component library** â€” Button, TextField, Card, BottomSheet, TopAppBar, etc.
...
- Extract large composables into sub-composables; each function â‰¤ 80 lines
...
- Back stack managed deliberately â€” don't push duplicates, use `popUpTo` / `launchSingleTop`
...
- No `!!` null assertions â€” use `?.let`, `?: return`, `requireNotNull` with message
...
- Never call methods on unchecked objects â€” null-check explicitly or use `Objects.requireNonNull`
- Always null `binding` reference in Fragment's `onDestroyView()` to prevent memory leaks
- Use `ExecutorService` (not `AsyncTask` â€” deprecated) for background work; or `LiveData` + Room's built-in threading
- Prefer `ListAdapter` + `DiffUtil` over manual `notifyDataSetChanged()` in RecyclerView
- Use `ViewBinding` â€” never `findViewById`

**Dart (Flutter):**
- Null safety required â€” no `!` without explicit null check above
...
- Avoid transitive dependency conflicts â€” use dependency resolution strategies
- Keep dependency count minimal â€” every added lib is a maintenance burden

### Code Review Checklist (PR gate)
- [ ] New public APIs have KDoc / DartDoc / JSDoc
- [ ] No hardcoded strings â€” use string resources / l10n
...
| Auth errors (401/403) | Refresh token â†’ re-request â†’ logout if fails |
...
```
         /\
        /E2E\        â† 10%  (UI tests: Espresso, Maestro, Appium)
       /------\
      / Integr \     â† 20%  (Repository, DB, API contract tests)
     /----------\
    /    Unit    \   â† 70%  (ViewModels, Use Cases, Utilities)
   /--------------\
```
...
- Coverage target: **â‰¥ 80%** on domain + presentation layers
...
### Build Variants
```
debug       â†’ dev API, logging on, no minification, debuggable
staging     â†’ staging API, logging on, minified, not debuggable
release     â†’ prod API, logging off, minified, signed
```

### Gradle Best Practices (Native)
- `build.gradle.kts` only â€” no Groovy DSL in new projects
...
```
PR Opened
  â””â”€ lint + unit tests + build debug APK          [< 5 min]

Merge to main
  â””â”€ unit + integration tests + staging build     [< 15 min]
  â””â”€ deploy to Firebase App Distribution (QA)

Release tag
  â””â”€ full test suite + E2E on device farm         [< 45 min]
  â””â”€ build release AAB
  â””â”€ upload to Play Console (internal track)
  â””â”€ promote: internal â†’ closed testing â†’ open â†’ production
```

**Recommended CI:** GitHub Actions, Bitrise, or CircleCI.

### Play Store Release Strategy
- Always release to **internal â†’ closed â†’ open testing** before production
- Use **staged rollouts**: 5% â†’ 20% â†’ 50% â†’ 100% with 24-48h monitoring
...
- Image loading: Coil (Compose) / Glide / Picasso â€” never load full-res in thumbnails
...
## Â§9 Debugging & Bug Fixing

### Debugging Process

1. **Reproduce reliably** â€” document exact steps, device, OS version, account state
2. **Isolate** â€” is it UI, business logic, network, or persistence?
3. **Instrument** â€” add targeted logs / breakpoints, NOT shotgun logging
4. **Hypothesize** â€” form 1-3 specific hypotheses before touching code
5. **Fix the root cause** â€” never patch symptoms; trace back to the source
6. **Regression test** â€” write a test that fails before fix, passes after
7. **Document** â€” comment explaining why the fix works, not just what it does
...
- Never log PII â€” mask emails, phone numbers, tokens in logs

### OEM-Specific Issues
- Test on **Samsung**, **Xiaomi/MIUI**, **OnePlus/OxygenOS**, **Huawei (no GMS)** for critical flows
- Background restrictions vary widely by OEM â€” test push, alarms, background sync
- Maintain a physical or cloud device farm with top market-share devices

---

## Â§10 Development Roadmap
...
### Phase 1 â€” Core Features (Weeks 3-8)
...
### Phase 3 â€” Hardening (Weeks 12-14)
...
- [ ] Crash-free rate â‰¥ 99.5% on staging
...
- `references/native-android.md` â€” Kotlin, Compose, Room, Hilt, Coroutines
- `references/java-android.md` â€” Java, XML Views, ViewBinding, LiveData, Retrofit, Room, Hilt, migration path
- `references/flutter.md` â€” Dart, BLoC/Riverpod, Drift, go_router
- `references/react-native.md` â€” TypeScript, RN architecture, Hermes, New Architecture
- `references/kmm.md` â€” KMM shared modules, SQLDelight, Ktor, Compose Multiplatform
- `references/hybrid.md` â€” Capacitor, Ionic, PWA considerations
