...
description: "Use when working with nv-generate-ct-rflow"
---
...
- Emit a single bash code block, and keep the `python -m pip install -r "$NV_GENERATE_ROOT/requirements.txt"` step in that same command â€” the runtime may be a fresh environment without `nibabel`/MONAI, so dropping the install fails with `ModuleNotFoundError`.
...
- rflow-ct requires CUDA and â‰ˆ 16 GB VRAM minimum for the default 256Â³ output_size. Larger output_size (e.g. 512Ã—512Ã—768) needs an A100/H100.
...
sampling, or autoencoder decoding â€” it shells out to the upstream
...
2. Download the `rflow-ct` weights **and** the mask-candidate datasets
   into the clone (one-time, â‰ˆ 5.5 GB):
...
3. NVIDIA GPU with â‰¥ 16 GB VRAM and CUDA. There is no CPU fallback.
