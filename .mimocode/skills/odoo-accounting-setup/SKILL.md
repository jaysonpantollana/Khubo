...
description: "Use when working with odoo-accounting-setup"
---
...
```text
Menu: Accounting â†’ Configuration â†’ Payment Terms â†’ New
...
```text
Menu: Accounting â†’ Configuration â†’ Fiscal Positions â†’ New
...
Tax Mapping:
  Tax on Sales (21% VAT) â†’ 0% Intra-Community VAT
  Tax on Purchases      â†’ 0% Reverse Charge
...
```text
Menu: Accounting â†’ Configuration â†’ Reconciliation Models â†’ New
...
- âœ… **Do:** Install your country's **localization module** first (`l10n_us`, `l10n_mx`, etc.) before manually creating accounts â€” it sets up the correct chart of accounts.
- âœ… **Do:** Use **Fiscal Positions** to automate B2B vs B2C tax switching â€” never change taxes manually on individual invoices.
- âœ… **Do:** Lock accounting periods (Accounting â†’ Actions â†’ Lock Dates) after month-end closing to prevent retroactive edits.
- âœ… **Do:** Use the **Early Payment Discount** feature (v16+) instead of splitting payment term lines for discount modelling.
- âŒ **Don't:** Delete journal entries â€” always reverse them with a credit note or the built-in reversal function.
- âŒ **Don't:** Mix personal and business transactions in the same journal.
- âŒ **Don't:** Create manual journal entries to fix bank reconciliation mismatches â€” use the reconciliation model workflow instead.

## Limitations

- Does not cover **multi-currency revaluation** or foreign exchange gain/loss accounting in depth.
- **Country-specific e-invoicing** (CFDI, FatturaPA, SAF-T) requires additional localization modules â€” use `@odoo-l10n-compliance` for those.
- Payroll accounting integration (salary journals, deduction accounts) is not covered here â€” use `@odoo-hr-payroll-setup`.
