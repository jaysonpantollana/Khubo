---
name: odoo-rpc-api
description: "Use when working with odoo-rpc-api"
---
...
- âœ… **Do:** Use **API Keys** (Settings â†’ Technical â†’ API Keys) instead of passwords â€” available from Odoo 14+.
- âœ… **Do:** Use `search_read` instead of `search` + `read` to reduce network round trips.
- âœ… **Do:** Always handle connection errors and implement retry logic with exponential backoff in production.
- âœ… **Do:** Store credentials in environment variables or a secrets manager (e.g., AWS Secrets Manager, `.env` file).
- âŒ **Don't:** Hardcode passwords or API keys directly in scripts â€” rotate them and use env vars.
- âŒ **Don't:** Call the API in a tight loop without batching â€” bulk operations reduce server load significantly.
- âŒ **Don't:** Use the master admin password for API integrations â€” create a dedicated integration user with minimum required permissions.

## Limitations

- Does not cover **OAuth2 or session-cookie-based authentication** â€” the examples use API key (token) auth only.
- **Rate limiting** is not built into the Odoo XMLRPC layer; you must implement throttling client-side.
- The XML-RPC endpoint (`/xmlrpc/2/`) does not support file uploads â€” use the REST-based `ir.attachment` model via JSON-RPC for binary data.
