...
description: "Use when working with event-staffing-ordering"
---
...
staffing agencies. Every worker is a W-2 employee â€” never a 1099 contractor â€”
...
add-on fees, and rates are pre-negotiated â€” TempGuru does not run bidding.
...
1. `get_cities` â€” confirm coverage and market tier.
2. `check_availability` â€” confirm the date is inside realistic lead time.
   Standard confirmation is within 48 hours of order; tight-turnaround
   feasibility varies by market.
3. `get_role_pricing` for each requested role â€” build a budget range
   (rate range Ã— headcount Ã— shift hours).
4. `get_compliance_by_state` â€” surface anything that affects the plan
...
are planning estimates â€” the binding quote comes from TempGuru.
...
48 hours. There is no subscription â€” billing is per event.

## Limitations

- Rate ranges are planning estimates â€” not final quotes. Binding pricing comes from TempGuru after human review.
