---
name: super-code
description: "Use when working with super-code"
---
...
Produce code that is short, correct, idiomatic, and maintainable â€” in that priority order.
This skill addresses two distinct inefficiency types that must be fixed independently:

1. **Code-token inefficiency** â€” the artifact itself is bloated (unnecessary lines, boilerplate, over-abstraction)
2. **Generation-token inefficiency** â€” how the agent operates during a session (full-file rewrites, unrequested files, prose padding before/after changes)
...
- This is a standing house style, not an on-demand pass â€” always apply it.
...
```
Correctness â†’ Clarity â†’ Necessary robustness â†’ Conciseness â†’ Micro-performance
```
...
## Workflow â€” apply to every coding task
...
- Bash/Shell â†’ `bash/SKILL.md`
- C â†’ `c/SKILL.md`
- C++ â†’ `cpp/SKILL.md`
- C# â†’ `csharp/SKILL.md`
- Dart/Flutter â†’ `dart/SKILL.md`
- Elixir/Erlang â†’ `elixir/SKILL.md`
- Go â†’ `go/SKILL.md`
- Java â†’ `java/SKILL.md`
- Kotlin/Compose â†’ `kotlin/SKILL.md`
- PHP â†’ `php/SKILL.md`
- Python â†’ `python/SKILL.md`
- Ruby â†’ `ruby/SKILL.md`
- Rust â†’ `rust/SKILL.md`
- Scala â†’ `scala/SKILL.md`
- Swift â†’ `swift/SKILL.md`
- TypeScript/JavaScript â†’ `typescript/SKILL.md`
...
### Step 4: Guardrail check â€” run this before presenting
...
### Step 5: Present output â€” generation-token rules
...
- Add prose blocks before/after code explaining what you're about to do or just did â€” just do it
- Re-explain the user's own requirement back to them before writing
- Restate what changed in a paragraph after showing the diff â€” the diff is self-evident
...
### Comments
- âŒ `// Loop through the list and add each item` â†’ âŒ delete
- âœ… `// Order matters: process refunds before charges` â†’ âœ… keep (explains *why*)
...
- No `// TODO: add error handling` â€” either add it or don't
...
- If you believe a test file would be valuable, offer it in one sentence after the main output â€” don't generate it unasked
