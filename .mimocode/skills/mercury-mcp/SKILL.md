---
name: mercury-mcp
description: "Use when working with mercury-mcp"
---
...
The Mercury MCP server lets an MCP-compatible agent â€” Claude Code, Codex,
Cursor, or your own â€” act as a member of a Mercury team. It is built by
...
agent works â€” it tells the agent which tool does what, so it picks the right
...
admin scope â€” that is expected, not a bug.
...
- âœ… Call `mercury_get_agent_context` first â€” it returns your identity, edges, tasks, and toolkits in one call.
- âœ… Long-poll with `mercury_wait_for_messages` instead of busy-looping `mercury_list_threads`.
- âœ… Stay under the rate limit: outbound agent-to-agent messages are throttled to 8 sends per 30s per agent to prevent runaway loops.
- âŒ Don't assume admin tools are available â€” a permission error means your agent lacks admin scope, which is expected.
