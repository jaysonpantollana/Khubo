...
description: "Use when working with nemo-data-designer-plugin"
---
...
Use **Autopilot** mode if the user implies they don't want to answer questions â€” e.g., they say something like "be opinionated", "you decide", "make reasonable assumptions", "just build it", "surprise me", etc. Otherwise, use **Interactive** mode (default).

Read **only** the workflow file that matches the selected mode, then follow it:

- **Interactive** â†’ read `workflows/interactive.md`
- **Autopilot** â†’ read `workflows/autopilot.md`
...
def load_config_builder() -> dd.DataDesignerConfigBuilder:
    config_builder = dd.DataDesignerConfigBuilder(
        ...
        # is a common default created during `nemo setup` â€” confirm it (or
...
Only include Pydantic models, custom generators, seed datasets, and extra dependencies when the task requires them. Prefer including `model_configs` when the dataset uses LLM columns â€” declaring it in the script keeps the config portable between local `run` and cluster `submit`, while the local YAML registry alternative only works for `run`.
