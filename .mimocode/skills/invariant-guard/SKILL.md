...
description: "Use when working with invariant-guard"
---
...
# invariant-guard â€” Correctness-First Coding

The model knows what a loop invariant is. It knows recursion needs a base case. It knows about empty lists, integer overflow, and the difference between `<` and `â‰¤`. It just does not write these down before producing code, so it ships subtle correctness bugs that tests do not catch.

invariant-guard fixes the behavior. State the invariants. State the base case. State the termination argument. State the edge cases. Then write the code â€” and verify that the code maintains what you stated.
...
- Postcondition stronger than the loop's natural invariant: Boyerâ€“Moore majority, Floyd's cycle detection, leftmost vs any binary search, QuickSelect partition.
...
- Any function where you catch yourself thinking "I know this algorithm" â€” the trap is usually in the contract, not the loop body.
...
If you cannot write the invariant in one sentence, you have not designed the loop. Write code anyway and you are coding by guess â€” and the bug will be in the case you did not enumerate.
...
1. **Every loop gets a one-line invariant.** Before writing any loop, state in one sentence what is true at the top of every iteration. Examples:
   ...
   - "At loop top: `lo â‰¤ target_position â‰¤ hi`."
...
2. **Every loop gets a one-line termination argument.** Name the quantity that strictly decreases (or strictly increases toward a bound) on every iteration. Examples:
   - "`hi âˆ’ lo` strictly decreases each iteration."
...
3. **Every recursion gets an explicit base case and a measure.** Before writing a recursive function, state:
   - The base case(s) â€” the smallest inputs that return without recursing.
   - The measure â€” a non-negative integer that strictly decreases on every recursive call (e.g. `len(xs)`, `hi âˆ’ lo`, `depth`, `n`).
   - The combination â€” how the recursive results combine into the answer.
...
4. **List edge cases before writing, not after.** For every function operating on a collection or number, list which of these apply and how they behave:
   ...
   - NaN, Â±Infinity, `-0`, denormals (for floats).
   - Off-by-one boundaries: index 0, index nâˆ’1, index n, length 0, length 1.
...
Before producing non-trivial code that has loops, recursion, or non-trivial state, your message must contain â€” in this order:

1. **Function contract** â€” preconditions, postconditions, and what the function returns. One line each.
2. **Loop invariants** â€” one per loop. (Rule 1.)
3. **Termination arguments** â€” one per loop or recursion. (Rules 2, 3.)
4. **Base cases and measure** â€” for recursion. (Rule 3.)
5. **Edge case table** â€” bullets, one per applicable case, with expected behavior. (Rule 4.)
6. **Illegal states made unrepresentable** â€” name the types or asserts that enforce invariants. (Rule 5.)
7. **The code.**
8. **Self-check** â€” one line per loop confirming the invariant holds at top, body preserves it, and exit implies postcondition.

If any of 1â€“6 is missing, do not emit code.

## Worked trap â€” Boyerâ€“Moore majority vote
...
These two statements are not equivalent. The loop invariant guarantees "if a majority exists, it is the candidate" â€” not "the candidate is a majority." Once you write both down, the gap is visible: you need a second pass to verify, or the postcondition is unmet.
...
function findMajority(arr: number[]): number | null {
  ...
  // Pass 2: verify â€” the voting invariant is strictly weaker than the postcondition.
  let tally = 0;
...
- **Floyd's cycle detection** â€” finding the meeting point tells you a cycle exists, *not* where it starts. You need a second walk.
- **Two-pointer "find any"** vs **"find leftmost"** â€” the loop invariant for one does not satisfy the postcondition of the other.
- **QuickSelect partition** â€” the loop returns a position; the postcondition is that the element at that position is the k-th smallest. Off by one in the partition invariant silently breaks it.
- **DP with reconstruction** â€” the table tells you the optimum value; reconstructing the optimum path needs separate invariants on the choice array.
...
## Canonical example â€” binary search for the leftmost match
...
### Without the protocol â€” returns any match
...
// leftmost([1,2,2,2,3], 2) â†’ may return 2, not 1
```
...
### With the protocol â€” contract-driven leftmost
...
function leftmost(a: number[], target: number): number {
  ...
  // inv: every index < lo has a[i] < target; every index â‰¥ hi has a[i] > target OR is past leftmost match
...
Same loop shape. The difference is the contract was written first â€” and the loop body was chosen to maintain an invariant that *implies* the postcondition.
...
| Two-pointer (sorted) | `target (if any) lies in a[lo..hi]` | `hi âˆ’ lo` strictly decreases |
| Binary search | `target (if present) âˆˆ a[lo..hi]` and `a[lo..hi]` non-empty | `hi âˆ’ lo` strictly halves |
| Sliding window | window `[l..r)` satisfies the constraint; answer â‰¥ best so far | `r` advances at least once per outer iter |
...
| Divide and conquer | result on `a[lo..hi]` = combine(results on the two halves) | `hi âˆ’ lo` strictly halves |
...
| In-place partition | `a[0..i)` < pivot; `a[i..j)` â‰¥ pivot; `a[j..n)` unseen | `n âˆ’ j` strictly decreases |

## Edge case table â€” defaults to consider
...
| Integer | 0, 1, âˆ’1, MIN, MAX, MAX âˆ’ 1, near overflow in arithmetic, division by 0 |
| Float | 0.0, âˆ’0.0, NaN, Â±Inf, denormal, exact comparison should be Îµ-based |
...
- Handle every edge case you listed in step 5, or explicitly delegate ("throws on empty â€” caller responsibility").
...
- The function is performance-critical and you have not picked the algorithm â€” go back to **`lemmaly`** first; pick the algorithm, then state its invariants here.
- The technique is mathematical (probabilistic, FFT, geometry) â€” load **`mathguard`**; invariants for approximate algorithms include Îµ-bounds, not equality.
- The code is concurrent â€” invariants must account for interleaving; explicitly state "single-threaded only" if that is the assumption.

## Rationalizations to watch for

| Excuse | Reality |
| --- | --- |
| "I know this algorithm â€” single pass, done." | Knowing the loop â‰  knowing the contract. The trap usually lives in the postcondition the loop does not enforce. |
...
| "The postcondition is implied." | If it were, the natural loop invariant would equal it. When they differ (Boyerâ€“Moore, leftmost search, QuickSelect), you need a second pass, an extra check, or auxiliary state. |
| "Adding a verification pass feels redundant." | Boyerâ€“Moore voting + verification is still O(n). "Feels redundant" is the rationalization that ships the bug. |

## Red flags â€” STOP and write the invariant first

- About to write `while (...)` without having stated what is true on entry.
- About to write `if (i === n âˆ’ 1)` or `if (i === n)` â€” boundary suspicious, restate the invariant.
- About to recurse without naming the base case in this message.
- About to write `// TODO: handle empty` â€” handle it now or change the type so empty is impossible.
...
- [ ] For approximate/randomized algorithms (escalated to mathguard): Îµ-bounds are part of the postcondition, not equality.
...
- **Documentation is the only enforcement.** If the author skips writing the invariants, this skill cannot detect that â€” pair with code review or a PR template that asks for the contract.
...
- `lemmaly` â€” algorithm choice must be settled before invariants; load lemmaly first if the algorithm family is unclear.
- `mathguard` â€” Îµ-bounded postconditions for approximate / randomized algorithms.
- `complexity-cuts` â€” if 3+ optimization transformations have failed tests, the bug is a missing contract, not a missing optimization â€” escalate here.
