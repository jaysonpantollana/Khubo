...
description: "Use when working with tao-validate-dataset-format"
---
...
or for `tao-daft info` / `tao-daft convert` â€” those have their own skills.
...
2. **Target is `--path PATH`**, not positional. It accepts a single
   dataset/scene or a parent directory â€” the validator walks the tree.
...
So the loop is: `tao-daft --version` â†’ `tao-daft validate --help` â†’
pick format (infer if unspecified, see below) â†’
`tao-daft validate <format> --help` â†’ run â†’ interpret.

### Format inference

Use directory markers, not filenames:

- `meta.json` next to `media/` and `text/` â‡’ `cosmos-reason-v1.0`.
- A directory (or nested directories) containing `contextual/`,
  typically alongside `raw/` and `task/` â‡’ `metropolis-v3.0`.
- Neither marker present â‡’ ask the user; do not guess.

### Reading errors

The CLI ends every run with a `VALIDATION RESULTS` block, then
`âœ… VALIDATION PASSED` or `âŒ VALIDATION FAILED`, and exits non-zero on
failure (safe to chain in scripts).

Output can be large on big trees â€” capture the full output to a file
...
- **`tao-daft: command not found`** â€” wheel not installed in the active
  env. `pip install nvidia-tao-daft`; verify `tao-daft --version`.
- **`error: argument --path is required`** â€” path passed positionally.
  Move it behind `--path`.
- **`invalid choice: '<format>'`** â€” slug isn't wired up in this
  version. Re-run `tao-daft validate --help` and pick from the list.
- **Auto-detection (raw type / contextual set) is wrong** â€” override
  via the format's scope-restriction flag; discover the name from the
  leaf `--help`.
- **CI wants warnings to fail** â€” add `--strict`.
