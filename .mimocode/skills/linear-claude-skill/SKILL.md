...
description: "Use when working with linear-claude-skill"
---
...
```bash
# âŒ NEVER - exposes key to Claude's context
linear config show
echo $LINEAR_API_KEY
...
âŒ Creating issues in a "holding" project and moving them later:
```bash
# Don't do this
...
|-----------|-------------|-------|
| Create issue | âœ… High | Full support |
| Update status | âœ… High | Use `state: "Done"` directly |
| List/Search issues | âœ… High | Supports filters, queries |
| Add comment | âœ… High | Works with issue IDs |
...
### Issues â†’ Projects â†’ Initiatives
