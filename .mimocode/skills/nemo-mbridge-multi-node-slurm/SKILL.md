...
description: "Use when working with nemo-mbridge-multi-node-slurm"
---
...
**Prefer srun-native** â€” simpler, avoids shell escaping issues with TRAIN_CMD. Megatron Bridge auto-derives `RANK`, `WORLD_SIZE`, `LOCAL_RANK`, `MASTER_ADDR`, `MASTER_PORT` from SLURM env vars (`SLURM_PROCID`, `SLURM_NTASKS`, `SLURM_LOCALID`, `SLURM_NODELIST`) via `common_utils.py` helpers called during `initialize.py` distributed init, so you never need to set them manually.
...
- Phase 2's `uv sync` is a fast no-op (everything is cached) â€” safe to run on all ranks without sleep guards
...
#SBATCH --nodes=<NNODES>
#SBATCH --ntasks-per-node=1          # ALWAYS 1 â€” torchrun handles per-node spawning
#SBATCH --gpus-per-node=8
#SBATCH --time=00:30:00
#SBATCH --account=<YOUR_ACCOUNT>
#SBATCH --partition=batch
...
`MASTER_ADDR` and `MASTER_PORT` are auto-derived from SLURM env vars by `initialize.py` / `common_utils.py` â€” no need to set them.
...
| `Unable to create step for job... Requested node configuration is not available` | `-w <node>` conflicts with allocation | Remove `-w` flag â€” HF cache is on shared filesystem, accessible from any node |
...
### NCCL Timeout at `dist.barrier()` â€” "crash on rank 0"
...
|---|---|---|
| `save_artifacts` hangs on rank 0 | Error is in `save_hf_weights` â†’ `dist.barrier()` | Increase timeout: `init_process_group("nccl", timeout=timedelta(minutes=60))` |
...
expert_memory_gb â‰ˆ experts_per_gpu * expert_params * 2 / 1e9  (bf16)
total_per_gpu â‰ˆ expert_memory_gb + attention_memory_gb + kv_cache_gb
```

**MiniMax-M2 example** (256 experts, ~230GB fp8 â†’ ~460GB bf16):
...
**Symptom**: Roundtrip completes but shows âŒ for all expert weights and raises `ValueError: Weight mismatch detected`.
...
**This is expected for FP8 models.** The conversion is correct; the comparison tolerance is insufficient for the FP8â†’BF16 precision gap.
...
3. **Escaping inside TRAIN_CMD**: Since `TRAIN_CMD` is a double-quoted string, escape inner `$` for Slurm variables that must expand at runtime (not sbatch time):
   ...
   - Host-side variables like `$GH_TOKEN`, `$LOGDIR`, `$WORKDIR` expand at sbatch time â€” no escaping needed.
