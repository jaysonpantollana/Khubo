...
description: "Use when working with azure-eventgrid-dotnet"
---
...
â””â”€â”€ EventGridPublisherClient
    â”œâ”€â”€ SendEventAsync(EventGridEvent)
    â”œâ”€â”€ SendEventsAsync(IEnumerable<EventGridEvent>)
    â”œâ”€â”€ SendEventAsync(CloudEvent)
    â””â”€â”€ SendEventsAsync(IEnumerable<CloudEvent>)

Pull Delivery (Namespaces)
â”œâ”€â”€ EventGridSenderClient
â”‚   â””â”€â”€ SendAsync(CloudEvent)
â””â”€â”€ EventGridReceiverClient
    â”œâ”€â”€ ReceiveAsync()
    â”œâ”€â”€ AcknowledgeAsync()
    â”œâ”€â”€ ReleaseAsync()
    â””â”€â”€ RejectAsync()
```
...
1. **Use CloudEvents** â€” Prefer CloudEvents for new implementations (industry standard)
2. **Batch events** â€” Send multiple events in one call for efficiency
3. **Use Entra ID** â€” Prefer managed identity over access keys
4. **Idempotent handlers** â€” Events may be delivered more than once
5. **Set event TTL** â€” Configure time-to-live for namespace events
6. **Handle partial failures** â€” Acknowledge/release events individually
7. **Use dead-letter** â€” Configure dead-letter for failed events
8. **Validate schemas** â€” Validate event data before processing
