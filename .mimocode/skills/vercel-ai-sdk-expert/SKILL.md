...
description: "Use when working with vercel-ai-sdk-expert"
---
    ...
    tools: {
      getWeather: tool({
        ...
        execute: async ({ location, unit = 'celsius' }) => {
          ...
          return `The weather in ${location} is ${temp}Â° ${unit}.`;
        },
      }),
    },
...
{m.role === 'assistant' && m.toolInvocations?.map((toolInvocation) => (
  <div key={toolInvocation.toolCallId} className="text-sm text-gray-500">
    {toolInvocation.state === 'result' ? (
      <p>âœ… Fetched weather for {toolInvocation.args.location}</p>
    ) : (
      <p>â³ Fetching weather for {toolInvocation.args.location}...</p>
    )}
  </div>
))}
```
...
- âœ… **Do:** Use `openai('gpt-4o')` or `anthropic('claude-3-5-sonnet-20240620')` format (from specific provider packages like `@ai-sdk/openai`) instead of the older edge runtime wrappers.
- âœ… **Do:** Provide a strict Zod `schema` and a clear `system` prompt when using `generateObject()`.
- âœ… **Do:** Set `maxDuration = 30` (or higher if on Pro) in Next.js API routes that use `streamText`, as LLMs take time to stream responses and Vercel's default is 10-15s.
- âœ… **Do:** Use `tool()` with comprehensive `description` tags on Zod parameters, as the LLM relies entirely on those strings to understand when and how to call the tool.
- âœ… **Do:** Enable `maxSteps: 5` (or similar) when providing tools, otherwise the LLM won't be able to reply to the user *after* seeing the tool result!
- âŒ **Don't:** Forget to return `result.toDataStreamResponse()` in Next.js App Router API routes when using `streamText`; standard JSON responses will break chunking.
- âŒ **Don't:** Blindly trust the output of `generateObject` without validation, even though Zod forces the shape â€” always handle failure states using `try/catch`.
