...
description: "Use when working with minimax-minimax-multimodal-toolkit"
---
...
# MiniMax CLI â€” Agent Skill Guide
...
Generate video. Default model: `MiniMax-Hailuo-2.3`. This is an async task â€” by default it polls until completion.
...
```bash
# stdout is always clean data â€” safe to pipe
mmx text chat --message "Hi" --output json | jq '.content'

# stderr has progress/spinners â€” discard if needed
mmx video generate --prompt "Waves" 2>/dev/null

# Chain: generate image â†’ describe it
...
CLI flags â†’ environment variables â†’ `~/.mmx/config.json` â†’ defaults.
