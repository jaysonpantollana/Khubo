...
description: "Use when working with holoscan-install-source"
---
...
# Holoscan SDK â€” Build from Source
...
- 10â€“30 min for a clean first build.

## Limitations

- Only recommended when published packages (Conda / container / apt / wheel) don't fit â€” debug symbols, custom CMake options, or unsupported configs.
- Still requires Docker â€” the `./run` script builds inside a container; this is not a true bare-metal build.
...
- If Docker is missing â†’ help install from https://docs.docker.com/engine/install/
- If GPU passthrough fails â†’ install NVIDIA Container Toolkit:
  ```bash
...
The `./run build` script handles container creation, CMake configuration, compilation, and install in one step. Warn the user this takes **10â€“30 minutes** on first run (downloads base image + compiles).
...
**Important:** Always single-quote the regex string when it contains `|` â€” without quotes, bash interprets `|` as a pipe and the command fails with `command not found`.
...
| Symptom | Fix |
|---------|-----|
| `bash: <TEST_NAME>: command not found` when running tests | The regex contains `\|` â€” wrap it in single quotes: `--options "-R '<regex>'"` |
