...
description: "Use when working with vss-deploy-profile"
---

---
name: vss-deploy-profile
description: Use to select, configure, deploy, verify, debug, or tear down a VSS profile (base, search, lvs, warehouse, edge). Not for standalone microservices â€” use the vss-deploy-* skill.
license: Apache-2.0
metadata:
  version: "3.2.0"
...
**Each profile's reference owns its sizing table.** Don't pick a deployment shape from this file â€” open the profile reference and check minimum GPU count for the host's hardware against the (mode Ã— platform) matrix there.
...
1. **Repo path** â€” auto-detect `video-search-and-summarization/` before
   asking the user. Use the detected path as `$REPO` for all subsequent
   commands.
2. **Credential gates** â€” see [`references/credentials.md`](references/credentials.md): `NGC_CLI_API_KEY` for local/local_shared NIM pulls, `NVIDIA_API_KEY` for remote NIM endpoints, and `HF_TOKEN` for edge recipes that use gated HF models.
3. **System prerequisites (GPU driver, Docker, NVIDIA Container Toolkit, kernel sysctls, and â€” if `ufw` is active â€” the [Docker-bridgeâ†’host firewall allow](references/prerequisites.md#firewall) so bridge NIMs can fetch clips from host-mode VST)** â€” full checks in [`references/prerequisites.md`](references/prerequisites.md). Canonical hardware/driver matrix is the [VSS prerequisites page](https://docs.nvidia.com/vss/3.2.0/prerequisites.html).
...
if sudo -n true 2>/dev/null; then
  echo "passwordless sudo â€” pre-flight will auto-install missing pieces"
else
  echo "sudo requires password â€” pre-flight will NOT auto-install; hand commands to the user"
fi
```
...
set but the user didn't ask for remote, surface it in Step 1 and ask â€” never
silently deploy remote because a var happened to exist.

If no combination on this host satisfies the profile's sizing requirements, **stop and report the blocker** â€” don't silently pick another shape.
...
### Step 0 â€” Tear down any existing deployment + clear data volumes
...
### Step 0a â€” Credentials gate (run before any env mutation)
...
30-second failure; the same 401 inside a NIM cold-start is a 10â€“20 min
...
| **`HOST_IP`** | In-cluster dial address: `ip route get 1.1.1.1` src (like `dev-profile.sh`; correct on LAN + cloud). If that interface is a VPN/tunnel, fall back to the LAN IP and **prompt the user** â€” [Network addressing](references/prerequisites.md#addressing). |
| **`EXTERNAL_IP`** | Browser-facing address; defaults to `${HOST_IP}`. Override when the browser path differs â€” cloud public IP, Brev secure-link (Step 1d), or tunnel; **ask the user where they browse from if unsure**. [Network addressing](references/prerequisites.md#addressing). |
...
### Step 1b â€” Prepare the data directory
...
### Step 1c â€” Initialize `generated.env`
...
### Step 1d â€” Brev only: detect first, then set `EXTERNAL_IP` to the secure-link domain

**Detect Brev before anything else** â€” a Brev-provisioned instance sets `BREV_ENV_ID` in `/etc/environment`; nothing else does:

```bash
grep -qE '^BREV_ENV_ID=' /etc/environment && echo "on Brev" || echo "not Brev"
```

- **not Brev** â†’ skip the rest of this step and **do not read [`references/brev.md`](references/brev.md)**; keep the normal `${HOST_IP}`-based `EXTERNAL_IP`.
- **on Brev** â†’ apply the Brev secure-link overrides from [`references/brev.md` Â§ Setup flow](references/brev.md#setup-flow) to `generated.env` (NOT `.env`). Those set `EXTERNAL_IP` / `VSS_PUBLIC_HOST` to the secure-link domain **and** `VSS_PUBLIC_HTTP_PROTOCOL=https` / `VSS_PUBLIC_WS_PROTOCOL=wss` / `VSS_PUBLIC_PORT=443` â€” setting `EXTERNAL_IP` alone leaves `http://â€¦:7777` UI/API/WS links that the browser blocks as mixed content.

### Step 2 â€” Build env_overrides
...
### Step 3 â€” Apply overrides + dry-run
...
### Step 3b â€” Verify resolved.yml has no unexpanded ${...} tokens
...
### Step 3c â€” Verify access to selected NGC artifacts
...
- Container images: `docker manifest inspect <nvcr.io/...>` after `docker
  login nvcr.io` â€” for gated `nvcr.io` repos a `401`/`403` here is a definitive
...
### Step 3d â€” Strip dangling optional `depends_on` from resolved.yml
...
If validation still fails after the normalizer runs, capture the error and inspect â€” that's a different bug (a dependency that's not optional, or another schema violation), not the dangling-depends_on case.
...
Ask: **"Looks good â€” deploy now?"** and wait for confirmation before Step 5.

**Exception â€” autonomous mode.** If the user's request already asks you to run autonomously (e.g. "deploy X autonomously", "run without confirmation", "non-interactive"), skip the confirmation prompt and proceed straight to Step 5. This path exists so automated eval / CI invocations don't hang waiting for a human reply they'll never get. In all other cases, a human must approve.
...
> **Avoid broad `--force-recreate` on ordinary retries** â€” it destroys warm
> NIM containers (another 3â€“5 min torch.compile + CUDA-graph capture each).
...
### Step 5b â€” Wait until the stack is actually healthy

**Gate 0 â€” container count must be > 0.** Refuse to proceed past `up -d` until the started count (`docker compose -f resolved.yml ps -q | wc -l`) is non-zero and â‰¥ the expected count (`config --services | wc -l`); a zero/short count almost always means a missing `--env-file` in Step 5. The exact gate plus the full readiness procedure live in [`references/readiness.md`](references/readiness.md).

Cold deploys can take 10â€“20 min, and each profile reference lists the required endpoints. **Never declare deploy done after `up -d`; only after every documented endpoint succeeds.**

## Tear Down

To tear down a deployment â€” full host reclaim or cache-preserving redeploy / profile
switch â€” follow [`references/teardown.md`](references/teardown.md). Always tear down
...
The LLM/VLM NIM probes â€” including the `*_MODE=remote` handling that skips
`localhost:3008x` (where a connection refused is expected) and probes the
selected `*_BASE_URL/v1/models` via `scripts/probe_remote_models.sh` â€” are in
...
The common-error quick reference, the full symptom â†’ cause â†’ fix table, the
unexpanded-`${...}` diagnostic, and the NIM endpoint probes are consolidated in
[`references/troubleshooting.md`](references/troubleshooting.md) â€” start there
