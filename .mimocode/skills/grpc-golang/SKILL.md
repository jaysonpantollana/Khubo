---
name: grpc-golang
description: "Use when working with grpc-golang"
---
...
- âœ… **Do:** Use Buf to standardize your toolchain and linting with `buf.yaml` and `buf.gen.yaml`.
- âœ… **Do:** Always use semantic versioning in package paths (e.g., `package api.v1`).
- âœ… **Do:** Enforce mTLS for all internal service-to-service communication.
- âœ… **Do:** Handle `ctx.Done()` in all streaming handlers to prevent resource leaks.
- âœ… **Do:** Map domain errors to standard gRPC status codes (e.g., `codes.NotFound`).
- âŒ **Don't:** Return raw internal error strings or stack traces to gRPC clients.
- âŒ **Don't:** Create a new `grpc.ClientConn` per request; always reuse connections.
