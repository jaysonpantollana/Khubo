---
name: blueprint
description: "Use when working with blueprint"
---

---
name: blueprint
description: "Turn a one-line objective into a step-by-step construction plan any coding agent can execute cold. Each step has a self-contained context brief â€” a fresh agent in a new session can pick up any step without reading prior steps."
category: planning
risk: safe
source: community
...
1. **Research** â€” Scans the codebase, reads project memory, runs pre-flight checks
2. **Design** â€” Breaks the objective into one-PR-sized steps, identifies parallelism, assigns model tiers
3. **Draft** â€” Generates the plan from a structured template with branch workflow rules, CI policy, and rollback strategies inline
4. **Review** â€” Delegates adversarial review to a strongest-model sub-agent (falls back to default model if unavailable)
5. **Register** â€” Saves the plan and updates project memory
...
- âœ… Use for tasks requiring 3+ PRs or multiple sessions
- âœ… Let Blueprint auto-detect git/gh availability â€” it degrades gracefully
- âŒ Don't invoke for tasks completable in a single PR
- âŒ Don't invoke when the user says "just do it"
...
- **Zero runtime risk**: Pure markdown â€” no hooks, no scripts, no executable code
