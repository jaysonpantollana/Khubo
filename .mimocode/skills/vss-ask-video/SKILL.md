---
name: vss-ask-video
description: "Use when working with vss-ask-video"
---
...
Use this skill when you need details about the video which requires VLM to look at the video frames â€” for example the agent has **no** usable prior answer and needs a **fresh look at the pixels** for a specific clip.
...
- The user asks for **details** that **cannot be answered** from existing messages, summaries, Elasticsearch/MCP results, or filenames aloneâ€”you need **model inference on the video**.
...
This skill requires a VSS profile that serves the `video_understanding` tool â€” typically **base** (recommended) or **lvs**. Before any request:
...
2. **If the probe fails**, ask the user:
   ...
   - If yes â†’ hand off to `/vss-deploy-profile -p base` (or `-p lvs` if the user prefers). Return here once it succeeds.
   - If no â†’ stop.
...
3. **If a matching sensor is present** â†’ proceed to the Agent workflow below.

4. **If no matching sensor is present** â€” upload the video first, then re-list to confirm the new sensor appears:
   ```bash
   # filename: must not contain whitespace
   # timestamp: ISO 8601 UTC â€” default 2025-01-01T00:00:00.000Z if user did not specify
...
1. **Clip** â€” Identify **sensor id**, **filename**, or **URL** for one video segment. If ambiguous, ask the user.
...
- **vss-manage-video-io-storage** â€” VST storage/replay URLs so **`VIDEO_URL`** is valid for the VLM.
- **vss-generate-video-report** â€” timestamped **reports** via **Mode A (direct VLM)** or **Mode B (video-analytics incidents)**; this skill is **VSS-agent `/generate`** for ad-hoc **video Q&A**.
