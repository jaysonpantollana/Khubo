...
description: "Use when working with snowflake-development"
---
...
- Use `snake_case` for all identifiers. Avoid double-quoted identifiers â€” they create case-sensitive names requiring constant quoting.
- Use CTEs (`WITH` clauses) over nested subqueries.
- Use `CREATE OR REPLACE` for idempotent DDL.
- Use explicit column lists â€” never `SELECT *` in production (Snowflake's columnar storage scans only referenced columns).

### Stored Procedures â€” Colon Prefix Rule
...
- `SELECT *` breaks on schema changes â€” use explicit column lists.
...
-- Tasks start SUSPENDED â€” you MUST resume them
...
- Do NOT include empty/null values in edit specs â€” clears existing values.
- Tool descriptions are the #1 quality factor.
- Never modify production agents directly â€” clone first.
...
- DataFrames are lazy â€” executed on `collect()`/`show()`.
- Do NOT use `collect()` on large DataFrames â€” process server-side.
