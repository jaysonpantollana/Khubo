---
name: clarity-gate
description: "Use when working with clarity-gate"
---
...
### HITL Claim Validation (Â§1.3.2-1.3.3)
| Code | Check | Severity |
|------|-------|----------|
...
Computes stable, hash-based claim IDs for HITL tracking (per Â§1.3.4).
...
**Test vectors:**
- `claim_id("Base price is $99/mo", "api-pricing/1")` â†’ `claim-75fb137a`
- `claim_id("The API supports GraphQL", "features/1")` â†’ `claim-eb357742`

### scripts/document_hash.py

Computes document SHA-256 hash per FORMAT_SPEC Â§2.2-2.4 with full canonicalization.
...
>
> This skill checks whether claims are properly marked as uncertainâ€”it cannot verify if claims are actually true. 
>
...
The 9 Verification Points guide **semantic review** â€” content quality checks that require judgment (human or AI). They answer questions like "Should this claim be hedged?" and "Are these numbers consistent?"
...
Claim Extracted --> Does Source of Truth Exist?
   ...
   Consistency & Verification     Two-Round Verification
           |                               |
   PASS / BLOCK                   Round A â†’ Round B â†’ APPROVE / REJECT
```
...
### Tier 2: Two-Round HITL Verification â€” MANDATORY
...
|---|-------|--------|-------------|------|
| 1 | [claim] | âœ“ Confirmed | [name] | [date] |
...
- YAML frontmatter with all required fields:
  - `clarity-gate-version` â€” Tool version (no "v" prefix)
  - `processed-date` â€” YYYY-MM-DD format
  - `processed-by` â€” Processor name
  - `clarity-status` â€” CLEAR or UNCLEAR
  - `hitl-status` â€” PENDING, REVIEWED, or REVIEWED_WITH_EXCEPTIONS
  - `hitl-pending-count` â€” Integer â‰¥ 0
  - `points-passed` â€” e.g., `1-9` or `1-4,7,9`
  - `hitl-claims` â€” List of verified claims (may be empty `[]`)
...
- `rag-ingestable` â€” **Computed by validators**, not manually set. Shows `true` only when `CLEAR | REVIEWED` with no exclusion blocks.
- `document-sha256` â€” Required. 64-char lowercase hex hash for integrity verification. See spec Â§2 for computation rules.
- `exclusions-coverage` â€” Optional. Fraction of body inside exclusion blocks (0.0â€“1.0).
...
**Why no explicit status field?** Field presence is self-enforcingâ€”you can't accidentally set status without providing who/when.
...
See CLARITY_GATE_FORMAT_SPEC.md Â§4 for complete rules.
...
- **Staleness markers:** Use `[STABLE]`, `[CHECK]`, `[VOLATILE]`, `[SNAPSHOT]` in content
  - `[STABLE]` â€” Safe to cite without rechecking
  - `[CHECK]` â€” Verify before citing
  - `[VOLATILE]` â€” Changes frequently; always verify
  - `[SNAPSHOT]` â€” Point-in-time data; include date when citing
...
### v2.1.3 (2026-03-02)
- **FIXED:** `document_hash.py` now implements full FORMAT_SPEC Â§2.1-2.4 compliance
- **FIXED:** Fence-aware end marker detection (Quine Protection per Â§2.3/Â§8.5)
...
- **ADDED:** YAML-aware `document-sha256` removal with multiline continuation support (Â§2.2)
...
- **ADDED:** New validation codes: E-ST10, W-ST11, W-HC01, W-HC02, E-SC06 (FORMAT_SPEC Â§1.2-1.3)
...
- **ADDED:** Validation rules mapping (9 points â†’ rule codes)
- **ADDED:** CGD output format template with `<!-- CLARITY_GATE_END -->` markers
- **ADDED:** Quine Protection note (Â§2.3 fence-aware marker detection)
- **ADDED:** Redacted Export feature (Â§8.11)
