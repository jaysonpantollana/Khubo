...
description: "Use when working with earth2studio-data-fetch"
---
...
- **Variables** â€” what do they want? Use Earth2Studio variable names
...
- **Time** â€” what date/time range? A single timestamp, a range, or multiple
  discrete times?
- **Data type** â€” analysis/reanalysis (historical state) or forecast (lead-time based)?
- **Lead time** (forecast only) â€” how far ahead? Which initialization time?
- **Region** â€” global or regional (e.g. North America for HRRR)?
- **Output format** â€” xarray DataArray (default), save to file (NetCDF/Zarr)?
...
- **Temporal coverage** â€” operational sources (GFS, HRRR) have limited
  history; reanalysis (ERA5 via ARCO/CDS/WB2) goes back decades
- **Spatial resolution** â€” HRRR is 3km NA-only; GFS is 0.25Â° global;
  WB2ERA5_32x64 is 5.625Â° global
- **Update frequency** â€” some are real-time, some have multi-day lag
...
The lexicon VOCAB maps Earth2Studio variable names â†’ source-specific
...
constructor arguments and call signature before writing the script â€” they can
...
1. Map plain language â†’ `z500`, `t2m`
...
- **Network required** â€” all data sources fetch from remote stores
  (GCS, S3, CDS API)
- **No local file loading** â€” for local NetCDF/Zarr, use
  `DataArrayFile`/`DataSetFile` directly
- **One source type per script** â€” cannot mix analysis and forecast
  sources in a single call
- **Variable availability varies** â€” not all sources provide all
  variables; always verify via lexicon
- **Rate limits** â€” CDS API has queue-based throttling; GCS/S3 sources
