...
description: "Use when working with cloudflare-workers-expert"
---
...
- âœ… **Do:** Use `env.VAR_NAME` for secrets and environment variables.
- âœ… **Do:** Use `Response.redirect()` for clean edge-side redirects.
- âœ… **Do:** Use `wrangler tail` for live production debugging.
- âŒ **Don't:** Import large libraries; Workers have limited memory and CPU time.
- âŒ **Don't:** Use Node.js specific libraries (like `fs`, `path`) unless using Node.js compatibility mode.
