...
description: "Use when working with nemoclaw-user-configure-inference"
---
...
The user-local path cannot bootstrap system packages without elevation, so if `zstd` is missing it prints per-distro install hints and exits â€” install `zstd` manually, then rerun onboarding.
...
response (including `401`) as proof the proxy is alive â€” they only fail
...
- **[references/tool-calling-reliability.md](references/tool-calling-reliability.md)** â€” Explains Ollama tool-call leak symptoms, when vLLM with a tool-call parser is recommended, and how to repoint NemoClaw to a parser-aware local endpoint.
...
- `nemoclaw-user-get-started` â€” Quickstart (use the `nemoclaw-user-get-started` skill) for first-time installation
