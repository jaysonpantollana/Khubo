...
description: "Use when working with tao-generate-video-reasoning-annotations"
---
...
description: >-
  ...
  Chain-of-Thought training data â€” multi-level captions, structured
...
Generate Chain-of-Thought training datasets from videos by producing multi-level captions, structured descriptions, and QA pairs (MCQ, binary, open-ended) with step-by-step reasoning traces. Domain-agnostic by default â€” customize prompts for any video domain.

## Purpose

Transform raw videos into CoT Q&A training data for video understanding models. VLMs (e.g., Gemini, Qwen) act as "teacher" annotators: Steps 0â€“1 require the model to see the video (VLM calls); Steps 2â€“3 are text-to-text (cheaper LLM calls).

## Pipeline architecture

```
Step 0:  [Optional] Filter & classify videos  â†’ Keep domain-relevant, classify anomaly vs normal
Step 1a: Global + dense captions               â†’ VLM: narrative summary + timestamped events
Step 1b: Chunk captions                         â†’ VLM: fixed-duration segment micro-captions
Step 1c: [Optional, anomaly only] Highlight     â†’ LLM extracts anomaly timestamp, VLM captions clip
Step 2:  Description synthesis                  â†’ LLM: synthesize captions into structured narrative
Step 3:  QA generation                          â†’ LLM: MCQ, binary, open-ended with reasoning
Step 4:  Parse outputs                          â†’ Per-task `tao-vl-reason-v1.0` JSON files
```

Steps are individually selectable via `workflow.steps`. The pipeline has built-in resume â€” each step skips already-processed videos, so re-running after a prompt tweak is safe.

## Initial consultation

When the user invokes this skill, walk through these questions in order. Don't skip â€” getting domain and VLM access right up front prevents wasted runs.
...
### 2. Domain â€” drives prompt selection
...
| **warehouse** (industrial site CCTV â€” safety, operations, security) | Same pattern. Set `prompts_module: "nvidia_tao_ds.auto_label.video_reasoning_annotation.prompts_warehouse"`, or copy `references/prompts_warehouse.py` and tune. |
| **custom** (any other domain) | **Run the workshop in [references/domain_adaptation.md](references/domain_adaptation.md)**. It walks through: Phase 1 â€” question types the user wants the model to answer; Phase 2 â€” caption-requirements checklist; Phase 3 â€” fill the `[PLACEHOLDER]` markers in `nvidia_tao_ds.auto_label.video_reasoning_annotation.prompt_template`. The two reference modules above are working examples to model after. Do this **before** any pipeline runs. |

### 3. Anomaly / normal / mixed

- Mixed dataset â†’ `workflow.mode: "auto"` (Step 0 classifies each video).
- Pre-split anomaly only â†’ `workflow.mode: "anomaly"`, drop Step 0.
- Pre-split normal only â†’ `workflow.mode: "normal"`, drop Steps 0 and 1c.

### 4. VLM / LLM endpoint â€” confirm access **before** running
...
- Steps 2â€“3 are text-only â€” a smaller/cheaper LLM is fine for `llm.backend` even when `vlm.backend` is a frontier video model.

If the user has **no endpoint at all** and wants to self-host, point them at the `skills/applications/tao-run-inference-service` skill â€” a workflow that stands up a network-specific TAO inference microservice locally and exposes an OpenAI-compatible endpoint. Should support Cosmos, Qwen, and Gemma. Check `skills/applications/tao-run-inference-service/references/service.yaml` for the current `valid_network_arch_config_basenames` list before relying on a specific model.

If the user doesn't have endpoint access ready and isn't ready to set one up, stop here and help them figure it out first.

### 5. Pilot vs full run

- **Recommend a 5â€“10 video pilot** when domain is `custom`, when any prompt was edited, or when this is the user's first run.
...
Use this when running a 5â€“10 video pilot:

1. Run the pipeline on the pilot subset with the chosen `prompts_module` and `workflow.mode`.
2. Inspect `results_dir/step_1a_caption/captions.jsonl` â€” captions accurate, capturing the right level of detail?
3. Inspect `results_dir/step_3_qa/qa_output.jsonl` â€” questions meaningful, answers correct, reasoning logical?
...
Quality compounds downstream â€” bad captions produce bad descriptions which produce bad QA. Focus iteration on Step 1a/1b output first; descriptions and QA usually improve once captions are right.
...
- **Built-in (general)**: `nvidia_tao_ds.auto_label.video_reasoning_annotation.prompts` â€” domain-agnostic, used by default.
- **Template**: `nvidia_tao_ds.auto_label.video_reasoning_annotation.prompt_template` â€” same 26 keys with `[PLACEHOLDER]` markers for domain customization.
...
All outputs go to `results_dir/` with per-step subdirectories (`step_0_filter/`, `step_1a_caption/`, â€¦, `step_4_output/`):

- **Steps 0â€“3**: JSONL â€” one JSON object per video per line.
...
- **VLM endpoint**: at least one â€” Gemini API key or OpenAI-compatible endpoint.
