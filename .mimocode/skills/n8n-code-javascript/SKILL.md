...
description: "Use when working with n8n-code-javascript"
---
...
```javascript
// âŒ WRONG - Will return undefined
...
```javascript
// âŒ WRONG: Object without array wrapper
return {
  json: {field: value}
};

// âŒ WRONG: Array without json wrapper
return [{field: value}];

// âŒ WRONG: Plain string
return "processed";

// âŒ WRONG: Raw data without mapping
...
```javascript
// âŒ WRONG: Using n8n expression syntax in code
...
```javascript
// âŒ WRONG: Returning object instead of array
...
```javascript
// âŒ WRONG: Crashes if field doesn't exist
...
// âŒ WASTEFUL: Transform everything, then filter
const processed = $input.all()
