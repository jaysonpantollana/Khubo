...
description: "Use when working with tao-run-platform"
---
...
Install `nvidia-tao-sdk[all]` before using this platform â€” the `[all]` extra pulls in every platform-specific dependency (Lepton, Brev, S3 utilities, etc.):
...
The package index is environment-specific â€” the runner/container is expected to have a working `pip` configuration (e.g. `~/.pip/pip.conf`, `PIP_INDEX_URL`, `PIP_EXTRA_INDEX_URL`, or proxy). If the install fails for index/network reasons, that's a runner setup issue; this skill stays agnostic to the registry.
...
Credentials come from **environment variables** â€” sourced from `~/.config/tao/.env` (auto-loaded by the skill bank's SessionStart hook).
...
# or
sdk = BrevSDK()      # reads BREV_API_TOKEN (optional â€” falls back to brev login)
```
...
|---|---|---|
| Lepton | `LEPTON_WORKSPACE_ID`, `LEPTON_AUTH_TOKEN` | â€” |
| Brev | â€” (manual `brev login` works) | `BREV_API_TOKEN` |
| S3 I/O (any platform) | `S3_BUCKET_NAME`, `ACCESS_KEY`, `SECRET_KEY` | `S3_ENDPOINT_URL`, `CLOUD_REGION` |
| Container env | `NGC_KEY` | `HF_TOKEN` |

The agent never reads credential values â€” it only checks presence with `[ -n "$VAR_NAME" ]`.
...
For background runs, persist `job.id` and the `state_file` path, then re-attach later by constructing the same SDK and calling `get_job_status(job_id)` â€” job state is read from the on-disk store.
...
specs["dataset"]["train_csv"] = f"{base}/train.csv"   # nested â€” see "spec is nested dicts"
```
...
SDK error â†’ root cause â†’ fix mappings are in
...
similar â€” the entries map exception text to the underlying cause.
