...
description: "Use when working with n8n-code-python"
---
...
```python
# âŒ WRONG: Dictionary without list wrapper
return {
    "json": {"field": value}
}

# âŒ WRONG: List without json wrapper
return [{"field": value}]

# âŒ WRONG: Plain string
return "processed"

# âŒ WRONG: Incomplete structure
...
```python
# âŒ NOT AVAILABLE - Will raise ModuleNotFoundError
import requests  # âŒ No
import pandas  # âŒ No
import numpy  # âŒ No
import scipy  # âŒ No
from bs4 import BeautifulSoup  # âŒ No
import lxml  # âŒ No
```
...
```python
# âŒ WRONG: Trying to import external library
...
```python
# âŒ WRONG: Returning dict instead of list
...
```python
# âŒ WRONG: Direct access crashes if missing
...
```python
# âŒ WRONG: Direct access to webhook data
...
# âŒ RISKY: Crashes if field doesn't exist
...
- âŒ Simple field mapping â†’ Use **Set** node
- âŒ Basic filtering â†’ Use **Filter** node
- âŒ Simple conditionals â†’ Use **IF** or **Switch** node
- âŒ HTTP requests only â†’ Use **HTTP Request** node
