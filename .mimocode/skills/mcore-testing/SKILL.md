---
name: mcore-testing
description: "Use when working with mcore-testing"
---
...
```text
tests/
â”œâ”€â”€ unit_tests/          # pytest, 1 node Ã— 8 GPUs, torch.distributed runner
â”œâ”€â”€ functional_tests/    # end-to-end shell + training scripts
â”‚   â””â”€â”€ test_cases/
â”‚       â””â”€â”€ {model}/{test_case}/
â”‚           â”œâ”€â”€ model_config.yaml          # training args
â”‚           â””â”€â”€ golden_values_{env}_{platform}.json
â””â”€â”€ test_utils/
    â”œâ”€â”€ recipes/
    â”‚   â”œâ”€â”€ h100/        # YAML recipes for H100 jobs
    â”‚   â””â”€â”€ gb200/       # YAML recipes for GB200 jobs
    â””â”€â”€ python_scripts/  # helpers (recipe_parser, golden-value download, â€¦)
```
...
HuggingFace connectivity, â€¦) before declaring a genuine failure.
...
with `-broken` â€” **do not delete the entry**:
...
- `pyproject.toml` sets `addopts = --durations=15 -s -rA` â€” stdout is not
...
3. Apply markers as needed:
   - `@pytest.mark.internal` â€” skipped on `legacy` tag
   - `@pytest.mark.flaky_in_dev` â€” skipped in `dev` environment (CI default; use this to disable a flaky test without blocking the standard pipeline)
   - `@pytest.mark.flaky` â€” skipped in `lts` environment
   - `@pytest.mark.experimental` â€” `latest` tag only
