...
description: "Use when working with minimax-vision-analysis"
---
...
**OpenCode** â€” add to `~/.config/opencode/opencode.json` or `package.json`:
```json
{
  "mcp": {
    "MiniMax": {
...
**Important:** If the user does not have a MiniMax Token Plan subscription, inform them that the `understand_image` tool requires one â€” it cannot be used with free or other tier API keys.
...
(1) Strengths â€” what works well, (2) Issues â€” usability or design problems,
