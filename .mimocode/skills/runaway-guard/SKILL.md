---
name: runaway-guard
description: "Use when working with runaway-guard"
---
...
# runaway-guard â€” $-Cost is the Third Complexity Dimension

Every loop has time complexity and space complexity. A loop that calls a paid API has a third: **dollars per execution**. The model tracks the first two automatically. It does not track the third, so it ships code where a single bug â€” a retry without bound, a stream reconnect storm, an agent that re-queues itself, a webhook that fires the same job twice â€” silently spends real money.
...
**Violating the letter of these rules is violating the spirit of the skill.** "I'm only testing locally" is the exact rationalization that ships the $200 bill â€” local code hits the same paid API as production.
...
A cap only in code can be bypassed by a bug in that code. A cap only at the provider can be hit during normal usage and degrade the product. You need both. If you cannot state both in one sentence each, you have not designed the call site â€” you have written a wish.
...
1. **Every call site gets a one-line cost contract.** Before writing any paid-API call, state in one sentence:
   ...
   - **Max $ per run:** `max_calls Ã— unit_cost` â€” compute it, don't estimate.
...
2. **Every loop calling a paid API gets an explicit iteration bound, not just a termination argument.** `invariant-guard` requires a termination measure. runaway-guard requires the bound to be a **concrete integer in code**, not just "eventually terminates":
   ...
   ```ts
   // âŒ Terminates in theory. Bills $200 in practice.
   while (job.status !== 'done') {
     await fal.run(...);
   }

   // âœ… Concrete bound â€” wallet invariant explicit.
   const MAX_CALLS = 20;
...
3. **Every retry path is bounded by attempts AND total elapsed cost, not by time alone.** Exponential backoff with no attempt cap is a wallet attack on yourself.
   - Max attempts: a small integer (3â€“5 for transient errors, 1 for 4xx).
   - Cap counts across the whole pipeline, not just one library â€” Inngest retries Ã— SDK retries Ã— your own retry wrapper multiply.
...
4. **Every fan-out path declares a concurrency limit.** Parallel calls multiply cost per wall-clock second. State the limit in code, at the queue (Inngest `concurrency`), and at the provider where supported:
   ...
   - In-process: `p-limit`, semaphore, or batched `Promise.all` chunks â€” never an unbounded `Promise.all(items.map(...))` on a paid API.
...
5. **Every paid API has a matching provider-side hard cap, configured out of band.** Defense in depth: if the code is wrong, the provider stops the bleeding. Document the cap in the same file as the call site so future readers know it exists.
   ...
   |---|---|
   | **Fal.ai** | Dashboard â†’ Billing â†’ **Spend Limit** (e.g. $50/day). Hard stop on exceed. |
   | **Anthropic** | Console â†’ Workspaces â†’ **Workspace Budget** with hard limit. Per-workspace, per-month. |
   | **OpenAI** | Org â†’ Settings â†’ **Usage limits** (org-level hard limit blocks requests). âš ï¸ Per-*project* monthly budgets are **soft thresholds only** â€” they alert but do not block. For a real hard cap use the org-level Usage limit, a billing gateway, or your own fail-closed budget check. |
   | **Replicate** | Account â†’ Billing â†’ **Spend limit**. Per account. |
   | **ElevenLabs** | Workspace â†’ **Usage limits** per workspace / API key. |
   | **Together / Groq / Cohere / Mistral** | Each has a billing dashboard with a monthly spend cap â€” set it before first deploy, not after. |
...
7. **Make the "amplifier" patterns explicit and forbidden by default.** These are the shapes that turn small bugs into large bills:
   ...
   - **Cache-miss stampede on a paid call.** N concurrent requests for the same uncached key â†’ N billed calls. Use `singleflight` / request coalescing.

## The pre-write protocol

Before producing code that calls a paid API, your message must contain â€” in this order:
...
3. **Max $ per run.** `max_calls Ã— unit_cost`. Compute it.
...
8. **The code** â€” with the cost contract in a comment above the call site.
9. **Self-check.** One line: "in the worst case, this code bills $X and the provider cap stops it at $Y."

If any of 1â€“7 is missing, do not emit code.

## Worked trap â€” the Inngest + Fal $200 night
...
export const generateCampaign = inngest.createFunction(
  { id: 'gen-campaign' },                              // âŒ no concurrency limit
  { event: 'campaign/start' },
  async ({ event, step }) => {
    const prompts = await step.run('fetch', () => fetchPrompts(event.data.id));
    // âŒ unbounded fan-out, no per-run cap, no idempotency
    await Promise.all(prompts.map(p => fal.run('fal-ai/flux-pro', { input: { prompt: p } })));
  }
);
```

**What went wrong.** `fetchPrompts` had a bug: on a transient DB error it returned the partial list *plus the previous run's list appended*. Inngest retried the function at its default retry count (multiple attempts in addition to the initial one). Each retry re-ran `fetchPrompts`, each retry doubled the list (40 â†’ 80 â†’ 160 â†’ 320 prompts). `Promise.all` fanned all 320 out concurrently. At $0.05/image: **$16/retry Ã— triangular growth across overnight retries on the schedule = ~$200 by morning.**

**Why each rule would have caught it.**

| Rule | Catch |
|---|---|
| 1. Cost contract | Forces writing "max calls per run". The number `prompts.length` is not a known integer â†’ rule fails â†’ write a cap. |
| 2. Concrete iteration bound | `Promise.all(prompts.map(...))` has no integer bound â†’ rule fails â†’ wrap in chunks with `MAX_IMAGES_PER_RUN`. |
| 3. Retry policy | Inngest default retries Ã— no idempotency key = double-billed work. Rule forces an idempotency key per `(campaignId, promptHash)`. |
...
| 7. Amplifier audit | "Self-rescheduling jobs" â€” Inngest's retry IS self-rescheduling. The audit forces you to consider it. |
...
//   idempotency: key = `${campaignId}:${sha1(prompt)}` â€” provider-side dedup window 24h
...
Note: the bug in `fetchPrompts` is still there. The protocol does not fix that bug â€” it makes the bug **cost $2.50 instead of $200** while you find it. That is the entire point of defense in depth.

## Common runaway patterns and their wallet invariants

| Pattern | Wallet invariant to write | Hard cap to set |
|---|---|---|
| Fan-out over a list of items | `total_cost â‰¤ list_len Ã— unit_cost â‰¤ MAX_$_PER_RUN` | provider daily spend limit |
| Retry on transient error | `total_cost â‰¤ attempts Ã— unit_cost`, attempts â‰¤ 5 | provider daily spend limit; alert at 50% |
| Agent loop ("ask model what to do next") | `total_cost â‰¤ MAX_STEPS Ã— per_step_cost`, depth â‰¤ MAX_DEPTH | per-agent-run cost ceiling, kill-switch |
| Polling for job completion | `total_cost â‰¤ ceil(MAX_WAIT_MS / poll_interval) Ã— poll_cost` | absolute deadline + alert |
| Webhook handler â†’ API call | idempotency key required; cycle if webhook is triggered by the same API | provider rate limit per key |
| Stream reconnect | `attempts â‰¤ MAX_RECONNECTS`, exponential backoff with cap | provider connection cap |
| Cache miss stampede | singleflight â†’ `cost â‰¤ 1 Ã— unit_cost` per key per window | n/a (deduped in code) |
| Self-scheduling job | recursion depth bounded by ledger row, not by code | scheduler-level dedup + max runs/day |
| Multi-provider fallback | sum across providers â‰¤ MAX_$_PER_RUN | hard cap on each provider separately |
...
### Fal.ai
- Dashboard â†’ **Billing â†’ Spend Limit**. Daily and monthly hard caps. Hard stop on exceed.
...
### Anthropic
- Console â†’ **Workspaces** â†’ create a Workspace per environment.
- Each Workspace gets a **Budget** with a **hard limit** (request blocking) and **soft limit** (email alert).
- Use a per-Workspace API key â€” a leaked dev key cannot exceed the dev Workspace budget.
- Prompt caching reduces cost ~90% for repeated context; cap is on unblended cost so caching extends the budget.

### OpenAI
- âš ï¸ **Per-project monthly budgets are soft only.** OpenAI's Help Center documents project budgets as "soft spending thresholds" that send alerts but do **not** enforce a hard cap. A runaway can continue past the documented project budget.
- For a real hard cap, use one of:
  - **Org-level Usage limits** (Org â†’ Settings â†’ Limits) â€” block requests on exceed.
...
### Replicate
- Account â†’ **Billing â†’ Spend limit**. Account-wide hard cap.
- Use separate tokens per environment; rotate on leak.

### ElevenLabs
- Workspace â†’ **Usage limits**. Set per API key.
- Voice cloning is billed per character; cap the character count in code AND per key.

### Inngest (queue layer â€” not paid AI but the multiplier)
- `concurrency: { limit: N }` on every function that calls a paid API.
- `retries: 2` (Inngest default is **4 retries**, i.e. up to 5 attempts including the initial â€” confirm against current Inngest docs) for paid call functions; fewer attempts on idempotent failures. Worst-case wallet math: `attempts = 1 + retries`, so a default `step.run()` can bill **5Ã—**, not 4Ã—.
- `NonRetriableError` for 4xx â€” never retry a 4xx into a paid API.
...
|---|---|
| Empty input list | 0 calls, 0 cost, return early â€” do not even auth |
...
| All calls fail with 5xx | bounded retries, total cost â‰¤ attempts Ã— unit, alert on full exhaustion |
...
- Use a named constant (`MAX_IMAGES_PER_RUN`, `MAX_AGENT_STEPS`) for the bound â€” never a magic number inline.
- Wrap fan-out in `p-limit` or equivalent â€” never raw `Promise.all` over a paid API.
...
- **invariant-guard** â€” when the loop or recursion calling the paid API has no written termination measure. Termination is a precondition for a wallet cap; invariant-guard establishes it, runaway-guard bounds the cost.
- **complexity-cuts** â€” when the runaway has already shipped and you are diagnosing an unexpected bill. Treat the unintended fan-out as a complexity bug, write a characterization test, then transform one step at a time.
- **lemmaly** â€” when designing a new agent loop or batch pipeline that fans out paid calls. Pick the algorithm and data structure first; come back here for the wallet invariant per step.
- **mathguard** â€” when the cost driver is compute on a sublinear-algorithm problem (vector search, sketching, FFT) rather than per-call billing; mathguard sets the algorithmic floor that determines per-call work.
...
| "The list is small, fan-out is fine." | The list is small *today*. Next week it is fetched from a table that grew 50Ã—. The cap exists for next week. |
| "Inngest already retries, so I don't need a retry policy." | Inngest retries Ã— your retry wrapper Ã— SDK retries = 27 attempts. Each one bills. |
| "The API call is cheap, $0.001." | At 10,000 unintended invocations that is $10 â€” and the count is exactly what you failed to bound. |
...
## Red flags â€” STOP and write the cost contract first
...
- "Just for tonight" â€” overnight is exactly when runaway loops bill the most.
...
- [ ] Retry policy is explicit: max attempts, 4xx â†’ no retry, idempotency key per call.
...
- [ ] In the worst case the code bills â‰¤ MAX_$_PER_RUN, and even with a bug the provider cap stops loss at MAX_$_PER_DAY.
...
- **Not a billing system.** runaway-guard enforces *intent* (caps, contracts, audits) at code-write time. It does not meter spend in production â€” pair it with the provider's hard cap and observability (LLM-cost dashboards, log alerts) for runtime enforcement.
...
- **Streaming and per-token pricing.** For per-token APIs (Anthropic, OpenAI), `max calls` is a proxy â€” the real cap is `max input tokens Ã— max output tokens Ã— per-token rate`. Adapt the protocol: replace `max calls per run` with `max tokens per run`.
...
- **Does not replace incident response.** When a bill has already arrived, escalate to `complexity-cuts` for the corrective rewrite â€” runaway-guard prevents the next one, not the current one.
