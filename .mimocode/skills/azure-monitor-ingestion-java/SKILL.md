...
description: "Use when working with azure-monitor-ingestion-java"
---
...
1. **Batch logs** â€” Upload in batches rather than one at a time
2. **Use concurrency** â€” Set `maxConcurrency` for large uploads
3. **Handle partial failures** â€” Use error consumer to log failed entries
4. **Match DCR schema** â€” Log entry fields must match DCR transformation expectations
5. **Include TimeGenerated** â€” Most tables require a timestamp field
6. **Reuse client** â€” Create once, reuse throughout application
7. **Use async for high throughput** â€” `LogsIngestionAsyncClient` for reactive patterns
