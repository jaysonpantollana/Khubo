...
description: "Use when working with n8n-expression-syntax"
---
...
âŒ $json.email  (no braces - treated as literal text)
âŒ {$json.email}  (single braces - invalid)
```
...
```javascript
âŒ {{$node["http request"].json}}  // lowercase
âŒ {{$node["Http Request"].json}}  // wrong case
