---
name: ab-test-setup
description: "Use when working with ab-test-setup"
---
...
2. **Variant attribution:** Verify that the variant assignment ID is attached to every fired event â€” not just the entry event. Use your analytics' raw event view to compare a sample of 5+ events per variant.
...
4. **Sample randomization:** Pull the first 100 assignment records from your assignment table; the variant split should be within Â±5% of the configured allocation.
5. **Guardrail metric pipeline:** Each guardrail metric defined in Â§6ï¸âƒ£ must have a working dashboard or alert by the time the test launches.
