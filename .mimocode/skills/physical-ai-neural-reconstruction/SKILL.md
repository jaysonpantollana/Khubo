...
description: "Use when working with physical-ai-neural-reconstruction"
---
...
- Order multi-step NuRec workflows (data â†’ conversion â†’ train â†’
  render â†’ cleanup) before opening the upstream recipe.
...
- SimReady packaging of CAD or source meshes â†’ use
  `omniverse-cad-to-simready`.
- Generic USD performance tuning unrelated to NuRec â†’ use
  `omniverse-usd-performance-tuning`.
- AKS / OSMO / NIM Operator infrastructure setup â†’ use
...
- **Docker + NVIDIA Container Toolkit + GPU** â€” for `nre`, `nre-tools`,
...
- **NGC API key** (`NGC_API_KEY`) â€” for pulling NGC containers.
...
radar, or stereo recordings â€” typically from a self-driving car or a
robot â€” and turns them into a 3D scene you can re-render from any
viewpoint. Names that come up a lot:

- **NRE** â€” "Neural Reconstruction Engine". NuRec is the product; NRE
  is the engine that trains and renders. Both route to the upstream
  `nre` skill.
- **USDZ** â€” the file format of a trained scene. A zip archive that
  Omniverse, Isaac Sim, and CARLA can open.
- **NCore V4** â€” the input format NRE consumes. Raw recordings must be
  converted to NCore V4 before training.
- **3DGUT / 3DGRT** â€” the two 3D Gaussian Splatting flavours used
...
1. **Get the input** â€” convert your own recording to NCore V4
   (`ncore`), or download a pre-converted dataset
   (`physical-ai-datasets`).
2. **Train the reconstruction** â€” feed NCore V4 to NRE; out comes a
   USDZ (`nre`).
3. **Render new views** â€” render images, videos, or LiDAR sweeps from
   the USDZ (`nre`).
...
| Train a 3D reconstruction from an NCore clip | `ncore` â†’ `nre` |
...
| Skip training and just render a NuRec scene NVIDIA already built | `physical-ai-datasets` â†’ `nre` |
| Extract individual 3D objects (cars, pedestrians) from a driving clip | `asset-harvester` |
| Add, remove, or replace cars / pedestrians in a NuRec scene | `asset-harvester` â†’ `nre` |
...
| Benchmark different reconstruction methods on the same scenes | `physical-ai-datasets` (`PhysicalAI-NuRec-PPISP`) â†’ `nre` |
...
| `nurec-fixer` | `.agents/skills/nurec-fixer/` | Standalone NVIDIA **DiffusionHarmonizer** workflow â€” public successor to the older Fixer / Difix3D+ recipes â€” that cleans rendered frames, harmonizes inserted actors, evaluates PSNR/LPIPS, and optionally fine-tunes the model. |
...
- Router only â€” do not duplicate upstream NuRec recipes here. Read
...
- Do not invent NRE / NCore / DiffusionHarmonizer commands from
  memory. Re-read the upstream sibling skill â€” versions move fast
...
section â€” read them in the order documented in
