...
description: "Use when working with hugging-face-vision-trainer"
---
...
Train object detection, image classification, and SAM/SAM2 segmentation models on managed cloud GPUs. No local GPU setup requiredâ€”results are automatically saved to the Hugging Face Hub.
...
- **MUST pass token in job secrets** â€” see directive #3 below for syntax (MCP tool vs Python API)

### Dataset Requirements â€” Object Detection
- Dataset must exist on Hub
- Annotations must use the `objects` column with `bbox`, `category` (and optionally `area`) sub-fields
- Bboxes can be in **xywh (COCO)** or **xyxy (Pascal VOC)** format â€” auto-detected and converted
- Categories can be **integers or strings** â€” strings are auto-remapped to integer IDs
- `image_id` column is **optional** â€” generated automatically if missing
- **ALWAYS validate unknown datasets** before GPU training (see Dataset Validation section)

### Dataset Requirements â€” Image Classification
- Dataset must exist on Hub
- Must have an **`image` column** (PIL images) and a **`label` column** (integer class IDs or strings)
- The label column can be `ClassLabel` type (with names) or plain integers/strings â€” strings are auto-remapped
- Common column names auto-detected: `label`, `labels`, `class`, `fine_label`
- **ALWAYS validate unknown datasets** before GPU training (see Dataset Validation section)

### Dataset Requirements â€” SAM/SAM2 Segmentation
- Dataset must exist on Hub
- Must have an **`image` column** (PIL images) and a **`mask` column** (binary ground-truth segmentation mask)
- Must have a **prompt** â€” either:
...
### Critical Settings
- **Timeout must exceed expected training time** â€” Default 30min is TOO SHORT. See directive #6 for recommended values.
- **Hub push must be enabled** â€” `push_to_hub=True`, `hub_model_id="username/model-name"`, token in `secrets`
...
**Option 1: Via HF Jobs (recommended â€” avoids local SSL/dependency issues):**
```python
hf_jobs("uv", {
...
- **`âœ“ READY`** â€” Dataset is compatible, use directly
- **`âœ— NEEDS FORMATTING`** â€” Needs preprocessing (mapping code provided in output)

## Automatic Bbox Preprocessing

The object detection training script (`scripts/object_detection_training.py`) automatically handles bbox format detection (xyxyâ†’xywh conversion), bbox sanitization, `image_id` generation, string categoryâ†’integer remapping, and dataset truncation. **No manual preprocessing needed** â€” just ensure the dataset has `objects.bbox` and `objects.category` columns.
...
AskUserQuestion({
    "questions": [
        ...
        },
        {
            ...
            "options": [
                {"label": "t4-small ($0.40/hr)", "description": "1x T4, 16 GB VRAM â€” sufficient for all OD models under 100M params"},
                {"label": "l4x1 ($0.80/hr)", "description": "1x L4, 24 GB VRAM â€” more headroom for large images or batch sizes"},
                {"label": "a10g-large ($1.50/hr)", "description": "1x A10G, 24 GB VRAM â€” faster training, more CPU/RAM"},
                {"label": "a100-large ($2.50/hr)", "description": "1x A100, 80 GB VRAM â€” fastest, for very large datasets or image sizes"}
            ],
            "multiSelect": false
        }
    ]
...
2. **Submit** using `hf_jobs` MCP tool (preferred) or `HfApi().run_uv_job()` â€” see directive #1 for both methods. Pass all config via `script_args`.
...
4. **Wait for user** to request status checks â€” don't poll automatically. Training jobs run asynchronously and can take hours.
...
**Job config** MUST include the token in secrets â€” syntax depends on submission method (see table above).
...
- Do NOT rely on implicit token resolution (`hub_token=None`) â€” unreliable in Jobs
- See the `hugging-face-jobs` skill â†’ *Token Usage Guide* for full details

### 3. JobInfo attribute

Access the job identifier using `.id` (NOT `.job_id` or `.name` â€” these don't exist):
...
`scripts/object_detection_training.py` uses `HfArgumentParser` â€” all config is passed via `script_args`. Boolean arguments have two syntaxes:
...
Trackio is **always enabled** in the object detection training script â€” it calls `trackio.init()` and `trackio.finish()` automatically. No need to pass `--report_to trackio`. The project name is taken from `--output_dir` and the run name from `--run_name`. For image classification, pass `--report_to trackio` in `TrainingArguments`.
...
|-------|--------|----------|
| `ustc-community/dfine-small-coco` | 10.4M | Best starting point â€” fast, cheap, SOTA quality |
...
| Model | Params | Use case |
|-------|--------|----------|
| `timm/mobilenetv3_small_100.lamb_in1k` | 2.5M | Ultra-lightweight â€” mobile/edge, fastest training |
| `timm/mobilevit_s.cvnets_in1k` | 5.6M | Mobile transformer â€” good accuracy/speed trade-off |
| `timm/resnet50.a1_in1k` | 25.6M | Strong CNN baseline â€” reliable, well-studied |
| `timm/vit_base_patch16_dinov3.lvd1689m` | 86.6M | Best accuracy â€” DINOv3 self-supervised ViT |
...
| Model | Params | Use case |
|-------|--------|----------|
| `facebook/sam2.1-hiera-tiny` | 38.9M | Fastest SAM2 â€” good for quick experiments |
| `facebook/sam2.1-hiera-small` | 46.0M | Best starting point â€” good quality/speed balance |
| `facebook/sam2.1-hiera-base-plus` | 80.8M | Higher capacity for complex segmentation |
| `facebook/sam2.1-hiera-large` | 224.4M | Best SAM2 accuracy â€” requires more VRAM |
| `facebook/sam-vit-base` | 93.7M | Original SAM â€” ViT-B backbone |
| `facebook/sam-vit-large` | 312.3M | Original SAM â€” ViT-L backbone |
| `facebook/sam-vit-huge` | 641.1M | Original SAM â€” ViT-H, best SAM v1 accuracy |
...
All recommended OD and IC models are under 100M params â€” **`t4-small` (16 GB VRAM, $0.40/hr) is sufficient for all of them.** Image classification models are generally smaller and faster than object detection models â€” `t4-small` handles even ViT-Base comfortably. For SAM2 models up to `hiera-base-plus`, `t4-small` is sufficient since only the mask decoder is trained. For `sam2.1-hiera-large` or SAM v1 models, use `l4x1` or `a10g-large`. Only upgrade if you hit OOM from large batch sizes â€” reduce batch size first before switching hardware. Common upgrade path: `t4-small` â†’ `l4x1` ($0.80/hr, 24 GB) â†’ `a10g-large` ($1.50/hr, 24 GB).

For full hardware flavor list: refer to the `hugging-face-jobs` skill. For cost estimation: run `scripts/estimate_cost.py`.

## Quick start â€” Object Detection
...
- `--model_name_or_path` â€” recommended: `"ustc-community/dfine-small-coco"` (see model table above)
- `--dataset_name` â€” the Hub dataset ID
- `--image_square_size` â€” 480 (fast iteration) or 800 (better accuracy)
- `--hub_model_id` â€” `"username/model-name"` for Hub persistence
- `--num_train_epochs` â€” 30 typical for convergence
- `--train_val_split` â€” fraction to split for validation (default 0.15), set if dataset lacks a validation split
- `--max_train_samples` â€” truncate training set (useful for quick test runs, e.g. `"785"` for ~10% of a 7.8K dataset)
- `--max_eval_samples` â€” truncate evaluation set

## Quick start â€” Image Classification
...
- `--model_name_or_path` â€” any `timm/` model or Transformers classification model (see model table above)
- `--dataset_name` â€” the Hub dataset ID
- `--image_column_name` â€” column containing PIL images (default: `"image"`)
- `--label_column_name` â€” column containing class labels (default: `"label"`)
- `--hub_model_id` â€” `"username/model-name"` for Hub persistence
- `--num_train_epochs` â€” 3-5 typical for classification (fewer than OD)
- `--per_device_train_batch_size` â€” 16-64 (classification models use less memory than OD)
- `--train_val_split` â€” fraction to split for validation (default 0.15), set if dataset lacks a validation split
- `--max_train_samples` / `--max_eval_samples` â€” truncate for quick tests

## Quick start â€” SAM/SAM2 Segmentation
...
- `--model_name_or_path` â€” SAM or SAM2 model (see model table above); auto-detects SAM vs SAM2
- `--dataset_name` â€” the Hub dataset ID (e.g., `"merve/MicroMat-mini"`)
- `--prompt_type` â€” `"bbox"` or `"point"` â€” type of prompt in the dataset
- `--prompt_column_name` â€” column with JSON-encoded prompts (default: `"prompt"`)
- `--bbox_column_name` â€” dedicated bbox column (alternative to JSON prompt column)
- `--point_column_name` â€” dedicated point column (alternative to JSON prompt column)
- `--mask_column_name` â€” column with ground-truth masks (default: `"mask"`)
- `--hub_model_id` â€” `"username/model-name"` for Hub persistence
- `--num_train_epochs` â€” 20-30 typical for SAM fine-tuning
- `--per_device_train_batch_size` â€” 2-4 (SAM models use significant memory)
- `--freeze_vision_encoder` / `--freeze_prompt_encoder` â€” freeze encoder weights (default: both frozen, only mask decoder trains)
- `--train_val_split` â€” fraction to split for validation (default 0.1)
...
The object detection training script handles this gracefully â€” it falls back to the `validation` split. Ensure you're using the latest `scripts/object_detection_training.py`.
...
- [scripts/object_detection_training.py](scripts/object_detection_training.py) â€” Production-ready object detection training script
- [scripts/image_classification_training.py](scripts/image_classification_training.py) â€” Production-ready image classification training script (supports timm models)
- [scripts/sam_segmentation_training.py](scripts/sam_segmentation_training.py) â€” Production-ready SAM/SAM2 segmentation training script (bbox & point prompts)
- [scripts/dataset_inspector.py](scripts/dataset_inspector.py) â€” Validate dataset format for OD, classification, and SAM segmentation
- [scripts/estimate_cost.py](scripts/estimate_cost.py) â€” Estimate training costs for any vision model (includes SAM/SAM2)
- [references/object_detection_training_notebook.md](references/object_detection_training_notebook.md) â€” Object detection training workflow, augmentation strategies, and training patterns
- [references/image_classification_training_notebook.md](references/image_classification_training_notebook.md) â€” Image classification training workflow with ViT, preprocessing, and evaluation
- [references/finetune_sam2_trainer.md](references/finetune_sam2_trainer.md) â€” SAM2 fine-tuning walkthrough with MicroMat dataset, DiceCE loss, and Trainer integration
- [references/timm_trainer.md](references/timm_trainer.md) â€” Using timm models with HF Trainer (TimmWrapper, transforms, full example)
- [references/hub_saving.md](references/hub_saving.md) â€” Detailed Hub persistence guide and verification checklist
- [references/reliability_principles.md](references/reliability_principles.md) â€” Failure prevention principles from production experience
...
- [HF Jobs Guide](https://huggingface.co/docs/huggingface_hub/guides/jobs) â€” Main Jobs documentation
- [HF Jobs Configuration](https://huggingface.co/docs/hub/en/jobs-configuration) â€” Hardware, secrets, timeouts, namespaces
- [HF Jobs CLI Reference](https://huggingface.co/docs/huggingface_hub/guides/cli#hf-jobs) â€” Command line interface
