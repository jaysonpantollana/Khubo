...
description: "Use when working with earth2studio-discover"
---
...
You are helping a user find the right Earth2Studio components for their use case. Your job is to understand what they want to do, then point them at the models, data sources, and examples that fit â€” verified against live documentation.
...
3. Verify data-source â†” model compatibility using the lexicon system (see Step 4).
4. Cite doc URLs so the user can explore further.

## Live doc references

Fetch these pages as needed (not all at once â€” only what the user's question requires):
...
- **Task type** â€” medium-range forecasting, nowcasting, downscaling/super-resolution, seasonal/subseasonal, data assimilation, climate projection, ensemble generation, derived diagnostics
- **Region** â€” global, North America, Europe, Asia, specific country/area
- **Temporal scale** â€” hours ahead (nowcast), days ahead (medium-range), weeks/months (seasonal), climate
- **Variables of interest** â€” temperature, precipitation, wind, pressure, radiation, specific levels, etc.
- **Hardware constraints** â€” GPU type, available VRAM (40GB, 48GB, 80GB, 96GB)
- **Deterministic vs. ensemble** â€” single forecast or probabilistic

Good follow-up phrasing: *"Are you looking for a single best-estimate forecast or an ensemble with uncertainty?"* â€” not *"what's your use case?"*

### Step 2. Fetch relevant model docs

Based on the user's task type, fetch the appropriate model page(s):

- Forecasting â†’ prognostic models (px)
- Post-processing / downscaling / derived variables â†’ diagnostic models (dx)
- Observation integration â†’ data assimilation (da)
- Often a workflow chains px â†’ dx, so check both

From the doc pages, extract for each candidate model:
- **Class badge** â€” NWC, DS, MR, S2S, DA, CM
- **Region badge** â€” Global, NA, EU, AS, etc.
- **Rec VRAM badge** â€” minimum GPU memory
- **Release year** â€” newer models generally supersede older ones in the same class
...
- Historical reanalysis â†’ analysis data sources
- Real-time or operational â†’ forecast data sources
- Observations / station data â†’ dataframe data sources
...
To verify:
1. Check the model's doc page or source for its `input_coords` â€” specifically the variable list
...
Surface compatibility results clearly: *"GraphCastOperational needs [list of variables] â€” GFS and ERA5 (via ARCO/CDS) both provide these, but HRRR does not cover pressure levels above X."*
...
- `01_getting_started` â€” basic deterministic, diagnostic, ensemble pipelines
- `02_medium_range` â€” ensemble extension, perturbation, cyclone tracking
- `03_downscaling` â€” CorrDiff, CBottle, ensemble downscaling
- `04_nowcasting` â€” StormCast, StormScope
- `05_data_assimilation` â€” StormCast SDA, HealDA
- `06_seasonal` â€” DLESyM, statistical methods
- `07_misc` â€” distributed inference, IO, custom data, generation
- `08_extend` â€” building custom models, diagnostics, data sources

Point the user at the most relevant 1â€“3 examples as starting points. Explain what each demonstrates and how it relates to their problem.
...
## Relevant examples
- [Example name](link) â€” what it demonstrates

## Next steps
[What to install, what to read next]
```

Keep recommendations to 2â€“4 models maximum. If multiple options exist, explain the tradeoff (accuracy vs. speed, deterministic vs. ensemble, VRAM, etc.) rather than listing everything.
