...
description: "Use when working with tao-finetune-huggingface-model"
---
...
tags:
  ...
  - computer-vision
  - training
compatibility: Requires docker + nvidia-container-toolkit, NVIDIA GPU (driver â‰¥ 545, â‰¥ 24 GB VRAM for â‰¤3B models), ~40 GB free disk. Optional credentials (loaded from `~/.config/tao/.env` by the SessionStart hook) â€” HF_TOKEN is read only when the model/dataset is gated or `push_to_hub` is on; WANDB_API_KEY and WANDB_PROJECT only when WandB logging is enabled.
metadata:
...
governed by the rules in this file â€” follow them, do not improvise.

**Order of authority (highest first):** (1) user input â†’ (2) live research
(model card, HF repo example, author script, task docs, paper â€” always fetched,
Step 3) â†’ (3) curated `references/*.md` (fallback when live research is silent) â†’
...
**Required:**
- `model_id` â€” HuggingFace model ID, e.g. `google/vit-base-patch16-224`

**Conditional credentials (loaded by the SessionStart hook from `~/.config/tao/.env`):**
- `HF_TOKEN` â€” only when the model/dataset is **gated** (read) or `push_to_hub` is on (write); public + `push_to_hub: false` runs don't need it. The agent never reads the value â€” only checks presence with `[ -n "$HF_TOKEN" ]`.
- `WANDB_API_KEY`, `WANDB_PROJECT` â€” only when WandB is enabled; set `WANDB_MODE=disabled` to opt out.

**Dataset â€” exactly one:**
- `dataset_id` â€” HuggingFace dataset ID *(source: `hf`)*
- `local_dataset_path` â€” local folder or file *(source: `local`)*; optional `local_dataset_format` âˆˆ {auto, imagefolder, coco, voc, jsonl, arrow, parquet, csv} (default auto-detect).
- *(omit)* â€” agent recommends popular datasets *(source: `recommend`)*
...
**Optional deliverables (off by default):** `emit_progress_log` â†’
`output_dir/PROGRESS.md` (per-step âœ…/âš ï¸/âŒ journal); `emit_report` â†’
`reports/report.{pdf,html}` with curves & samples; `emit_unit_tests` â†’
...
(GPU host runtime â€” driver 580, CUDA Toolkit 13.0, NVIDIA Container Toolkit
...
(local Docker job preflight â€” daemon reachable, GPU smoke).

**Default platform:** `local-docker` â€” build a one-off image
...
## References â€” fallback safety net
...
`cv-scripts.md` / `vlm-scripts.md` are a research checklist, not code to inline â€”
if a block has no Step 3 finding, refetch the listed URL.

See `references/reference-index.md` for the complete index â€” every always-on
reference plus the three opt-in ones gated by a flag (`progress-tracking.md` â†
`emit_progress_log`, `testing.md` â† `emit_unit_tests`, `reporting.md` â†
...
- **Never silently substitute** model_id, dataset_id, or training_method â€” stop and ask.
- **Error recovery is minimal-change.** OOM â†’ halve batch, double grad_accum,
  enable gradient checkpointing (don't switch to LoRA without approval); NaN â†’
  reduce LR 10Ã—; flat loss â†’ inspect collator; same error 3Ã— â†’ stop and ask.
- **Dataset columns verified BEFORE the collator.** Rename â†’ `prepare_data.py`;
  restructuring â†’ stop and ask.
- **Hardware sizing (bf16):** â‰¤3B â†’ 24 GB, 7â€“13B â†’ 80 GB, 30B+ â†’ multi-GPU or
  LoRA on 1Ã— 80 GB, 70B+ â†’ 8Ã— 80 GB or LoRA. Won't fit + no LoRA request â†’ ask.
...
`AutoModelFor...` â†’ `references/model-discovery.md`, log under `notes:`). For
`source = recommend`, present 3â€“5 picks from
...
bind-mounted by Steps 4â€“5), dataset-column verification + rename rule, the full
...
### Step 2 â€” Hardware audit & NGC image
...
`gpu_name`, `driver_major`, `vram_gb_per_gpu`. **2b. Pick NGC image (live)** â€”
...
suffix); WebFetch fail â†’ `references/hardware-container.md` fallback. **2c.
Re-evaluate** `hw`-dependent compat rules. **2d. Model-fit check** â€” bf16
`param_bytes â‰ˆ 2Ã—param_count`; if > 60% of `vram_gb_per_gpu Ã— 1e9`, recommend
LoRA.
...
### Step 3 â€” Research the recipe

Fetch the live recipe â€” the agent's `transformers`/`trl`/`peft` memory is
suspect, so Step 3 is non-negotiable. Walk `references/research-priorities.md`
in priority order (Priority 1 â†’ 6).
...
`vlm-scripts.md`) and log "fallback to scaffold â€” no live source for <slot>"
...
header, and the Dockerfile template (deps â†’ compat â†’ code layer order).

**4b. Build, prepare, smoke** â€” `docker build -t run-<short>:latest .`, then run
`references/docker-runs.md` Â§1 (build), Â§2 (prepare_data), Â§3 (smoke,
`--smoke --max_steps 1`); Â§3 lists the smoke pass criteria (no exception, loss
finite, `grad_norm > 0` at step 1). If `emit_unit_tests: true`, also run
`pytest tests/` inside the container. Any failure â†’ STOP.

**4c. Preflight summary** â€” print the boxed `â”€ PREFLIGHT â”€` summary (reference
...
### Step 5 â€” Train, evaluate, infer

Run in order, all commands in `references/docker-runs.md`: **5a** baseline eval
(Â§4, skip if `skip_baseline: true`), **5b** full training detached (Â§5), **5c**
LoRA merge (Â§6, only VLM-with-LoRA), **5d** post-train eval (Â§7), **5e**
inference 5 samples (Â§8). Multi-GPU: prepend `torchrun --nproc_per_node=$gpu_count`
to `python train.py`. Watch `docker logs -f hft_train`: loss should drop within
10-20 steps (flat â†’ stop; NaN â†’ reduce LR; OOM â†’ halve batch; full recovery in
...
**Gate:** all of â€” `checkpoints/final/` (or `checkpoints/merged/` for LoRA)
...
### Step 6 â€” Push & emit rerun skill

Publish the run and make it reproducible without re-research.

**6a. Push to HF Hub** â€” use `references/hub-push.md` (pushes weights merged or
...
**Gate (Done criteria):** all of â€” Step 5 gate met; HF Hub repo exists at the
...
**Final message to user** â€” terse, with direct URLs: wandb URL; HF Hub URL;
primary metric baseline â†’ fine-tuned (Î”); path to `reports/inference_samples/`;
...
redesigning anything â€” its symptom â†’ minimal-fix table covers NGC ENTRYPOINT,
...
"Option A/B/C" when the answer is clear â€” act). Full text:
`references/core-rules.md`.

## Example pipelines

- [tao-rerun-convnext-cifar10](references/tao-rerun-convnext-cifar10.md) â€” facebook/convnext-tiny-224 on cifar10 (image-classification, 10 classes, subset 5000/1000).
- [tao-rerun-detr-cppe5](references/tao-rerun-detr-cppe5.md) â€” facebook/detr-resnet-50 on cppe-5 (object-detection, 5 classes, subset 800/200).
- [tao-rerun-segformer-foodseg103](references/tao-rerun-segformer-foodseg103.md) â€” nvidia/mit-b0 on EduardoPacheco/FoodSeg103 (semantic segmentation, 103 classes + background, subset 1000/200).
- [tao-rerun-smolvlm-vqav2](references/tao-rerun-smolvlm-vqav2.md) â€” HuggingFaceTB/SmolVLM-256M-Instruct on merve/vqav2-small (image-text-to-text VLM LoRA, subset 500/100, 5 epochs).
