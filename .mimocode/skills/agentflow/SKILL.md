---
name: agentflow
description: "Use when working with agentflow"
---
...
Tasks flow through: Backlog, Research, Build, Review, Test, Integrate, Done. Each stage has specific gates. The Kanban board IS the orchestration layer â€” no separate database, no message queue, no custom infrastructure.
...
```bash
# Terminal 2 â€” Builder
claude -p "/sdlc-worker --slot T2"

# Terminal 3 â€” Builder
claude -p "/sdlc-worker --slot T3"

# Terminal 4 â€” Reviewer
...
- Do: Review LEARNINGS.md periodically â€” it captures common failure patterns
- Don't: Skip the deterministic quality gates â€” they catch most issues cheaply
- Don't: Force-push to main â€” AgentFlow uses `git revert` for safety
