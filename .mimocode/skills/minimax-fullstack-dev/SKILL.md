...
description: "Use when working with minimax-fullstack-dev"
---
...
## MANDATORY WORKFLOW â€” Follow These Steps In Order
...
5. **Real-time**: Needed? If yes â€” SSE, WebSocket, or polling?
6. **Auth**: Needed? If yes â€” JWT, session, OAuth, or third-party (Clerk, Auth.js)?
...
Use the appropriate checklist below. Ensure ALL checked items are implemented â€” do not skip any.
...
## Quick Start â€” New Backend Service Checklist
...
## Quick Start â€” Frontend-Backend Integration Checklist
...
- [ ] **Error handling** â€” API errors mapped to user-facing messages
...
```
1. âœ… Organize by FEATURE, not by technical layer
2. âœ… Controllers never contain business logic
3. âœ… Services never import HTTP request/response types
4. âœ… All config from env vars, validated at startup, fail fast
5. âœ… Every error is typed, logged, and returns consistent format
6. âœ… All input validated at the boundary â€” trust nothing from client
7. âœ… Structured JSON logging with request ID â€” not console.log
```
...
```
âœ… Feature-first                    âŒ Layer-first
src/                                src/
  orders/                             controllers/
...
| Layer | Responsibility | âŒ Never |
|-------|---------------|---------|
...
class OrderService {
  constructor(
    private readonly orderRepo: OrderRepository,    // âœ… injected interface
...
class OrderService:
    def __init__(self, order_repo: OrderRepository, email_service: EmailService):
        self.order_repo = order_repo                 # âœ… injected
...
type OrderService struct {
    orderRepo    OrderRepository                      // âœ… interface
...
class Settings(BaseSettings):
    database_url: str                        # required â€” app won't start without it
...
```
âœ… All config via environment variables (Twelve-Factor)
âœ… Validate required vars at startup â€” fail fast
âœ… Type-cast at config layer, not at usage sites
âœ… Commit .env.example with dummy values

âŒ Never hardcode secrets, URLs, or credentials
âŒ Never commit .env files
âŒ Never scatter process.env / os.environ throughout code
```
...
```
âœ… Typed, domain-specific error classes
âœ… Global error handler catches everything
âœ… Operational errors â†’ structured response
âœ… Programming errors â†’ log + generic 500
âœ… Retry transient failures with exponential backoff

âŒ Never catch and ignore errors silently
âŒ Never return stack traces to client
âŒ Never throw generic Error('something')
```
...
```
âœ… Schema changes via migrations, never manual SQL
âœ… Migrations must be reversible
âœ… Review migration SQL before production
âŒ Never modify production schema manually
```

### N+1 Prevention

```typescript
// âŒ N+1: 1 query + N queries
...
Pool size = `(CPU cores Ã— 2) + spindle_count` (start with 10-20). Always set connection timeout. Use PgBouncer for serverless.
...
> **Full reference:** [references/auth-flow.md](references/auth-flow.md) â€” JWT bearer flow, automatic token refresh, Next.js server-side auth, RBAC pattern, backend middleware order.

### Standard Middleware Order

```
Request â†’ 1.RequestID â†’ 2.Logging â†’ 3.CORS â†’ 4.RateLimit â†’ 5.BodyParse
       â†’ 6.Auth â†’ 7.Authz â†’ 8.Validation â†’ 9.Handler â†’ 10.ErrorHandler â†’ Response
```

### JWT Rules

```
âœ… Short expiry access token (15min) + refresh token (server-stored)
âœ… Minimal claims: userId, roles (not entire user object)
âœ… Rotate signing keys periodically

âŒ Never store tokens in localStorage (XSS risk)
âŒ Never pass tokens in URL query params
```
...
```typescript
// lib/api-client.ts â€” transparent refresh on 401
...
```typescript
// âœ… Structured â€” parseable, filterable, alertable
...
// âŒ Unstructured â€” useless at scale
...
|-------|------|------------|
| error | Requires immediate attention | âœ… Always |
| warn | Unexpected but handled | âœ… Always |
| info | Normal operations, audit trail | âœ… Always |
| debug | Dev troubleshooting | âŒ Dev only |

### Rules

```
âœ… Request ID in every log entry (propagated via middleware)
âœ… Log at layer boundaries (request in, response out, external call)
âŒ Never log passwords, tokens, PII, or secrets
âŒ Never use console.log in production code
```
...
```
âœ… All jobs must be IDEMPOTENT (same job running twice = same result)
âœ… Failed jobs â†’ retry (max 3) â†’ dead letter queue â†’ alert
âœ… Workers run as SEPARATE processes (not threads in API server)

âŒ Never put long-running tasks in request handlers
âŒ Never assume job runs exactly once
```
...
```
âœ… ALWAYS set TTL â€” never cache without expiry
âœ… Invalidate on write (delete cache key after update)
âœ… Use cache for reads, never for authoritative state

âŒ Never cache without TTL (stale data is worse than slow data)
```
...
```
Client â†’ GET /api/uploads/presign?filename=photo.jpg&type=image/jpeg
Server â†’ { uploadUrl: "https://s3.../presigned", fileKey: "uploads/abc123.jpg" }
Client â†’ PUT uploadUrl (direct to S3, bypasses your server)
Client â†’ POST /api/photos { fileKey: "uploads/abc123.jpg" }  (save reference)
```
...
// Note: do NOT set Content-Type header â€” browser sets boundary automatically
```
...
### Option A: Server-Sent Events (SSE) â€” One-Way Server â†’ Client
...
### Option B: WebSocket â€” Bidirectional
...
|--------|-----------|------------|------|
| Polling | Client â†’ Server | Low | Simple status checks, < 10 clients |
| SSE | Server â†’ Client | Medium | Notifications, feeds, AI streaming |
...
### API Error â†’ User-Facing Message
...
```
âœ… Map every API error code to a human-readable message
âœ… Show field-level validation errors next to form inputs
âœ… Auto-retry on 5xx (max 3, with backoff), never on 4xx
âœ… Redirect to login on 401 (after refresh attempt fails)
âœ… Show "offline" banner when fetch fails with TypeError

âŒ Never show raw API error messages to users ("NullPointerException")
âŒ Never silently swallow errors (show toast or log)
âŒ Never retry 4xx errors (client is wrong, retrying won't help)
```
...
â”‚
â”œâ”€ YES, both TypeScript
â”‚   â””â”€ tRPC (end-to-end type safety, zero codegen)
â”‚
â”œâ”€ YES, different languages
â”‚   â””â”€ OpenAPI spec â†’ generated client (type safety via codegen)
â”‚
â”œâ”€ NO, public API
â”‚   â””â”€ REST + OpenAPI â†’ generated SDKs for consumers
â”‚
â””â”€ Complex data needs, multiple frontends
    â””â”€ GraphQL + codegen (flexible queries per client)

Real-time needed?
â”‚
â”œâ”€ Server â†’ Client only (notifications, feeds, AI streaming)
â”‚   â””â”€ SSE (simplest, auto-reconnect, works through proxies)
â”‚
â”œâ”€ Bidirectional (chat, collaboration)
â”‚   â””â”€ WebSocket (need heartbeat + reconnection logic)
â”‚
â””â”€ Simple status polling (< 10 clients)
    â””â”€ React Query refetchInterval (no infrastructure needed)
```
...
```
âœ… CORS: explicit origins (never '*' in production)
âœ… Security headers (helmet / equivalent)
âœ… Rate limiting on public endpoints
âœ… Input validation on ALL endpoints (trust nothing)
âœ… HTTPS enforced
âŒ Never expose internal errors to clients
```
...
| 16 | Upload large files through API server | Presigned URL â†’ direct to S3 |
...
**Rule:** If it involves HTTP (request parsing, status codes, headers) â†’ controller. If it involves business decisions (pricing, permissions, rules) â†’ service. If it touches the database â†’ repository.

### Issue 2: "Service is getting too big"

**Symptom:** One service file > 500 lines with 20+ methods.

**Fix:** Split by sub-domain. `OrderService` â†’ `OrderCreationService` + `OrderFulfillmentService` + `OrderQueryService`. Each focused on one workflow.
