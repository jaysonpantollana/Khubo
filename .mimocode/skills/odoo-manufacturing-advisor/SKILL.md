...
description: "Use when working with odoo-manufacturing-advisor"
---
...
```text
Menu: Manufacturing â†’ Products â†’ Bills of Materials â†’ New
...
> **BoM Types explained:**
>
> - **Manufacture This Product** â€” standard production BoM, creates a Manufacturing Order
> - **Kit** â€” sold as a bundle; components are delivered separately (no MO created)
> - **Subcontracting** â€” components are sent to a subcontractor who returns the finished product

### Example 2: Configure a Work Center

```text
Menu: Manufacturing â†’ Configuration â†’ Work Centers â†’ New
...
Menu: Inventory â†’ Operations â†’ Replenishment â†’ Run Scheduler
(or Manufacturing â†’ Planning â†’ Replenishment in some versions)

After running, review procurement exceptions:
Menu: Inventory â†’ Operations â†’ Replenishment

Message Types:
  "Replenish"   â€” Stock is below minimum; needs a PO or MO
  "Reschedule"  â€” An order's scheduled date conflicts with demand
  "Cancel"      â€” Demand no longer exists; the order can be cancelled
```

## Best Practices

- âœ… **Do:** Enable **Work Orders** in Manufacturing Settings to use routing and time-tracking per operation.
- âœ… **Do:** Use **BoM with variants** (via product attributes) for products that come in multiple configurations (color, size, voltage) â€” avoids duplicate BoMs.
- âœ… **Do:** Set **Lead Times** on components (vendor lead time + security lead time) so MRP schedules purchase orders in advance.
- âœ… **Do:** Use **Scrap Orders** when discarding defective components during production â€” never adjust stock manually.
- âŒ **Don't:** Manually create purchase orders for MRP-managed items â€” override MRP suggestions only when justified.
- âŒ **Don't:** Confuse **Kit** BoM with **Manufacture This Product** â€” a Kit never creates a Manufacturing Order.
...
- MRP calculations assume demand comes from **Sale Orders** and **Reordering Rules** â€” forecasts from external systems require custom integration.
