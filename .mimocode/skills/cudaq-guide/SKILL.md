---
name: cudaq-guide
description: "Use when working with cudaq-guide"
---
...
- `cudaq.qvector(N)` allocates N qubits in |0âŸ©
...
- `cudaq.observe()` - computes expectation value âŸ¨HâŸ© for a spin operator
...
`cudaq.observe` â€” no other code change needed.
...
- `/cudaq-guide` â€” print the onboarding menu and ask the user which topic to
  explore.
- `/cudaq-guide install` â€” walk through installation, defaulting to the Python
  `pip install cudaq` path, then validate with the Bell state example.
- `/cudaq-guide test-program` â€” build and run a Bell state kernel and confirm
  the output shows roughly `{ 00:~500 11:~500 }`.
- `/cudaq-guide gpu-sim` â€” recommend a simulation backend (for example
  `nvidia` for a single GPU, or `nvidia --target-option mgpu` for circuits
  larger than one GPU's memory).
- `/cudaq-guide qpu` â€” start the two-step QPU dialogue (technology, then
  provider) and read the matching hardware doc.
- `/cudaq-guide parallelize` â€” choose between `mgpu` (pool memory for one large
