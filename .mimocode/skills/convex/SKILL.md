...
You are an expert in Convex â€” the open-source, reactive backend platform where queries are TypeScript code. You have deep knowledge of schema design, function authoring (queries, mutations, actions), real-time data subscriptions, authentication, file storage, scheduling, and deployment workflows across React, Next.js, Angular, Vue, Svelte, React Native, and server-side environments.
...
- **TypeScript-first**: All backend logic â€” queries, mutations, actions, schemas â€” is written in TypeScript
...
- **End-to-end type safety**: Types flow from schema â†’ backend functions â†’ client hooks

### Function Types

| Type            | Purpose                   | Can Read DB    | Can Write DB      | Can Call External APIs | Cached/Reactive |
| :-------------- | :------------------------ | :------------- | :---------------- | :--------------------- | :-------------- |
| **Query**       | Read data                 | âœ…             | âŒ                | âŒ                     | âœ…              |
| **Mutation**    | Write data                | âœ…             | âœ…                | âŒ                     | âŒ              |
| **Action**      | Side effects              | via `runQuery` | via `runMutation` | âœ…                     | âŒ              |
| **HTTP Action** | Webhooks/custom endpoints | via `runQuery` | via `runMutation` | âœ…                     | âŒ              |
...
my-app/
â”œâ”€â”€ convex/
â”‚   â”œâ”€â”€ _generated/        â† Auto-generated (DO NOT EDIT)
â”‚   â”‚   â”œâ”€â”€ api.d.ts
â”‚   â”‚   â”œâ”€â”€ dataModel.d.ts
â”‚   â”‚   â””â”€â”€ server.d.ts
â”‚   â”œâ”€â”€ schema.ts          â† Database schema definition
â”‚   â”œâ”€â”€ tasks.ts           â† Query/mutation functions
â”‚   â””â”€â”€ http.ts            â† HTTP actions (optional)
â”œâ”€â”€ .env.local             â† CONVEX_DEPLOYMENT, NEXT_PUBLIC_CONVEX_URL
â””â”€â”€ convex.json            â† Project config (optional)
```
...
Queries are reactive â€” clients automatically get updates when data changes.

````typescript
import { query } from "./_generated/server";
import { v } from "convex/values";

// Simple query â€” list all tasks
...
Actions can call third-party services but cannot directly access the database â€” they must use `ctx.runQuery` and `ctx.runMutation`.
...
```typescript
// app/layout.tsx â€” wrap children
...
export function TaskList() {
  // Reactive query â€” auto-updates when data changes
...
Better Auth provides email/password, social logins, two-factor authentication, and session management â€” all running inside Convex functions rather than an external auth server.
...
export class ConvexService implements OnDestroy {
  ...
  // Reactive signal â€” updates automatically when data changes
...
- âœ… Define schemas â€” adds type safety across your entire stack
- âœ… Use indexes for queries â€” avoids full table scans
- âœ… Use compound indexes with equality filters first, range filter last
- âœ… Rely on native determinism â€” `Date.now()` and `Math.random()` are 100% safe to use in queries and mutations because Convex freezes time at the start of every function execution!
- âœ… Use `v.id("tableName")` for document references instead of plain strings
- âœ… Use actions for external API calls (never call external APIs from queries or mutations)
- âœ… Use `ctx.runQuery` / `ctx.runMutation` from actions â€” never access `ctx.db` directly in actions
- âœ… Add argument validators to all functions â€” they enforce runtime type safety
- âœ… Return `null` when a document isn't found instead of throwing an error unless missing is exceptional
- âœ… Prefer `withIndex` over `.filter()` for query performance

## Anti-Patterns to Avoid

1. **âŒ External API calls in queries/mutations**: Only actions can call external services. Queries and mutations run in the Convex transaction engine.
2. **âŒ Doing slow CPU-bound work in mutations**: Mutations block database commits; offload heavy processing to actions.
3. **âŒ Using `.collect()` on large tables without limits**: Fetches all documents into memory. Use `.take(N)` or `.paginate()`.
4. **âŒ Skipping schema definition**: Without a schema you lose end-to-end type safety, the main Convex advantage.
5. **âŒ Using `.filter()` instead of indexes**: `.filter()` does a full table scan. Define an index and use `.withIndex()`.
6. **âŒ Storing large blobs in documents**: Use Convex file storage (`_storage`) for files; keep documents lean.
7. **âŒ Circular `runQuery`/`runMutation` chains**: Actions calling mutations that schedule actions can create infinite loops.

## Common Pitfalls

- **Problem:** "Query returns `undefined` on first render"
  **Solution:** This is expected â€” Convex queries are async. Check for `undefined` before rendering (this means loading, not empty).
...
- No raw SQL â€” you work with the Convex query builder API
...
- `@firebase` â€” Alternative BaaS with Firestore (compare: Convex is TypeScript-first with ACID transactions)
- `@supabase-automation` â€” Alternative with PostgreSQL backend (compare: Convex is document-relational with built-in reactivity)
- `@prisma-expert` â€” ORM for traditional databases (Convex replaces both ORM and database)
- `@react-patterns` â€” Frontend patterns that pair well with Convex React hooks
- `@nextjs-app-router` â€” Next.js App Router integration patterns
- `@authentication-oauth` â€” Auth patterns (Convex supports Clerk, Auth0, Convex Auth)
- `@stripe` â€” Payment integration via Convex actions and HTTP webhooks
