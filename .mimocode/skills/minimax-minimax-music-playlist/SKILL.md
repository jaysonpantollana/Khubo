...
description: "Use when working with minimax-minimax-music-playlist"
---
...
description: >
  ...
  music recommendations. Supports multilingual triggers â€” match equivalent phrases in
  any language.
license: MIT
metadata:
  version: "2.0"
...
# MiniMax Music Playlist â€” Personalized Playlist Generator
...
user invocation â€” adapt interaction style to context.

## Prerequisites

- **mmx CLI** â€” music & image generation. Install: `npm install -g mmx-cli`. Auth: `mmx auth login --api-key <key>`.
- **Python 3** â€” for scanning scripts you write on the fly (stdlib only, no pip).
- **Audio player** â€” `mpv`, `ffplay`, or `afplay` (macOS built-in).

## Language

Detect the user's language from their message. **All user-facing text must be
in the same language as the user's prompt** â€” do not mix languages. If the user
...
Each song's lyrics language follows its genre (K-pop â†’ Korean, J-pop â†’ Japanese, etc.),
...
```
1. Scan local music apps â†’ 2. Build taste profile â†’ 3. Plan playlist
â†’ 4. Generate songs (mmx music) â†’ 5. Generate cover (mmx image) â†’ 6. Play â†’ 7. Save & feedback
```
...
6. Skip Spotify for now and continue with other sources â€” tell the user they can
...
- `ms_played` â€” playback duration in milliseconds (use as weight: longer = stronger signal)
- `ts` â€” timestamp
...
- **Genre distribution** â€” what styles the user listens to (e.g., J-pop 20%, R&B 15%, Classical 10%)
- **Mood tendencies** â€” emotional tone preferences (melancholic, energetic, calm, romantic, etc.)
- **Vocal preference** â€” male vs female voice ratio
- **Tempo preference** â€” slow / moderate / upbeat / fast distribution
- **Language distribution** â€” zh, en, ja, ko, etc.
- **Top artists** â€” most listened artists
...
1. Look up artists in the local mapping table at `<SKILL_DIR>/data/artist_genre_map.json`
   â€” this table covers 20,000 popular artists with pre-mapped genres, vocal type, and language
2. For artists not in the mapping table, query the MusicBrainz API:
   ...
   â€” extract genre tags from the response; respect rate limit (1 req/sec)
   â€” cache results to `<SKILL_DIR>/data/artist_cache.json` to avoid re-querying
...
- "Late night chill" â€” relaxing slow songs
- "Commute" â€” upbeat and energizing
- "Rainy day" â€” melancholic & cozy
- "Surprise me" â€” random based on your taste

Or tell me your own vibe!
```

Once the user picks a theme, proceed automatically through generation, cover,
playback, and saving â€” no further confirmations needed.

Determine playlist parameters:
- **Theme/mood** â€” from user input, or default to top mood from profile
- **Song count** â€” from user input, or default to 5
- **Genre mix** â€” weighted by profile, with variety
...
passed to `mmx` should be in English â€” this is internal and should NOT be shown to
the user. Example:

```
Playlist Plan: Late Night Chill (5 songs)

1. Neo-soul R&B â€” introspective  English/male vocal
   A mellow neo-soul R&B ballad with warm baritone, electric piano, smooth bass

2. Lo-fi hip-hop â€” dreamy  Instrumental
   Dreamy lo-fi with sampled piano, vinyl crackle, soft electronic drums

3. Smooth jazz â€” romantic  English/female vocal
   Silky female voice, saxophone, piano, romantic starlit night

4. Indie folk â€” melancholic  English/male vocal
   Tender male voice, acoustic guitar, harmonica, quiet solitude

5. Ambient electronic â€” calm  Instrumental
   Soft synth pads, gentle arpeggios, dreamy atmosphere
```

After showing the plan, proceed directly to generation â€” no confirmation needed.
...
**Key flags:**
- `--lyrics-optimizer` â€” auto-generate lyrics from prompt (for vocal tracks)
- `--instrumental` â€” no vocals
- `--vocals "<description>"` â€” vocal style (e.g., "warm Chinese male baritone")
- `--genre`, `--mood`, `--tempo`, `--instruments` â€” fine-grained control
- `--quiet --non-interactive` â€” suppress interactive output for batch mode
- `--out <path>` â€” save to file
...
should feel like an album cover â€” artistic, evocative, not literal.
...
Only play the songs generated in this session â€” if the directory has old files
