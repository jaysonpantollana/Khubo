...
description: "Use when working with tao-train-single-step"
---
...
1. **train** â€” executed through AutoML when the selected model has
   `automl_enabled: true` and `automl_policy` is `auto`; set
   `automl_policy=off` for a plain single training run
2. **eval** â€” executed if `eval_dataset_uri` is resolved
3. **export** â€” optional, on user request after training
...
### Optional
- **eval_dataset_uri**: Some model skills mark this as required â€” check the resolved model skill before treating it as optional.
