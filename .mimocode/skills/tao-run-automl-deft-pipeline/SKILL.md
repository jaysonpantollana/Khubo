...
description: "Use when working with tao-run-automl-deft-pipeline"
---

---
name: tao-run-automl-deft-pipeline
description: >
  Run the canonical NVIDIA AOI three-phase training pipeline â€” Phase 1 AutoML baseline (HPO),
  Phase 2 DEFT loop (RCA â†’ SDG â†’ mining â†’ plain-train retrain), Phase 3 AutoML refinement on
  ...
  with AutoML" request â€” route here instead of tao-run-deft-aoi directly unless the user
  explicitly asks for the DEFT loop ONLY (e.g. "run JUST the DEFT loop", "skip AutoML, only
  DEFT"). Also handles the same three-phase pattern for non-AOI DEFT applications â€” AutoML
...
A workflow-bridge skill that runs **three phases** in sequence by delegating to two existing skills â€” `tao-run-automl` for HPO and a DEFT application skill (default `tao-run-deft-aoi` for AOI; other `skills/applications/deft-*` skills for non-AOI cases) for the iterative data-improvement loop.
...
- User asks to "run the AOI workflow" or "improve my AOI ChangeNet model" â€” **default to this skill**, not `tao-run-deft-aoi` directly. The bare DEFT loop is the inner stage of this pipeline.
...
- User explicitly asks for the DEFT loop only ("run JUST the DEFT loop", "skip AutoML") â†’ use `tao-run-deft-aoi` directly
- User wants only AutoML with no follow-on DEFT â†’ use `tao-run-automl` directly
...
train/base/training_set.csv       â€” DEFT skips its baseline train)       train_combined_iter${N}.csv
        â”‚                                       â”‚                                       â”‚
        â–¼                                       â–¼                                       â–¼
[ AutoML HPO sweep ]               [ DEFT: baseline-inference â†’ RCA       [ AutoML HPO sweep ]
   N recommendations                 â†’ iter 1..N (plain retrain) ]        re-tunes HPs against the
   pick best by val_loss / FAR      RCA / route / SDG / mining             DEFT-augmented dataset
        â”‚                                       â”‚                                       â”‚
        â–¼                                       â–¼                                       â–¼
best HPs spec + ckpt â”€â”€â”€â”€â”€â–º      DEFT-augmented CSV â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º        final best checkpoint
...
- **Phase 1 â†’ Phase 2**: a *spec file* AND the *winning checkpoint*. Retraining the same HPs in DEFT's baseline step is wasted compute, so the bridge deep-merges Phase 1's winning HPs onto `baseline_spec.yaml`, copies the winning checkpoint into `${RESULTS_DIR}/baseline/train/` under the filename DEFT expects, and pre-populates `deft_state.json` + `loop_log.jsonl` so DEFT resumes at baseline inference â†’ evaluate â†’ RCA â†’ iter 1. DEFT itself stays plain-train (`automl_policy: off` preserved). Verbatim 4-step procedure in `references/handoff.md`.
- **Phase 2 â†’ Phase 3**: a *training CSV* AND the *iter winner's checkpoint*. The CSV (`train_combined_iter${N_final}.csv`) is AutoML's training data; the checkpoint (`iterations.<best>.best_ckpt_path` from `deft_state.json`) is wired into each rec's `train.pretrained_model_path` so Phase 3 **fine-tunes from Phase 2's winner** rather than from scratch. Without this warm-start Phase 3 routinely regresses vs the iter winner. Phase 3's winning checkpoint is the deliverable â€” no separate retrain after Phase 3. See `references/handoff.md`.
...
- **Phase 2 alone** (just DEFT) fills the gaps but uses whatever HPs `specs/baseline_spec.yaml` was hand-authored with â€” usually not optimal.
...
| `eval_dataset_uri` | `<workspace>/train/base/validation_set.csv` | Held-out â€” must NOT be the KPI test set (`<workspace>/kpi/testing_set.csv`), since that set is reserved for DEFT's final reporting |
| `metric` | FAR @ 100% recall (preferred) or `val_loss` | See `references/pitfalls.md` â€” ChangeNet AOI is class-imbalanced, val_loss alone can mode-collapse |
| `algorithm` | `bayesian` | LLM-brain or `autoresearch` if compute is tight |
| `automl_max_recommendations` | 5â€“10 for AOI | More recs = better HPs but linear in compute |
...
Phase 1 hands over **two artifacts**: the winning *spec* and the winning *checkpoint*. Instead of retraining the same HPs in DEFT's baseline step, pre-seed DEFT's baseline state from Phase 1's outputs so DEFT starts at baseline inference â†’ evaluate â†’ RCA â†’ iter 1. The four steps â€” write the merged `baseline_spec_automl.yaml`, copy the winning checkpoint into `${RESULTS_DIR}/baseline/train/`, initialise `deft_state.json` with `iterations.baseline.stage_completed == "train"` (and append the matching `loop_log.jsonl` entry), then invoke DEFT â€” are given verbatim with the exact code in `references/handoff.md`. `automl_policy: off` inside the loop is preserved.
...
## Phase 2 â€” DEFT loop (plain training, baseline pre-seeded from Phase 1)
...
**The DEFT loop's baseline-train sub-step is skipped.** Phase 1 already produced a checkpoint trained at the winning HPs, and Phase 1's handoff (see above) pre-populated `${RESULTS_DIR}/baseline/train/` and `${RESULTS_DIR}/deft_state.json` so DEFT resumes at baseline inference â†’ evaluate â†’ RCA â†’ iter 1. The rest of the DEFT loop runs unchanged. **Do not modify its `automl_policy: off` invariant.**

The DEFT loop owns:

- The Pre-Flight Summary display step â€” **not** a fresh user gate. The Consolidated Pre-Flight (above) is the single gate; the DEFT summary still prints as an audit-trail display of the pre-seeded `baseline/train/` source but must not re-prompt, since every input was collected in the consolidated gate.
- Baseline inference â†’ evaluate â†’ RCA on the pre-seeded checkpoint, and the full per-iteration RCA â†’ routing â†’ SDG â†’ mining â†’ assemble â†’ train cycle.
...
- `iterations.<best>.best_ckpt_path` â€” the loop's best plain-train checkpoint
- The final iteration label `N_final` â€” used to locate the augmented training CSV

If the DEFT loop hard-stops on an unrecoverable gate, **skip Phase 3**. There is no validated augmented CSV to feed AutoML.

---

## Phase 3 â€” AutoML refinement on the DEFT-augmented dataset
...
| `eval_dataset_uri` | Same as Phase 1 (`<workspace>/train/base/validation_set.csv`) â€” keep the comparison apples-to-apples |
| `metric` | Same metric as Phase 1 |
| `algorithm` | Same as Phase 1 |
| `automl_max_recommendations` | 5â€“10 |
| Initial spec | Start from `<workspace>/specs/baseline_spec_automl.yaml` (Phase 1's winner) â€” gives the sweep a strong centroid to refine around |
| **Warm-start checkpoint** | **`iterations.<best>.best_ckpt_path` from `${RESULTS_DIR}/deft_state.json`** â€” set `spec_overrides["train"]["pretrained_model_path"]` to this path. Each Phase 3 rec then **fine-tunes from Phase 2's winner** instead of training from scratch. |

The warm-start is **mandatory**: with no warm-start, every rec starts from random init with only 10-20 epochs to reconverge, Phase 3's `val_loss` regresses 0.03-0.05 vs iter1, and the `_pick_best` safety net silently rolls back to the iter winner â€” wasting Phase 3's compute. The concrete `spec_overrides` code (selecting the lowest-`far_pct` iteration, excluding any prior `final_automl`), the broad-exploration tradeoff, output to `${RESULTS_DIR}/final_automl/`, and wiring Phase 3's checkpoint back into the DEFT report via `iterations.final_automl` + re-running `prepare_inference_spec.py` (with the `_pick_best` regression safety net) are all in `references/handoff.md`.

---

## Pitfalls and quality checks

These apply to both AutoML phases â€” bake them into agent behavior, don't just paste once. The full detail is in `references/pitfalls.md`:
...
- **Run-to-run noise.** AutoML can show 2â€“3Ã— metric variance for the same config. If the winner looks suspiciously better than the runner-up, re-run with a fresh seed before committing the spec to Phase 2.
...
When starting fresh from "run the AOI workflow", the agent delivers a three-phase worded message to the user (Phase 1 AutoML baseline â†’ Phase 2 DEFT loop â†’ Phase 3 AutoML refinement, with the cost framing and "OK to proceed?" close), then after confirmation invokes `tao-run-automl` (Phase 1), writes the merged spec, pre-seeds `deft_state.json`, invokes `tao-run-deft-aoi` (Phase 2) with every input pre-supplied, and invokes `tao-run-automl` again (Phase 3) â€” with no further pauses unless a downstream skill hits an unrecoverable hard-stop gate â€” then summarizes the trajectory (baseline AutoML best â†’ DEFT iter 1 â†’ ... â†’ DEFT iter N_final â†’ Phase 3 best).
...
The same three-phase pattern applies to other DEFT skills â€” swap `network_arch`, the Phase 2 DEFT skill, the spec/checkpoint path conventions, and the Phase 3 augmented-CSV path. The handoff shape (Phase 1 emits spec + checkpoint that pre-seeds the DEFT baseline, Phase 2 emits an augmented dataset, Phase 3 emits the final checkpoint) is identical, and the baseline-skip mechanism is generic to any DEFT-style loop with a resumable baseline state. See `references/quick-start.md`.

---

## See also

- `tao-skill-bank:tao-run-automl` â€” AutoML interface, algorithms, HP ranges
- `tao-skill-bank:tao-run-deft-aoi` â€” full DEFT AOI loop (Phase 2 default)
- `tao-skill-bank:tao-train-visual-changenet` â€” underlying ChangeNet train/eval/infer skill (used by both AutoML and DEFT)
- Other `skills/applications/deft-*` skills â€” non-AOI Phase 2 targets
- `references/preflight.md` â€” building the consolidated pre-flight gate
- `references/handoff.md` â€” Phase 1â†’2 pre-seed, Phase 2 quality check, Phase 3 warm-start + report wiring
- `references/pitfalls.md` â€” metric, noise, leakage, and compute-budget guidance
- `references/quick-start.md` â€” verbatim worked-example message and non-AOI variant
