...
description: "Use when working with minimax-android-native-dev"
---
...
```
MyApp/
â”œâ”€â”€ gradle.properties          # Configure AndroidX and other settings
â”œâ”€â”€ settings.gradle.kts
â”œâ”€â”€ build.gradle.kts           # Root level
â”œâ”€â”€ gradle/wrapper/
â”‚   â””â”€â”€ gradle-wrapper.properties
â”œâ”€â”€ app/
â”‚   â”œâ”€â”€ build.gradle.kts       # Module level
â”‚   â””â”€â”€ src/main/
...
**Build Variant Naming**: `{flavor}{BuildType}` â†’ e.g., `devDebug`, `prodRelease`
...
```
app/src/
â”œâ”€â”€ main/           # Shared code for all flavors
â”œâ”€â”€ dev/            # Dev-only code and resources
â”‚   â”œâ”€â”€ java/
â”‚   â””â”€â”€ res/
â”œâ”€â”€ staging/        # Staging-only code and resources
â”œâ”€â”€ prod/           # Prod-only code and resources
â”œâ”€â”€ debug/          # Debug build type code
â””â”€â”€ release/        # Release build type code
```
...
**Null Safety**:
```kotlin
// âŒ Avoid: Non-null assertion !! (may crash)
val name = user!!.name

// âœ… Recommended: Safe call + default value
val name = user?.name ?: "Unknown"

// âœ… Recommended: let handling
user?.let { processUser(it) }
```

**Exception Handling**:
```kotlin
// âŒ Avoid: Random try-catch in business layer swallowing exceptions
fun loadData() {
    try {
        val data = api.fetch()
    } catch (e: Exception) {
...
// âœ… Recommended: Let exceptions propagate, handle at appropriate layer
...
// âœ… Recommended: Unified handling in ViewModel
...
**Common Mistakes**:
```kotlin
// âŒ Wrong: Updating UI on IO thread
viewModelScope.launch(Dispatchers.IO) {
    val data = api.fetch()
    _uiState.value = data  // Crash or warning!
}

// âŒ Wrong: Executing time-consuming operation on Main thread
viewModelScope.launch {
    val data = api.fetch()  // Blocking main thread! ANR
}

// âœ… Correct: Fetch on IO, update on Main
viewModelScope.launch {
...
```kotlin
// âŒ Wrong: Accessing uninitialized lateinit
class MyViewModel : ViewModel() {
    lateinit var data: String
    fun process() = data.length  // May crash
}

// âœ… Correct: Use nullable or default value
class MyViewModel : ViewModel() {
    var data: String? = null
    fun process() = data?.length ?: 0
}

// âŒ Wrong: Using return in lambda
list.forEach { item ->
    if (item.isEmpty()) return  // Returns from outer function!
}

// âœ… Correct: Use return@forEach
list.forEach { item ->
...
```kotlin
// âŒ Wrong: Fields declared as non-null (server may not return them)
...
// âœ… Correct: All fields declared as nullable
data class UserResponse(
    @SerializedName("id")
    val id: String? = null,
    @SerializedName("name")
...
```kotlin
// âŒ Wrong: Only adding Observer, not removing
class MyView : View {
...
// âœ… Correct: Paired add and remove
class MyView : View {
    override fun onAttachedToWindow() {
        super.onAttachedToWindow()
        activity?.lifecycle?.addObserver(this)
...
```kotlin
// âŒ Wrong: Calling Composable from non-Composable function
fun showError(message: String) {
    Text(message)  // Compile error!
}

// âœ… Correct: Mark as @Composable
@Composable
fun ErrorMessage(message: String) {
    Text(message)
}

// âŒ Wrong: Using suspend outside LaunchedEffect
@Composable
fun MyScreen() {
    val data = fetchData()  // Error!
}

// âœ… Correct: Use LaunchedEffect
@Composable
fun MyScreen() {
...
```kotlin
// âŒ Wrong: Creating objects in Composable (created on every recomposition)
@Composable
fun MyScreen() {
    val viewModel = MyViewModel()  // Wrong!
}

// âœ… Correct: Use viewModel() or remember
@Composable
...
```xml
<!-- âŒ Wrong: Using reserved names -->
<color name="background">#FFFFFF</color>
<color name="icon">#000000</color>

<!-- âœ… Correct: Add prefix or specific naming -->
<color name="app_background">#FFFFFF</color>
<color name="icon_primary">#000000</color>
```

```kotlin
// âŒ Wrong: Variable names conflict with system
...
```xml
<!-- âŒ Wrong: Drawable name conflicts -->
<ImageView android:src="@drawable/icon" />

<!-- âœ… Correct: Add prefix -->
...
|------|------|
| Minimum | 48 Ã— 48dp |
| Recommended (primary actions) | 56 Ã— 56dp |
| Kids apps | 56dp+ |
...
A well-tested Android app uses layered testing: fast local unit tests for logic, instrumentation tests for UI and integration, and Gradle Managed Devices to run emulators reproducibly on any machine â€” including CI.

### 8.1 Test Dependencies

Before adding test dependencies, inspect the project's existing versions to avoid conflicts:

1. Check `gradle/libs.versions.toml` â€” if present, add test deps using the project's version catalog style
...
**Dependencies Reference** â€” add only the groups you need:
...
> **Note**: If the project uses a Compose BOM, `ui-test-junit4` and `ui-test-manifest` don't need explicit versions â€” the BOM manages them.
