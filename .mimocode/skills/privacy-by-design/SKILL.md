...
description: "Use when working with privacy-by-design"
---

---
name: privacy-by-design
description: "Use when building apps that collect user data. Ensures privacy protections are built in from the startâ€”data minimization, consent, encryption."
risk: safe
source: community
...
- Use when deploying to productionâ€”verify privacy controls before launch

## Legal Frameworks

**GDPR (EU)** â€” Primary reference. Article 25 mandates "data protection by design and by default." Applies to EU users and often adopted globally.

**CCPA (California)** â€” Right to know, delete, opt-out of sale. Similar principles: minimize, disclose, allow control.

**LGPD (Brazil)** â€” Aligned with GDPR. Purpose limitation, necessity, transparency. Applies to Brazil users.
...
**Data minimization** â€” Less data = less breach impact, lower storage cost, simpler compliance. Each field is a liability.

**Purpose limitation** â€” Reusing data without consent is illegal under GDPR. Document purpose in schema or metadata.

**Retention** â€” Indefinite storage increases risk and violates GDPR. Define `retention_days` per data type; automate cleanup.

**Logging** â€” Logs often leak PII. Redact emails, IDs, tokens. Use structured logging with allowlists.

**Third parties** â€” Every SDK (analytics, crash reporting, ads) may send data elsewhere. Audit dependencies; require consent before loading.

---

## Code Examples

### JavaScript/Node â€” Minimal User Model
...
### JavaScript â€” Consent Before Tracking
...
### SQL â€” Schema with Purpose and Retention
...
### API â€” Return Only Needed Fields
...
- âœ… Ask "do we need this?" for every new data field
- âœ… Design deletion and export flows from day one
- âœ… Use hashing or tokenization for sensitive identifiers when possible
- âœ… Document purpose and retention in schema or metadata
- âŒ Don't log passwords, tokens, or PII in plain text
- âŒ Don't share data with third parties without explicit consent
- âŒ Don't assume "we'll add privacy later"â€”it rarely happens
- âŒ Don't expose stack traces or internal errors to clients
