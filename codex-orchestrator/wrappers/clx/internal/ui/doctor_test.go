...
func TestPrintDoctorRichCardIsResponsiveAndSanitized(t *testing.T) {
	...
	report := doctorRendererReport("clx")
	...
	for _, want := range []string{"CLX", "DOCTOR", "CHECKS", "GUIDANCE", "VERDICT", "✓", "!", "×"} {
...
func TestPrintDoctorPlainModesUseExplicitStatusWords(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			var buf bytes.Buffer
			PrintDoctor(&buf, caps, doctorRendererReport("clx"))
			out := buf.String()
			for _, want := range []string{"CLX DOCTOR", "OK", "WARN", "FAIL", "GUIDANCE", "VERDICT"} {
...
func doctorRendererReport(engine string) DoctorReport {
	return DoctorReport{
		...
		Rows: []DoctorRow{
			{Label: "Deps", Tone: ToneOK, Value: "available: curl, node"},
