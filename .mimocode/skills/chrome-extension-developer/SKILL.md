...
description: "Use when working with chrome-extension-developer"
---
...
- âœ… **Do:** Use `chrome.runtime.onInstalled` for extension initialization.
- âœ… **Do:** Use modern ES modules in scripts if configured in manifest.
- âœ… **Do:** Validate external input in content scripts before acting on it.
- âŒ **Don't:** Use `innerHTML` or `eval()` - prefer `textContent` and safe DOM APIs.
- âŒ **Don't:** Block the main thread in the service worker; it must remain responsive.
