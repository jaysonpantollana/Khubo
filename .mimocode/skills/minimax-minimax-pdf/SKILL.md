...
description: "Use when working with minimax-minimax-pdf"
---
...
|---|---|---|
| Generate a new PDF from scratch | **CREATE** | `palette.py` â†’ `cover.py` â†’ `render_cover.js` â†’ `render_body.py` â†’ `merge.py` |
| Fill / complete form fields in an existing PDF | **FILL** | `fill_inspect.py` â†’ `fill_write.py` |
| Reformat / re-style an existing document | **REFORMAT** | `reformat_parse.py` â†’ then full CREATE pipeline |

**Rule:** when in doubt between CREATE and REFORMAT, ask whether the user has an existing document to start from. If yes â†’ REFORMAT. If no â†’ CREATE.

---

## Route A: CREATE

Full pipeline â€” content â†’ design tokens â†’ cover â†’ body â†’ merged PDF.
...
**Doc types:** `report` Â· `proposal` Â· `resume` Â· `portfolio` Â· `academic` Â· `general` Â· `minimal` Â· `stripe` Â· `diagonal` Â· `frame` Â· `editorial` Â· `magazine` Â· `darkroom` Â· `terminal` Â· `poster`
...
- `--abstract "text"` â€” abstract text block on the cover (magazine/darkroom)
- `--cover-image "url"` â€” hero image URL/path (magazine, darkroom, poster)

**Color overrides â€” always choose these based on document content:**
- `--accent "#HEX"` â€” override the accent color; `accent_lt` is auto-derived by lightening toward white
- `--cover-bg "#HEX"` â€” override the cover background color

**Accent color selection guidance:**

You have creative authority over the accent color. Pick it from the document's semantic context â€” title, industry, purpose, audience â€” not from generic "safe" choices. The accent appears on section rules, callout bars, table headers, and the cover: it carries the document's visual identity.
...
**Rule:** choose a color that a thoughtful designer would select for this specific document â€” not the type's default. Muted, desaturated tones work best; avoid vivid primaries. When in doubt, go darker and more neutral.
...
| `bullet` | Unordered list item (â€¢ prefix) | `text` |
| `numbered` | Ordered list item â€” counter auto-resets on non-numbered blocks | `text` |
| `callout` | Highlighted insight box with accent left bar | `text` |
| `table` | Data table â€” accent header, alternating row tints | `headers`, `rows`, `col_widths`?, `caption`? |
...
| `math` | Display math â€” LaTeX syntax via matplotlib mathtext | `text`, `label`?, `caption`? |
...
| `divider` | Accent-colored full-width rule | â€” |
| `caption` | Small muted label | `text` |
| `pagebreak` | Force a new page | â€” |
...
Parse an existing document â†’ content.json â†’ CREATE pipeline.
