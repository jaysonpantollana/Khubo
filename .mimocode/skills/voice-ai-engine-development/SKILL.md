...
description: "Use when working with voice-ai-engine-development"
---
...
```
Audio In â†’ Transcriber â†’ Agent â†’ Synthesizer â†’ Audio Out
...
class SynthesisResult:
    ...
    get_message_up_to: Callable[[float], str]  # seconds â†’ partial text
...
- Handle audio format conversion (MP3 â†’ PCM)

### 4. Output Device (Audio â†’ Client)
...
**Scenario**: Bot is saying "I think the weather will be nice today and tomorrow andâ€”" when user interrupts with "Stop".
...
```python
# Log key events
logger.info(f"ðŸŽ¤ [TRANSCRIBER] Received: '{transcription.message}'")
logger.info(f"ðŸ¤– [AGENT] Generating response...")
logger.info(f"ðŸ”Š [SYNTHESIZER] Synthesizing {len(text)} characters")
logger.info(f"âš ï¸ [INTERRUPT] User interrupted bot")
...
```python
# âŒ Bad: Wait for entire response
async def generate_response(prompt):
    response = await openai.complete(prompt)  # 5 seconds
    return response

# âœ… Good: Stream chunks as they arrive
async def generate_response(prompt):
...
- âœ… Async worker pipeline for concurrent processing
- âœ… Queue-based communication between components
- âœ… Streaming at every stage (transcription, LLM, synthesis)
- âœ… Interrupt system for natural conversations
- âœ… Rate limiting for real-time audio playback
