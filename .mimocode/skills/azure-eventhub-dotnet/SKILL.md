...
description: "Use when working with azure-eventhub-dotnet"
---
...
1. **Use `EventProcessorClient` for receiving** â€” Never use `EventHubConsumerClient` in production
2. **Checkpoint strategically** â€” After N events or time interval, not every event
3. **Use partition keys** â€” For ordering guarantees within a partition
4. **Reuse clients** â€” Create once, use as singleton (thread-safe)
5. **Use `await using`** â€” Ensures proper disposal
6. **Handle `ProcessErrorAsync`** â€” Always register error handler
7. **Batch events** â€” Use `CreateBatchAsync()` to respect size limits
8. **Use buffered producer** â€” For high-volume scenarios with automatic batching
