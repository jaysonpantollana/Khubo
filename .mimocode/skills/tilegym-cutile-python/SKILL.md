...
description: "Use when working with tilegym-cutile-python"
---
...
**cuTile Language Specification** â€” <https://docs.nvidia.com/cuda/cutile-python>. Covers
...
- **Inside a TileGym checkout** (`<repo>/skills/tilegym-cutile-python/`, or `<repo>/.agents/skills/tilegym-cutile-python/` / `<repo>/.claude/skills/tilegym-cutile-python/` via the backward-compat symlinks) â€” TileGym ops are at `<repo>/src/tilegym/ops/cutile/`.
- **Installed elsewhere** (e.g. `~/.agents/skills/tilegym-cutile-python/`, `~/.claude/skills/tilegym-cutile-python/`, or inside a different repo) â€” clone TileGym once to `${TILEGYM_SKILL_CACHE_DIR:-~/.cache/tilegym}/TileGym` and use its `src/tilegym/ops/cutile/`.
...
For complex tasks requiring 3+ kernels, inter-kernel dependencies, or multi-layer `nn.Module` decomposition, use the orchestrated multi-agent pipeline. The main agent acts as an **orchestrator** (not a coder) â€” sub-agents handle reference reading and code generation.

**Pipeline**: Op Tracer (optional) â†’ Analyzer â†’ Kernel Agents (parallel) â†’ Composer â†’ Main Agent validates
...
2. **Study relevant cuTile examples**: Review examples for similar operations â€” existing examples often provide the exact patterns you need. Copy and adapt working patterns rather than reinventing the wheel.
...
**Reference Documentation**:
- **Language Spec** â€” <https://docs.nvidia.com/cuda/cutile-python>
- **Implementation Guidelines** (`guidelines/` 01â€“03) â€” Lessons, rules, and concepts
...
â”‚  1. Generate Code                                           â”‚
â”‚     - Write cuTile kernel with inline validation to file    â”‚
â”‚                                                             â”‚
â”‚  2. Execute Code                                            â”‚
â”‚     - Run: python <filename>.py                             â”‚
â”‚                                                             â”‚
â”‚  3. Check Results                                           â”‚
â”‚     â”œâ”€ Compilation error? â†’ Fix syntax/type issues â†’ Retry  â”‚
â”‚     â”œâ”€ Runtime error? â†’ Fix kernel logic â†’ Retry            â”‚
â”‚     â”œâ”€ Validation FAIL? â†’ Fix numerical issues â†’ Retry      â”‚
â”‚     â””â”€ Validation PASS? â†’ Done âœ“                            â”‚
...
- **Example validation pattern**:
  ...
  if is_close:
      ...
      print(f"âœ— Validation FAILED - max diff: {max_diff}")
...
See `guidelines/03_concepts.md` â†’ "Default Rules When User Does Not Specify" for tolerance values, default dtypes, and default tensor shapes.

### Testing Checklist
- âœ“ Verify cuTile output matches reference implementation within tolerance
- âœ“ Test with various tensor sizes (aligned and unaligned to tile size)
- âœ“ Test boundary conditions and edge cases
- âœ“ Ensure all tensors are on CUDA device before kernel launch
- âœ“ Verify dtype consistency across inputs and outputs
...
1. **Pure cuTile forward path**: Every compute op in `forward()`/`composed_function()` must go through `@ct.kernel` + `ct.launch`. Do not call `nn.Conv2d()(x)`, `F.conv2d(x, w)`, `F.linear(x, w)`, or any other `nn.*`/`F.*` compute op as a runtime operation in the forward path.
   ...
   - **Permitted in `__init__()`**: Using `nn.Conv2d`, `nn.Linear`, etc. solely for **weight initialization and storage** is fine â€” as long as `forward()` extracts the weights (e.g., `self.conv.weight.data`) and passes them to `ct.launch` instead of calling `self.conv(x)`.
   - See Rule 15 and Rule 17 in `guidelines/02_code_generation_rules.md` for common violations and detailed examples.
2. **Tile indices, not element indices**: `ct.load(A, index=(bid_m, k), shape=(BLOCK_M, K))` âœ… not `(bid_m * BLOCK_M, k)` âŒ
...
For detailed guidelines on memory operations, tile sizing, common pitfalls, and optimization strategies, see the `guidelines/` directory (01â€“03).
...
7. **Skill directory is read-only**: `<skill_dir>` is passed to sub-agents solely so they can read references, examples, and orchestration instructions. No agent â€” main or sub â€” may ever write, create, or save any file under `<skill_dir>`. Use it only with read tools (Read, Glob, Grep, Bash `cat`/`grep`). Never pass it to Write, Edit, or any file-creating command.
...
1. âœ… **Pure cuTile forward path**: No `nn.*`/`F.*` compute calls in `forward()`/`composed_function()` â€” all compute routed through `ct.launch` (weight-init-only usage in `__init__` is fine)
2. âœ… Existing examples were searched before implementation
3. âœ… Packaged `examples/` were searched if TileGym had no match
4. âœ… Only ONE .py file created (no READMEs, no separate examples unless requested)
5. âœ… No source citations in code (no mentions of TileGym files or reference files in comments/docstrings)
6. âœ… Generated cuTile code compiles without errors
7. âœ… Numerical results match reference implementation within tolerance
8. âœ… All constants have proper type annotations
9. âœ… All tile dimensions are powers of 2
10. âœ… Grid dimensions correctly cover all tensor elements
11. âœ… Code includes inline validation and test code in the same file

**Additional criteria when using orchestration (complex tasks):**

12. âœ… Complexity was assessed and orchestration was chosen for the right reasons
13. âœ… Analyzer produced clear kernel specs with PyTorch references
14. âœ… Independent kernels were generated in parallel (not sequentially)
15. âœ… Each individual kernel was validated before composition
16. âœ… Composed solution passes end-to-end validation against original PyTorch reference
