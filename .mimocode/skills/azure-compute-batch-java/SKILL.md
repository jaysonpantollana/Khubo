...
description: "Use when working with azure-compute-batch-java"
---
...
1. **Use Entra ID** â€” Preferred over shared key for authentication
2. **Use management SDK for pools** â€” `azure-resourcemanager-batch` supports managed identities
3. **Batch task creation** â€” Use `createTaskCollection` or `createTasks` for multiple tasks
4. **Handle LRO properly** â€” Pool resize, delete operations are long-running
5. **Monitor task counts** â€” Use `getJobTaskCounts` to track progress
6. **Set constraints** â€” Configure `maxWallClockTime` and `maxTaskRetryCount`
7. **Use low-priority nodes** â€” Cost savings for fault-tolerant workloads
8. **Enable autoscale** â€” Dynamically adjust pool size based on workload
