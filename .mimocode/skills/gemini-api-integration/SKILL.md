...
description: "Use when working with gemini-api-integration"
---
...
This skill guides AI agents through integrating Google Gemini API into applications â€” from basic text generation to advanced multimodal, function calling, and streaming use cases. It covers the full Gemini SDK lifecycle with production-grade patterns.
...
- âœ… **Do:** Use `gemini-1.5-flash` for most tasks â€” it's fast and cost-effective
- âœ… **Do:** Always stream responses for user-facing chat UIs to reduce perceived latency
- âœ… **Do:** Store API keys in environment variables, never hard-code them
- âœ… **Do:** Implement exponential backoff for rate limit (429) errors
- âœ… **Do:** Use `systemInstruction` to set persistent model behavior
- âŒ **Don't:** Use `gemini-pro` for simple tasks â€” Flash is cheaper and faster
- âŒ **Don't:** Send large base64 images inline for files > 20MB â€” use File API instead
- âŒ **Don't:** Ignore safety ratings in responses for production apps
...
try {
  ...
  if (error.status === 429) {
    ...
    // Invalid request â€” check prompt or parameters
