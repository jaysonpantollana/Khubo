...
description: "Use when working with agent-framework-azure-ai-py"
---
...
```
User Query â†’ AzureAIAgentsProvider â†’ Azure AI Agent Service (Persistent)
                    â†“
...
async def main():
    async with (
        ...
        print(f"{weather.location}: {weather.temperature}Â°{weather.unit}")
```
