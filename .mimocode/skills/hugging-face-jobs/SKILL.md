...
description: "Use when working with hugging-face-jobs"
---
...
Run any workload on fully managed Hugging Face infrastructure. No local setup requiredâ€”jobs run on cloud CPUs, GPUs, or TPUs and can persist results to the Hugging Face Hub.
...
- **HF_TOKEN for Hub Access** âš ï¸ CRITICAL - Required for any Hub operations (push models/datasets, download private repos, etc.)
- Token must have appropriate permissions (read for downloads, write for uploads)

### âœ… **Token Usage** (See Token Usage section for details)
...
```python
# hf_jobs MCP tool â€” $HF_TOKEN is auto-replaced with real token:
{"secrets": {"HF_TOKEN": "$HF_TOKEN"}}

# HfApi().run_uv_job() â€” MUST pass actual token:
from huggingface_hub import get_token
secrets={"HF_TOKEN": get_token()}
```

**âš ï¸ CRITICAL:** The `$HF_TOKEN` placeholder is ONLY auto-replaced by the `hf_jobs` MCP tool. When using `HfApi().run_uv_job()`, you MUST pass the real token via `get_token()`. Passing the literal string `"$HF_TOKEN"` results in a 9-character invalid token and 401 errors.
...
hf_jobs("uv", {
    ...
    "secrets": {"HF_TOKEN": "$HF_TOKEN"}  # âœ… Automatic replacement
})
```
...
hf_jobs("uv", {
    "script": "your_script.py",
    "secrets": {"HF_TOKEN": "hf_abc123..."}  # âš ï¸ Hardcoded token
})
```
...
hf_jobs("uv", {
    "script": "your_script.py",
    "env": {"HF_TOKEN": "hf_abc123..."}  # âš ï¸ Less secure than secrets
})
```
...
print("âœ… Dataset pushed successfully!")
""",
    "flavor": "cpu-basic",
    "timeout": "30m",
    "secrets": {"HF_TOKEN": "$HF_TOKEN"}  # âœ… Token provided securely
})
```
...
âš ï¸ **Important:** There are *two* "script path" stories depending on how you run Jobs:
...
```python
# âŒ Will fail (remote container can't see your local path)
...
```python
# âœ… Inline: read the local script file and pass its *contents*
from pathlib import Path
script = Path("hf-jobs/scripts/foo.py").read_text()
hf_jobs("uv", {"script": script})

# âœ… URL: host the script somewhere reachable
...
**âš ï¸ EPHEMERAL ENVIRONMENTâ€”MUST PERSIST RESULTS**
...
```
Total Cost = (Hours of runtime) Ã— (Cost per hour)
```
...
### Pattern 1: Dataset â†’ Model Responses (vLLM) â€” `scripts/generate-responses.py`
...
### Pattern 2: CoT Self-Instruct Synthetic Data â€” `scripts/cot-self-instruct.py`
...
### Pattern 3: Streaming Dataset Stats (Polars + HF Hub) â€” `scripts/finepdfs-stats.py`
...
3. Upgrade hardware: cpu â†’ t4 â†’ a10g â†’ a100
...
2. Verify token in secrets â€” MCP: `"$HF_TOKEN"`, Python API: `get_token()` (NOT `"$HF_TOKEN"`)
...
**Common issues:**
- Job times out â†’ Increase timeout, optimize code
- Results not saved â†’ Check persistence method, verify HF_TOKEN
- Out of Memory â†’ Reduce batch size, upgrade hardware
- Import errors â†’ Add dependencies to PEP 723 header
- Authentication errors â†’ Check token, verify secrets parameter
...
- `scripts/generate-responses.py` - vLLM batch generation: dataset â†’ responses â†’ push to Hub
- `scripts/cot-self-instruct.py` - CoT Self-Instruct synthetic data generation + filtering â†’ push to Hub
...
5. **Use tokens securely** - MCP: `secrets={"HF_TOKEN": "$HF_TOKEN"}`, Python API: `secrets={"HF_TOKEN": get_token()}` â€” `"$HF_TOKEN"` only works with MCP tool
