...
description: "Use when working with minimax-frontend-dev"
---
...
metadata:
  ...
  category: frontend
  sources:
    - taste-skill by Leonxlnx (https://github.com/Leonxlnx/taste-skill) â€” Design engineering framework
    - canvas-design by Anthropic (https://github.com/anthropics/skills/tree/main/skills/canvas-design) â€” Static visual art workflow
    - algorithmic-art by Anthropic (https://github.com/anthropics/skills/tree/main/skills/algorithmic-art) â€” Generative art workflow
...
â”‚   â”œâ”€â”€ minimax_tts.py            # Text-to-speech
â”‚   â”œâ”€â”€ minimax_music.py          # Music generation
â”‚   â”œâ”€â”€ minimax_video.py          # Video generation (async)
â”‚   â””â”€â”€ minimax_image.py          # Image generation
â”œâ”€â”€ references/                   # Detailed guides (read as needed)
â”‚   â”œâ”€â”€ minimax-cli-reference.md  # CLI flags quick reference
â”‚   â”œâ”€â”€ asset-prompt-guide.md     # Asset prompt engineering rules
â”‚   â”œâ”€â”€ minimax-tts-guide.md      # TTS usage & voices
â”‚   â”œâ”€â”€ minimax-music-guide.md    # Music prompts & lyrics format
â”‚   â”œâ”€â”€ minimax-video-guide.md    # Camera commands & models
â”‚   â”œâ”€â”€ minimax-image-guide.md    # Ratios & batch generation
â”‚   â”œâ”€â”€ minimax-voice-catalog.md  # All voice IDs
â”‚   â”œâ”€â”€ motion-recipes.md         # Animation code snippets
â”‚   â”œâ”€â”€ env-setup.md              # Environment setup
â”‚   â””â”€â”€ troubleshooting.md        # Common issues
â”œâ”€â”€ templates/                    # Visual art templates
â”‚   â”œâ”€â”€ viewer.html               # p5.js interactive art base
â”‚   â””â”€â”€ generator_template.js     # p5.js code reference
â””â”€â”€ canvas-fonts/                 # Static art fonts (TTF + licenses)
```
...
assets/
â”œâ”€â”€ images/
â”‚   â”œâ”€â”€ hero-landing-1710xxx.webp
â”‚   â”œâ”€â”€ icon-feature-01.webp
â”‚   â””â”€â”€ bg-pattern.svg
â”œâ”€â”€ videos/
â”‚   â”œâ”€â”€ hero-bg-1710xxx.mp4
â”‚   â””â”€â”€ demo-preview.mp4
â””â”€â”€ audio/
    â”œâ”€â”€ bgm-ambient-1710xxx.mp3
    â””â”€â”€ tts-intro-1710xxx.mp3
```
...
â”‚   â””â”€â”€ styles.css
â””â”€â”€ js/
    â””â”€â”€ main.js           # Animations (GSAP/vanilla)
```
...
â”‚   â”‚   â””â”€â”€ motion/       # RevealSection, StaggerGrid
â”‚   â”œâ”€â”€ lib/
â”‚   â”œâ”€â”€ styles/
â”‚   â””â”€â”€ app/              # Pages
â””â”€â”€ package.json
...
â”‚   â”œâ”€â”€ pages/
â”‚   â””â”€â”€ assets/           # Processed assets (optional)
â””â”€â”€ package.json
```
...
â”œâ”€â”€ public/assets/
â”œâ”€â”€ src/
â”‚   â”œâ”€â”€ components/       # .astro, .tsx, .vue, .svelte
â”‚   â”œâ”€â”€ layouts/
â”‚   â”œâ”€â”€ pages/
â”‚   â””â”€â”€ styles/
â””â”€â”€ package.json
...
**All rules in this skill are mandatory. Violating any rule is a blocking error â€” fix before proceeding or delivering.**

---

## Workflow
### Phase 1: Design Architecture
1. Analyze the request â€” determine page type and context
...
3. Execute via scripts, save to project â€” do NOT proceed to Phase 5 until all assets are saved locally

### Phase 4: Copywriting & Content
Follow copywriting frameworks (AIDA, PAS, FAB) to craft all text content. Do NOT use "Lorem ipsum" â€” write real copy.
...
- **Styling:** Tailwind CSS. Check version in `package.json` â€” NEVER mix v3/v4 syntax.
...
| Rule | Directive |
|------|-----------|
| Typography | Headlines: `text-4xl md:text-6xl tracking-tighter`. Body: `text-base leading-relaxed max-w-[65ch]`. **NEVER** use Inter â€” use Geist/Outfit/Satoshi. **NEVER** use Serif on dashboards. |
...
- **Magnetic Buttons:** Use `useMotionValue`/`useTransform` â€” never `useState` for continuous animations
...
**5-Card Archetypes:**
1. Intelligent List â€” auto-sorting with `layoutId`
2. Command Input â€” typewriter + blinking cursor
3. Live Status â€” breathing indicators
4. Wide Data Stream â€” infinite horizontal carousel
5. Contextual UI â€” staggered highlight + float-in toolbar
...
| Scroll storytelling (pin, scrub) | **GSAP + ScrollTrigger** â€” frame-accurate control |
| Looping icons | **Lottie** â€” lazy-load (~50KB) |
| 3D/WebGL | **Three.js / R3F** â€” isolated `<Canvas>`, own `"use client"` boundary |
...
| 3-4 Smooth | CSS keyframes + Framer animate, stagger â‰¤3 items |
...
**NEVER animate:** `width`, `height`, `top`, `left`, `margin`, `padding`, `font-size` â€” if you need these effects, use `transform: scale()` or `clip-path` instead.
...
| Video | `scripts/minimax_video.py` | Async (create â†’ poll â†’ download) |
...
4. **Save:** `<project>/public/assets/{images,videos,audio}/` as `{type}-{descriptor}-{timestamp}.{ext}` â€” **MUST save locally**
...
- `references/minimax-cli-reference.md` â€” CLI flags
- `references/asset-prompt-guide.md` â€” Prompt rules
- `references/minimax-voice-catalog.md` â€” Voice IDs
- `references/minimax-tts-guide.md` â€” TTS usage
- `references/minimax-music-guide.md` â€” Music generation (prompts, lyrics, structure tags)
- `references/minimax-video-guide.md` â€” Camera commands
- `references/minimax-image-guide.md` â€” Ratios, batch
...
Identify subtle, niche reference â€” sophisticated, not literal. Jazz musician quoting another song.
...
- [ ] **No placeholder URLs** â€” grep the output for `unsplash`, `picsum`, `placeholder`, `placehold`, `via.placeholder`, `lorem.space`, `dummyimage`. If ANY found, STOP and replace with generated assets before delivering.
