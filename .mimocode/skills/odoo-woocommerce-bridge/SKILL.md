...
description: "Use when working with odoo-woocommerce-bridge"
---
...
- âœ… **Do:** Use **SKU** as the unique identifier linking WooCommerce products to Odoo products.
- âœ… **Do:** Run inventory sync on a **schedule** (every 15-30 min) rather than real-time to avoid rate limits.
- âœ… **Do:** Log all API calls and errors to a database table for debugging.
- âŒ **Don't:** Process the same WooCommerce order twice â€” flag it as processed immediately after import.
- âŒ **Don't:** Sync draft or cancelled WooCommerce orders to Odoo â€” filter by `status = processing` or `completed`.
