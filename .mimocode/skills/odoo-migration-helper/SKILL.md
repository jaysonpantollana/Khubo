...
description: "Use when working with odoo-migration-helper"
---
...
- âœ… **Do:** Test with `--update=your_module` on each version before pushing to production.
- âœ… **Do:** Use the official [Odoo Upgrade Guide](https://upgrade.odoo.com/) to get an automated pre-upgrade analysis report.
- âœ… **Do:** Check OCA migration notes and the module's `HISTORY.rst` for community modules.
- âœ… **Do:** Run `npm run validate` after migration to catch manifest or frontmatter issues early.
- âŒ **Don't:** Skip intermediate versions â€” go v14â†’v15â†’v16â†’v17 sequentially; never jump.
- âŒ **Don't:** Forget to update `version` in `__manifest__.py` (e.g., `17.0.1.0.0`).
- âŒ **Don't:** Assume OCA modules are migration-ready; check their GitHub branch for the target version.
