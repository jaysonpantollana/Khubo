...
description: "Use when working with tao-run-on-brev"
---
...
# 2. brev-cli agent skill installed â€” provides the brev CLI's command reference to the agent
...
# 3. brev login active â€” always token-login first when running headless.
...
run `brev login --token "$BREV_API_TOKEN"` before any other `brev` call** â€”
...
- **Ephemeral**: Omit `instance_id` â€” the handler creates a new instance per job. Clean but slower (instance boot ~2-5 min).

### Creating an instance â€” placement info
...
**Multi-node is not supported on Brev.** Brev is instance-based â€” one job runs on one instance, with no cross-instance coordination.

Multi-GPU **on a single instance** is supported (instances available with up to 8Ã— H100 / A100 / L40S). `gpu_count` maps to the GPU count on the instance; `torchrun --nproc-per-node=N` or PyTorch DDP work within the instance.
...
```bash
# Wait up to 5 minutes for shell readiness â€” covers the SSH bring-up window.
...
**â‰¥ 600 s (10 min)** for the first exec on a new instance; the previous
60â€“120 s default truncates remote startup and surfaces as a spurious
...
```bash
brev delete <instance>      # plain delete â€” no flags
```
...
persists across a single retry, the token itself is stale â€” mint a fresh one.
...
*Creating an instance â€” placement info* above.
...
timeout to â‰¥ 600 s for cold-start runs (see *`brev exec` timeout for
