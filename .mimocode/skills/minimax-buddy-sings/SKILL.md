---
name: minimax-buddy-sings
description: "Use when working with minimax-buddy-sings"
---
...
description: >
  ...
  singing or music. Supports multilingual triggers â€” match equivalent phrases in any
  language.
license: MIT
metadata:
  version: "1.1"
...
# Buddy Sings â€” Let Your Claude Code Pet Sing

Turn your Claude Code pet into a singer. Each pet gets a unique vocal identity
based on its name and personality â€” the same pet always sounds the same.
...
- **Audio player** (for playback â€” at least one of):
  - `mpv` (recommended â€” interactive controls: space = pause, q = quit)
  - `ffplay` (from FFmpeg)
...
```
Check pet â†’ Build vocal identity â†’ Gather context â†’ Generate music â†’ Play & feedback
```
...
throughout the entire session. All examples below are in English â€” translate them
naturally when responding in other languages.

**User-facing text localization rule**:
- ALL text shown to the user â€” including pet info, voice description, lyrics preview,
  prompt preview, playback info, and feedback prompts â€” MUST be fully translated into
  the user's language.
- The **API prompt** sent to the model should always be written in English for best
  ...
  is an internal implementation detail â€” the user does not need to see it.
...
- `name` â€” the pet's name
- `personality` â€” the pet's personality description
...
vocal identity. No template lookups â€” interpret the personality freely.

### How to interpret personality into voice

Read the personality text and craft vocal attributes:

- **Timbre**: What does this personality sound like? e.g., "few words" â†’
  low, warm, deliberate; "energetic" â†’ bright, punchy; "mysterious" â†’ breathy,
  dark; "legendary chonk" â†’ thick, warm, cozy
- **Singing style**: How would they deliver a song? e.g., "of few words" â†’
  sparse, dramatic pauses; "playful" â†’ bouncy, rhythmic; "poetic" â†’ flowing, legato
- **Mood**: What emotional tone fits? e.g., "chill" â†’ relaxed, laid-back;
  "fierce" â†’ intense, powerful
...
**First time**: No cache exists â†’ interpret personality â†’ save to cache file.

**Subsequent calls**: Read cache â†’ use the saved `prompt_fragment` directly.
Do NOT re-interpret â€” consistency matters.

**Cache invalidation**: If the `personality` in `~/.claude.json` differs from what's
cached, the pet has changed â€” regenerate and save a new cache.
...
- **Time-based references**: "today", "this week", "recently", "yesterday" â†’ scan
  current conversation history and memory files for what happened in that period
- **Personal references**: "my work", "my day", "what I did" â†’ scan memory
  and conversation for the user's activities
- **Relationship references**: "our story", "what we did together" â†’ scan memory for
...
1. **Current conversation context**: Look at what the user has been doing in
   this Claude Code session â€” files edited, commands run, topics discussed.
...
Use gathered context to enrich the lyrics prompt â€” make the song personal and
...
Don't present a mode menu â€” ask a specific question:

```
What should <name> sing about?

For example:
  - "Today's work" â€” I'll check what you've been up to
...
personality:
- Quiet/reserved personality â†’ midnight lullaby, gentle sunset, quiet morning
- Energetic personality â†’ party jam, adventure song, victory march
- Mysterious personality â†’ moonlit serenade, secret whisper, dream journey
...
1. **Construct the full prompt**: The prompt has two parts that MUST both be present:
   ...
   **Part A â€” Vocal identity (MUST come first)**: Always start the prompt with the
   cached `prompt_fragment`. This is the most important part â€” it defines who is
   singing. Place it at the beginning of the prompt so the API prioritizes it.

   **Part B â€” Genre/style/mood tags**: Choose tags that **match the theme**, NOT
   ...
   **Genre matching guidelines** â€” pick a genre that fits the theme's energy:
   ...
   **Prompt structure** â€” write as vivid English sentences, not comma-separated tags:
   ```
...
2. **Generate lyrics**: Use `--lyrics-optimizer` to auto-generate lyrics, or write lyrics
   ...
   **Important â€” perspective & personality-driven lyrics**:
   ...
   - NOT "I rub my sleepy eyes" (owner's perspective â€” wrong)
   - NOT "Then you woke up, my little Moth" (owner talking about pet â€” wrong)

   The pet's personality should shape the lyrics' tone and word choice:
   - "of few words" â†’ short, impactful lines, minimal filler
   - "playful" â†’ rhyming, bouncy phrasing, fun wordplay
   - "poetic" â†’ metaphor-rich, flowing imagery
   - "fierce" â†’ direct, powerful declarations
...
3. **Preview (MUST show full content)**: Before generating, show the user the
   **complete lyrics** and **full prompt** â€” no abbreviation, no `...`, no summary.
   This is part of the fun â€” the user wants to read and enjoy the lyrics before
   hearing them sung.
   ...
   internal implementation detail â€” do not show it to the user. Translate all
   labels (Singer, Theme, Description, Confirm, etc.) into the user's language.

   Template (English reference â€” localize all labels at runtime):
...
Do NOT show playback controls (e.g. keyboard shortcuts) â€” they don't work in this
...
| Companion field is empty | Same â€” guide to `/buddy` |
...
| User wants a specific genre | Let them override â€” append their genre to the prompt |
...
- Lyrics should always be **original** â€” never reproduce copyrighted lyrics.
