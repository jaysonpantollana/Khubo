---
name: tao-train-dino
description: "Use when working with tao-train-dino"
---
...
**Required datasets â€” MUST resolve both:**

| Dataset | Required | Why |
|---|---|---|
| Train dataset URI | Yes | Training data (COCO format) |
| Validation dataset URI | **Yes â€” ALWAYS** | DINO unconditionally builds a val dataloader. Omitting `val_data_sources` causes `FileNotFoundError` at startup regardless of the metric or workflow. If the user has no separate eval split, reuse the train URI. |

**Required inputs before generating any training spec:**

1. **Train dataset URI** â€” S3 path to COCO-format training data
2. **Validation dataset URI** â€” S3 path to COCO-format val data (can be same as train)
3. **`num_classes`** â€” How many object classes? Default 91 (COCO). Must be >= `max(category_id) + 1`. Too low causes `CUDA error: device-side assert triggered`.
...
Data source overrides are **mandatory for every action** â€” DINO's `config.json` has empty `data_sources` because the runner cannot auto-resolve array-of-objects spec keys. The agent MUST build data source paths and include them in `spec_overrides`.
