...
description: "Use when working with django-access-review"
---
...
```
â–¡ Where are permission checks implemented?
...
```
â–¡ What models contain user data?
â–¡ Which have ownership fields (owner_id, user_id, organization_id)?
â–¡ Which are accessed via ID in URLs or request bodies?
```
...
```
â–¡ For list endpoints: Does the query filter to user's data, or return everything?

â–¡ For create endpoints: Who sets the owner - the server or the request?

â–¡ For bulk operations: Are they scoped to user's data?

â–¡ For related resources: If I can access a document, can I access its comments?
  What if the document belongs to someone else?

â–¡ For tenant/org resources: Can User in Org A access Org B's data by changing
...
1. Find the view handling this URL
   â†’ DocumentViewSet.retrieve() in api/views.py

2. Check what DocumentViewSet inherits from
   â†’ class DocumentViewSet(viewsets.ModelViewSet)
   â†’ No custom base class with authorization

3. Check permission_classes
   â†’ permission_classes = [IsAuthenticated]
   â†’ Only checks login, not ownership

4. Check get_queryset()
   â†’ def get_queryset(self):
   â†’     return Document.objects.all()
   â†’ Returns ALL documents!

5. Check for has_object_permission()
   â†’ Not implemented

6. Check retrieve() method
   â†’ Uses default, which calls get_object()
   â†’ get_object() uses get_queryset(), which returns all
...
```
â–¡ I understand how authorization is typically implemented in this codebase
â–¡ I've identified the ownership model (user, org, tenant, etc.)
â–¡ I've mapped the key endpoints that handle user data
â–¡ For each sensitive endpoint, I've traced the flow and asked:
  - Where does the ID come from?
  - Where is data fetched?
  - What checks exist between input and data access?
â–¡ I've verified my findings by checking parent classes and middleware
â–¡ I've only reported issues I've confirmed through investigation
```
