...
description: "Use when working with nemo-mbridge-perf-memory-tuning"
---

---
name: nemo-mbridge-perf-memory-tuning
description: Techniques for reducing peak GPU memory in Megatron Bridge â€” expandable segments, parallelism resizing, activation recompute, CPU offloading constraints, and common OOM fixes.
license: Apache-2.0
...
- **Parameter + optimizer state memory** â€” controlled by TP, PP, DP sharding
  (distributed optimizer, FSDP)
- **Activation memory** â€” controlled by activation recompute, sequence length,
  micro-batch size
- **Temporary / workspace memory** â€” CUDA kernels, NCCL buffers, CUDA graphs
...
3. **Avoid increasing TP** as a memory fix â€” doubling TP dramatically increases
   NVLink all-reduce volume and often kills throughput (-28% on Llama3 70B).
4. **Avoid increasing PP at the cost of DP** â€” halving DP doubles gradient
...
effect on peak memory is minimal â€” changing VPP does not meaningfully change
...
(VPP 5â†’10). The actual fix was `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`
...
- **PP=8 works for memory but loses DP** (2â†’1), meaning 32 gradient accumulation
