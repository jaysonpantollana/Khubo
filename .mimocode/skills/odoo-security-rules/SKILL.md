...
description: "Use when working with odoo-security-rules"
---
...
> **Note:** Use `base.group_erp_manager` for ERP managers, not `base.group_system` â€” that group is reserved for Odoo's technical superusers. Always create a custom group for module-specific manager roles:
>
> ```xml
...
### Example 2: Record Rule â€” Users See Only Their Own Records
...
- âœ… **Do:** Start with the most restrictive access and open up as needed.
- âœ… **Do:** Use `company_ids` (plural) in multi-company rules â€” it includes all companies the user belongs to.
- âœ… **Do:** Test rules using a non-admin user in debug mode â€” `sudo()` bypasses all record rules entirely.
- âœ… **Do:** Create dedicated security groups per module rather than reusing core Odoo groups.
- âŒ **Don't:** Give `perm_unlink = 1` to regular users unless deletion is explicitly required by the business process.
- âŒ **Don't:** Leave `group_id` blank in `ir.model.access.csv` unless you intend to grant public (unauthenticated) access.
- âŒ **Don't:** Use `base.group_system` for module managers â€” that grants full technical access including server configurations.

## Limitations

- Does not cover **field-level access control** (`ir.model.fields` read/write restrictions) â€” those require custom OWL or Python overrides.
- **Portal and public user** access rules have additional nuances not fully covered here; test carefully with `base.group_portal`.
- Record rules are **bypassed by `sudo()`** â€” any code running in superuser context ignores all `ir.rule` entries.
- Does not cover **row-level security via PostgreSQL** (RLS) â€” Odoo manages all security at the ORM layer.
