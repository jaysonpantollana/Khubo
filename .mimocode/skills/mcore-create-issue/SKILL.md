...
description: "Use when working with mcore-create-issue"
---
...
test file. The file may not exist on `main` â€” first determine the PR's base
...
- **Error message**: the assertion failure, exception type, or first meaningful
  traceback frame â€” keep it under ~30 lines.
...
gh issue create \
  ...
  --title "ðŸ› CI failure: <failed-test-node-id>" \
  --label "bug" \
...
```
<core error message / traceback â€” 30 lines max>
```
...
- Never create an issue if a duplicate already exists â€” link the existing one instead.
...
- Keep the error snippet concise (â‰¤30 lines). Truncate long tracebacks and note that the full log is available via the job URL.
- Do not guess the root cause â€” quote the actual log output verbatim.
