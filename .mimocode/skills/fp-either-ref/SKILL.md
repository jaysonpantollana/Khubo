---
name: fp-either-ref
description: "Use when working with fp-either-ref"
---
...
E.fromNullable(err)(x)   // null â†’ Left(err), else Right(x)
E.tryCatch(fn, toError)  // try/catch â†’ Either
```
...
E.filterOrElse(pred, toErr) // Right â†’ Left if pred fails
```
...
```typescript
// âŒ try/catch - errors not in types
try {
...
// âœ… Either - errors explicit in types
pipe(
