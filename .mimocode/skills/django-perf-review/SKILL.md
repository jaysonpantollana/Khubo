...
description: "Use when working with django-perf-review"
---
...
Validate by tracing: View â†’ Queryset â†’ Template/Serializer â†’ Loop access
...
# Template:
# {% for user in users %}
#     {{ user.profile.bio }}  â† triggers query per user
# {% endfor %}
...
class UserSerializer(serializers.ModelSerializer):
    ...
    def get_order_count(self, obj):
        return obj.orders.count()  # â† query per user
...
class User(models.Model):
    email = models.EmailField()  # â† no db_index
...
**Validation:**
- Traced: UserListView â†’ users queryset â†’ user_list.html â†’ `{{ user.profile.bio }}` in loop
...
- Does the severity match the actual impact? ("Minor inefficiency" â‰  CRITICAL)
