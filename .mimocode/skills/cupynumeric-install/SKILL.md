...
description: "Use when working with cupynumeric-install"
---

---
name: cupynumeric-install
description: Install and verify cuPyNumeric for Python â€” requirements, commands, verification. Source builds are out of scope.
...
Use this skill to install cuPyNumeric for *use* from Python and to verify the install actually works (including GPU usage). Apply it whenever a user wants cuPyNumeric running via conda or pip. Do not use it to build from source (to modify or contribute) â€” that is out of scope.
...
- **GPU**: Compute Capability â‰¥ 7.0 (Volta+). CPU-only also supported.
- **CUDA**: 12.2+.
...
### Bootstrap â€” install a package manager first

If neither `conda` nor `pip` is available, install one. **Provide the command and the docs link; do not run it** â€” `curl | bash` requires user trust.
...
Run a self-contained script through the `legate` launcher â€” no repo checkout needed.
...
Expect `sum: 45` and `matmul: 64.0`. If `legate` is missing, the env is not activated â€” see Troubleshooting.

#### GPU usage check (mandatory when a supported GPU is present)

A passing smoke test does **not** prove GPU usage â€” a CPU-variant install on a GPU box produces correct results too. Run both steps.
...
**2. Confirm the GPU was touched.** Run a deadline-bounded matmul loop alongside `nvidia-smi`, all from one shell â€” no second-terminal race:
...
for _ in $(seq 10); do                      # 10 samples at 1s â€” covers slow startup
...
Expect `memory.used` in the GiB range across most samples and non-trivial `utilization.gpu` in several. If both stay at baseline across every sample, the GPU variant is not installed â€” check `conda list cupynumeric` for `*_gpu` (not `*_cpu`).
...
- **Verify `conda --version` â‰¥ 24.1.** Older releases silently break variant selection.
...
- **`ModuleNotFoundError: No module named 'cupynumeric'`** â†’ Run `which python` and `pip list | grep cupynumeric` (or `conda list | grep cupynumeric`) from the same shell to find the env mismatch.
- **`ImportError` mentioning CUDA / `libcudart`** â†’ Reinstall with `CONDA_OVERRIDE_CUDA="<your-cuda-version>"`; the CPU variant is on a GPU box, or CUDA versions are mismatched.
- **`legate: command not found`** â†’ Activate the env, then run `which legate` to confirm.
- **Slower than NumPy on a laptop** â†’ Expect this for small problems (Legate per-task overhead). See the cuPyNumeric FAQ.

## See also

- [references/verification_examples.md](references/verification_examples.md) â€” verification + troubleshooting recipes.
