---
name: hasdata
description: "Use when working with hasdata"
---
...
|---|---|---|---|
| **Web Scraping API** | seconds | Arbitrary URL â€” JS rendering, CSS/AI extraction, screenshots | `POST /scrape/web` |
| **Scraper APIs** (sync) | seconds | Pre-parsed JSON for known platforms (Google, Amazon, Zillow, â€¦) | `GET /scrape/<vertical>/<resource>` |
| **Scraper Jobs** (async) | minutesâ€“hours | Bulk extraction, recursive crawling, webhook fan-out | `POST /scrapers/<slug>/jobs` |

**Decision rule.** Default to a **Scraper API** when one exists for the platform (pre-parsed JSON, no selector maintenance). Use **Web Scraping** for arbitrary URLs not covered by an API. Reach for a **Scraper Job** only when no API equivalent exists â€” `crawler`, `contacts`, `sec-edgar`, `amazon-bestsellers`, `amazon-product-reviews` â€” *or* when async fan-out + webhooks save engineering time over a paginated client loop.

## Always-true response shape

```json
{ "requestMetadata": { "id": "â€¦", "status": "ok", "url": "â€¦" }, "...": "endpoint-specific" }
```
...
- **AI Mode + verify.** `/scrape/google/ai-mode` for the answer + references â†’ `/scrape/web` (markdown) on each reference URL â†’ cited RAG context, no vector DB.
- **Maps â†’ leads.** `/scrape/google-maps/search` returns business websites and phones; collect contact details only from public, permitted sources and apply opt-out, rate, and privacy-law constraints before any outreach use.
- **Crawler â†’ corpus.** `crawler` Scraper Job with `outputFormat: ["markdown"]` + `includePaths: "/docs/.+"` produces an LLM-ready corpus in one submission.
...
- **Timeouts:** **set client timeout â‰¥ 300 s.** HasData's own deadline is 300 s; shorter clients produce phantom failures while still being billed on completion.
- **Retries:** `429` and `5xx` only â€” exponential backoff, jitter. Never retry `4xx` (auth, validation).
- **Concurrency:** cap at your plan limit. The free tier is 1; anything higher just generates `429`s.
- **Async jobs:** the submit response handle is `body.id` (integer), **not `jobId`**. Persist it immediately. Poll `GET /scrapers/jobs/<id>` every 10â€“30 s with backoff; treat webhooks as best-effort and always pair with polling. On `finished` the status carries `data: {csv, json, xlsx}` short-lived URLs â€” download immediately.
...
- **Disable `jsRendering` first**, enable only if the page needs it â€” most static pages parse fine without a headless browser.
- **No `cookies` parameter** â€” cookies go through `headers["Cookie"]`.
...
- [`references/web-scraping.md`](references/web-scraping.md) â€” `POST /scrape/web` parameters, JS scenarios, AI extraction, cookie auth.
- [`references/search.md`](references/search.md) â€” Google SERP / Light / AI Mode / News / Shopping / Bing / Trends + pagination.
- [`references/ecommerce.md`](references/ecommerce.md) â€” Amazon (product, search, seller, seller-products) and Shopify.
- [`references/real-estate.md`](references/real-estate.md) â€” Zillow, Redfin (bracketed filters).
- [`references/travel.md`](references/travel.md) â€” Airbnb, Booking, Google Flights (occupancy rules, token pagination, IATA codes).
- [`references/local-business.md`](references/local-business.md) â€” Maps (search/place/reviews/photos/posts), Yelp, YellowPages.
- [`references/jobs.md`](references/jobs.md) â€” Indeed and Glassdoor.
- [`references/youtube.md`](references/youtube.md) â€” YouTube search / video / channel / transcript.
- [`references/scraper-jobs.md`](references/scraper-jobs.md) â€” async submit/poll/results, Crawler, Contacts, SEC EDGAR, webhook receiver.
- [`references/code-recipes.md`](references/code-recipes.md) â€” Python / TypeScript clients with retry, backoff, concurrency, polling.
