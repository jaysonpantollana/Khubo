...
description: "Use when working with azure-security-keyvault-keys-dotnet"
---
...
â”œâ”€â”€ CreateKey / CreateRsaKey / CreateEcKey
â”œâ”€â”€ GetKey / GetKeys
â”œâ”€â”€ UpdateKeyProperties
â”œâ”€â”€ DeleteKey / PurgeDeletedKey
â”œâ”€â”€ BackupKey / RestoreKey
â””â”€â”€ GetCryptographyClient() â†’ CryptographyClient

CryptographyClient (cryptographic operations)
â”œâ”€â”€ Encrypt / Decrypt
â”œâ”€â”€ WrapKey / UnwrapKey
â”œâ”€â”€ Sign / Verify
â””â”€â”€ SignData / VerifyData

KeyResolver (key resolution)
â””â”€â”€ Resolve(keyId) â†’ CryptographyClient
```
...
1. **Use Managed Identity** â€” Prefer `DefaultAzureCredential` over secrets
2. **Enable soft-delete** â€” Protect against accidental deletion
3. **Use HSM-backed keys** â€” Set `HardwareProtected = true` for sensitive keys
4. **Implement key rotation** â€” Use automatic rotation policies
5. **Limit key operations** â€” Only enable required `KeyOperations`
6. **Set expiration dates** â€” Always set `ExpiresOn` for keys
7. **Use specific versions** â€” Pin to versions in production
8. **Cache CryptographyClient** â€” Reuse for multiple operations
