...
description: "Use when working with vss-search-archive"
---
...
Search video archives by natural language using Cosmos Embed1 embeddings. Requires the search profile â€” deploy with the `vss-deploy-profile` skill (`-p search`). These videos sources can be ingested files or RTSP streams.
...
1. Probe the stack:
   ...
   ```
   (The second check confirms Elasticsearch is up â€” unique to the search profile.)
...
2. **If the probe fails**, ask the user:
   ...
   - If yes â†’ hand off to the `/vss-deploy-profile` skill. Return here once it succeeds.
   - If no â†’ stop. Do not run this skill against a missing or wrong-profile stack.

   (If your caller has granted explicit pre-authorization to deploy
   autonomously â€” e.g. the request says "pre-authorized to deploy
   prerequisites", or you are running in a non-interactive evaluation
   harness with that permission â€” skip the confirmation and invoke
...
# 3. Tell the agent the upload finished â€” this fans out to RTVI-CV + RTVI-embed
...
The response shape is `{status, message, error}` â€” no `sensorId` (the agent keys the stream by the `name` you provided). On any step's failure earlier steps roll back. The `start_embedding_generation` step is fire-and-verify: a 2xx confirms the request was accepted and the embedding pipeline is running in the background, **not** that the stream is searchable yet. Search hits will start appearing only after enough chunks land in Elasticsearch â€” poll with a low-`top_k` query a few seconds in if you need a readiness signal.

### Delete source â€” agent-backed cleanup
...
1. **Ingest** â€” Files come in through the agent's three-step universal flow; RTSP streams through `/api/v1/rtsp-streams/add`. Both routes hand the source to RTVI-CV (attribute detection) and RTVI-Embed (Cosmos Embed1) which generates vector embeddings for video segments.
2. **Index** â€” Embeddings are stored in Elasticsearch via the Kafka pipeline.
3. **Query** â€” Natural-language queries are embedded and matched against stored vectors by similarity.
4. **Results** â€” Timestamped video segments ranked by relevance, with clip playback links.
...
1. **Resolve inputs from user instructions â€” HARD STOP if `$HOST_IP`
   is not explicitly provided.** See Â§ Input resolution below. Do NOT
...
2. **Resolve the source â€” HARD STOP before any `/generate` call.**
   ...
   Then:
   - **If the named source (or a clearly substring-matching name) IS in the list** â†’ proceed to step 3. Forward the user's natural-language query verbatim â€” the agent's own search tool decomposer (`services/agent/src/vss_agents/tools/search.py`) extracts `video_sources` from the prose given the available sources, so the skill does NOT need to construct a structured `video sources` payload.
   - **If the named source is NOT in the list** â†’ STOP. Do NOT fire `/generate` as a probe. Respond to the user with the registered source names and ask whether they meant one of those, want to ingest the missing source (point them at *Ingestion prerequisite* and run the matching file or RTSP recipe through the **agent backend**, not bare VIOS), or want to abandon the query. Wait for clarification.
   - **If the query names no specific source** ("find forklifts in the ingested videos", "search across all sources") â†’ skip the substring check, but `sensor/list` must still return non-empty (otherwise no sources are ingested â†’ HARD STOP).
...
4. Present the results to the user query. Format response as a professional inspection report but name it `Video Search Results`:
   ...
   - If criteria results are non-null, then in addition to a column "Critic result" ("confirmed" | "rejected" | "skipped"), include a column "Criteria" with all the criteria for this search result ({criteria_n}: âœ“ | âœ—)
...
Pick and choose some of these tuning options. Adjust them as needed for the userâ€™s situation and query. 
