...
description: "Use when working with vss-manage-alerts"
---

---
name: vss-manage-alerts
description: Use for VSS alert workflows â€” real-time monitoring, Alert-Bridge subscriptions, Slack notifications, incident queries, camera onboarding. Not for non-alert analytics.
license: Apache-2.0
metadata:
  version: "3.2.0"
...
- **Connection refused** â†’ microservice not running: probe `/docs` or `/health`, redeploy via `vss-deploy-profile`.
- **HTTP 401/403 on NGC pulls** â†’ missing/expired `NGC_CLI_API_KEY`: `docker login nvcr.io` and re-export the key.
- **OOM / model load failure** â†’ insufficient GPU memory: use a smaller variant or `docker compose down` to free GPUs.

# VSS Alert Management

The alerts profile runs in one of two modes (chosen at `/vss-deploy-profile -p alerts -m {verification,real-time}`) â€” see **The Two Modes** table below. This skill routes by **deployed mode + user intent** (monitoring vs subscription CRUD vs Slack webhook).
...
If the probe fails, ask which mode to deploy and hand off to `/vss-deploy-profile -p alerts -m <mode>` (decline â†’ stop; pre-authorized autonomous deploy â†’ run directly with `verification` by default). If it passes, detect the mode per Step 1.
...
**Switching modes** uses the `vss-deploy-profile` teardown + deploy flow with the other `-m` flag (VLM â†’ CV adds the CV pipeline; CV â†’ VLM tears it down). `rtvi-vlm` runs in both modes.

---

## Step 1 â€” Detect the Currently Deployed Mode

Before running any alert workflow, check which mode is live. Use **CV-only** containers as the signal â€” `vss-rtvi-vlm` is **not** a reliable mode signal because it runs in both modes.
...
If `vss-behavior-analytics` is present â†’ **CV mode** (which also has `vss-rtvi-vlm`).
If only `vss-rtvi-vlm` is present (and no CV pipeline) â†’ **VLM mode**.
If neither matches, the alerts profile is not deployed â€” direct the user to the `vss-deploy-profile` skill.
...
# MODE=2d_cv   â†’ CV mode (full superset)
# MODE=2d_vlm  â†’ VLM real-time mode (vss-rtvi-vlm only; no vss-rtvi-cv)
```

---

## Step 2 â€” Route by Deployed Mode

| Deployed mode | User asks aboutâ€¦ | Action |
|---|---|---|
| **VLM real-time** | Slack webhook setup/status/test/stop | **Workflow E** â€” `references/alert-notify.md` |
| **VLM real-time** | rule CRUD, or a realtime alert on a sensor with a detection condition, or stop/delete a named alert (by `alert_type`/condition or rule ID) | **Workflow D** â€” `references/alert-subscriptions.md` (incl. two-step stop/confirm) |
| **CV verification** | subscription/rule CRUD or Slack/notification setup | Refuse â€” see canonical refusal text below |
| **CV or VLM** | generic start/stop monitoring **without** a detection condition | **Workflow B (VLM)** â€” call the VSS Agent; `rtvi-vlm` runs in both modes |
| **CV or VLM** | incident lookup / *what happened* (recent alerts, time-range, casual "any alerts today?") | **Workflow C (Query)** â€” works on both; **always run the query, never answer from memory** |
| **CV** | static CV alert onboarding / verdict-prompt customization | **Workflow A (CV)** â€” onboard RTSP via `vss-manage-video-io-storage`; pipeline auto-picks it up |
| **VLM** | a CV / behavior-analytics / PPE-rule alert needing the static CV pipeline | **Redeployment required** â€” confirm first, then `vss-deploy-profile -m verification` |
...
1. **Workflow E (Slack)** â€” Slack-specific keywords (`slack`, `webhook` + `slack`, `bot token`, `slack channel`). `notify` alone is **not** sufficient.
2. **Workflow D (Subscriptions)** â€” sensor **plus** a detection condition, rule CRUD keywords (`rule`, `subscription`, rule ID), **or stopping/deleting a named alert by type/condition** ("stop the PPE alert", "delete the collision rule"). A named `alert_type`/condition = an existing **rule** â†’ D's two-step stop protocol (`GET /api/v1/realtime` â†’ yes/no confirm â†’ delete), never Workflow B.
3. **Workflow B (VLM monitoring)** â€” generic start/stop on a sensor with **no** detection condition and **no** alert-type qualifier ("start/stop real-time alert for sensor X"). A stop that names a type ("stop the **PPE** alert") is a rule stop â†’ Workflow D.
4. **Workflow C (Query)** â€” incident lookup / *what happened* (`show/list incidents`, `recent alerts`, time-range queries, **and casual "any alertsâ€¦?" / "any alerts so far today?" / "what's been triggered?" phrasings**). Bare `alerts` (without `rule`/`subscription`/`active rules`) means **incidents** â†’ Workflow C, never Workflow D.
5. **Workflow A (CV)** â€” CV deployment handling for anything not matched above.

> **`alerts` vs `alert rules` (C vs D) â€” pick exactly one, never both:**
> *what happened / has been triggered* (incidents) â†’ **Workflow C**
> (`POST /generate`). *What
> rules/subscriptions are configured or active* â†’ **Workflow D** (the
> **bare** `GET /api/v1/realtime`, no `/incidents`). Bare `alerts` =
> incidents (C); `alert rules` / `subscriptions` / `active rules` =
> inventory (D). Never answer from memory; run the one correct call â€”
...
**Stop routing (B vs D):** "Stop the **&lt;type&gt;** alert" (names an `alert_type`/condition like PPE, collision, fire) = stop a **subscription rule** â†’ **Workflow D** (find via `GET /api/v1/realtime`, then the two-step stop/confirm protocol in `references/alert-subscriptions.md`; do **not** call `POST /generate`). A bare "stop real-time alert / stop monitoring on &lt;sensor&gt;" with **no** type qualifier = Workflow B.
...
- RTSP URL / IP camera â†’ add it with `POST /sensor/add` (that skill's Section 6); record the `sensorId` / name.
- Named existing sensor â†’ confirm it appears in `GET /sensor/list` before proceeding.
...
Do not call the `rtvi-vlm` microservice endpoints directly â€” always go through the agent. The agent internally dispatches to `rtvi_vlm_alert`, `rtvi_prompt_gen`, and `video_analytics_mcp.get_incidents`.

---

## Workflow A â€” CV Mode (`-m verification` / `MODE=2d_cv`)

CV alerts are **deployment-driven, not request-driven** â€” there is no agent
call to "create" one.

1. Check if the sensor is in VIOS via `vss-manage-video-io-storage`'s `GET /sensor/list` (idempotent â€” don't blindly `POST /sensor/add`).
...
4. Alerts land in Elasticsearch (Behavior Analytics â†’ `alert-bridge` verification per `alert_type_config.json`). Query with **Workflow C**.

A static-CV-pipeline alert on a VLM-only deployment is a mode mismatch â€” see the routing table above.

---

## Workflow B â€” VLM Real-time Monitoring (CV or VLM mode)
...
Under the hood: `rtvi_prompt_gen` â†’ `rtvi_vlm_alert action="start"`.
...
deployment is a mode mismatch â€” see the routing table.

---

## Workflow D â€” Alert Subscriptions (VLM real-time mode only)
...
**Not here:** generic start/stop without a condition (â†’ Workflow B) or Slack
operations (â†’ Workflow E).
...
## Workflow E â€” Slack Notifications (VLM real-time mode only)
...
> **`alert-notify` (port 9090) â‰  `vss-alert-bridge` (`/api/v1/realtime`).**
> Do NOT touch `vss-alert-bridge` for Slack ops.

Routes here: "Set up Slack notifications", "Check if alert-notify is running", "Send a test alert to Slack". Does **not** route here: "Notify me when someone enters the zone" (â†’ D/B), "Alert and notify on my phone" (ambiguous â€” ask).

Load and follow `references/alert-notify.md`. Code lives in `scripts/alert-notify/`. VLM real-time mode only.

---

## Workflow C â€” Query / List Alerts (works on either mode)
...
natural-language requests to `$AGENT/generate` â€” "Show me recent alerts
...
detected lately?" are incident queries â€” issue a `POST /generate` (e.g.
...
> is wrong for "what happened" â€” never call/probe it or load the Workflow
...
> found / the count is 0** for the requested period and STOP â€” do not fall
...
| Task | Skill |
|---|---|
| Deploy, redeploy, or switch alert mode | **`vss-deploy-profile`** â€” `-p alerts -m {verification,real-time}` |
...
- **`alert-notify` (port 9090) â‰  `vss-alert-bridge`.** Slack ops â†’ Workflow E (`alert-notify`); never route Slack to `vss-alert-bridge`'s `/api/v1/realtime`.
...
- **Don't use `vss-rtvi-vlm` as a mode signal** â€” it runs in both modes. Use `vss-behavior-analytics` (CV-only) or the `MODE` env var.
- **A mode switch tears down the current deployment** â€” running VLM streams and un-persisted CV alert state are lost.
- **Always go through `$AGENT/generate`** â€” never call `rtvi-vlm` directly. The VLM trigger is a `"yes"`/`"true"` token match (case-insensitive); `rtvi_prompt_gen` enforces the Yes/No pattern, so don't hand-craft prompts that break it.
