...
description: "Use when working with minimax-shader-dev"
---

---
name: shader-dev
description: Comprehensive GLSL shader techniques for creating stunning visual effects â€” ray marching, SDF modeling, fluid simulation, particle systems, procedural generation, lighting, post-processing, and more.
license: MIT
metadata:
  version: "1.0"
  category: graphics
...
```
shader-dev/
â”œâ”€â”€ SKILL.md                      # Core skill (this file)
â”œâ”€â”€ techniques/                   # Implementation guides (read per routing table)
â”‚   â”œâ”€â”€ ray-marching.md           # Sphere tracing with SDF
â”‚   â”œâ”€â”€ sdf-3d.md                 # 3D signed distance functions
â”‚   â”œâ”€â”€ lighting-model.md         # PBR, Phong, toon shading
â”‚   â”œâ”€â”€ procedural-noise.md       # Perlin, Simplex, FBM
â”‚   â””â”€â”€ ...                       # 34 more technique files
â””â”€â”€ reference/                    # Detailed guides (read as needed)
    â”œâ”€â”€ ray-marching.md           # Math derivations & advanced patterns
    â”œâ”€â”€ sdf-3d.md                 # Extended SDF theory
    â”œâ”€â”€ lighting-model.md         # Lighting math deep-dive
    â”œâ”€â”€ procedural-noise.md       # Noise function theory
    â””â”€â”€ ...                       # 34 more reference files
```

## How to Use

1. Read the **Technique Routing Table** below to identify which technique(s) match the user's request
2. Read the relevant file(s) from `techniques/` â€” each file contains core principles, implementation steps, and complete code templates
...
| Color grading / palettes | [color-palette](techniques/color-palette.md) | â€” |
| Bloom / tone mapping / glitch | [post-processing](techniques/post-processing.md) | multipass-buffer |
| Multi-pass ping-pong buffers | [multipass-buffer](techniques/multipass-buffer.md) | â€” |
| Texture / sampling techniques | [texture-sampling](techniques/texture-sampling.md) | â€” |
| Camera / matrix transforms | [matrix-transform](techniques/matrix-transform.md) | â€” |
| Surface normals | [normal-estimation](techniques/normal-estimation.md) | â€” |
...
| Procedural audio / music | [sound-synthesis](techniques/sound-synthesis.md) | â€” |
...
| WebGL2 shader errors / debugging | [webgl-pitfalls](techniques/webgl-pitfalls.md) | â€” |

## Technique Index

### Geometry & SDF
- **sdf-2d** â€” 2D signed distance functions for shapes, UI, anti-aliased rendering
- **sdf-3d** â€” 3D signed distance functions for real-time implicit surface modeling
- **csg-boolean-operations** â€” Constructive solid geometry: union, subtraction, intersection with smooth blending
- **domain-repetition** â€” Infinite space repetition, folding, and limited tiling
- **domain-warping** â€” Distort domains with noise for organic, flowing shapes
- **sdf-tricks** â€” SDF optimization, bounding volumes, binary search refinement, hollowing, layered edges, debug visualization

### Ray Casting & Lighting
- **ray-marching** â€” Sphere tracing with SDF for 3D scene rendering
- **analytic-ray-tracing** â€” Closed-form ray-primitive intersections (sphere, plane, box, torus)
- **path-tracing-gi** â€” Monte Carlo path tracing for photorealistic global illumination
- **lighting-model** â€” Phong, Blinn-Phong, PBR (Cook-Torrance), and toon shading
- **shadow-techniques** â€” Hard shadows, soft shadows (penumbra estimation), cascade shadows
- **ambient-occlusion** â€” SDF-based AO, screen-space AO approximation
- **normal-estimation** â€” Finite-difference normals, tetrahedron technique

### Simulation & Physics
- **fluid-simulation** â€” Navier-Stokes fluid solver with advection, diffusion, pressure projection
- **simulation-physics** â€” GPU-based physics: springs, cloth, N-body gravity, collision
- **particle-system** â€” Stateless and stateful particle systems (fire, rain, sparks, galaxies)
- **cellular-automata** â€” Game of Life, reaction-diffusion (Turing patterns), sand simulation

### Natural Phenomena
- **water-ocean** â€” Gerstner waves, FFT ocean, caustics, underwater fog
- **terrain-rendering** â€” Heightfield ray marching, FBM terrain, erosion
- **atmospheric-scattering** â€” Rayleigh/Mie scattering, god rays, SSS approximation
- **volumetric-rendering** â€” Volume ray marching for clouds, fog, fire, explosions

### Procedural Generation
- **procedural-noise** â€” Value noise, Perlin, Simplex, Worley, FBM, ridged noise
- **procedural-2d-pattern** â€” Brick, hexagon, truchet, Islamic geometric patterns
- **voronoi-cellular-noise** â€” Voronoi diagrams, Worley noise, cracked earth, crystal
- **fractal-rendering** â€” Mandelbrot, Julia sets, 3D fractals (Mandelbox, Mandelbulb)
- **color-palette** â€” Cosine palettes, HSL/HSV/Oklab, dynamic color mapping

### Post-Processing & Infrastructure
- **post-processing** â€” Bloom, tone mapping (ACES, Reinhard), vignette, chromatic aberration, glitch
- **multipass-buffer** â€” Ping-pong FBO setup, state persistence across frames
- **texture-sampling** â€” Bilinear, bicubic, mipmap, procedural texture lookup
- **matrix-transform** â€” Camera look-at, projection, rotation, orbit controls
- **polar-uv-manipulation** â€” Polar/log-polar coordinates, kaleidoscope, spiral mapping
- **anti-aliasing** â€” SSAA, SDF analytical AA, temporal anti-aliasing (TAA), FXAA post-process
- **camera-effects** â€” Depth of field (thin lens), motion blur, lens distortion, film grain, vignette
- **texture-mapping-advanced** â€” Biplanar mapping, texture repetition avoidance, ray differential filtering

### Audio
- **sound-synthesis** â€” Procedural audio in GLSL: oscillators, envelopes, filters, FM synthesis

### Debugging & Validation
- **webgl-pitfalls** â€” Common WebGL2/GLSL errors: `fragCoord`, `main()` wrapper, function order, macro limitations, uniform null
...
- Vertex shader: `attribute` â†’ `in`, `varying` â†’ `out`
- Fragment shader: `varying` â†’ `in`, `gl_FragColor` â†’ `fragColor`, `texture2D()` â†’ `texture()`
...
- WebGL2 requires standard `void main()` entry point â€” always wrap mainImage:
```glsl
...
- GLSL requires functions to be declared before use â€” either declare before use or reorder:
```glsl
// WRONG â€” getAtmosphere() calls getSunDirection() before it's defined
vec3 getAtmosphere(vec3 dir) { return getSunDirection(); } // Error!
vec3 getSunDirection() { return normalize(vec3(1.0)); }

// CORRECT â€” define callee first
vec3 getSunDirection() { return normalize(vec3(1.0)); }
vec3 getAtmosphere(vec3 dir) { return getSunDirection(); } // Works
```

### Macro Limitations
- `#define` cannot use function calls â€” use `const` instead:
```glsl
// WRONG
...
- When extracting shader source from `<script>` tags, ensure `#version` is the **first character** â€” use `.trim()`:
```javascript
const fs = document.getElementById('fs').text.trim();
```

### Common Pitfalls
- **Unused uniforms**: Compiler may optimize away unused uniforms, causing `gl.getUniformLocation()` to return `null` â€” always use uniforms in a way the compiler cannot optimize out
- **Loop indices**: Use runtime constants in loops, not `#define` macros in some ES versions
- **Terrain functions**: Functions like `terrainM(vec2)` need XZ components â€” use `terrainM(pos.xz + offset)` not `terrainM(pos + offset)`
...
### JS Variable Declaration Order (TDZ â€” causes white screen crash)
...
Reason: `let`/`const` have a Temporal Dead Zone â€” referencing them before declaration throws `ReferenceError`, causing a white screen.
...
- **Strict type matching**: `vec3 x = 1.0` is illegal â€” use `vec3 x = vec3(1.0)`; cannot use `.z` to access a `vec2`
- **No ternary on structs**: ESSL does not allow ternary operator on struct types â€” use `if`/`else` instead

### Performance Budget

Deployment environments may use headless software rendering with limited GPU power. Stay within these limits:

- Ray marching main loop: â‰¤ 128 steps
- Volume sampling / lighting inner loops: â‰¤ 32 steps
- FBM octaves: â‰¤ 6 layers
- Total nested loop iterations per pixel: â‰¤ 1000 (exceeding this freezes the browser)

## Quick Recipes

Common effect combinations â€” complete rendering pipelines assembled from technique modules.
...
Visual debugging methods â€” temporarily replace your output to diagnose issues.
