...
description: "Use when working with shader-programming-glsl"
---
...
- âœ… **Do:** Use `mix()` for linear interpolation instead of manual math.
- âœ… **Do:** Use `step()` and `smoothstep()` for thresholding and soft edges (avoid `if` branches).
- âœ… **Do:** Pack data into vectors (`vec4`) to minimize memory access.
- âŒ **Don't:** Use heavy branching (`if-else`) inside loops if possible; it hurts GPU parallelism.
- âŒ **Don't:** Calculate constant values inside the shader; pre-calculate them on the CPU (uniforms).
