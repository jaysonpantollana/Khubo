---
name: linkedin-cli
description: "Use when working with linkedin-cli"
---
...
You have access to `linkedin` â€“ a CLI tool for LinkedIn automation. Use it to fetch profiles, search people and companies, send messages, manage connections, create posts, react, comment, and more.

Each command sends a request to Linked API, which runs a real cloud browser to perform the action on LinkedIn. Operations are **not instant** â€“ expect 30 seconds to several minutes depending on complexity.
...
Always respect LinkedInâ€™s terms of service, local regulations, and your organisationâ€™s compliance policies when using automation against real accounts.
...
Exit code 0 means the API call succeeded â€“ always check the `success` field for the action outcome. Non-zero exit codes indicate infrastructure errors:

| Exit Code | Meaning                                                                                     |
| --------- | ------------------------------------------------------------------------------------------- |
| 0         | Success (check `success` field â€“ action may have returned an error like "person not found") |
...
- `--posts` â€“ recent posts (with `--posts-limit N`, `--posts-since TIMESTAMP`)
- `--comments` â€“ recent comments (with `--comments-limit N`, `--comments-since TIMESTAMP`)
- `--reactions` â€“ recent reactions (with `--reactions-limit N`, `--reactions-since TIMESTAMP`)

Only request additional data when needed â€“ each flag increases execution time.
...
- `--employees` â€“ include employees
- `--dms` â€“ include decision makers
- `--posts` â€“ include company posts
...
- **Not instant.** A real browser navigates LinkedIn â€“ expect 30 seconds to several minutes per operation.
