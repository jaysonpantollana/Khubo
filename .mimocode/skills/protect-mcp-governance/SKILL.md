...
description: "Use when working with protect-mcp-governance"
---

---
name: protect-mcp-governance
description: "Agent governance skill for MCP tool calls â€” Cedar policy authoring, shadow-to-enforce rollout, and Ed25519 receipt verification."
risk: safe
source: community
...
```
Agent â†’ protect-mcp â†’ Cedar policy evaluation â†’ MCP Server
                â†“
...
1. **Shadow mode** (default) â€” logs decisions without blocking. Use this to observe what your policies would do before enforcing them.
2. **Enforce mode** â€” blocks tool calls that violate policy. Use after shadow-mode validation.
3. **Hooks mode** â€” integrates with Claude Code hooks for pre/post tool-call governance.

## Core Concepts

### Cedar Policies

Cedar is a policy language designed for authorization. Policies are evaluated locally via WASM â€” no network calls required.
...
```cedar
// Start permissive â€” allow everything in shadow mode
permit(
  principal,
  action == Action::"call_tool",
  resource
...
```bash
# Shadow mode is the default â€” logs decisions without blocking
...
- âœ… **Do:** Start in shadow mode and observe before enforcing
- âœ… **Do:** Use `policy_digest` to track which policy version produced each decision
- âœ… **Do:** Store receipts alongside your application logs for correlation
- âœ… **Do:** Pin the verifier version when integrating into CI (`@veritasacta/verify@0.2.5`)
- âŒ **Don't:** Skip shadow mode and go straight to enforce in production
- âŒ **Don't:** Trust `claimed_issuer_tier` without independent verification
- âŒ **Don't:** Treat a valid signature as proof the signer is trustworthy â€” it only proves the receipt has not been tampered with since signing
...
**Solution:** Check your Cedar policy ordering. Cedar evaluates `forbid` rules before `permit` rules â€” a broad `forbid` will override specific `permit` rules.
...
- `@security-auditor` â€” General security auditing and compliance
- `@security-audit` â€” Code vulnerability scanning
- `@mcp-development` â€” MCP server development patterns

## Additional Resources

- [protect-mcp on npm](https://www.npmjs.com/package/protect-mcp) â€” MIT licensed
- [Cedar Policy Language](https://www.cedarpolicy.com/) â€” AWS open-source policy engine
- [IETF Draft: Signed Receipts](https://datatracker.ietf.org/doc/draft-farley-acta-signed-receipts/) â€” Receipt format specification
- [@veritasacta/verify](https://www.npmjs.com/package/@veritasacta/verify) â€” Apache-2.0 verifier, works offline
