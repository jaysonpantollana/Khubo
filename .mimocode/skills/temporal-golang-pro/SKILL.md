...
description: "Use when working with temporal-golang-pro"
---
...
- âœ… **Do:** Always handle errors from `ExecuteActivity` and `client.Dial`.
- âœ… **Do:** Use `workflow.Go` and `workflow.Channel` for concurrency.
- âœ… **Do:** Sort map keys before iteration to maintain determinism.
- âœ… **Do:** Use `activity.RecordHeartbeat` for activities lasting > 1 minute.
- âœ… **Do:** Test logic compatibility using `replayer.ReplayWorkflowHistoryFromJSON`.
- âŒ **Don't:** Swallow errors with `_` or `log.Fatal` in production workers.
- âŒ **Don't:** Perform direct Network/Disk I/O inside a Workflow function.
- âŒ **Don't:** Rely on native `time.Now()` or `rand.Int()`.
- âŒ **Don't:** Apply this to simple cron jobs that don't require durability.
