---
name: hsb-test
description: "Use when working with hsb-test"
---
...
**Gate 2 â€” Present the phase plan, ask for the test document, and get confirmation.** Before taking any action:

1. Show the phase plan:

```
HSB Test â€” Phase Plan
...
2. **If the user has not provided a test document path or URL, STOP and ask for it â€” do not proceed to Phase 0 or any phase until the user provides it**: `Please provide the path or URL to your test document:`. If the user has already specified specific tests (e.g., "connectivity checks only"), state which phases will run and which will be skipped, and note that tests will be filtered to the user's platform/board/sensor configuration and classified as automatable vs. manual.

3. Ask explicitly: `Shall I proceed with Phase 0? [Y/n]` â€” do not start Phase 0 until the user confirms.
...
Then execute:
- Phase 1 (test plan intake and test list building â€” setup confirmation is skipped)
...
After completing each phase (Phases 0â€“3), **always prompt the user for confirmation** before starting the next phase.
...
- **Phase 4** (cleanup) is the final phase â€” it runs automatically after Phase 3 completes or after the user declines to run another test plan.
...
OPTIONS
  ...
  --y               Auto-approve all phase gates and skip interactive
                    debugging on test failures. Not recommended â€” a
...
Phase 1. Failed tests are logged but not interactively debugged â€”
...
2. **Test plan approval is skipped** â€” the generated test plan executes automatically.
3. **Failed tests do not pause** â€” failures are logged and testing continues to the next test case automatically.
4. **Inter-test prompts are skipped** â€” tests run back-to-back without confirmation.
5. **Default timeout applies** â€” 120 seconds per test, or `--timeout N` if specified.
