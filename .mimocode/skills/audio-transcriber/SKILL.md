...
description: "Use when working with audio-transcriber"
---
...
if [[ ! "$AUTO_INSTALL" =~ ^[Nn] ]]; then
    ...
    if [[ -f "$SKILL_DIR/scripts/install-requirements.sh" ]]; then
        ...
        echo "âŒ Installation script not found"
        echo ""
    ...
    if python3 -c "import faster_whisper" 2>/dev/null || python3 -c "import whisper" 2>/dev/null; then
        ...
        echo "âŒ Installation failed. Please install manually."
        exit 1
    fi
else
    echo ""
...
if [[ ! -f "$AUDIO_FILE" ]]; then
    echo "âŒ File not found: $AUDIO_FILE"
    exit 1
fi
```
...
if [[ ! " ${SUPPORTED_FORMATS[@]} " =~ " ${EXTENSION,,} " ]]; then
    ...
    if command -v ffmpeg &>/dev/null; then
        ...
        echo "âŒ Install ffmpeg to convert formats: brew install ffmpeg"
        exit 1
    fi
fi
```
...
1. **Display user's prompt:**
   ```
   ðŸ“ Prompt fornecido pelo usuÃ¡rio:
...
2. **Automatically improve with prompt-engineer (if available):**
   ```bash
   ðŸ”§ Melhorando prompt com prompt-engineer...
...
4. **Ask which to use:**
   ```bash
   ðŸ’¡ Usar versÃ£o melhorada? [s/n] (default: s):
   ```
...
def process_with_llm(transcript, prompt, cli_tool='claude'):
    ...
    with Progress(
        ...
        progress.add_task(
            description=f"ðŸ¤– Processando com {cli_tool}...",
            total=None
        )
...
ðŸ§¹ Removidos arquivos temporÃ¡rios: metadata.json, transcription.json
