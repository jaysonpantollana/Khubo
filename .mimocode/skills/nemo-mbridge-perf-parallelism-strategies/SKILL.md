...
description: "Use when working with nemo-mbridge-perf-parallelism-strategies"
---
...
parameters are active per token, TP can often stay at 1 or 2 â€” the active
...
stage these two meshes share the same set of GPUs â€” they overlap, they
...
| Config | Wrong (PPÂ·TPÂ·CPÂ·EPÂ·ETP) | Correct (PPÂ·max(TPÂ·CP, EPÂ·ETP)) |
|---|---|---|
...
| PP=2, TP=2, CP=1, EP=8, ETP=1 | 32 | **16** (2 Â· max(2, 8)) |
...
Example â€” TP=2, CP=1, EP=8, ETP=1, PP=1:

- **8 GPUs** (`min_gpus`): dense `DP = 8/2 = 4`, MoE `EDP = 8/8 = 1`
- **16 GPUs**: dense `DP = 8`, MoE `EDP = 2` â†’ 2Ã— global batch
- **32 GPUs**: dense `DP = 16`, MoE `EDP = 4` â†’ 4Ã— global batch
...
data_parallel_size = world_size / (tensor_model_parallel_size Ã— pipeline_model_parallel_size Ã— context_parallel_size)
```
