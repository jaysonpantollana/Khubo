...
description: "Use when working with kotlin-coroutines-expert"
---
...
- âœ… **Do:** Use `Dispatchers.IO` for blocking I/O operations.
- âœ… **Do:** Cancel scopes when they are no longer needed (e.g., `ViewModel.onCleared`).
- âœ… **Do:** Use `TestScope` and `runTest` for unit testing coroutines.
- âŒ **Don't:** Use `GlobalScope`. It breaks structured concurrency and can lead to leaks.
- âŒ **Don't:** Catch `CancellationException` unless you rethrow it.
