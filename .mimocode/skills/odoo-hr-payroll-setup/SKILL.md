...
description: "Use when working with odoo-hr-payroll-setup"
---
...
This skill guides HR managers and payroll accountants through setting up Odoo HR and Payroll correctly. It covers salary structure creation with Python-computed rules, time-off policies, employee contract types, and the payroll â†’ accounting journal posting flow.
...
```text
Menu: Payroll â†’ Configuration â†’ Salary Structures â†’ New

Name: US Employee Monthly
Payslip Code: MONTHLY

Rules (executed top-to-bottom â€” order matters):
...
```text
Menu: Time Off â†’ Configuration â†’ Time Off Types â†’ New
...
- âœ… **Do:** Install your country's **payroll localization** (`l10n_us_hr_payroll`, `l10n_mx_hr_payroll`, etc.) before building custom rules â€” it provides pre-configured tax structures.
- âœ… **Do:** Use **salary rule inputs** (`inputs.ALLOWANCE.amount`) to pass variable values (bonuses, allowances, withholding rates) rather than hardcoding them in the rule formula.
- âœ… **Do:** Archive old salary structures rather than deleting them â€” active payslips reference their structure and will break if the structure is deleted.
- âœ… **Do:** Always set an active **Employee Contract** with correct dates and salary before generating payslips.
- âŒ **Don't:** Manually edit posted payslips â€” cancel and regenerate the payslip batch if corrections are needed.
- âŒ **Don't:** Use `contract.wage` in deduction rules without verifying whether the structure is monthly or annual â€” always check the contract wage period.

## Limitations

- **Odoo Payroll is Enterprise-only** â€” the Community Edition does not include the Payroll module (`hr_payroll`).
