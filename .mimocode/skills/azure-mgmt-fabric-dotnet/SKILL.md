...
description: "Use when working with azure-mgmt-fabric-dotnet"
---
...
3. **Always use `DefaultAzureCredential`** â€” never hardcode credentials
4. **Handle `RequestFailedException`** for ARM API errors
5. **Use `CreateOrUpdateAsync`** for idempotent operations
6. **Suspend when not in use** â€” Fabric capacities bill for compute even when idle
7. **Check provisioning state** before performing operations on a capacity
8. **Use appropriate SKU** â€” Start small (F2/F4) for dev/test, scale up for production
...
1. **Capacity names must be globally unique** â€” Fabric capacity names must be unique across all Azure subscriptions
2. **Suspend doesn't delete** â€” Suspended capacities still exist but don't bill for compute
3. **SKU changes may require downtime** â€” Scaling operations can take several minutes
4. **Admin UPNs must be valid** â€” Capacity administrators must be valid Azure AD users
5. **Location constraints** â€” Not all SKUs are available in all regions; use `GetSkusFabricCapacitiesAsync` to check
6. **Long provisioning times** â€” Capacity creation can take 5-15 minutes
