...
description: "Use when working with spline-3d-integration"
---
...
Spline is a browser-based 3D design tool â€” think Figma, but for 3D. Designers create interactive 3D scenes (objects, materials, animations, physics, events) in the Spline editor, then export them for the web via a hosted `.splinecode` file URL.

---

## STEP 1 â€” Identify the Stack
...
## STEP 2 â€” Get the Scene URL

The user must go to their Spline editor â†’ **Export** â†’ **Code Export** â†’ copy the `prod.spline.design` URL:
...
- âœ… Toggle **Hide Background** ON if the site has a dark or custom background
- âœ… Toggle **Hide Spline Logo** ON if they have a paid plan
- âœ… Set **Geometry Quality** to Performance for faster load
- âœ… Disable **Page Scroll**, **Zoom**, **Pan** if those aren't needed (reduces hijacking risk)
- âœ… Click **Generate Draft** or **Promote to Production** after any settings change â€” the URL does NOT auto-update

---

## STEP 3 â€” Read the Relevant Guide

Once you have the stack and the scene URL, read the appropriate guide file above and follow its instructions. Always read COMMON_PROBLEMS.md before finishing integration â€” it contains critical gotchas that will otherwise only surface in production.
