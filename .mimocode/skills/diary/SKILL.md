---
name: diary
description: "Use when working with diary"
---
...
> ðŸš¨ **Agent One-Shot Integrity Constraint (Highest Priority)**: Steps 1-4 are an **indivisible atomic workflow**. The AI **MUST use Continuous Tool Calling** to complete all actions in one breath.
...
> âš ï¸ **Ironclad Rule 1**: The local project diary in Step 1 **must NEVER be polluted with global data**. It only records the current project.
> âš ï¸ **Ironclad Rule 2**: When using tools like `run_command` to execute predefined python scripts (Step 2, Step 4), you must set them to `SafeToAutoRun: true` to avoid getting stuck waiting for user authorization.
> âš ï¸ **Ironclad Rule 3**: **Never guess the project name**. The AI must confirm the current directory name as the project identifier using a terminal command BEFORE taking any action in Step 1.
...
- **Isolation and Naming Rules (Ironclad Rules)**:
  - ðŸ“„ **Mandatory Filename Suffix**: The local diary **MUST** include the project name detected just now. It is **absolutely forbidden** to use a global-level filename (like `2026-02-23.md`) locally.
  - âœ… **Pure Content**: Only record content exclusive to the current project. Do not mix in other projects.
  - ðŸ“ **Append Mode**: If the project diary already exists, update it using "append", never overwrite the original content.
  - ðŸ“ **Auto-Creation**: Create subfolders `diary/YYYY/MM/` based on the year and month.
  - âš¡ **Force Continue**: Once writing is complete, **do not interrupt the conversation; immediately call the terminal tool and proceed to Step 2.**
...
## Step 3: AI Smart Fusion & Global Archiving (AI Execution) ðŸ§ 
...
- **Context Firewall (Core Mechanism)**:
  ...
  2. **Priority Definition**: The content marked as `ðŸ“ [Current Project Latest Progress]` in Step 2 is the protagonist of today's diary.
- **Rewrite Rules**:
  ...
  2. **Precise Zoning**: Ensure there is a dedicated `### ðŸ“ ProjectName` zone for this project. Do not mix content into other project zones.
...
## Step 4: Cloud Sync & Experience Extraction (Script + Human) ðŸ›‘
...
- **Action 2 (Extraction & Forced Pause)**:
  ...
  2. Confirm if it contains entirely new key points lacking in the past (ðŸ“Œ New Rules), or better approaches (ðŸ”„ Evolved Rules).
...
---
**ðŸŽ¯ Task Acceptance Criteria**:
1. âœ… Project local diary generated (no pollution).
2. âœ… `fetch_diaries.py` called with absolute path and successfully printed materials.
3. âœ… AI executed high-quality rewrite and precisely wrote to global diary (appended successfully if file existed).
4. âœ… `--sync-only` successfully pushed to Notion + Obsidian.
5. âœ… Experience extraction presented to the user and authorized.

---

## ðŸ“ Templates and Writing Guidelines

Strictly apply the following Markdown templates to ensure clarity during Step 1 (Local) and Step 3 (Global Fusion).

### ðŸ’¡ Writing Guidelines (For AI)
1. **Dynamic Replacement**: The `{Project Name}` in the template MUST strictly use the folder name grabbed by `pwd` in Step 1.
2. **Concise Deduplication**: When writing the global diary in Step 3, the AI must condense the "ðŸ› ï¸ Execution Details" from the local diary. The global diary focuses only on "General Direction and Output Results."
...
### ðŸ“ Template 1: Project Local Diary (Step 1 Exclusive)

```markdown
# Project DevLog: {Project Name}
* **ðŸ“… Date**: YYYY-MM-DD
* **ðŸ·ï¸ Tags**: `#Project` `#DevLog`
...
### ðŸ› ï¸ Execution Details & Changes
* **Git Commits**: (List if any)
* **Core File Modifications**:
  * ðŸ“„ `path/filename`: Explanation of changes.
* **Technical Implementation**:
  * (Record key logic or architecture structural changes)

### ðŸš¨ Troubleshooting
> ðŸ› **Problem Encountered**: (e.g., API error, package conflict)
> ðŸ’¡ **Solution**: (Final fix, leave key commands)
...
### ðŸŒ Template 2: Global Diary (Step 3 Exclusive)

```markdown
# ðŸ“” YYYY-MM-DD Global Progress Overview

> ðŸŒŸ **Daily Highlight**
> (1-2 sentences summarizing all project progress for the day, synthesized by AI)

---

## ðŸ“ Project Tracking
(âš ï¸ AI Rule: If file exists, find the corresponding project title and append; NEVER overwrite, keep it clean.)

### ðŸ”µ {Project A, e.g., auto-video-editor}
* **Today's Progress**: (Condense Step 2 local materials into key points)
* **Action Items**: (Extract next steps)

### ðŸŸ¢ {Project B, e.g., GSS}
* **Today's Progress**: (Condense key points)
* **Action Items**: (Extract next steps)

---

## ðŸ§  Improvements & Learnings
(âš ï¸ Dedicated to Experience Extraction)

ðŸ“Œ **New Rules / Discoveries**
(e.g., Found hidden API limit, or a more efficient python syntax)

ðŸ”„ **Optimizations & Reflections**
(Improvements from past methods)

---

## âœ… Global Action Items
