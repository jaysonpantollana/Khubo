---
name: angular
description: "Use when working with angular"
---
...
- Migrating from AngularJS (1.x) â†’ use `angular-migration` skill
- Working with legacy Angular apps that cannot upgrade
- General TypeScript issues â†’ use `typescript-expert` skill
...
| ----------------------- | --------------- | -------------------------------- |
| Local component state   | âœ… Preferred    | Overkill                         |
| Derived/computed values | âœ… `computed()` | `combineLatest` works            |
| Side effects            | âœ… `effect()`   | `tap` operator                   |
| HTTP requests           | âŒ              | âœ… HttpClient returns Observable |
| Event streams           | âŒ              | âœ… `fromEvent`, operators        |
| Complex async flows     | âŒ              | âœ… `switchMap`, `mergeMap`       |
