---
name: deepstream-dev
description: "Use when working with deepstream-dev"
---
...
```
Source â†’ Stream Muxer â†’ Inference â†’ [Tracker] â†’ OSD â†’ Renderer
```
...
7. **Queue Types**:
   - `queue.Queue` â†’ Use with `threading.Thread`
   - `multiprocessing.Queue` â†’ Use with `multiprocessing.Process`
...
13. **Ultralytics YOLO Output Format Depends on Model Generation** â€” newer models (v10+/v26+) output post-NMS results; older models (v8/v11) output raw pre-NMS tensors. The custom parser and `cluster-mode` **must** match the actual output:

   | Model generation | Output tensor shape | Fields | `cluster-mode` |
   |------------------|--------------------|---------------------------------|----------------|
   | v8 / v11 | `[batch, 84, 8400]` | `[features(4+80), anchors]` â€” raw cx/cy/w/h + class scores, no NMS | `2` (NMS) |
   | v10 / v26+ | `[batch, 300, 6]` | `[max_det, (x1,y1,x2,y2,conf,cls)]` â€” already post-NMS, pixel coords | `4` (none) |

   **How to identify at runtime**: log `inferDims.d[0]` and `inferDims.d[1]` inside the custom parser.
   - `d={84, 8400}` â†’ pre-NMS (v8/v11 style)
   - `d={300, 6}` â†’ post-NMS (v10/v26+ style)

   **Symptom of mismatch**: If `cluster-mode: 2` is used with a post-NMS `[N, 6]` output, bounding boxes appear shifted by 45Â° or 135Â° from the actual objects (DeepStream's NMS incorrectly re-processes already-final coordinates).
