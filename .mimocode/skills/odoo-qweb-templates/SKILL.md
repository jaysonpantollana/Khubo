...
description: "Use when working with odoo-qweb-templates"
---
...
- âœ… **Do:** Use `t-field` for model fields â€” Odoo auto-formats dates, monetary values, and booleans correctly.
- âœ… **Do:** Use `t-out` (Odoo 15+) for safe HTML output of non-field strings. Use `t-esc` only on Odoo 14 and below (it HTML-escapes output).
- âœ… **Do:** Call `web.external_layout` for PDF reports to automatically include the company header, footer, and logo.
- âœ… **Do:** Use `_lt()` (lazy translation) for translatable string literals inside Python report helpers, not inline `t-esc`.
- âŒ **Don't:** Use raw Python expressions inside QWeb â€” compute values in the model or a report `_get_report_values()` helper.
- âŒ **Don't:** Forget `t-as` when using `t-foreach`; without it, you can't access the current record in the loop body.
- âŒ **Don't:** Use `t-esc` where you intend to render HTML content â€” it will escape the tags and print them as raw text.

## Limitations

- Does not cover **website controller routing** for dynamic QWeb pages â€” that requires Python `http.route` knowledge.
- **Email template** QWeb has different variable scope than report QWeb (`object` vs `docs`) â€” this skill primarily focuses on PDF reports.
