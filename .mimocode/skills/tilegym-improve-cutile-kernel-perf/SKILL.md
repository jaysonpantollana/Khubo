...
description: "Use when working with tilegym-improve-cutile-kernel-perf"
---
...
- Subject to the **core constraint**: Correctness shall not regress â€” every optimization MUST preserve numerical correctness. `latency (ms)` shall not regress > 2% compared to baseline.
...
For each iteration:
1. Run pytest benchmark: `python -m pytest ... --print-record` â†’ extract latency (ms)
...
3. Verify correctness â€” if fails, **revert immediately**. Common causes: `flush_to_zero`/`rounding_mode=APPROX` changed results, tile size OOB, `allow_tma=False` semantics, persistent loop bound error
