...
description: "Use when working with odoo-ecommerce-configurator"
---
...
```text
Menu: Website â†’ eCommerce â†’ Products â†’ Select Product
...
Fields to complete for a great product listing:
  ...
  Sales Price:        $299.00
  Website Description (website tab): 150â€“300 words of unique content

Publishing:
  Toggle "Published" in the top-right corner of the product form
  or via: Website â†’ Go to Website â†’ Toggle "Published" button

SEO (website tab â†’ SEO section):
  ...
  Meta Description: Discover the most comfortable ergonomic mesh office
                    chair, designed for all-day support...  (â‰¤160 chars)
...
```text
Menu: Website â†’ Configuration â†’ Payment Providers â†’ Stripe â†’ Configure
(or: Accounting â†’ Configuration â†’ Payment Providers â†’ Stripe)

State: Test  (use Test mode until fully validated, then switch to Enabled)

Credentials (from your Stripe Dashboard â†’ Developers â†’ API Keys):
...
Webhook:
  Add Odoo's webhook URL in Stripe Dashboard â†’ Webhooks
...
```text
Menu: Inventory â†’ Configuration â†’ Delivery Methods â†’ New

Name: Standard Shipping (3â€“5 business days)
Provider: Fixed Price
Delivery Product: [Shipping] Standard  (used for invoicing)

Pricing:
  Price: $9.99
  â˜‘ Free if order amount is above: $75.00
...
Publish to website:
  â˜‘ Published  (visible to customers at checkout)
```

### Example 4: Set Up Abandoned Cart Recovery

```text
Menu: Email Marketing â†’ Mailing Lists â†’ (create a list if needed)

For automated abandoned cart emails in Odoo 16/17:
Menu: Marketing â†’ Marketing Automation â†’ New Campaign
...
Actions:
  ...
  3. Wait 24 hours
  4. Send Email: "Last chance â€” items selling fast"
...
- âœ… **Do:** Use **Product Variants** (color, size) instead of duplicate products â€” cleaner catalog and shared inventory tracking.
- âœ… **Do:** Enable **HTTPS** (SSL certificate) via your hosting provider and set HSTS in Website â†’ Settings â†’ Security.
- âœ… **Do:** Set up **Abandoned Cart Recovery** using Marketing Automation or a scheduled email sequence.
- âœ… **Do:** Add a **Stripe webhook** so Odoo is notified of payment events in real time â€” without it, failed payments may not update correctly.
- âŒ **Don't:** Leave the payment provider in **Test mode** in production â€” no real charges will be processed.
- âŒ **Don't:** Publish products without an **Internal Reference (SKU)** â€” it breaks inventory tracking and order fulfillment.
- âŒ **Don't:** Use the same Stripe key for Test and Production environments â€” always rotate to live keys before going live.
...
- Does not cover **multi-website** configuration â€” running separate storefronts with different pricelists and languages requires Enterprise.
...
- Odoo eCommerce does not support **subscription billing** natively â€” that requires the Enterprise **Subscriptions** module.
