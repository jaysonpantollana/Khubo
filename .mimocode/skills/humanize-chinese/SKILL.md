...
description: "Use when working with humanize-chinese"
---

---
name: humanize-chinese
description: Detect and rewrite AI-like Chinese text with a practical workflow for scoring, humanization, academic AIGC reduction, and style conversion. Use when the user asks to åŽ»AIå‘³, é™AIGC, åŽ»é™¤AIç—•è¿¹, è®ºæ–‡é™é‡, çŸ¥ç½‘æ£€æµ‹, ç»´æ™®æ£€æµ‹, humanize chinese, detect AI text, or make Chinese text sound more natural.
category: content
risk: safe
source: community
tags:
...
## When to Use
- Use when the user says `åŽ»AIå‘³`, `é™AIGC`, `åŽ»é™¤AIç—•è¿¹`, `è®©æ–‡å­—æ›´è‡ªç„¶`, `æ”¹æˆäººè¯`, or `é™ä½ŽAIçŽ‡`
...
- mechanical connectors such as `ç»¼ä¸Šæ‰€è¿°`, `å€¼å¾—æ³¨æ„çš„æ˜¯`, `ç”±æ­¤å¯è§`
...
- `æœ¬æ–‡æ—¨åœ¨` -> `æœ¬æ–‡å°è¯•` or `æœ¬ç ”ç©¶å…³æ³¨`
- `å…·æœ‰é‡è¦æ„ä¹‰` -> `å€¼å¾—å…³æ³¨` or `æœ‰ä¸€å®šå‚è€ƒä»·å€¼`
- `ç ”ç©¶è¡¨æ˜Ž` -> `å‰äººç ”ç©¶å‘çŽ°` or `å·²æœ‰æ–‡çŒ®æ˜¾ç¤º`
