...
description: "Use when working with emblemai-crypto-wallet"
---
...
- `GET /balance/{chain}/{address}` â€” Check wallet balance
- `POST /swap` â€” Execute token swap
- `GET /portfolio/{address}` â€” Portfolio overview
- `GET /token/{chain}/{contract}` â€” Token information
- `POST /transfer` â€” Send tokens

## Key Behaviors

1. **Always confirm** before executing transactions â€” show the user what will happen
...
5. **Never expose private keys** â€” all signing happens server-side via vault
