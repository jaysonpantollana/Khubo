---
name: go-rod-master
description: "Use when working with go-rod-master"
---
...
- **Resource Cleanup:** Designed around Go's `defer` pattern â€” browsers and pages close automatically.
...
Rod manages three layers: **Browser â†’ Page â†’ Element**.
...
- **Remove `navigator.webdriver`** â€” the primary headless detection signal.
- **Spoof WebGL vendor/renderer** â€” presents real GPU info (e.g., "Intel Inc." / "Intel Iris OpenGL Engine") instead of headless markers like "Google SwiftShader".
- **Fix Chrome plugin array** â€” reports proper `PluginArray` type with realistic plugin count.
- **Patch permissions API** â€” returns `"prompt"` instead of bot-revealing values.
- **Set realistic languages** â€” reports `en-US,en` instead of empty arrays.
- **Fix broken image dimensions** â€” headless browsers report 0x0; stealth fixes this to 16x16.
...
**Using stealth.JS directly (advanced â€” for custom page creation):**
...
- `examples/basic_scrape.go` â€” Minimal scraping example
- `examples/stealth_page.go` â€” Anti-detection with go-rod/stealth
- `examples/request_hijacking.go` â€” Intercepting and modifying network requests
- `examples/concurrent_pages.go` â€” Page pool for concurrent scraping

---

## Best Practices

- âœ… **ALWAYS use `stealth.MustPage(browser)`** instead of `browser.MustPage()` for real-world sites.
- âœ… **ALWAYS `defer browser.MustClose()`** immediately after connecting.
- âœ… Use the error-returning API (not `Must*`) in production code.
- âœ… Set explicit timeouts with `.Timeout()` â€” never rely on defaults for production.
- âœ… Use `browser.MustIncognito().MustPage()` for isolated sessions.
- âœ… Use `PagePool` for concurrent scraping instead of spawning unlimited pages.
- âœ… Use `MustWaitStable()` before clicking elements that might be animating.
- âœ… Use `MustWaitRequestIdle()` after actions that trigger AJAX calls.
- âœ… Use `launcher.New().Headless(false).Devtools(true)` for debugging.
- âŒ **NEVER** use `time.Sleep()` for waiting â€” use Rod's built-in wait methods.
- âŒ **NEVER** create a new `Browser` per task â€” create one Browser, use multiple `Page` instances.
- âŒ **NEVER** use `browser.MustPage()` for production scraping â€” use `stealth.MustPage()`.
- âŒ **NEVER** ignore errors in production â€” always handle them explicitly.
- âŒ **NEVER** forget to defer-close browsers, pages, and hijack routers.

## Common Pitfalls

- **Problem:** Element not found even though it exists on the page.
  **Solution:** The element may be inside an iframe or shadow DOM. Use `page.MustSearch()` instead of `page.MustElement()` â€” it searches across all iframes and shadow DOMs.
...
- [Official Documentation](https://go-rod.github.io/) â€” Guides, tutorials, FAQ
- [Go API Reference](https://pkg.go.dev/github.com/go-rod/rod) â€” Complete type and method documentation
- [go-rod/stealth](https://github.com/go-rod/stealth) â€” Anti-bot detection plugin
- [Examples (source)](https://github.com/go-rod/rod/blob/main/examples_test.go) â€” Official example tests
- [Rod vs Chromedp Comparison](https://github.com/nichochar/go-rod.github.io/blob/main/lib/examples/compare-chromedp) â€” Migration reference
- [Chrome DevTools Protocol Docs](https://chromedevtools.github.io/devtools-protocol/) â€” Underlying protocol reference
- [Chrome CLI Flags Reference](https://peter.sh/experiments/chromium-command-line-switches) â€” Launcher flag documentation
- `references/api-reference.md` â€” Quick-reference cheat sheet
