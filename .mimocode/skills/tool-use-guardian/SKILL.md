...
description: "Use when working with tool-use-guardian"
---

---
name: tool-use-guardian
description: "FREE â€” Intelligent tool-call reliability wrapper. Monitors, retries, fixes, and learns from tool failures. Auto-recovers from truncated JSON, timeouts, rate limits, and mid-chain failures."
...
The reliability wrapper every AI agent needs. Monitors tool calls, auto-retries failures, fixes truncated responses, and learns which tools are unreliable â€” so you never lose your chain of thought.
...
For multi-step tool chains, Guardian maintains checkpoints. If step 4 of 7 fails, it resumes from step 4 â€” never restarts from scratch.
...
- âœ… Let Guardian wrap all external tool calls automatically
- âœ… Review Guardian's reliability reports to identify flaky tools
- âœ… Use checkpoint recovery for long chains
- âŒ Don't disable retry logic for rate-limited APIs
- âŒ Don't ignore repeated failure warnings
