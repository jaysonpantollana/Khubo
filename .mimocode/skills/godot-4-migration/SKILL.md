...
description: "Use when working with godot-4-migration"
---
...
- âœ… **Do:** Use `@export_range`, `@export_file`, etc., for better inspector UI.
- âœ… **Do:** Type all variables (`var x: int`) for performance gains in GDScript 2.0.
- âœ… **Do:** Use `super()` to call parent methods instead of `.function_name()`.
- âŒ **Don't:** Use string names for signals (`emit_signal("name")`) if you can use the signal object (`name.emit()`).
