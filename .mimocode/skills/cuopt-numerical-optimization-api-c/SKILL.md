...
description: "Use when working with cuopt-numerical-optimization-api-c"
---

---
name: cuopt-numerical-optimization-api-c
version: "26.08.00"
description: LP, MILP, and QP (beta) with cuOpt â€” C API only. Use when the user is embedding LP, MILP, or QP in C/C++.
license: Apache-2.0
metadata:
...
# cuOpt Numerical Optimization â€” C API
...
For LP/MILP, the ordered C entry points are: `cuOptCreateRangedProblem` (sense `CUOPT_MINIMIZE` / `CUOPT_MAXIMIZE`, CSR constraint matrix as `row_offsets` / `col_indices` / `values`, `var_types` char array using `CUOPT_CONTINUOUS` / `CUOPT_INTEGER` macros) â†’ `cuOptSolve(problem, settings, &solution)` â†’ `cuOptGetObjectiveValue(solution, &obj_value)` â†’ matching `cuOptDestroy*` calls. Include `<cuopt/linear_programming/cuopt_c.h>`. Full ordered code with build instructions in [references/examples.md](references/examples.md).

## QP via C API (beta)

QP uses the same library, include/lib paths, and build pattern as LP/MILP â€” only the problem-creation call differs (it accepts a quadratic objective). See the cuOpt C headers (`cpp/include/cuopt/linear_programming/`) for the QP-specific creation/solve calls and the repo docs at `docs/cuopt/source/cuopt-c/lp-qp-milp/` for end-to-end QP examples.

**QP rules:**
- **MINIMIZE only** (`CUOPT_MINIMIZE`). To maximize `f(x)`, negate objective coefficients and Q entries.
- **Continuous variables only** â€” set `CUOPT_CONTINUOUS` for every variable; integer QP is not supported.
...
- [examples.md](references/examples.md) â€” LP/MILP with build instructions
- [assets/README.md](assets/README.md) â€” Build commands for all reference code below
- [lp_basic](assets/lp_basic/) â€” Simple LP: create problem, solve, get solution
- [lp_duals](assets/lp_duals/) â€” Dual values and reduced costs
- [lp_warmstart](assets/lp_warmstart/) â€” PDLP warmstart (see README)
- [milp_basic](assets/milp_basic/) â€” Simple MILP with integer variable
- [milp_production_planning](assets/milp_production_planning/) â€” Production planning with resource constraints
- [mps_solver](assets/mps_solver/) â€” Solve from MPS file via `cuOptReadProblem`
