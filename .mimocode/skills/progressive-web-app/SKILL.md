...
description: "Use when working with progressive-web-app"
---
...
A Progressive Web App is a web application that uses modern browser capabilities to deliver a fast, reliable, and installable experience â€” even on unreliable networks. The three required pillars are:

1. **HTTPS** â€” Required in production for service workers to register (localhost is exempt for development).
2. **Web App Manifest** (`manifest.json`) â€” Makes the app installable and defines its appearance on device home screens.
3. **Service Worker** (`sw.js`) â€” A background script that intercepts network requests, manages caches, and enables offline functionality.
...
- Use when the user asks "can my website be installed like an app?" or "how do I make my site work offline?" â€” even if they don't use the word PWA.

## Deliverables Checklist

Every PWA implementation must include these files at minimum:

- [ ] `index.html` â€” Links manifest, registers service worker
- [ ] `manifest.json` â€” Full app metadata and icon set
- [ ] `sw.js` â€” Service worker with install, activate, and fetch handlers
- [ ] `app.js` â€” Main app logic with SW registration and install prompt handling
- [ ] `offline.html` â€” Fallback page shown when navigation fails offline (required â€” missing file will cause install to fail)
...
- `purpose: "maskable"` on icons enables adaptive icons on Android (safe zone matters â€” keep content in center 80%).
...
// â”€â”€â”€ Install Prompt (Add to Home Screen) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
let deferredPrompt;
const installBtn = document.getElementById('install-btn'); // may be null if omitted

// Capture the browser's install prompt â€” it fires before the browser's own UI
...
### Cache Versioning (critical â€” always increment on deploy)
...
self.addEventListener('fetch', (event) => {
  ...
  // Strategy A: Cache-First (for static assets â€” fast, tolerates stale)
  if (url.pathname.match(/\.(css|js|png|jpg|svg|woff2)$/)) {
    event.respondWith(cacheFirst(request));
    return;
  }

  // Strategy B: Network-First (for HTML pages â€” fresh, falls back to cache)
  if (request.headers.get('Accept')?.includes('text/html')) {
    event.respondWith(networkFirst(request));
    return;
  }

  // Strategy C: Stale-While-Revalidate (for API data â€” fast and eventually fresh)
...
// â”€â”€â”€ Strategy Implementations â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
### iOS / Safari Quirks
- Safari supports manifests and service workers but **does not support `beforeinstallprompt`** â€” users must install via the Share â†’ "Add to Home Screen" menu manually.
...
- Requests to external origins (e.g., CDN fonts, third-party APIs) return "opaque" responses that cannot be inspected. Cache them with caution â€” a failed opaque response still gets a `200` status.
...
```javascript
// With Workbox (via CDN for simplicity â€” use npm + bundler in production)
...
- [ ] `sw.js` registers without errors in DevTools â†’ Application â†’ Service Workers
...
- [ ] Lighthouse PWA audit passes (Chrome DevTools â†’ Lighthouse tab)
