...
description: "Use when working with variant-analysis"
---
...
**Example:** Bug found in `api/handlers/` â†’ only searching that directory â†’ missing variant in `utils/auth.py`
...
**Example:** Bug uses `isAuthenticated` check â†’ only searching for that exact term â†’ missing bugs using related properties like `isActive`, `isAdmin`, `isVerified`
...
**Example:** Original bug is "return allow when condition is false" â†’ only searching that pattern â†’ missing:
...
**Example:** Testing auth checks only with valid users â†’ missing bypass when `userId = null` matches `resourceOwnerId = null`
