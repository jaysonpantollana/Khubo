...
description: "Use when working with vss-generate-video-calibration"
---
...
- Read [`references/calibration-tail.md`](references/calibration-tail.md) when you need the reusable Python implementation of the verify â†’ calibrate â†’ poll â†’ results tail.
...
**Disambiguation rule:** if the user is asking to launch / deploy / set up AMC (no calibration verb) â†’ `deploy`. If they provide RTSP URLs â†’ `rtsp`. If they mention local files / a videos directory â†’ `videos`. If they ask to verify install or test the bundled sample â†’ `sample-dataset`. Combined intents (e.g. "launch AMC and calibrate my videos") â†’ walk `deploy` first, then the calibration mode. When ambiguous, ask via `AskUserQuestion`.
...
- Microservice reachable at `http://<HOST_IP>:${VSS_AUTO_CALIBRATION_PORT:-8010}/v1/ready` â†’ `{"code":0,...}`.
- Projects directory writable by the container user. If you didn't just deploy (so Step 5 of the deploy reference hasn't run), confirm the write test in [`references/deploy-auto-calibration-service.md` Â§ Step 5](references/deploy-auto-calibration-service.md#step-5--confirm-the-projects-directory-is-writable) â€” otherwise the first `create_project` returns `[Errno 13] Permission denied`.
...
The verify â†’ calibrate â†’ poll â†’ results sequence is identical regardless of input mode. After the mode-specific reference has uploaded videos / ingested RTSP clips / uploaded the bundled sample, run this tail. Use [`references/calibration-tail.md`](references/calibration-tail.md) for the shared Python snippet.

### Step A â€” Verify Project

```
POST /v1/verify_project/<project_id>
```

Response: `{"project_state": "READY"}` â€” must be `READY` before calibrating. If not READY, re-check that videos + alignment + layout are present (either via API or via UI manual alignment).

### Step B â€” Start Calibration

**Confirm the plan before calibrating.** Whether the settings file and detector were auto-detected or asked, present a short summary and confirm via `AskUserQuestion` before the `POST /calibrate`. The resolved values are the defaults, so confirming is one click â€” but the user can switch the detector or skip an auto-detected settings file. Summarize:

- **Detector** â€” `resnet` or `transformer` (the value to be sent).
- **Calibration settings** â€” the file being applied (path), or default parameters (with the option to tune them in the UI first â€” see below).
- **Optional overrides** â€” ground-truth zip and focal lengths, if any.
...
`detector_type` is a separate `/calibrate` parameter â€” **not** consumed by `/v1/config/<id>`. If the user provided a calibration settings file, parse it for `"detector"` / `"detector_type"` and use that value. If the file doesn't specify one, the default (`resnet`) is the value shown in the confirmation above â€” the user can switch it there before calibrating. If there's no settings file at all, ask the user via `AskUserQuestion`:

- `resnet` â€” default, fast.
- `transformer` â€” slower, better under heavy occlusion.
...
- **Proceed with the default parameters** â€” well-suited to typical warehouse scenes; recommended unless the user has specific tuning in mind.
- **Adjust parameters in the UI first** â€” open the project, go to Step 3: Parameters, change values, and click Save; then continue.

Wait for the user's choice â€” and, if they choose to tune, for them to confirm they've Saved â€” before calling `/calibrate`.

### Step C â€” Poll for Completion
...
| `COMPLETED` | Finished |
| `ERROR` | Failed â€” pull log via `GET /v1/amc/calibrate/<id>/log` |
...
Typical time: **10â€“60 min** (your-own videos), **10â€“30 min** (bundled sample).
...
Evaluation response includes `Average L2 distance(m)` and `Average reprojection error 0(px)`. Evaluation metrics are produced **only when a ground-truth `GT.zip` was uploaded** â€” a missing `evaluation_statistics` result is normal otherwise and is not the end of result reporting.
...
- **UI** â€” `http://<HOST_IP>:${VSS_AUTO_CALIBRATION_UI_PORT:-5000}`; open the project, then the Results page to view the overlay.
- **Overlay image on disk** â€” `${VSS_APPS_DIR}/services/auto-calibration/projects/project_<id>/output/multi_view_results/BA_output/results_ba_scaled_world/overlay_img_*.png` (single-camera projects use `output/single_view_results/cam_00/verification_map_overlay.png`).
- **Project files** â€” `${VSS_APPS_DIR}/services/auto-calibration/projects/project_<id>/`.

### Step E â€” VGGT Refinement
...
The file replaces what the user would otherwise tune in UI Step 3 (rectification, bundle-adjustment, evaluation knobs, detector, â€¦). After a successful POST, **also** parse the file for `"detector"` / `"detector_type"` â€” if it's `"resnet"` or `"transformer"`, use that value for the `/calibrate` call in Step B (detector is a separate API parameter, not consumed by `/config`).

Non-2xx is surfaced â€” do not silently fall back. Skip this call entirely if the user chose the UI-fallback path.

## UI Fallback Pattern

When alignment / layout files aren't on disk, direct the user to the appropriate AMC UI step:

- **Settings missing** â†’ "Open UI project `<project_id>`, go to **Step 3: Parameters**, tune via the settings dialog (or accept defaults), click Save." **Also**: before the `/calibrate` call, ask the user via `AskUserQuestion` whether to use the `resnet` or `transformer` detector â€” Step 3 doesn't cover detector choice.
- **Layout missing** â†’ "Open UI project `<project_id>`, go to **Step 2: Video Configuration**, upload `layout.png` only (do NOT re-upload videos â€” they're already attached via API/RTSP), click Save."
- **Alignment missing** â†’ "Open UI project `<project_id>`, go to **Step 4: Alignment**, either upload `alignment_data.json` or mark correspondence points on the layout, click Save."
...
```
project_<project_id>/
â”œâ”€â”€ manual_adjustment/
â”‚   â”œâ”€â”€ alignment_data.json
â”‚   â””â”€â”€ layout.png
â”œâ”€â”€ output/
â”‚   â”œâ”€â”€ single_view_results/cam_XX/
â”‚   â”‚   â”œâ”€â”€ camInfo_hyper_XX.yaml
â”‚   â”‚   â””â”€â”€ trajDump_Stream_0_3d.txt
â”‚   â”œâ”€â”€ multi_view_results/BA_output/results_ba/
â”‚   â”‚   â”œâ”€â”€ initial/camInfo_XX.yaml
â”‚   â”‚   â””â”€â”€ refined/camInfo_XX.yaml          # â† final calibration
â”‚   â””â”€â”€ multi_view_results/BA_output/results_ba_scaled_world/
â”‚       â””â”€â”€ overlay_img_XX.png               # â† visual overlay for review
...
| Calibration stuck `RUNNING` > 90 min | `GET /v1/amc/calibrate/<id>/log` â€” usually insufficient tracklets (scene too static). See "Custom Dataset" guidelines in root `README.md`. |
| Immediate `ERROR` state | Check video naming: must be `cam_00.mp4`, `cam_01.mp4`, â€¦ contiguous (videos mode) / camera_name labels (RTSP mode). |
| Low L2 but high reprojection | Provide explicit `focal_length` override during input upload (see videos / rtsp references). |
| VGGT `INIT`, never `READY` | VGGT model not loaded â€” see [`references/deploy-auto-calibration-service.md`](references/deploy-auto-calibration-service.md) Step 2. |
| Upload timeout | Large videos â€” bump `timeout=300` to e.g. `600` in the per-mode Python script. |
| Port scan finds no backend | Backend not running â€” walk [`references/deploy-auto-calibration-service.md`](references/deploy-auto-calibration-service.md) first. |

## For Downstream Skills â€” MV3DT Export
...
# Response: application/zip â€” mv3dt_output.zip containing transforms.yml
```
...
# Response: application/zip â€” vggt_mv3dt_output.zip
```
...
3. `GET /v1/result/{project_id}/mv3dt_result?result_type=amc` â€” save the ZIP locally.
4. If VGGT also ran, optionally fetch `?result_type=vggt` for the refined MV3DT.

## Related Skills

- [`vss-manage-video-io-storage`](../vss-manage-video-io-storage/SKILL.md) â€” VIOS API skill; only the `rtsp` calibration mode depends on VIOS being reachable.
