...
description: "Use when working with not-a-vibe-coder"
---
...
A skill that turns any project idea â€” no matter how vague â€” into 8 living planning
...
1. **User command > files > AI assumptions.** If the user says something that
   contradicts a file, the user wins â€” and the relevant file(s) should then be
   updated to reflect the new instruction.
2. **No silent additions.** Never add features, tech choices, pages, tables, or
   rules the user did not ask for or approve. If something seems missing, ask â€”
   don't assume. Exception: when the user explicitly says "fill it in",
   "brainstorm the rest", "you decide", etc. â€” see Phase 3.
...
4. **One file at a time, in order**, during initial planning â€” don't dump all 8
   files at once unless the user explicitly asks for that.
5. **Tracker.md is append-only progress tracking** â€” update it whenever work is
...
7. **Read before you write.** At the start of any session, if these files
   already exist in the project, read all 8 before doing anything else â€” they
   are your memory.
...
- If the user gives a fully detailed spec already, you can still create the
  files but populate them directly from what they said â€” skip redundant
  questions.

### Phase 1 â€” PRD.md first

This is the foundation. Everything else depends on it.

- Take whatever the user gave you (even just "restaurant app") and ask a small
  number of clarifying questions to flesh out the PRD â€” target audience, core
...
- The user can also choose to skip Q&A and just write directly into PRD.md
  themselves â€” if they say "I'll fill it in", create a skeleton PRD.md with
...
- Do not invent features. If the user's answer is vague, ask again or offer
  options â€” don't fill gaps with assumptions.
- Once the PRD feels solid, write PRD.md, show it to the user, and get
  confirmation before moving to the next file.

### Phase 2 â€” Remaining files, one by one (except Design.md)

In this order: TechSpec.md â†’ AppFlow.md â†’ Schema.md â†’ ImplementationPlan.md â†’
Rules.md â†’ Tracker.md â†’ Design.md (last, see Phase 2.5).
...
properly" â€” this means: make reasonable, justifiable choices consistent with
...
intent" â€” not "ask about every detail."

### Phase 2.5 â€” Design.md (always interactive)

Never write Design.md without asking the user:
- Overall style direction (e.g. minimal / modern / playful / corporate / retro /
  brutalist / glassmorphism / dark-first) â€” offer `ask_user_input_v0` choices
  if helpful.
- Color palette â€” either ask for specific colors/hex codes, or offer 2-3
...
### Phase 3 â€” Final review

- Once all 8 files are drafted, present a short summary of the whole plan and
  ask the user to review everything (especially Rules.md â€” ask if they want to
...
- Ambiguous feature request â†’ ask, don't assume.
- User explicitly says "you decide" / "brainstorm it" â†’ make a reasoned,
  PRD-consistent choice, document it, present for review â€” don't silently bake
  it in.
- Conflict between user's current message and a file â†’ user wins; then sync
  the file.
- Design.md â†’ always ask style + colors first, no exceptions.
- Any completed task â†’ update Tracker.md immediately.
- Mid-project pivot â†’ update all affected files proactively, summarize changes.
