...
description: "Use when working with hugging-face-evaluation"
---
...
## âš ï¸ CRITICAL: Check for Existing PRs Before Creating New Ones
...
1) `get-prs` â†’ check for existing open PRs first
2) `inspect-tables` â†’ find table numbers/columns  
3) `extract-readme --table N` â†’ prints YAML by default  
...
âš ï¸ **Important:** This approach is only possible on devices with `uv` installed and sufficient GPU memory.
...
- From file: `leaderboard|mmlu|0` â†’ Use: `leaderboard|mmlu|0` (or change to `5` for 5-shot)
- From file: `bigbench|abstract_narrative_understanding|0` â†’ Use: `bigbench|abstract_narrative_understanding|0`
- From file: `lighteval|wmt14:hi-en|0` â†’ Use: `lighteval|wmt14:hi-en|0`
...
- Compares token sets: `"OLMo-3-32B"` â†’ `{"olmo", "3", "32b"}` matches `"**Olmo 3 32B**"` or `"Olmo-3-32B`
