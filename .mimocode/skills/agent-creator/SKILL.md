---
name: agent-creator
description: "Use when working with agent-creator"
---
...
Follow these steps in order. Do NOT skip the interview â€” even a one-line
...
1. **Agent name** â€” What should this agent be called?
   - Guide: short, lowercase, hyphenated (e.g., `code-reviewer`, `sql-expert`, `test-writer`)

2. **Purpose** â€” What is this agent for? (even a single line is fine)
   - Example: "review code", "write SQL queries", "generate unit tests"

3. **Plugin placement** â€” Should this go into an existing plugin or a new one?
   - List the user's existing plugins from `<appDataDir>\config\plugins\`
   - Default: create a new plugin named `<agent-name>-plugin`

4. **Companion skill** â€” Should I also create a routing skill that auto-triggers
...
"for reviewing code" â€” your job is to expand that into a rich, detailed persona
...
â””â”€â”€ skills/                    (only if companion skill requested)
    â””â”€â”€ use-<agent-name>/
        â””â”€â”€ SKILL.md
```
...
<The full generated persona from Step 2. This is the agent's system prompt and identity. Write it in second person ("You are..."). Be specific and detailed â€” this is what makes the agent good at its job.>
...
- **Add personality**: "You are direct and concise" vs "You are thorough and explain your reasoning" â€” these produce very different agents
