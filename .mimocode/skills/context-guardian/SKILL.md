...
description: "Use when working with context-guardian"
---
...
aproxima do limite da janela. Essa compactacao e heuristica â€” ela resume mensagens
...
â”œâ”€â”€ references/
â”‚   â”œâ”€â”€ extraction-protocol.md        # Protocolo detalhado de extracao
â”‚   â””â”€â”€ verification-checklist.md     # Checklist de verificacao e redundancia
â””â”€â”€ scripts/
    â””â”€â”€ context_snapshot.py           # Script de snapshot automatico
```
...
context-guardian (PRE-compactacao)    context-agent (POS-sessao)
         â”‚                                    â”‚
         â”œâ”€â”€ Detecta contexto grande          â”œâ”€â”€ Salva resumo ao final
         â”œâ”€â”€ Extrai dados criticos            â”œâ”€â”€ Atualiza ACTIVE_CONTEXT.md
         â”œâ”€â”€ Verifica integridade             â”œâ”€â”€ Sincroniza MEMORY.md
         â”œâ”€â”€ Salva snapshot verificado        â”œâ”€â”€ Indexa busca FTS5
         â””â”€â”€ Gera briefing de transicao       â””â”€â”€ Arquiva sessoes antigas
```
...
**P0 â€” Perda Fatal (preservar com redundancia tripla)**
...
| Correcoes aplicadas | Bug, causa raiz, solucao exata, arquivos afetados | "instagram/db.py: SQL injection via f-string â†’ ? placeholders" |
...
| Erros encontrados | Mensagem exata, stack trace relevante, como resolveu | "TypeError at line 45 â†’ cast para int" |
| Comandos que funcionaram | Comando completo que produziu resultado correto | "python verify_zips.py â†’ 22/22 OK" |

**P1 â€” Perda Grave (preservar com verificacao)**
...
```
â–¡ Cada arquivo modificado tem: caminho, natureza da mudanca, motivo
â–¡ Cada bug corrigido tem: sintoma, causa raiz, solucao, arquivo
â–¡ Cada decisao tem: o que, por que, alternativas descartadas
â–¡ Cada tarefa pendente tem: descricao, prioridade, dependencias
â–¡ Cada padrao/convencao tem: regra, motivo, exemplos
â–¡ Nenhuma informacao de uma secao contradiz outra
â–¡ Referencias cruzadas estao consistentes (ex: "18 queries testadas" aparece em
  multiplos lugares com o mesmo numero)
â–¡ Caminhos de arquivo estao completos (absolutos, nao relativos)
```
...
**Camada 1 â€” Snapshot estruturado (arquivo .md)**
...
**Camada 2 â€” MEMORY.md atualizado**
...
1. [tarefa 1 â€” resultado]
2. [tarefa 2 â€” resultado]
...

## O Que Falta Fazer

1. [tarefa pendente â€” prioridade] [dependencia se houver]
2. ...
...
1. **30 segundos** â€” Escrever um mini-briefing com: tarefas pendentes, decisoes
   criticas, caminhos de arquivo modificados
2. **1 minuto** â€” Atualizar MEMORY.md com informacoes P0
3. **2 minutos** â€” Executar context-agent save
