...
description: "Use when working with minimax-react-native-dev"
---
...
metadata:
  ...
  category: mobile
  sources:
    - expo/skills by Expo (https://github.com/expo/skills) â€” Native UI, navigation, and animation patterns
...
- [references/navigation.md](references/navigation.md) â€” Expo Router: Stack, Tabs, NativeTabs (`headerLargeTitle`, `headerBackButtonDisplayMode`), links, modals, sheets, context menus
- [references/components.md](references/components.md) â€” FlashList patterns, `expo-image`, safe areas (`contentInsetAdjustmentBehavior`), native controls, blur/glass effects, storage
- [references/styling.md](references/styling.md) â€” StyleSheet, NativeWind/Tailwind, platform styles, theming, dark mode
- [references/animations.md](references/animations.md) â€” Reanimated 3: entering/exiting, shared values, gestures, scroll-driven
- [references/state-management.md](references/state-management.md) â€” Zustand (selectors, persist), Jotai (atoms, derived), React Query, Context
- [references/forms.md](references/forms.md) â€” React Hook Form + Zod: validation, multi-step, dynamic arrays
- [references/networking.md](references/networking.md) â€” fetch wrapper, React Query (optimistic updates), auth tokens, offline, API routes, webhooks
- [references/performance.md](references/performance.md) â€” Profiling workflow, FlashList + `memo`, bundle analysis, TTI, memory leaks, animation perf
- [references/testing.md](references/testing.md) â€” Jest, React Native Testing Library, E2E with Maestro
- [references/native-capabilities.md](references/native-capabilities.md) â€” Camera, location, permissions (`use*Permissions` hooks), haptics, notifications, biometrics
- [references/engineering.md](references/engineering.md) â€” Project layout (`components/ui/`, `stores/`, `services/`), path aliases, SDK upgrades, EAS build/submit, CI/CD, DOM components
...
**Animation rule**: only animate `transform` and `opacity` â€” GPU-composited, no layout thrash.

**Imports**: always import directly from source, not barrel files â€” avoids bundle bloat.

**Lists and images**: before using `FlatList` or RN `Image`, check the Component Preferences table above â€” `FlashList` and `expo-image` are almost always the right choice.
...
Flutter development â†’ see `flutter-dev` skill.
iOS native (UIKit/SwiftUI) â†’ see `ios-application-dev` skill.
Android native (Kotlin/Compose) â†’ see `android-native-dev` skill.
