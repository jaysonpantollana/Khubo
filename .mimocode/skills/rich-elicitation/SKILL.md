...
description: "Use when working with rich-elicitation"
---
...
This skill governs how Antigravity resolves task ambiguity before starting work. When a user's request has too many unanswered dimensions â€” each with several reasonable answers â€” Antigravity asks targeted clarifying questions across multiple rounds rather than silently picking defaults.
...
**If 2+ rows apply â†’ trigger this skill.**

### Step 2: Ask Round 1 Questions

Ask up to 3 questions using `ask_user_input_v0`. Group related questions in a single call. Lead with 1â€“2 sentences explaining why you're asking. Mark one option per question as **(Recommended)**.
...
|---|---|---|
| Round 1 | Blocking questions â€” shape the entire output | 3 |
| Round 2 | Follow-ups unlocked by Round 1 answers | 3 |
| Round 3 | Final details â€” use sparingly | 2 |

Transition between rounds naturally. Don't announce "Round 2" mechanically. Use phrasing like:
> "Got it â€” that helps a lot. One more thing before I start:"
...
### Example 1: Multi-Round â€” Investor Pitch Deck

**User:** "Help me create a presentation for my project."

**Round 1** â€” Three blocking questions:
...
Q3: How much content do you already have?
  ...
  - Full draft â€” just needs polish
  - Previous version to update
```

*User answers: Investors | Pitch | Rough notes â†’ Round 2 is warranted.*

**Round 2** â€” Unlocked by Round 1:

```
Framing: "Perfect â€” investor pitch it is. A couple more things:"
...
Q2: How long should the deck be?
  - Short and punchy, 8â€“10 slides (Recommended)
  - Standard, 12â€“15 slides
  - Comprehensive, 20+ slides
```

*No Round 3 needed â€” proceed.*

---

### Example 2: Single-Round â€” Business Email
...
- âœ… Always mark one option per question as **(Recommended)**
- âœ… Lead with a 1â€“2 sentence framing before the question widget
- âœ… Group up to 3 related questions in a single `ask_user_input_v0` call
- âœ… Re-evaluate after each round â€” stop as soon as you have enough context
- âœ… Use `single_select` for mutually exclusive choices, `multi_select` when combinations are valid
- âœ… State remaining assumptions explicitly before proceeding after Round 3
- âŒ Don't ask 6 separate question calls when 2 grouped calls would do
- âŒ Don't mark two options as Recommended in the same question
- âŒ Don't use vague option labels like "Other" or "It depends" without elaborating
- âŒ Don't mechanically label rounds in the UI ("Round 1:", "Round 2:")
- âŒ Don't run a follow-up round for minor details that have safe defaults

---

## Limitations

- This skill does not validate whether the user's answers are internally consistent â€” it trusts them as given.
- Round structure is a guideline, not a rigid contract; judgment is required on when to stop.
- Works best with `ask_user_input_v0` â€” in environments without that tool, question quality may degrade.
...
This skill is pure reasoning â€” it issues no shell commands, reads no files, makes no network requests, and mutates no state. Risk level is `none`.
...
- `@ask-user-questions` â€” Single-round elicitation with recommended options. Use that skill for simpler tasks; use rich-elicitation when answers to early questions open up new meaningful choices.
