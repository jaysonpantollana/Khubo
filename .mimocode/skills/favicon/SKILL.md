---
name: favicon
description: "Use when working with favicon"
---
...
3. **Rails `config/application.rb`** - Extract the module name (e.g., `module MyApp` â†’ "MyApp")
4. **Directory name** - Use the current working directory name as fallback

Convert the name to title case if needed (e.g., "my-app" â†’ "My App").
...
- If static files are in `public/` or `static/` and served from root â†’ use `/favicon.ico`
- If static files are in `src/assets/` â†’ use `/assets/favicon.ico`
- If static files are in the same directory as HTML â†’ use `./favicon.ico` or just `favicon.ico`
