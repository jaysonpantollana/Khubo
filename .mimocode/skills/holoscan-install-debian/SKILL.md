...
description: "Use when working with holoscan-install-debian"
---
...
- No Python bindings from apt â€” pair with `/holoscan-install-wheel` if the user needs Python.
- Ubuntu-only. Other distros must use the container or wheel install.
- Package variant must match the host CUDA driver (`holoscan-cuda-12` vs `holoscan-cuda-13`); wrong variant â†’ "CUDA driver version is insufficient".
...
Determine OS version and CUDA variant if not already known â€” run in parallel:
...
**CUDA variant rule â€” pick the apt package:**
...
# hello_world â€” expected: "Hello World!"
$HS/examples/hello_world/cpp/hello_world

# tensor_interop â€” expected: tensors doubling each pass, "Graph execution finished."
...
# video_replayer (10 frames, headless) â€” expected: Vulkan selects NVIDIA GPU, "Graph execution finished."
...
- **`python3 -c "import holoscan"` fails after apt install.** Expected â€” the Debian package has been C++ only since v3.0.0. Run `/holoscan-install-wheel` to add Python bindings.
...
- **`error while loading shared libraries: libholoscan_core.so`.** `LD_LIBRARY_PATH` is unset. Use the env snippet from Step 4 â€” `export LD_LIBRARY_PATH=/opt/nvidia/holoscan/lib`.
