...
description: "Use when working with vss-deploy-detection-tracking-2d"
---

---
name: vss-deploy-detection-tracking-2d
description: "Use this skill when the user wants to deploy, run, debug, tear down, or call the REST API of the RTVI-CV 2D detection / tracking microservice. Trigger when the user says things like 'deploy rtvi-cv', 'start warehouse 2d', 'add a stream', 'check rtvi-cv health', or 'stop the perception container'. Not for VLM, embedding, or analytics â€” use the matching vss-* skill."
license: Apache-2.0
metadata:
  version: "3.2.0"
...
Follow the routing tables and step-by-step workflows below. Each section that ends in *workflow*, *quick start*, or *flow* is intended to be executed top-to-bottom. Detailed reference material lives in `references/` and helper scripts live in `scripts/` â€” call them via `run_script` when the skill points to a script by name.
...
# RTVI-CV â€” Detection & Tracking (Unified Skill)

Unified skill for the **Real Time Video Intelligence CV (RTVI-CV)** microservice. Two action surfaces in one skill:

- **Deploy / operate / debug / tear down** the RTVI-CV container locally â†’ see [`references/deploy-vss-detection-tracking-2d.md`](references/deploy-vss-detection-tracking-2d.md)
- **Call the RTVI-CV REST API** (streams, health, metrics, embeddings) on a running instance â†’ see [`references/usage-vss-detection-tracking-2d.md`](references/usage-vss-detection-tracking-2d.md)

> **Service**: `rtvi-cv` (`metropolis_perception_app`)
> **Image**: `nvcr.io/<org>/<repo>:<tag>` â€” user-supplied at deploy time
> **REST port**: `9000` (`/api/v1` â€” `/live`, `/ready`, `/startup`, `/metrics`, `/stream/add`, `/stream/remove`, embeddings)
> **Hardware**: x86/aarch64 dGPU (T4, A100, L40, H100, B200, RTX), SBSA (Spark, Grace-Hopper), Jetson (Thor, Orin, Xavier)

---

## Action routing â€” pick once per invocation
...
| `stop rtvi-cv`, `tear down`, `kill the perception container`, `cleanup rtvicv-perception-docker` | **TEARDOWN** (handled by deploy doc â†’ "Mode Selection") | [`references/deploy-vss-detection-tracking-2d.md`](references/deploy-vss-detection-tracking-2d.md) + [`references/teardown-flow.md`](references/teardown-flow.md) |
...
**Selection rule:** match the user's phrasing against the table above and immediately load the corresponding reference file. Do not mix the flows â€” DEPLOY assumes no running container yet; API USAGE assumes the container is already running on `http://<host>:9000`.
...
â”œâ”€â”€ SKILL.md          # this file (routing + contracts)
â”œâ”€â”€ assets/           # data files (deploy-defaults.yml â€” single source of truth for tags / refs / paths / GPU)
â”œâ”€â”€ evals/            # Tier-3 eval manifests (deploy-evals.json, usage-evals.json)
â”œâ”€â”€ scripts/          # 23 bash + python helpers (see `scripts/` for the full inventory)
â””â”€â”€ references/       # workflow runbooks (deploy / api-usage / teardown / troubleshooting / â€¦)
```
...
All scripts are invoked from the skill root via `$SKILL_DIR/scripts/<name>` â€” paths inside the deploy reference doc are preserved verbatim and resolve correctly when the agent runs from skill root.

---

## Available Scripts

Helpers live in `scripts/` and are invoked from the skill root by name â€”
...
1. **Read this file first.** It only routes â€” it does not contain workflows.
2. **Match the user's intent** against the routing table above.
3. **Load exactly one reference doc** (DEPLOY or API USAGE). Don't preload both â€” each reference is large and contains its own full contract.
4. **Follow the loaded reference exactly.** The reference docs are the byte-for-byte preserved contracts from the predecessor skills `vss-deploy-detection-tracking-2d` (deploy/teardown/debug) and `rtvicv-api` (REST API) â€” every step ordering invariant, bash-batching rule, box-rendering rule, and `AskQuestion` contract is retained.
5. **For DEPLOY**, the reference doc enforces its own startup contract: one-line acknowledgement â†’ planning-tool call (`TodoWrite` array of 5 todos, OR 5 successive `TaskCreate` calls on newer Claude Code) â†’ Step 1 question. Do not narrate, do not pre-flight, and never print "loading TodoWrite/TaskCreate" or any deferred-tool resolution prose â€” the planning tool is loaded silently.

---

## Output contract â€” DEPLOY flow
...
1. **Render every step's exit in a fixed-width box** â€” Step 1 *Deploy
   ...
   *Apply configuration*, Step 5 *Plan* + *Results*. Not just the final
   ...
   Â§ "Universal box format" below). Per-step **content** rules (what
...
2. **After the Step 5 Results box, issue the Step 6 `AskUserQuestion`**
   from [`references/next-steps.md`](references/next-steps.md) Â§ "11.c"
   â€” never replace it with a free-form *Next steps* bullet list. The
...
3. **After the user picks a Step 6 bucket, issue the follow-up
   ...
   Â§ "11.d" â€” never substitute prose + ready-to-copy curl examples + a
   ...
   - **Manage streams** â†’ Add / Remove / List. **Remove builds its
     options dynamically from `/stream/get-stream-info`** â€” one option
     per active stream labelled `<camera_id> Â· <camera_url>` plus
     "Remove ALL" when `ACTIVE > 1` (full spec: Â§ "`remove_streams`
     sub-flow").
   - **Stop the deployment** â†’ Stop app / Stop container / Full teardown.
   - **Check metrics & FPS** â†’ no follow-up; run `collect_metrics.sh`
     directly after printing the `/api/v1/metrics` API box.
   - **Check liveness / readiness** â†’ no follow-up; probe all three
     health endpoints after printing their API boxes.
4. **Render the FULL per-step content, not an overview row** â€”
   ...
   where the agent collapses most often** â€” its canonical
   per-use-case key list lives in
   [`references/apply-config.md`](references/apply-config.md)
   Â§ "Per-use-case complete edit list", and the agent MUST emit one
   `âœ” [section] key=value  â€” annotation` row per key in that table for
   the active use case + settings. A section with 5 keys â†’ 5 rows; a
   section with 6 keys â†’ 6 rows. Never one overview row per section.

Forbidden (these are the shortcuts the agent falls back to under
pressure, and they break the user's UX):

- âŒ **Internal tool-loading narration.** Never print "I need to load
  TodoWrite (a deferred tool the skill calls for the task widget)",
  "Loading TaskCreateâ€¦", "Calling ToolSearch for the planning toolâ€¦",
  or any other text about resolving / loading / fetching deferred tools.
  The agent loads tools **silently**. The user only ever sees the `âœ”
  <pinned-values>` summary line followed by the widget â€” never any
  scaffolding around tool resolution.
- âŒ **Collapsing all 5 deploy steps into a single `TaskCreate`'s
  `description` field.** When `TaskCreate` is the available planning
  tool, issue **5 separate `TaskCreate` calls** back-to-back (one per
  step). See `references/task-list.md` Â§ "Initial `TaskCreate` calls"
  for the verbatim template. Same rule for `TodoWrite` â€” one call with
  all 5 todos in the `todos:[â€¦]` array; never one todo whose `content`
  is a multi-line list.
- âŒ **Silently choosing `dynamic` stream-mode.** The skill default is
  `stream_mode=static` â€” the agent bakes auto-discovered `file://` URLs
  ...
  Â§ "Defaults â€” the skill is static-mode by default" for the full
  rationale.
- âŒ A one-line `âœ” App ready in Ns, N streams, fps total Y` in place of
  the Step 5 Results box.
- âŒ ASCII box-drawing chars (`+`, `-`, `=`, `*`) instead of light
  box-drawing chars (`â”Œ â”€ â” â”‚ â”” â”˜`).
- âŒ Skipping Step 6 on the assumption "the user knows what to do next".
- âŒ After Step 6, dumping a markdown wall of prose + multiple curl
  blocks + a closing "want me to run any of these?" â€” that's the
...
- âŒ Step 4 overview collapses â€” these are explicitly banned by the
  deploy doc's Step 4 content rule:
    - `âœ” Batch size 3 (tile grid: 1Ã—3)` â†’ required: 5 separate rows
      (`[streammux] batch-size=3`, `[primary-gie] batch-size=3`,
      `[source-list] max-batch-size=3`, `[tiled-display] rows=1`,
      `[tiled-display] columns=3`).
    - `âœ” Output sink eglsink` â†’ required: one row per sink key
      (4 keys for eglsink, e.g. `[sink0] enable=1`, `type=2`,
      `sync=0`, `qos=0` â€” read apply-config.md for the exact list).
    - `âœ” Sources static (3 streams, http-port=9000)` â†’ required: six
      annotated `[source-list]` rows.
    - `âœ” Tile grid 1 row Ã— 3 cols` (single row) â†’ required: two
...
- **Width: 128 chars** corner-to-corner â€” `â”Œ` at column 1, `â”` at
  ...
  each side inside the `â”‚` borders).
- **Light box-drawing chars only**: `â”Œ â”€ â” â”‚ â”” â”˜`. No `+`, `-`, `=`,
  `*` ASCII fallbacks.
- **Top border â€” title CENTERED**: `â”Œ` + Nâ‚ dashes + `â£` + title + `â£`
  + Nâ‚‚ dashes + `â”`, where `Nâ‚ + Nâ‚‚ + len(title) + 2 = 126`. Distribute
  the pad: `Nâ‚ = floor((126 âˆ’ len(title) âˆ’ 2) / 2)`,
  `Nâ‚‚ = 126 âˆ’ len(title) âˆ’ 2 âˆ’ Nâ‚`. Nâ‚ and Nâ‚‚ differ by at most 1.
- **Body**: one `â”‚ <content padded to inner-content 124> â”‚` per fact.
  Each fact line uses the `  âœ” <key-padded-to-13>  <value>` form (two
  spaces in, glyph, key right-padded to 13, two spaces, value).
- **Blank lines between groups**: render `â”‚ <124 spaces> â”‚` between
  logical groups (e.g. Identity / Model / Videos in Step 1) so the
  user can scan the box at a glance.
- **Bottom border**: `â””` + 126 dashes + `â”˜` â€” solid border, no title.

Standard step titles (used at the top of each step's box):

```
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Deploy targets â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Pipeline configuration â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Container â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Apply configuration â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Perception Application â€” Plan â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Perception Application â€” Results â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
```
...
[`references/deploy-vss-detection-tracking-2d.md`](references/deploy-vss-detection-tracking-2d.md)
under "Step N box content rule" â€” read those when rendering the
corresponding step.
