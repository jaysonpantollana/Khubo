...
description: "Use when working with tilegym-adding-cutile-kernel"
---
...
2. Execute steps **in order** â€” do NOT skip ahead or combine steps
...
5. Each step MUST result in a file write or explicit skip decision â€” no silent omissions
...
Add a `@dispatch` function â€” this is the **single entry point** for all backends.
...
2. test_op / test_op_xxx â€” missing @pytest.mark.parametrize("backend", _backends), backend parameter, and tilegym.is_backend_available / tilegym.set_backend pattern
```
...
- Call the op via `tilegym.ops.my_op(a, b, ..., backend=backend)` â€” do **not** use `set_backend`.
