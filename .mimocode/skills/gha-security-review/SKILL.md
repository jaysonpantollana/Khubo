...
description: "Use when working with gha-security-review"
---

---
name: gha-security-review
description: "Find exploitable vulnerabilities in GitHub Actions workflows. Every finding MUST include a concrete exploitation scenario â€” if you can't build the attack, don't report it."
risk: safe
source: community
...
Find exploitable vulnerabilities in GitHub Actions workflows. Every finding MUST include a concrete exploitation scenario â€” if you can't build the attack, don't report it.

This skill encodes attack patterns from real GitHub Actions exploits â€” not generic CI/CD theory.
...
- `.github/workflows/*.yml` â€” all workflow definitions
- `action.yml` / `action.yaml` â€” composite actions in the repo
- `.github/actions/*/action.yml` â€” local reusable actions
...
Only report vulnerabilities exploitable by an **external attacker** â€” someone **without** write access to the repository. The attacker can open PRs from forks, create issues, and post comments. They cannot push to branches, trigger `workflow_dispatch`, or trigger manual workflows.

**Do not flag** vulnerabilities that require write access to exploit:
- `workflow_dispatch` input injection â€” requires write access to trigger
...
1. **Entry point** â€” How does the attacker get in? (fork PR, issue comment, branch name, etc.)
2. **Payload** â€” What does the attacker send? (actual code/YAML/input)
3. **Execution mechanism** â€” How does the payload run? (expression expansion, checkout + script, etc.)
4. **Impact** â€” What does the attacker gain? (token theft, code execution, repo write access)
5. **PoC sketch** â€” Concrete steps an attacker would follow
...
Load references selectively â€” only what's relevant to the triggers found.
...
- Confirm the value is attacker-controlled (PR title, branch name, comment body â€” not numeric IDs, SHAs, or repository names)
...
| `${{ github.event.pull_request.number }}` in `run:` | Numeric only â€” not injectable |
...
| Any expression in `workflow_dispatch`/`schedule`/`push` to protected branches | Requires write access â€” outside threat model |
...
1. **Read the full workflow** â€” don't rely on grep output alone
2. **Trace the trigger** â€” confirm the event and check `if:` conditions that gate execution
3. **Trace the expression/checkout** â€” confirm it's in a `run:` block or actually references fork code
4. **Confirm attacker control** â€” verify the value maps to something an external attacker sets
5. **Check existing mitigations** â€” env var wrapping, author_association checks, restricted permissions, SHA pinning
...
- **Confidence**: HIGH â€” confirmed through attack path tracing
