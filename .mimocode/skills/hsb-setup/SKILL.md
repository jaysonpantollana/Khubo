---
name: hsb-setup
description: "Use when working with hsb-setup"
---
...
HSB_PLATFORM    Platform hint â€” may be empty; will detect from hardware.
HSB_REPO        Custom repo URL â€” defaults to https://github.com/nvidia-holoscan/holoscan-sensor-bridge.git
```

**SSH_TARGET and REMOTE_ROOT are required. Stop and ask the user for them if either is missing.**

**Gate 2 â€” Present the phase plan.** Before taking any action, show the user this exact plan and wait for acknowledgement:

```
HSB Setup â€” Phase Plan
  ...
  Phase 3: Native CLI build (AGX Thor only â€” skipped for other platforms)
...
**Gate 3 â€” Token-budget preflight (Phase 0).** Run this before any SSH connection or devkit change. See `## Token-budget preflight` section for the full procedure. Do not proceed to Phase 1 until the budget check passes.
...
- **IGX Orin with dGPU OS/configuration** â†’ build with `sh docker/build.sh --dgpu`
- **IGX Orin iGPU** â†’ build with `sh docker/build.sh --igpu`
- **AGX Orin** â†’ build with `sh docker/build.sh --igpu`
- **AGX Thor** â†’ build with `sh docker/build.sh --igpu`
- **DGX Spark** â†’ build with `sh docker/build.sh --igpu`

If the user says only â€œIGX Orinâ€, explicitly ask whether it is **iGPU** or **dGPU OS/configuration**.
...
The following table maps known `/sys/class/dmi/id/product_name` values to supported `HSB_PLATFORM` values. Match using **case-insensitive substring** search â€” the product name may contain additional text (e.g., "Developer Kit", revision numbers).
...
1. **`DETECTED_PLATFORM` is non-empty and `HSB_PLATFORM` is empty** â†’ set `HSB_PLATFORM` to `DETECTED_PLATFORM`. Alert the user:
   ```
   Platform auto-detected from hardware: <DETECTED_PLATFORM> (product_name: <DETECTED_PRODUCT>).
   HSB_PLATFORM was not set â€” updating to "<DETECTED_PLATFORM>".
   ```

2. **`DETECTED_PLATFORM` is non-empty and differs from `HSB_PLATFORM`** â†’ override `HSB_PLATFORM` with `DETECTED_PLATFORM`. Alert the user:
   ```
...
3. **`DETECTED_PLATFORM` is non-empty and matches `HSB_PLATFORM`** â†’ no change needed. Confirm:
   ```
   Platform verified: <HSB_PLATFORM> matches hardware (product_name: <DETECTED_PRODUCT>).
   ```

4. **`DETECTED_PLATFORM` is empty** (file missing, unreadable, or unrecognized product name) **and `HSB_PLATFORM` is set** â†’ keep the existing `HSB_PLATFORM`. Warn:
   ```
   Could not auto-detect platform from hardware (product_name: "<DETECTED_PRODUCT>").
   Keeping existing HSB_PLATFORM: "<HSB_PLATFORM>".
   ```

5. **Both `DETECTED_PLATFORM` and `HSB_PLATFORM` are empty** â†’ fall through to the manual platform question in step 5.
...
2. **Check remaining usage** using the best available Claude Code/account usage source for the current subscription plan. Prefer machine-readable or product-provided usage data when available. If no reliable usage source is available, ask the user to provide their current remaining usage/quota from the Claude Code account or plan UI.
   ...
   1. **I can't verify â€” stop**: The user cannot determine remaining usage. Stop before Phase 1.
   2. **I have < {estimate} available â€” stop**: The user checked their plan/account UI and confirms less than the estimated budget remains. Stop before Phase 1.
   3. **I have â‰¥ {estimate} available â€” proceed**: The user checked their plan/account UI and confirms at least the estimated budget remains. Proceed to Phase 1.
...
```
Holoscan Sensor Bridge â€” Demo Bring-Up Skill

USAGE
  /hsb-setup [PLATFORM] [OPTIONS]

PLATFORM (optional â€” will prompt if omitted)
