...
description: "Use when working with seo-aeo-schema-generator"
---
...
|------|---------------------|
| FAQPage | FAQ accordion in SERP â€” AEO critical |
...
- âœ… **Do:** Always include FAQPage schema on any page with a FAQ section â€” it is the strongest AEO signal
- âœ… **Do:** Use one `<script>` block per schema type â€” never combine multiple types
- âœ… **Do:** Test every output in Google's Rich Results Test before deploying
- âŒ **Don't:** Use relative URLs anywhere in schema â€” all URLs must start with `https://`
- âŒ **Don't:** Leave placeholder text in any field before deploying
- âŒ **Don't:** Use HTML tags inside JSON-LD string values
...
- **Problem:** Product schema missing star rating display
  **Solution:** Add AggregateRating object with ratingValue, reviewCount, bestRating, and worstRating â€” all four fields required.

## Related Skills

- `@seo-aeo-landing-page-writer` â€” provides the FAQ and product data for schema population
- `@seo-aeo-content-quality-auditor` â€” flags schema gaps during the audit
