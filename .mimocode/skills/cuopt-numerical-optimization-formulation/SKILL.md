...
description: "Use when working with cuopt-numerical-optimization-formulation"
---

---
name: cuopt-numerical-optimization-formulation
version: "26.08.00"
description: LP, MILP, QP â€” concepts, problem-text parsing, and formulation patterns (parameters, constraints, decisions, objective). Concepts only; no API.
license: Apache-2.0
metadata:
...
- **QP**: Quadratic objective (e.g., xÂ², xÂ·y terms â€” portfolio variance, least squares), linear constraints. **QP support in cuOpt is currently in beta.**

## Identifying problem type

| Property | LP | MILP | QP |
|---|---|---|---|
| Objective | Linear | Linear | Quadratic (xáµ€Qx + cáµ€x) |
...
If the objective is purely linear, prefer LP/MILP â€” do not artificially introduce quadratic terms. If any variable is integer or binary, the problem is MILP regardless of the rest.

## Required formulation questions

Ask these if not already clear:

1. **Decision variables** â€” What are they? Bounds?
2. **Objective** â€” Minimize or maximize? Linear or quadratic? For QP: any squared or cross terms (xÂ², xÂ·y)? If maximize a quadratic, the user must negate and minimize.
3. **Constraints** â€” Linear inequalities/equalities? (Quadratic constraints are not supported.)
4. **Variable types** â€” All continuous (LP / QP) or some integer/binary (MILP)?
5. **Convexity (QP only)** â€” For minimization, the quadratic form (matrix Q) should be positive semi-definite for well-posed problems.

## Typical modeling elements

- **Continuous variables** â€” production amounts, flow, allocations, portfolio weights.
- **Binary variables** â€” open/close, yes/no (e.g., facility open, item selected).
- **Linking constraints** â€” e.g., production only if facility open (Big-M or indicator).
- **Resource constraints** â€” linear cap on usage (materials, time, capacity).
- **Quadratic objective terms** â€” variance (xáµ€Qx), squared error (â€–Ax âˆ’ bâ€–Â²), interaction terms.

## Typical QP use cases

- Portfolio optimization â€” minimize variance subject to return and budget.
- Least squares â€” minimize â€–Ax âˆ’ bâ€–Â² subject to linear constraints.
...
**Classify every sentence** as **parameter/given**, **constraint**, **decision**, or **objective**. Watch for **implicit constraints** (e.g., committed vs optional phrasing) and **implicit objectives** (e.g., "determine the plan" + costs â†’ minimize total cost).
...
### ðŸ”’ MANDATORY: When in Doubt â€” Ask
...
### ðŸ”’ MANDATORY: Complete-Path Runs â€” Try All Variants
...
| **Objective** | What to minimize or maximize. May be **explicit** ("minimize cost") or **implicit** ("determine the plan" with costs given). | "Minimize total cost." "Determine the production plan" (with costs) â†’ minimize total cost. |

### Implicit constraints: committed vs optional phrasing

**Committed/fixed phrasing** â†’ treat as **parameter** or **implicit constraint** (everything mentioned is given or must happen). Not a decision.
...
| "Has a capacity of C" | **Parameter** (C) + **constraint**: usage â‰¤ C. | Capacity is fixed. |
| "Must meet all demand" | **Constraint**: demand satisfaction. | Explicit requirement. |

**Optional/decision phrasing** â†’ treat as **decision**.

| Phrasing | Interpretation | Why |
|----------|-----------------|-----|
| "May produce up to â€¦" | **Decision**: how much to produce. | Optional level. |
...
| "Wants to minimize/maximize â€¦" | **Objective** (drives decisions). | Goal; decisions are the levers. |

### Implicit objectives â€” do not miss
...
|-------------------|---------------------------|-----|
| "Determine the production plan" + costs given (per unit, per hour, etc.) | **Minimize total cost** (production + inspection/sales + overtime, etc.) | Plan is chosen; costs are specified â†’ natural goal is to minimize total cost. |
| "Determine the plan" + costs and revenues given | **Maximize profit** (revenue âˆ’ cost) | Both sides of the ledger â†’ optimize profit. |
| "Try to determine the monthly production plan" + workshop hour costs, inspection/sales costs | **Minimize total cost** | All cost components are given; no revenue to maximize â†’ minimize total cost. |
...
3. **Identify the objective (explicit or implicit):** If the problem says "minimize/maximize X", that's the objective. If it only says "determine the plan" (or "find", "establish") but gives costs (and possibly revenues), the objective is **implicit** â€” state it (e.g., minimize total cost, or maximize profit) and confirm with the user if ambiguous.
4. **Flag implicit constraints**: For each sentence, ask â€” "Does this state a fixed fact or a requirement (â†’ parameter/constraint), or something we choose (â†’ decision)?"
5. **Resolve ambiguity** by checking verbs and modals:
   - "is", "has", "operates", "employs", "plans to" (fixed/committed) â†’ parameter or implicit constraint.
   - "may", "can choose", "considers", "decides", "wants to" (optional) â†’ decision or objective.
6. **ðŸ”’ MANDATORY â€” If anything is still ambiguous** (e.g., a value or constraint could be read two ways): ask the user which interpretation is correct, or solve all plausible interpretations and report all outcomes. Do not assume a single interpretation.
...
- [ ] **Objective is identified:** Explicit ("minimize/maximize X") or implicit ("determine the plan" + costs â†’ minimize total cost; + revenues â†’ maximize profit). Never formulate without stating the objective.
- [ ] Committed phrasing ("plans to", "operates", "employs") â†’ not decisions.
- [ ] Optional phrasing ("may", "can choose", "considers") â†’ decisions.
- [ ] Implicit constraints from committed phrasing are written out (e.g., "all X must be produced").
- [ ] **ðŸ”’ MANDATORY â€” Ambiguity:** Any phrase that could be read two ways â†’ I asked the user or I will solve all interpretations and report all outcomes (no silent single interpretation).
...
**Implicit-objective example:** A problem that asks to "determine the production plan" (or similar) and gives cost components (e.g., workshop, inspection, sales) but does not state "minimize" or "maximize" â†’ **Objective is implicit: minimize total cost**. Always state it explicitly: "The objective is to minimize total cost."
...
The remaining sections cover specific LP/MILP modeling patterns. Each is independent â€” read the one that matches your problem.
...
```
x_total  â€” INTEGER (total production of a product)
s1, s2, â€¦ â€” CONTINUOUS (amount sold in each price segment, bounded by segment width)

Link: x_total = s1 + s2 + â€¦
Resource constraints use x_total.
Objective uses segment variables Ã— segment profit rates.
```

### Cutting stock / trim loss problems

In cutting stock problems, **waste area** includes both **trim loss** (unused width within each cutting pattern) and **over-production** (excess strips produced beyond demand). Minimizing only trim loss (waste width Ã— length per pattern) ignores over-production and yields an incorrect objective.
...
```
minimize  sum_j (roll_width_j Ã— x_j)
```

where `x_j` is the length cut using pattern `j`. The waste area is then:

```
waste = total_material_area âˆ’ required_useful_area
```

where `required_useful_area = sum_i (order_width_i Ã— order_length_i)`.

#### Gotcha

Using `sum_j (waste_width_j Ã— x_j)` as the objective only captures trim loss â€” the unused strip within each pattern. It does **not** penalize over-production of an order. The solver will over-produce narrow orders to fill patterns efficiently, but that excess material is still waste. Always use total material area as the objective.

### Goal programming (preemptive / lexicographic)

Goal programming optimizes multiple objectives in priority order. Implement it as **sequential solves** â€” one per priority level.

#### Formulation pattern

1. **Hard constraints** â€” capacity limits, non-negativity, etc. These hold in every phase.
2. **Goal constraints** â€” for each goal, introduce deviation variables (dâ» for underachievement, dâº for overachievement) and write an equality: `expression + dâ» âˆ’ dâº = target`.
...
Deviation variables (dâ», dâº) and slack/idle-time variables are always **continuous**. However, **decision variables must still be INTEGER when they represent discrete/countable quantities** (units produced, vehicles, workers, etc.). Do not let the presence of continuous deviation variables cause you to make all variables continuous â€” the integrality of decision variables directly affects feasibility and objective values.
...
- **End-of-period capacity** (variable bound): `stock[t] <= capacity` â€” always needed.
- **After-purchase capacity** (explicit constraint): `stock[t-1] + buy[t] <= capacity` â€” prevents buying more than the warehouse can hold before any sales occur within the period.
...
In some blending problems, a subset of raw materials must be **mixed together first** (e.g., in a mixing tank) before being allocated to different products. The resulting intermediate has a **uniform composition** â€” you cannot independently assign different raw materials to different products.
...
1. **Single-product allocation:** If analysis shows the intermediate is profitable in only one product, allocate all intermediate to that product (set intermediate allocation to other products to zero). The proportionality constraint becomes trivially satisfied. This is the most common case â€” check profitability of intermediate in each product before attempting a general split.

2. **Parametric over intermediate concentration:** Fix the sulfur/quality concentration of the intermediate as a parameter `Ïƒ`. For each fixed `Ïƒ`, the problem is a standard LP (intermediate becomes a virtual raw material with known properties). Solve for a grid of `Ïƒ` values or use the structure to find the optimum analytically.

3. **Scenario enumeration:** When only 2â€“3 products exist, enumerate which products receive the intermediate (all-to-A, all-to-B, split). For each scenario with a single recipient, the LP is standard. For split scenarios, use strategy 2.
