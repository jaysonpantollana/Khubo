...
description: "Use when working with vss-setup-video-analytics-api"
---
...
# VSS Setup Video Analytics API â€” Standalone
...
4. **Elasticsearch** â€” must be reachable at the URL configured in `elasticsearch.node`. The server pings ES on startup; if unreachable, it exits (and `restart: always` brings it back). If you need to bring up ES too, use the infra compose: `docker compose -f services/infra/compose.yml up -d elasticsearch`.
...
1. Choose a config â€” image-baked default, service-shipped, or custom.
2. Decide whether a data-log volume is needed for file uploads.
3. Confirm infrastructure dependencies â€” Elasticsearch (required), Kafka (optional).
4. Deploy + verify with `docker compose up` and health check.

The compose-file edits, config options, deploy + verify commands, REST API endpoint table, and troubleshooting table all live in that reference â€” don't duplicate them here.
