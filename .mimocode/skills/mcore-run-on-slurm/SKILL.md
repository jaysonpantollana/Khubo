...
description: "Use when working with mcore-run-on-slurm"
---
...
- Submit from the worktree you intend to run, or `cd` to it in the script. All nodes must reach the same path on a shared filesystem (NFS, Lustre, or similar) â€” node-local paths will not be visible to peer ranks.
...
- **Pre-Blackwell (Hopper, Ampere) with TP>1 or CP>1, non-FSDP:** set to `1`. The relevant code path asserts on this â€” you will get an assertion error if it is not `1`, not a silent deadlock.
...
- **Shape / divisibility error**: check `WORLD_SIZE = TP Ã— DP Ã— CP Ã— PP` and head-count divisibility (`num_attention_heads % TP == 0`).
