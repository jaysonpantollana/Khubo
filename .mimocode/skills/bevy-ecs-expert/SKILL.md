...
description: "Use when working with bevy-ecs-expert"
---
...
- âœ… **Do:** Use `Query` filters (`With`, `Without`, `Changed`) to reduce iteration count.
- âœ… **Do:** Prefer `Res` over `ResMut` when read-only access is sufficient to allow parallel execution.
- âœ… **Do:** Use `Bundle` to spawn complex entities atomically.
- âŒ **Don't:** Store heavy logic inside Components; keep them as pure data.
- âŒ **Don't:** Use `RefCell` or interior mutability inside components; let the ECS handle borrowing.
