...
description: "Use when working with tao-generate-referring-expressions"
---
...
```
Step 0: Region expression  â”€â”€â”
                              â”œâ”€â”€â–¶  Step 2: Grounding expression  â”€â”€â–¶  [Step 3: Double check]
Step 1: Image caption  â”€â”€â”€â”€â”€â”€â”˜                                                   (optional)
```

- **Step 0 (region_expr)** â€” VLM emits one short discriminative phrase per KITTI bbox (`bbox_2d`, `type`, `color`, `description`).
- **Step 1 (image_caption)** â€” VLM emits a holistic, location-agnostic scene caption.
- **Step 2 (grounding_expr)** â€” VLM groups Step 0 objects into grouping phrases and returns one bbox list per group, optionally using Step 1's caption as extra context.
- **Step 3 (double_check)** â€” VLM re-checks each Step 2 bbox against the image; bad matches are removed, slightly-off boxes get tightened.
...
4. **API access**: Ask the user which VLM endpoint they want to use. Present these five options and act on the choice:
   1. **Gemini** â€” set `vlm.backend: "gemini"`; require `GOOGLE_API_KEY` (env var or `vlm.gemini.api_key`).
   2. **NIM** (e.g. `https://inference-api.nvidia.com/v1`) â€” set `vlm.backend: "openai"`; collect `base_url`, `model_name`, and `api_key`.
   3. **TAO inference microservice** (self-hosted, OpenAI-compatible). Confirm whether the server is already running:
      - **Running** â€” collect `base_url`, `model_name`, and (optionally) `api_key`; set `vlm.backend: "openai"`.
      - **Not running** â€” guide the user through the `skills/applications/tao-run-inference-service` skill, which stands up a local TAO inference microservice with an OpenAI-compatible API. Before promising a specific model, check `skills/applications/tao-run-inference-service/references/service.yaml` for `valid_network_arch_config_basenames`. Once the server is up, collect `base_url`, `model_name`, and (optionally) `api_key`; set `vlm.backend: "openai"`.
   4. **vLLM** (self-hosted, OpenAI-compatible). Confirm whether the server is already running:
      - **Running** â€” collect `base_url`, `model_name`, and (optionally) `api_key`; set `vlm.backend: "openai"`.
      - **Not running** â€” follow [references/vllm_server.md](references/vllm_server.md) to install and launch a vLLM server, then collect `base_url`, `model_name`, and (optionally) `api_key`; set `vlm.backend: "openai"`.
   5. **Custom** (any other OpenAI-compatible endpoint) â€” set `vlm.backend: "openai"`; collect `base_url`, `model_name`, and (optionally) `api_key`.
...
2. Inspect `step_0_region_expr/annotations.jsonl` â€” are object types, colors, and discriminating phrases accurate?
3. Inspect `step_2_grounding_expr/annotations.jsonl` â€” are objects grouped sensibly, and do bbox coordinates match the described groups?
4. Inspect `step_3_double_check/annotations.jsonl` â€” were mismatched bboxes removed or tightened? Are any new errors introduced (rare)?
...
- `seed_annotations.jsonl` â€” initial per-image records (unless `input_annotations_jsonl` was supplied).
- `step_0_region_expr/annotations.jsonl` â€” adds `regions[]` (each with `bbox`/`bbox_2d`, `type`, `color`, `description`).
- `step_1_image_caption/annotations.jsonl` â€” adds `caption` (string).
- `step_2_grounding_expr/annotations.jsonl` â€” adds `expressions[]` (each `{text, instances: [{bbox: [x1,y1,x2,y2]}]}`).
- `step_3_double_check/annotations.jsonl` â€” same shape as Step 2, with bboxes removed/updated.
- `results_dir/annotations.jsonl` â€” copy of the last completed step's output.
