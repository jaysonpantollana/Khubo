...
description: "Use when working with odoo-sales-crm-expert"
---
...
```text
Menu: CRM â†’ Configuration â†’ Stages â†’ New
...
Typical B2B Pipeline:
  ...
  Stage 5: Won               (is_won: YES â€” marks opportunity as closed-won)
...
```text
Menu: Sales â†’ Configuration â†’ Quotation Templates â†’ New
(Requires the "Sales Management" module â€” enabled in Sales Settings)
...
Signature & Payment:
  â˜‘ Online Signature required before order confirmation
  â˜‘ Online Payment (deposit) â€” 50% upfront
...
```text
Menu: Sales â†’ Configuration â†’ Settings
  â˜‘ Enable Pricelists

Menu: Sales â†’ Configuration â†’ Pricelists â†’ New

Name: VIP Customer â€” 15% Off
Currency: USD
...
Assign to a customer:
  Customer record â†’ Sales & Purchase tab â†’ Pricelist â†’ VIP Customer
```

## Best Practices

- âœ… **Do:** Use **Lost Reasons** (CRM â†’ Configuration â†’ Lost Reasons) to build a dataset of why deals are lost â€” invaluable for sales coaching.
- âœ… **Do:** Enable **Sales Teams** with revenue targets so pipeline forecasting is meaningful per team.
- âœ… **Do:** Set **Expected Revenue** and **Closing Date** on every opportunity â€” these feed the revenue forecast dashboard.
- âœ… **Do:** Use **Quotation Templates** to standardize offers and reduce quoting time across the team.
- âŒ **Don't:** Skip the CRM opportunity when selling â€” going directly from lead to invoice breaks pipeline analytics.
- âŒ **Don't:** Manually edit prices on quotation lines as a workaround â€” set up proper pricelists instead.
- âŒ **Don't:** Ignore the **Predictive Lead Scoring** feature in v16+ â€” configure it with historical data for accurate forecasting.

## Limitations

- **Commission rules** are not built into Odoo CRM out of the box â€” they require custom development or third-party modules.
...
- Odoo CRM does not have native **email sequence / cadence** automation â€” use the **Email Marketing** or **Marketing Automation** modules for drip campaigns.
